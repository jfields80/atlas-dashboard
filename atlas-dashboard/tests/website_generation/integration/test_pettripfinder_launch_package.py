...
class TestRealSeedFilesParse:
    def test_seed_businesses_csv_parses(self):
        ...
        # 116 columbus-oh + 21 cleveland-akron-canton-oh + 47 dayton-oh.
        # Dayton grew 33 -> 44 in PTF-DAYTON-CANDIDATE-PROMOTION-001, which
        # promoted eleven reviewed dayton-recovery-002 candidates, then
        # 44 -> 47 in PTF-DAYTON-WORK-BROWSER-INTEGRATION-001 (two Best
        ...
        # PTF-PITTSBURGH-PASS2-DECISION-APPLICATION-001, plus eight
        ...
        # PTF-INVENTORY-001: the seed now carries two evidence states. A blank
        # pet_policy means "pending official-source attestation", and those rows
        # are excluded at the render boundary rather than published. Assert the
        # split exactly -- a row silently losing its evidence must still fail.
        pending = [r for r in seed if not str(r.get("pet_policy", "")).strip()]
        assert len(pending) == 0
        assert all(r["category"] == "pet-friendly-hotels" for r in pending)
        assert len(seed) - len(pending) == 281
...
class TestRealPackageConversion:
    ...
    def test_every_seed_row_becomes_a_unique_valid_listing(self):
        result = self._build()
        assert result.ok
        assert len(result.dataset.listings) == 281
    ...
    def test_no_fake_default_listings(self):
        ...
        # 89 columbus + 81 cleveland + 47 dayton + 29 pittsburgh + 8 Indianapolis
        assert counts[ids_by_slug["pet-friendly-hotels"]] == 254
...
class TestRealPackageReadiness:
    ...
    def test_ready_only_counting(self):
        ...
        assert readiness["total_unique_listings"] == 281
        assert readiness["counts_by_state"]["READY"] == 281
        assert readiness["counts_by_state"]["READY_WITH_WARNINGS"] == 0
        assert readiness["counts_by_state"]["NOT_READY"] == 0
        assert readiness["ready_total"] == 281
        assert readiness["ready_by_category"]["pet-friendly-hotels"] == 254
    ...
    def test_load_launch_package_helper_matches_direct_reads(self):
        ...
        assert len(package["seed_businesses"]) == 281
