...
def test_authoritative_hotel_discovery():
    cs = _candidates()
    assert len(cs) == 86                                  # authoritative Columbus/Dublin count
...
def test_checkpoint_reports_exact_nano_and_worst_case_cost():
    ...
    assert cp["hotels_found"] == 86 and cp["assignments_ready"] == 86
...
def test_live_pilot_routes_and_persists(monkeypatch, tmp_path):
    ...
    assert report["mode"] == "live" and report["calls_made"] == 86
    agg = report["aggregate"]
    assert sum(agg["routes"].values()) == 86
    assert agg["provider_failures"] == 0 and agg["structurally_valid"] == 86
    ...
    for sub in ("assignments", "model_results", "validated_results", "routing_envelopes"):
        assert len(list((tmp_path / sub).glob("*.json"))) == 86
...
def test_live_pilot_deterministic_idempotent_rerun(monkeypatch, tmp_path):
    ...
    CP.run_pilot(classified, CP.PilotCaps(), live=True, store=store, provider_factory=_fake_factory())
    assert len(list((tmp_path / "assignments").glob("*.json"))) == n1 == 86
...
def test_operator_summary_counts_and_cost(monkeypatch, tmp_path):
    ...
    assert s["inventory"]["authoritative_hotel_candidates"] == 86
    assert s["inventory"]["successful_model_responses"] == 86
    assert sum(s["routing"]["counts"].values()) == 86
...
def test_cli_dry_run_no_writes(tmp_path, capsys):
    ...
    assert "hotels found               : 86" in out and "DRY RUN" in out
