...
class TestPolicyAuthorityIsEmpty:
    ...
    def test_registering_milwaukee_changed_no_other_market_s_authority(self):
        ...
        # Routes are stated the same way as exclusions and seeds below: the
        # fixed 277 was the pre-Grand-Rapids total, and PTF-GRAND-RAPIDS-
        # HOLLAND-STAGE-001 registered a routing shard of its own after this
        # market. Milwaukee's routing shard is empty; everything else is
        # somebody else's shard.
        routes = MA.assemble_routing_document()["routes"]
        my_routes = [row for row in routes
                     if (row.get("hotel_ref") or {}).get("market_id") == MARKET]
        assert my_routes == MA.load_market_routes(MARKET)
        assert len(routes) - len(my_routes) == sum(
            len(MA.load_market_routes(m)) for m in MA.sharded_market_ids()
            if m != MARKET)
