...
def test_committed_authority_validates(routes):
    ...
    # that absence obeys.
    assert len(routes) == 131  # after PTF-CLEVELAND-PASS2-FOUNDER-DECISIONS-001
...
def test_committed_authority_split(routes):
    ...
    assert len(confirmed) == 120  # after PTF-CLEVELAND-PASS2-FOUNDER-DECISIONS-001
    assert len(held) == 9
...
def test_every_committed_record_preserves_index_binding(routes):
    ...
    assert len(rendered) == 9  # after PTF-CLEVELAND-PASS2-FOUNDER-DECISIONS-001
...
def test_routing_adds_capture_ready_hotels(queues):
    ...
    assert len(routed.selected) - len(base.selected) == 49  # after PTF-CLEVELAND-PASS2-FOUNDER-DECISIONS-001
...
def test_routing_carries_more_than_one_market(queues):
    ...
    assert len(by_market["cleveland-akron-canton-oh"]) == 102  # after PTF-CLEVELAND-PASS2-FOUNDER-DECISIONS-001
    ...
    assert len(added) == 49  # after PTF-CLEVELAND-PASS2-FOUNDER-DECISIONS-001
