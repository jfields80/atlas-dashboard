...
def test_routing_does_not_enter_seed_inventory():
    ...
    assert len(hotels) == 78, "seed hotel rows must be untouched by routing"
...
def test_routing_adds_capture_ready_hotels(queues):
    ...
    assert len(routed.selected) - len(base.selected) == 16
