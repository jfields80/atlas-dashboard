...
@pytest.fixture(scope="module")
def census_row():
    return next(h for h in _load(CENSUS)["hotels"]
                if h["identity_key"] == KEY)


@pytest.fixture(scope="module")
def route():
    return next(r for r in _load(AUTH / "identity_routing.json")["routes"]
                if r["hotel_ref"]["identity_key"] == KEY)
...
def test_no_census_identity_was_added_or_renamed(census_row):
    ...
    census = _load(CENSUS)
    assert len(census["hotels"]) == 256
    keys = [h["identity_key"] for h in census["hotels"]]
    assert len(set(keys)) == len(keys)


def test_the_authority_counts_did_not_move():
...
def test_the_row_is_unresolved_and_says_why(finding):
    item = next(i for i in _load(PARTITION)["items"]
                if i["identity_key"] == KEY)
    assert item["resolved"] is False
    assert item["final_state"] == "AWAITING_IDENTITY_RESOLUTION"
    assert "TWO Choice properties" in item["state_override_reason"]
    assert item["determined_by"] == M.WORK_ORDER


def test_the_route_stays_active(route, finding):
...
def test_running_it_again_refuses(census_row):
    """Idempotence by refusal: the guard notices the correction is already
    made rather than layering a second identity_review on the row."""
