...
def test_current_live_inventory_preserves_all_assemblable_market_profiles(markets):
    """Section 28's live inventory is market-derived.
    ...
    assert counts == {COLUMBUS: 88, CLEVELAND: 99, DAYTON: 47,
                      # Pittsburgh Pass 3 (+3) and Pass 4 (+8) founder
                      # decisions on top of the 26 live from Pass 2.
                      "pittsburgh-pa": 37, INDIANAPOLIS: 8,
                      "milwaukee-wi": 73,
                      ...
                      "louisville-ky": 46}
    assert sum(counts.values()) == 480   # 469 + Pittsburgh Pass 3/4 (11)
...
def test_combined_bundle_assembles_with_every_gate_passing(short_out):
    ...
    assert sum(len(f["hotel_routes"]) for f in manifest["fragments"].values()) == sum(
