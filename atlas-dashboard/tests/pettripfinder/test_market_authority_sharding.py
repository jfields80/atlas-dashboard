...
class TestTheSplitIsRecordedAndLostNothing:
    """SS10, as a claim that stays true.

    The baseline manifest records what every market held immediately BEFORE the
    split. Its job was to prove the split itself moved nothing, and it did --
    that is settled history, on main, in the sharding commit.

    It must NOT be re-asserted as live per-market equality. PTF-MARKET-AUTHORITY-
    SHARDING-001 exists so that a market can add a record without touching
    anything another market reads; a test demanding every market still hold its
    pre-split count would make every market work order edit THIS file, which is
    the shared-file contention the whole project removed. The first market to
    prove it was Detroit-Ann Arbor, whose seven founder-approved exclusions
    landed the day after the split.

    What is asserted here is what remains true forever: the recorded evidence is
    internally sound, and no market that existed at the split has since lost its
    authority storage. Live per-market counts are pinned in each market's own
    suite, where only that market's work can change them.
    """

    def test_the_recorded_baseline_is_internally_consistent(self, baseline):
        """The migration evidence must add up, or it proves nothing."""
        totals = baseline["global_totals"]
        per_market = baseline["per_market_totals"]
        assert sum(m["routing"] for m in per_market.values()) == totals["routing"]
        assert sum(m["exclusions"] for m in per_market.values()) == totals["exclusions"]
        assert sum(m["seed"] for m in per_market.values()) == totals["seed_rows"]
        assert baseline["baseline_main_sha"] ==             "20279f4b6f66a073f69823275c23f5c3481f173b"

    def test_no_market_present_at_the_split_has_lost_its_shards(self, baseline):
        """A market may gain records, retire records, or hold none at all. What
        it may never do is stop owning the place its records live."""
        for market_id in baseline["per_market_totals"]:
            assert market_id in MA.sharded_market_ids(), market_id
            for path in (MA.routing_shard_path(market_id),
                         MA.exclusions_shard_path(market_id),
                         MA.seed_shard_path(market_id)):
                assert path.is_file(), path

    def test_the_registered_market_set_only_grows(self, baseline):
        """Registering a market is additive; un-registering one silently would
        strand its committed authority."""
        assert set(baseline["registered_market_ids"]) <= set(MA.registered_market_ids())
