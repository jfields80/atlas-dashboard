...
class TestTheRuleIsOptIn:
    ...
    def test_the_phone_key_is_untouched_either_way(self):
        """``presentation_variants`` changes the NAME key and leaves the phone
        key exactly where it was.

        This test used to prove that with a sighting called "Totally Different",
        on the reasoning that a phone match should carry a row whatever the name
        said. PTF-GRAND-RAPIDS-HOLLAND-CHOICE-ROUTING-REPAIR-007 disproved that
        reasoning with a real routing error: a Comfort Inn bound on its own
        telephone to a Comfort Suites sighting, because two Choice brands share
        one switchboard in one building, and the Comfort Inn was routed to the
        Comfort Suites' page. ``names_may_share_a_url`` now refuses that, and
        the refusal is correct.

        So the ORIGINAL intent is tested with a compatible name -- the phone key
        is unmoved by the presentation flag -- and the interaction with 007 is
        asserted directly below rather than left as a surprise.
        """
        row = dict(self._row(), phone="3175551212")
        observation = URC.Observation(
            provider=URC.GOOGLE_PLACES, source="test",
            name="Candlewood Suites Indianapolis Northwest by IHG",
    ...
    def test_a_shared_switchboard_does_not_carry_an_unrelated_name(self):
        """007's rule, stated where the phone key is tested so the two are read
        together: a telephone match is not enough when the sighting names a
        property whose name neither contains nor is contained by this one."""
        row = dict(self._row(), phone="3175551212")
        observation = URC.Observation(
            ...
            url="https://www.ihg.com/candlewood/hotels/us/en/indianapolis/"
                "indnw/hoteldetail", street="")
        for variants in (False, True):
            found, binding = URC.bind(row, [observation],
                                      presentation_variants=variants)
            assert found is None and binding == ""
