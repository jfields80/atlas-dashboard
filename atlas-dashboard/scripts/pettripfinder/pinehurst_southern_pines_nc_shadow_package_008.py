"""PTF-PINEHURST-SOUTHERN-PINES-NC-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Fayetteville is live (deploy 6aa750d8), and Jacksonville, Greenville, Atlanta, Outer Banks and
Boone - Blowing Rock (source-ready, not registered) must become the live parent before
Pinehurst - Southern Pines can be released.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/pinehurst-southern-pines-nc/launch_package/``) from the proposed
  ...
  3. runs the FAST lane (rules A-O) over it, which builds the joining market's
     ...
     current live parent (Fayetteville) -- no whole-site candidate is assembled;
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/pinehurst-southern-pines-nc/``: never ``markets/packages/`` or
...
WHAT BINDS TO THE FAYETTEVILLE PARENT, AND HOW IT IS RECONCILED
------------------------------------------------------------
...
Jacksonville (and later Greenville, Atlanta, Outer Banks and Boone) is live, rule N FAILS this package on purpose; the registration
...
REPORT = PKG / "markets" / "reports" / "pinehurst_southern_pines_nc_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "pinehurst_southern_pines_nc_proposed_authority_002.json"
SEALED_AT = "2026-09-14T03:00:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the CURRENT live release (Fayetteville, deploy 6aa750d8...). That is what this "
         "package was validated against. Once Jacksonville is live, FAST rule N fails this package BY DESIGN; the "
         "registration order -- after Jacksonville, Greenville, Atlanta, Outer Banks AND Boone - Blowing Rock are "
         "live -- re-seals the same committed inputs against that parent as a NEW package id. Nothing here is a "
         "production candidate, and no whole-site build was assembled."),
