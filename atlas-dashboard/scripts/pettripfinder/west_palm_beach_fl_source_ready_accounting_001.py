"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- Phases 10, 13, 20-25 and 29: the machine-readable accounting.
...
Output: launch_packages/pettripfinder/markets/reports/west_palm_beach_fl_source_ready_accounting_001.json
"""
...
from scripts.pettripfinder.west_palm_beach_fl_nonhotel_rulings_001 import exclusion_class  # noqa: E402

WORK_ORDER = "PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001"
PKG = os.path.join(_DASH, "launch_packages", "pettripfinder")
R = os.path.join(PKG, "markets", "reports")
RAW = os.path.join(PKG, "markets", "staging", "west-palm-beach-fl", "raw_captures")
OUT = os.path.join(R, "west_palm_beach_fl_source_ready_accounting_001.json")
PARTITION = os.path.join(PKG, "markets", "staging", "west-palm-beach-fl", "launch_package", "west_palm_beach_fl_final_partition_001.json")
...
FAMILIES = [
    ("MARRIOTT", r"marriott|courtyard by marriott|courtyard marriott|courtyard palm beach|courtyard west palm beach|courtyard oceanside|courtyard by|courtyard(?= (palm|west|downtown|airport|boca|delray|jupiter|wellington))|residence inn|springhill|fairfield|towneplace|ac hotel|aloft|westin|sheraton|"
...
LUXURY_RX = re.compile(r"\b(ritz.?carlton|four seasons|the breakers|eau palm beach|the colony|the chesterfield|the vineta|white elephant|the ben|amrit|the boca raton|tideline|opal grand|"
                       r"pier 66|lago mar|bahia mar|the atlantic hotel|hotel maren|kimpton shorebreak|"
                       r"the diplomat|signia|margaritaville|seminole hard rock|guitar hotel|ocean manor|"
                       r"plunge beach|pelican grand|casa cordova|riverside hotel|b ocean|hyde beach|"
                       r"the ray|circ hotel|ocean sky|grand resort and spa|worthington|the westin beach)\b", re.I)
...
def main():
    ...
    fc2 = L("west_palm_beach_fl_firecrawl_pass_002.json", {}) or {}
    fc3 = L("west_palm_beach_fl_firecrawl_pass_003.json", {}) or {}
    bpr = L("west_palm_beach_fl_brand_page_reads_001.json", {}) or {}
    disc = L("west_palm_beach_fl_firecrawl_discovery_001.json", {}) or {}
    static = L("west_palm_beach_fl_free_static_capture_001.json", {}) or {}
    comp = L("west_palm_beach_fl_competitor_challenge_001.json", {}) or {}
    recon = L("west_palm_beach_fl_competitor_reconciliation_001.json", {}) or {}
    dbpr = L("west_palm_beach_fl_dbpr_lane_001.json", {}) or {}
    osm = L("west_palm_beach_fl_osm_lane_001.json", {}) or {}
    brand = L("west_palm_beach_fl_brand_inventory_001.json", {}) or {}
    roster = L("west_palm_beach_fl_destination_roster_001.json", {}) or {}
    routing = L("west_palm_beach_fl_routing_001.json", {}) or {}
    wyndham = L("west_palm_beach_fl_wyndham_lane_001.json", {}) or {}
    esa = L("west_palm_beach_fl_esa_lane_001.json", {}) or {}
    places = L("west_palm_beach_fl_places_route_discovery_001.json", {}) or {}
    ...
    creport = L("west_palm_beach_fl_census_reconciliation_001.json", {}) or {}

    # corridors
    cor = OrderedDict()
    for slug, name, _a, klass, _m, _z, _d in GEO.CORRIDORS:
        cid = "west-palm-beach-fl__" + slug
    ...
    # PALM BEACH COUNTY: the attended-browser lane this order actually exercised (west_palm_beach_fl_browser_lane_001),
    # counted per brand family so the brand table reports reads and denials, not a placeholder zero.
    _blp = os.path.join(R, "west_palm_beach_fl_browser_lane_001.json")
    ...
    doc = OrderedDict([
        ("schema", "ptf-source-ready-accounting/1.0"), ("work_order", WORK_ORDER), ("market_id", "west-palm-beach-fl"),
        ...
        ("providers", OrderedDict([
            ...
            ("firecrawl", OrderedDict([
                ...
                ("retry_pass_why", "first-pass FIRECRAWL_MISMATCH rows re-attempted once after the census restated "
                                   "Palm Beach County grid streets in the ordinal spelling the pages use"),
