"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- Phases 15-19: policy adjudication over one identity per row.
...
Output:
  launch_packages/pettripfinder/markets/reports/west_palm_beach_fl_clean_authority_001.json
"""
...
from scripts.pettripfinder.west_palm_beach_fl_census_reconciliation_001 import canonical_street  # noqa: E402
...
STAGING = os.path.join(PKG, "markets", "staging", "west-palm-beach-fl", "raw_captures")
CENSUS = os.path.join(PKG, "identity_census", "west-palm-beach-fl.json")
ROUTING = os.path.join(REPORTS, "west_palm_beach_fl_routing_001.json")
STATIC = os.path.join(REPORTS, "west_palm_beach_fl_free_static_capture_001.json")
FIRECRAWL = os.path.join(REPORTS, "west_palm_beach_fl_firecrawl_pass_001.json")
#: The retry pass (after the census street restatement); a retry row supersedes the first pass's row for its key.
FIRECRAWL_RETRY = os.path.join(REPORTS, "west_palm_beach_fl_firecrawl_pass_002.json")
...
FIRECRAWL_PROBE2 = os.path.join(REPORTS, "west_palm_beach_fl_firecrawl_pass_003.json")
...
OUT = os.path.join(REPORTS, "west_palm_beach_fl_clean_authority_001.json")
...
def build_evidence_index(census_hotels):
    ...
    # bound to its own premises by west_palm_beach_fl_browser_closure_002 (property code, or the brand page's own name with
...
BROWSER_BLOCKERS = {
    "MARRIOTT": "the router sends Marriott to the attended browser (Firecrawl is a measured capability wall, "
                "re-probed this order and answering SCRAPE_ALL_ENGINES_FAILED at zero credits). This session "
                "opened 13 paced windows on marriott.com and met an Akamai Access Denied page in most of "
                "them; the challenge was never bypassed and no window was retried from inside",
}

#: Families whose own host this session's attended browser DID reach. A row of one of these families that the
#: browser did not read is NOT awaiting a browser -- the browser answered for its brand -- so it must fall
#: through to the routing / access-exhaustion reasons instead of being parked in BROWSER_CAPTURE_NEEDED.
#: Parking it there would be the Phase 11 rule 7 mislabel: a page this order DID reach never stays labelled as
#: still awaiting the reader. Fort Lauderdale recorded extension read-permission refusals on all three of
#: these hosts; this session had none, which is why the text is measured here rather than inherited.
BROWSER_REACHED = {
    "HILTON": "all 28 routed Hilton properties were read first-party on hilton.com this session",
    "HYATT": "all 3 routed Hyatt properties were read first-party on hyatt.com this session",
    "BEST_WESTERN": "both reachable Best Western properties were read first-party on bestwestern.com this "
                    "session; the one Best Western row still unread is unread because the brand publishes no "
                    "property page for that premises -- its own property-code route redirects to a "
                    "hotel-search page and the brand's own Boca Raton results do not list it -- not because "
...
BROWSER_OUTCOME_STATE = {
    ...
    # PALM BEACH COUNTY -- the outcomes this market's own attended-browser lane records. Phase 11 rule 7: a page this
...
def build():
    PLACES_BY_KEY.clear()
    SITES_BY_KEY.clear()
    for r in (_load(os.path.join(REPORTS, "west_palm_beach_fl_places_route_discovery_001.json"), {}) or {}).get("rows", []):
    ...
    # Motel family ran six licensed premises behind one site there, and Lake Worth Beach's and Delray's
