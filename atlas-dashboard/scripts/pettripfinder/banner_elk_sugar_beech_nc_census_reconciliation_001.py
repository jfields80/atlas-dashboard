"""PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.

Cloned from the Boone - Blowing Rock NC census helper (itself from Outer Banks NC). Independent discovery lanes,
none authoritative on its own:

  OSM_OVERPASS        tourism=hotel / motel / guest_house elements inside the
                      observation box, read from the local Geofabrik North Carolina
                      extract (ODbL).
  BRAND_INVENTORY     Marriott's North Carolina hotel sitemap page; Hilton's own North
                      Carolina city pages (property cards stating each hotel's
                      address); Wyndham's own sitemap.
  DESTINATION_ORGANIZATION  the Banner Elk TDA (bannerelk.com: Hotels, Bed &
                      Breakfast Inns, Cabins, Condos, Vacation Homes), the Beech
                      Mountain Visitor Center, the Avery County Chamber, High Country
                      Host, Sugar Mountain Resort's partner list, and Explore Boone's
                      Avery listings owned from the Boone build -- each with the
                      roster's own category.
...
-----------------------
The market contract's postal partition (banner_elk_sugar_beech_nc_geography_001).
Banner Elk / Sugar Mountain / Seven Devils / Beech Mountain (28604) are CORE;
Linville / Montezuma / Pineola CORRIDOR; Newland / Elk Park FRINGE. Boone, Blowing
Rock, Valle Crucis and the rest of Watauga County are the LIVE boone-blowing-rock-nc
market and every such row is OUTSIDE with that reason -- never a duplicate identity.

CABINS, CONDOS AND TIMESHARES ARE NOT HOTELS
-------------------------------------------
High Country lodging is dominated by cabins, chalets, condo complexes, cabin
rental companies and vacation-ownership resorts. None is admitted: a row whose
only evidence types it as a cabin / cottage / condo, a cabin rental company or a
campground is NON_LODGING; named timeshare resorts and cabin collections are
refused BY NAME with the reason (banner_elk_sugar_beech_nc_nonhotel_rulings_001).
Ski-town condo towers and individually owned "inn" condominiums are refused the same way.
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/banner-elk-sugar-beech-nc.json
  launch_packages/pettripfinder/markets/reports/banner_elk_sugar_beech_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/banner_elk_sugar_beech_nc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import banner_elk_sugar_beech_nc_geography_001 as GEO  # noqa: E402
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "banner-elk-sugar-beech-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "banner-elk-sugar-beech-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_destination_roster_001.json")
...
from scripts.pettripfinder.banner_elk_sugar_beech_nc_nonhotel_rulings_001 import (  # noqa: E402
    NOT_LODGING_WHY, LODGING_UNCONFIRMED, IDENTITY_REVIEWS)
...
NON_HOTEL_BUREAU_SUBCATEGORIES = {
    # the bureau's own spellings, as its listing service prints them
    "Cabins, Cottages & Condos", "Cabin Rental Company", "RV & Campgrounds", "Farm Stays",
    # the Banner Elk TDA's and High Country Host's own spellings
    "Cabins", "Condos", "Vacation Homes", "Cabins, Condos and Chalets", "Campgrounds",
...
#: BOONE (kept): a roster files a listing under several sub-categories, and its PRIMARY one
#: is sometimes a meeting-space label ("Meetings Facilities" for the Holiday Inn Express and
#: La Quinta). The vacation-rental rule therefore reads EVERY sub-category the bureau gives a
#: listing: one that carries none of the bureau's hotel-type sub-categories is not a hotel
#: establishment on the bureau's own typing.
HOTEL_BUREAU_SUBCATEGORIES = {"Hotels & Motels", "Resorts & Inns", "Bed & Breakfasts",
                              # the Banner Elk TDA's and High Country Host's hotel-type categories
                              "Hotels", "Bed & Breakfast Inns", "Hotels, Motels and Inns",
                              "Bed & Breakfasts and Country Inns", "Resorts"}
...
def read_osm():
    ...
    # BANNER ELK: an identity-only read (the property's own address, no policy) supersedes a map row too.
    read_routes |= {_route_key(p[5]) for p in IDENTITY_ONLY_PAGES}
    out = []
...
def read_brand_city_pages():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if state_code(card.get("state")) != "NC":
            continue
...
def read_brand_state_sitemap():
    """Marriott's own North Carolina hotel sitemap page: MARSHA code, title and route."""
    out = []
    for r in _brand_doc().get("leads", []):
...
_PROPERTY_ROUTE = re.compile(
    r"wyndhamhotels\.com/(?![a-z]{2}-[a-z]{2}/)[a-z0-9-]+/[a-z-]+-north-carolina/[a-z0-9-]+/overview$",
    re.I)
...
from scripts.pettripfinder.banner_elk_sugar_beech_nc_static_rulings_001 import (  # noqa: E402
...
def read_identity_only_pages():
    ...
    for name, street, city, postal, phone, url, _sha, note in IDENTITY_ONLY_PAGES:
        out.append(observation(
            "PROPERTY_PAGE_IDENTITY_ONLY", 1, url, name, street=street, city=city, region="NC",
...
MAP_ROWS_SUPERSEDED_BY_A_READ = {
    # The map draws the Courtyard at 3998 NC Highway 105 South; the brand's own page for Courtyard Sugar Mountain
    # Banner Elk (tribc) states 3988 NC 105 Hwy, Sugar Mountain 28604 -- one building, the page's number wins.
    "way/1324894716": ("https://www.marriott.com/en-us/hotels/tribc-courtyard-sugar-mountain-banner-elk/overview/",
                       "Courtyard at 3998 NC Highway 105 South (map)"),
    # The map still names 1615 Tynecastle Highway 'Best Western Mountain Lodge' with a Best Western route; the
    # property's own site (and the Banner Elk TDA: 'formerly the Best Western Mountain Lodge') is The Lodge at
    # Banner Elk at the same street. The map label is kept as evidence of the former flag.
    "way/388101080": ("https://lodgeatbannerelk.com/faq/", "Best Western Mountain Lodge at 1615 Tynecastle Highway (map)"),
    # The map places Linville Falls Lodge & Cottages at 8890 NC Highway 183, Newland 28657; the lodge's own site
    # states 48 North Carolina 183, 28647 (Linville Falls). The property's own address beats the map's.
    "node/6657833507": ("https://www.linvillefallslodge.com/", "Linville Falls Lodge & Cottages at 8890 NC Highway 183, Newland 28657 (map)"),
}
...
def read_destination_roster():
    ...
    for r in doc.get("listings", []):
        ...
        out.append(observation(
            ...
            street="" if joined else r.get("street", ""), city=r.get("city", ""), region="NC",
...
def read_regional_visitor_center():
    """High Country Host's own lodging roster: names only, tier 2, never policy."""
    return [observation("REGIONAL_VISITOR_CENTER", 2, REGIONAL_VISITOR_CENTER_SOURCE, name,
                        merge_alias=normalize_name(name),
                        lead_source="High Country Host regional visitor center lodging roster (names only)")
...
_GENERIC_TOKENS = {
    ...
    "nc", "carolina", "bed", "breakfast", "b",
    # BOONE: High Country marketing vocabulary names no chain ("Westglow Luxury Resort and Spa",
    # "Horton Hotel & Rooftop Lounge", "Quality Inn & Suites - University").
    "spa", "luxury", "rooftop", "lounge", "member", "mountain", "mountains", "parkway", "valley",
    "appalachian", "university", "app", "high", "log", "cabins",
    # BANNER ELK: ski-town vocabulary names no chain ("Sugar Ski & Country Club", "Beech Mountain Inns").
    "ski", "golf", "club", "chalet", "chalets", "condos", "cottages", "lodges", "cabin",
}
...
_IN_MARKET_MUNICIPALITIES = {
    "banner elk", "sugar mountain", "seven devils", "beech mountain", "linville", "montezuma", "pineola",
    "newland", "elk park",
}
#: The live Boone - Blowing Rock market's places: never absorbed, never duplicated.
_NEIGHBOUR_MARKET_PLACES = {
    "boone", "blowing rock", "valle crucis", "vilas", "deep gap", "sugar grove", "zionville", "todd",
    "foscoe",
}
_OUT_OF_MARKET_PLACES = _NEIGHBOUR_MARKET_PLACES | {
    "west jefferson", "jefferson", "lenoir", "wilkesboro", "north wilkesboro", "spruce pine",
    "little switzerland", "mountain city", "elizabethton", "roan mountain", "lansing", "creston",
    "crossnore", "minneapolis", "plumtree", "boomer", "collettsville", "patterson",
    "asheville", "hickory", "morganton", "marion", "black mountain",
}
_ALL_PLACES = _IN_MARKET_MUNICIPALITIES | _OUT_OF_MARKET_PLACES
_DIRECTIONALS = {
    "north", "south", "east", "west", "northwest", "northeast", "southwest", "southeast",
    "village", "milepost", "mp", "bypass", "area", "near", "blvd", "road", "rd", "center", "downtown",
}
...
def fold_map_rows_beside_a_read(nodes):
    ...
    read = [n for n in nodes if n.street and n.postal and any(
        str(o.get("lane", "")).startswith("PROPERTY_PAGE") for o in n.observations)]
    folded, absorbed = [], set()
    for n in nodes:
        if n in read or not n.street or not n.postal:
            continue
        ...
        for h in read:
            ...
            if hnum is None or h.postal[:5] != n.postal[:5] or abs(hnum - num) > 10:
                continue
...
def _neighbour_census_names():
    """normalised name -> 'street, ZIP' for every identity the registered Boone - Blowing Rock census holds in
    one of its OWN postal codes (hotels and held rows alike)."""
    doc = _load(os.path.join(PKG, "identity_census", "%s.json" % GEO.NEIGHBOUR_MARKET), {}) or {}
    out = {}
    for r in (doc.get("hotels") or []) + (doc.get("non_admitted") or []):
        if GEO.is_neighbour_market(r.get("postal_code")):
            for nm in [r.get("canonical_name")] + list(r.get("identity_key_aliases") or []):
                if nm:
                    out.setdefault(normalize_name(nm), "%s %s" % (r.get("street") or "", r.get("postal_code") or ""))
    return out


NEIGHBOUR_CENSUS_NAMES = _neighbour_census_names()


def cross_market_collisions(rows):
    """Admitted identity keys that another REGISTERED market's census already admits. Reported, never merged."""
    import glob
    mine = {r["identity_key"]: r for r in rows if r["classification"] == TRUE_HOTEL_IDENTITY}
    out = []
    for path in sorted(glob.glob(os.path.join(PKG, "identity_census", "*.json"))):
        doc = _load(path, {}) or {}
        for h in doc.get("hotels") or []:
            if h.get("identity_key") in mine:
                out.append(OrderedDict([("identity_key", h["identity_key"]), ("other_market", doc.get("market_id")),
                                        ("other_street", h.get("street")), ("other_postal", h.get("postal_code")),
                                        ("this_street", mine[h["identity_key"]]["street"])]))
    return out
...
CORRIDOR_CITIES = {
    # 28604 is the four ski towns' shared postal code. A Watauga town (Foscoe, Valle Crucis, Boone) stated
    # beside 28604 is a GEOGRAPHY_HOLD: the ZIP alone never decides which of the two facts is wrong.
    "banner-elk-sugar-beech": {"banner elk", "sugar mountain", "beech mountain", "seven devils"},
    "linville-grandfather": {"linville", "montezuma", "pineola"},
    "newland-elk-park": {"newland", "elk park"},
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    # BANNER ELK: 28604 reaches over the Watauga line, so a refused municipality beside an ADMITTED postal
    # code is a GEOGRAPHY_HOLD (below), never an OUTSIDE decided on the town name alone.
    if normalize_name(node.city) in _OUT_OF_MARKET_PLACES and (node.postal or "")[:5] not in zips:
        _nb = normalize_name(node.city) in _NEIGHBOUR_MARKET_PLACES
        return OUTSIDE_MARKET, (("NEIGHBOUR_MARKET %s -- " % GEO.NEIGHBOUR_MARKET if _nb else "")
    ...
    if n in IDENTITY_REVIEWS:
        return IDENTITY_REVIEW_REQUIRED, "IDENTITY_REVIEW -- " + IDENTITY_REVIEWS[n]
    # A LIVE NEIGHBOUR'S IDENTITY IS NEVER DUPLICATED. A row this market cannot place in its own postal codes whose
    # name is exactly an identity the registered Boone - Blowing Rock census holds (Graystone Lodge, Sleep Inn Boone,
    # Hidden Valley Motel ...) belongs to that market.
    if (node.postal or "")[:5] not in zips and n in NEIGHBOUR_CENSUS_NAMES:
        return OUTSIDE_MARKET, ("NEIGHBOUR_MARKET %s -- the name is exactly an identity the registered %s census "
                                "holds (%s); never duplicated here" % (GEO.NEIGHBOUR_MARKET, GEO.NEIGHBOUR_MARKET,
                                                                       NEIGHBOUR_CENSUS_NAMES[n]))
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and (_subcats & NON_HOTEL_BUREAU_SUBCATEGORIES) and not any(
        ...
        return NON_LODGING, ("the destination roster files this listing under %s -- a cabin, condo, vacation-"
                             "home, rental-company, retreat or campground category, not a hotel establishment; "
                             "refused under the order's vacation-rental / cabin filter" % sorted(_subcats))
    ...
    if lanes == {"OSM_OVERPASS"} and _osm_cats and _osm_cats <= {"apartment", "chalet"} and not \
            re.search(r"\b(motel|motor court|motor lodge|hotel|inn)\b", node.name or "", re.I):
    ...
    # THE BOONE SIDE AND SOUTHERN AVERY BY PIN. A row with NO postal code of its own that every source
    # pins east of the Watauga line (longitude > -81.775: Foscoe, Valle Crucis -- the live Boone market),
    # south of 35.995 (Crossnore / southern Avery) or west of -82.02 (Roan Mountain, Tennessee) cannot be
    # an admitted identity whatever else is unknown about it. Refusal only: a pin never ADMITS a building.
    _pins = [(o.get("lat"), o.get("lng")) for o in node.observations
             if o.get("lat") is not None and o.get("lng") is not None]
    if not node.postal and _pins and all(float(g) > -81.78 for _t, g in _pins):
        return OUTSIDE_MARKET, ("NEIGHBOUR_MARKET %s -- no postal code of its own, and every source pins it on "
                                "the Watauga side (Foscoe / Valle Crucis, lng %.4f)"
                                % (GEO.NEIGHBOUR_MARKET, float(_pins[0][1])))
    if not node.postal and _pins and all(float(t) < 35.995 for t, _g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it south of Pineola (Linville "
                                "Falls / Crossnore, lat %.4f), where no admitted postal code can be assumed"
                                % float(_pins[0][0]))
    if not node.postal and _pins and all(float(g) < -82.02 for _t, g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it west of Elk Park toward "
                                "Roan Mountain, Tennessee (lng %.4f)" % float(_pins[0][1]))
    ...
    if node.postal and node.postal[:5] not in zips:
        _fs = (" NEIGHBOUR_MARKET %s --" % GEO.NEIGHBOUR_MARKET) if GEO.is_neighbour_market(node.postal) else ""
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Banner Elk - Sugar - Beech "
...
SOURCE_AUTHORITIES = [
    "OpenStreetMap via the Geofabrik North Carolina extract (2026-09-10 snapshot), reduced to this market's "
    "observation box (ODbL, (c) OpenStreetMap contributors)",
    "https://www.marriott.com/en-us/hotel-sitemap/usa-north-carolina-hotel-sitemap (the brand's own North "
    "Carolina hotel sitemap page)",
    "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (14 Avery County and neighbour city pages, with property cards)",
    ...
    "https://www.bannerelk.com lodging categories (Banner Elk TDA); https://beechmtn.com lodging (Beech Mountain "
    "Visitor Center); https://averycounty.com Accommodations directory (Avery County Chamber); "
    "https://www.highcountryhost.com lodging (Avery listings); https://skisugar.com/lodging/ (Sugar Mountain Resort)",
    "Explore Boone listing service rows in Avery County, owned from the Boone - Blowing Rock build (zero requests)",
...
def build():
    ...
    for n in nodes:
        ...
        if bureau:
            n.name = bureau[0]
    # BANNER ELK: the name the property's OWN page states is taken BEFORE name attachment as well, so a competitor
    # lead naming "The Lodge at Banner Elk" meets the read building rather than its stale map label
    # ("Best Western Mountain Lodge").
    for n in nodes:
        _pn = [o.get("name") for o in n.observations
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 Banner Elk - Sugar Mountain - Beech Mountain census, built from zero "
    ...
    report = OrderedDict([
        ...
        ("cross_market_identity_collisions", cross_market_collisions(rows)),
        ("neighbour_census_names_checked", len(NEIGHBOUR_CENSUS_NAMES)),
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Banner Elk - Sugar - Beech authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "banner-elk-sugar-beech-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "banner_elk_sugar_beech_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "banner_elk_sugar_beech_nc_competitor_gap_matrix_001.json"))
