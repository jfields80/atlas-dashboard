"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phase 26: the SHADOW sealed package (cloned from the Orlando FL V2 helper).

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
------------------------------------------------------------------
Jacksonville is not at the front of the serialized release queue: the current verified live release is WEST PALM
BEACH (deploy 6ab599163f833ab7f48b395d, 33 markets / 2,424 profiles / 2,701 release-index routes / 2,767 served
routes, source e3efa221, resolved mechanically in Phase 2 with --verify-host). The generic lane would reissue
shared, parent-sensitive state this order is forbidden to touch. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/jacksonville-fl/launch_package/``) from the proposed market document, the proposed census, the
  ...
  4. writes the package, the receipt and a report ONLY under ``markets/staging/jacksonville-fl/``.
...
REPORT = PKG / "markets" / "reports" / "jacksonville_fl_shadow_package_001.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "jacksonville_fl_proposed_authority_001.json"
PARTITION_SRC = STAGING / "launch_package" / "jacksonville_fl_final_partition_001.json"
...
def stage_tree():
    ...
    # the partition already lives under LP from jacksonville_fl_final_partition_001; nothing to copy
