"""PTF-PINEHURST-SOUTHERN-PINES-NC-PARALLEL-SOURCE-READY-001 -- Phase 4A: the OpenStreetMap lane.
...
at over an hour on the 428 MB North Carolina extract before it died without writing an index.
...
Output:
  launch_packages/pettripfinder/markets/reports/pinehurst_southern_pines_nc_osm_lane_001.json
"""
...
CONFIG = os.path.join(_DASH, "scripts", "pettripfinder", "discovery", "config",
                      "pinehurst_southern_pines_nc.json")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "pinehurst_southern_pines_nc_osm_lane_001.json")

#: guest_house is kept too: Sandhills inns are often tagged guest_house, and the census
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ...
            ("pbf_snapshot",
             "the 2026-09-10 Geofabrik North Carolina download, reused (hard link) from the Boone - Blowing "
             "Rock run's local copy; the upstream file has since been republished, so this md5 is the "
             "snapshot's own"),
