"""PTF-GREENVILLE-NC-PARALLEL-SOURCE-READY-001 -- Phases 4-8: one identity graph.

Independent discovery lanes ran for Greenville, none of them
authoritative on its own:
  ...
  BRAND_INVENTORY     Marriott's PGV* roster OWNED at zero requests from the
                      ...
                      brand's own destination pages; Red Roof's from its sitemap.
  DESTINATION_ORGANIZATION  the Greenville-Pitt County CVB's own lodging listings.
  PROPERTY_PAGE       the address each property's OWN page states, from the
                      attended pass (Hilton, Marriott, Wyndham, IHG, Red Roof) and the
                      independent / extended-stay properties' own websites.
...
-----------------------
The market contract's postal partition, and nothing else: the Convention Center /
Medical District / PGV Airport postal code 27834 (with Greenville's PO-box postal
codes) and Downtown / ECU 27858 (CORE), Winterville (CORRIDOR), Ayden and Simpson
(FRINGE). Farmville, Grifton, Kinston, Washington, Tarboro, Williamston and
Greenville, South Carolina are OUTSIDE however a row is named.

SHADOW UNTIL REGISTERED
-----------------------
Greenville is built while two other markets must go live first, so the census is
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/greenville-nc.json
  launch_packages/pettripfinder/markets/reports/greenville_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/greenville_nc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import greenville_nc_geography_001 as GEO  # noqa: E402
...
#: REGISTERED by PTF-GREENVILLE-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002; the source-ready census in identity_census_proposed/ is history.
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "greenville-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "greenville-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "greenville_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "greenville_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "greenville_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "greenville_nc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "greenville_nc_destination_roster_001.json")
...
IDENTITY_ONLY_PAGES = [
    ("Affordable Suites Extended Stay Hotel in Greenville, NC", "101 Dansey Road", "Greenville", "27834",
     "(252) 355-5717", "https://www.affordablesuites.com/hotels/affordable-suites-greenville-nc/",
     "8ca58941b32d926000ef8d131b832a120eb814aa23021c326c9bfc927b696ad2",
     "the brand's own property page (attended) states its Hotel JSON-LD address; its only pet wording is "
     "a 'Pet-Friendly Rooms' amenity chip, which is never operative"),
    ("The 5th Street Inn", "1105 E 5th St", "Greenville", "27858", "",
     "https://the5thstreetinn.com/", "0d82ecce2fcd256bf275150c260f625e2c739323365a2dcddff447d8e6923d43",
     "the inn's own site (a bed and breakfast with bookable rooms on its own premises) states its "
     "LocalBusiness JSON-LD address; its home page states no pet policy"),
]

#: Phase 6. Competitor directory leads, as web search result pages summarised the
#: BringFido Greenville NC list and a Memorial Drive / Greenville Boulevard hotel
...
_LEAD_SRC = "web search result: pet friendly hotels Greenville NC (BringFido summaries) and a Memorial Drive / Greenville Boulevard hotel search"
COMPETITOR_LEADS = [
    ("Economy Inn of Greenville NC", _LEAD_SRC),
    ("Holiday Inn Greenville", _LEAD_SRC),
    ("Knights Inn Greenville NC", _LEAD_SRC),
    ("Red Roof Inn Greenville NC", _LEAD_SRC),
    ("Motel 6 Greenville NC - ECU-Medical Center", _LEAD_SRC),
    ("Best Western Plus Suites Greenville NC", _LEAD_SRC),
    ("Candlewood Suites Greenville NC", _LEAD_SRC),
    ("Greenville Motel", _LEAD_SRC),
    ("Americas Best Value Inn Greenville", _LEAD_SRC),
    ("Quality Inn Greenville near University", _LEAD_SRC),
    ("Comfort Inn & Suites Greenville Near Convention Center", _LEAD_SRC),
    ("Rodeway Inn Greenville NC", "web search result: a Choice Hotels Greenville NC brand page (choicehotels.com/north-carolina/greenville/rodeway-inn-hotels/nc455) -- name only; choicehotels.com refused this client"),
]
...
_GENERIC_TOKENS = {
    ...
    # Greenville: the state abbreviation every lead appends ("Knights Inn Greenville NC").
    "nc",
}
...
_IN_MARKET_MUNICIPALITIES = {
    "greenville", "winterville", "ayden", "simpson", "pactolus",
}
_OUT_OF_MARKET_PLACES = {
    "farmville", "grifton", "grimesland", "chocowinity", "bethel", "fountain", "kinston",
    "washington", "tarboro", "williamston", "rocky mount", "wilson", "goldsboro", "new bern",
    "edenton", "jacksonville", "weldon",
}
...
_DIRECTIONALS = {
    ...
    # Greenville's same-brand names differ only in these: "University Med" / "Medical
    # Center" / "University Area" vs a bare "Greenville NC".
    # "nc" is deliberately NOT here: every Greenville name carries it, so it separates
    # nothing, and as an "area" it refused "Knights Inn Greenville NC" for its own building.
    "near", "blvd", "ecu", "med", "carolina",
}
...
CORRIDOR_MUNICIPALITY = {
    "convention-center-medical-district": "greenville", "downtown-ecu": "greenville",
    "winterville": "winterville", "ayden": "ayden", "simpson": "simpson",
}
...
RETIRED_WYNDHAM_FLAGS = ("wingate", "travelodge")
...
def classify(node, zips, name_counts, street_counts):
    ...
    if node.postal and node.postal[:5] not in zips:
        return OUTSIDE_MARKET, ("postal code %s is claimed by no corridor of the Greenville "
                                "contract; Farmville, Grifton, Kinston, Washington, Tarboro, "
                                "Williamston, Rocky Mount, Wilson, New Bern and Greenville SC are "
                                "refused by name in the geography"
                                % node.postal[:5])
    ...
    if name_counts.get((n, normalize_name(node.city)), 0) > 1 and not _hard_read:
...
def build():
    ...
    name_counts = Counter((normalize_name(n.name), normalize_name(n.city)) for n in nodes
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-GREENVILLE-NC-PARALLEL-SOURCE-READY-001 Greenville census, built from zero "
         "under the modern factory and registered to identity_census/ by PTF-GREENVILLE-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Every row carries "
        ...
        ("source_authorities", [
            ...
            "https://www.wyndhamhotels.com/hotels/greenville-north-carolina (the brand's own search)",
            "https://www.ihg.com/destinations/us/en/united-states/north-carolina/<city>-hotels (attended same-origin)",
            "https://www.redroof.com/sitemap.xml (attended same-origin) and the property pages it lists",
            "https://www.visitgreenvillenc.com/sitemap.xml and its lodging listing pages (Greenville-Pitt County CVB)",
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Greenville authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "greenville-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "greenville_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "greenville_nc_competitor_gap_matrix_001.json"))
