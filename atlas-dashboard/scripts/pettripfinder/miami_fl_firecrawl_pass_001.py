"""PTF-MIAMI-FL-HARDENED-SOURCE-READY-001 -- Phase 13, the Firecrawl rung (cloned from the Orlando FL V2 harness).
...
-----------------------------------------
  ...
  DISCOVERED    Choice property routes read from the brand's own city pages through Firecrawl
                (miami_fl_firecrawl_discovery_001, when it exists), in-market towns only, that no static read
...
Output:
  launch_packages/pettripfinder/markets/reports/miami_fl_firecrawl_pass_001.json
"""
...
STATIC_REPORT = os.path.join(REPORTS, "miami_fl_free_static_capture_001.json")
DISCOVERY = os.path.join(REPORTS, "miami_fl_firecrawl_discovery_001.json")
PROBE_PER_FAMILY = 4
#: MIAMI: independents are the market's largest unresolved family, so EVERY escalatable independent static failure
...
_IN_MARKET_CHOICE_TOWNS = ("miami", "miami-beach", "doral", "coral-gables", "miami-springs", "hialeah",
                          "miami-lakes", "north-miami", "north-miami-beach", "aventura", "sunny-isles-beach",
                          "homestead", "florida-city", "kendall", "cutler-bay", "miami-gardens", "opa-locka",
                          "key-biscayne", "bal-harbour", "surfside", "sweetwater", "medley", "south-miami")
SPACING_SECONDS = 2.0
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="miami_fl_firecrawl_001")
