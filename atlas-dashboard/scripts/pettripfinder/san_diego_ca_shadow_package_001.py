"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 26: the SHADOW sealed package (cloned from the Orlando FL V2 helper).

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
------------------------------------------------------------------
San Diego is not at the front of the serialized release queue: the current verified live release is JACKSONVILLE
(deploy 6ab6c8798bd9daf5038ae3e5, 34 markets / 2,519 profiles / 2,807 release-index routes / 2,874 served
routes, source e9a059e5, resolved mechanically in Phase 1 with --verify-host). The generic lane would reissue
shared, parent-sensitive state this order is forbidden to touch. So this helper:

  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/san-diego-ca/launch_package/``) from the proposed market document, the proposed census, the
  ...
  4. writes the package, the receipt and a report ONLY under ``markets/staging/san-diego-ca/``.
...
REPORT = PKG / "markets" / "reports" / "san_diego_ca_shadow_package_001.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "san_diego_ca_proposed_authority_001.json"
PARTITION_SRC = STAGING / "launch_package" / "san_diego_ca_final_partition_001.json"
SEALED_AT = "2026-09-25T02:00:00Z"
...
def stage_tree():
    ...
    # the partition already lives under LP from san_diego_ca_final_partition_001; nothing to copy
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the CURRENT live release (Jacksonville, deploy 6ab6c8798bd9...). That is what this "
         "package was validated against. Once any later market is live, FAST rule N fails this package BY "
         "DESIGN; the registration order -- when San Diego reaches the front of the release queue -- re-seals the "
