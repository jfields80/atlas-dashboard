"""PTF-JACKSONVILLE-NC-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.
...
yet deployed) must become the live parent before Jacksonville can be released.
...
to touch and that would conflict with Fayetteville's own registration of the
same documents.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/jacksonville-nc/launch_package/``) from the proposed
  ...
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/jacksonville-nc/``: never ``markets/packages/`` or
...
Fayetteville is live, rule N FAILS this package on purpose; the registration
...
REPORT = PKG / "markets" / "reports" / "jacksonville_nc_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "jacksonville_nc_proposed_authority_002.json"
SEALED_AT = "2026-09-13T16:00:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         ...
         "registration order re-seals the same committed inputs against the Fayetteville-live parent as a NEW "
