"""PTF-MIAMI-FL-HARDENED-SOURCE-READY-001 -- Phase 9A: the Florida DBPR public-lodging REGISTRY lane.
...
units. That makes the registry the census backbone for Greater Miami: a licensed HOTEL (rank HOTL) or MOTEL
...
-------------------------------------------------
  ...
  TAPT   transient apartment        a census LEAD only when its own name reads as a hotel / suites /
                                    ...
                                    single units dominate this class around Miami Beach)
...
Inputs (downloaded by this order, gitignored, hashed here):
  data/miami_fl/dbpr/hrlodge1.csv .. hrlodge7.csv
Output:
  launch_packages/pettripfinder/markets/reports/miami_fl_dbpr_lane_001.json
"""
...
MARKET_ID = "miami-fl"
SRC_DIR = os.path.join(_DASH, "data", "miami_fl", "dbpr")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports", "miami_fl_dbpr_lane_001.json")
...
RETRIEVED_AT = "2026-09-19T19:58:00Z"
...
OBSERVED_COUNTIES = {"Dade", "Miami-Dade", "Broward", "Palm Beach", "Monroe"}
...
#: Refused neighbours this lane OBSERVES row by row: every OUTSIDE postal code the geography names (Fort Lauderdale,
#: Hollywood, Hallandale, Dania Beach, Pompano / Deerfield, Broward suburbs, Boca Raton, West Palm / Palm Beach,
#: the Keys). Broward / Palm Beach / Monroe licences outside those named codes are COUNTED by county for the South
#: Florida boundary audit and never enumerated as leads.
...
_BOUNDARY_CITIES = ("FORT LAUDERDALE", "HOLLYWOOD", "DANIA BEACH", "DANIA", "HALLANDALE BEACH", "HALLANDALE",
                    "WEST PALM BEACH", "PALM BEACH", "KEY LARGO", "ISLAMORADA", "KEY WEST", "MARATHON")
...
def build():
    ...
    for i in range(1, 8):
        ...
        obs, _skipped = DBPR.parse_hrlodge_csv(
            text, observed_at="2026-09-19", retrieved_at=RETRIEVED_AT,
