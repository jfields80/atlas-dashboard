"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phase 13: the independents' and plain-client brands' operative quotes.
...
Output:
  launch_packages/pettripfinder/markets/staging/columbia-sc/raw_captures/brand_pages.json
"""
...
RAW = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "columbia-sc", "raw_captures")
OUT = os.path.join(RAW, "brand_pages.json")
POLICY_LANE = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                           "columbia_sc_policy_pages_lane_001.json")
...
ROWS = [
    # pet-friendly
    ("chesnut-cottage", "INDEPENDENT", "Chesnut Cottage Bed & Breakfast", "1718 Hampton Street", "Columbia", "29201", "",
     "15828ef39315", "Pet are allowed only in the Garden Suite.",
     # a room restriction ("only in the Garden Suite") beside the acceptance; no fee, weight, count or species stated
     {"pets_allowed": True}, None),
    ("the-lantern-hotel", "INDEPENDENT", "The Lantern Hotel Columbia", "1001 Senate Street", "Columbia", "29201", "",
     "85ffba64d142",
     "The Lantern Hotel proudly welcomes pets. Guests may bring up to two dogs, with a combined weight of 50 pounds or less. "
     "A flat pet fee of $150 applies per stay.",
     {"pets_allowed": True, "pet_count_limit": 2, "species_allowed": ["dog"], "weight_limit": 50.0,
      "weight_limit_unit": "lb", "weight_basis": "combined", "pet_fee": 15000, "fee_currency": "USD",
      "fee_basis": "per_stay"}, None),
    ("woodspring-suites-columbia-fort-jackson", "WOODSPRING", "WoodSpring Suites Columbia Fort Jackson",
     "931 Spears Creek Court", "Elgin", "29045", "", "a7476ffba8bd",
     "Pets allowed. A Non refundable pet registration fee of 50.00 USD and 10.00 USD per night per pet. Max 75 lbs, 2 dogs "
     "per room. No cats.",
    ...
    ("woodspring-suites-columbia-irmo", "WOODSPRING", "WoodSpring Suites Columbia - Irmo", "941 Columbiana Drive", "Irmo",
     "29063", "", "3517ca984391",
...
DOCUMENT_URL = {
    "chesnut-cottage": "https://www.chesnutcottage.com/policies",
    "the-lantern-hotel": "https://www.lanterncolumbia.com/faq",
    "woodspring-suites-columbia-fort-jackson":
        "https://www.woodspring.com/extended-stay-hotels/locations/south-carolina/columbia/woodspring-suites-columbia-fort-jackson",
    "woodspring-suites-columbia-irmo":
        "https://www.woodspring.com/extended-stay-hotels/locations/south-carolina/irmo/woodspring-suites-columbia-irmo",
}
...
REDROOF_PAYLOAD = ("redroof_rows.json", "8577184139a68f3193f8951e58055b86c72728e80278e2cc7c63e808f1cae64b")
...
BROWSER_READ_ROWS = [
    ("stayapt-suites-columbia-irmo-harbison", "STAYAPT", "stayAPT Suites Columbia-Irmo/Harbison", "851 Columbiana Drive", "Irmo",
     "29063", "https://www.stayapt.com/hotels/stayapt-suites-columbia-irmo-harbison/",
     "6ab01494ebf96ba435466e6a0995853856a2e00209785087ea6d5d98f1ff5494", 253486,
     "Both small pets and service animals are always welcome in our pet-friendly rooms. Two-animal maximum per room. There is "
     "a $25 non-refundable charge per pet, per day, with a maximum of $150 per pet. There is no charge for service animals.",
     # a daily charge with a per-pet cap is not one fee; "two-animal maximum" counts service animals too, so no pet count
     {"pets_allowed": True}, True),
]
...
def build():
    ...
    for seed, brand, name, street, city, postal, phone, prefix, quote, ext, conflict in ROWS:
        files = sorted({os.path.basename(f): f for f in
                        glob.glob(os.path.join(ACQ, "columbia_sc_*", prefix + "*.bin"))}.values())
        if len(files) != 1:
