"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phase 12G: independents' own policy / FAQ / pet pages (plain client).
...
The sentences are the candidate quote; facts are read by the shared reader in jacksonville_fl_policy_reads_001 and
judged by the first-party gate and the negation guard in the clean authority. Documents persisted by sha256 under
data/acquisition/jacksonville_fl_policy_pages_001/.

Output:
  launch_packages/pettripfinder/markets/staging/jacksonville-fl/raw_captures/policy_pages_rows.json
"""
...
B.DOCS = os.path.join(_DASH, "data", "acquisition", "jacksonville_fl_policy_pages_001")
PKG = os.path.join(_DASH, "launch_packages", "pettripfinder")
ROUTING = os.path.join(PKG, "markets", "reports", "jacksonville_fl_routing_001.json")
RAW = os.path.join(PKG, "markets", "staging", "jacksonville-fl", "raw_captures")
OUT = os.path.join(RAW, "policy_pages_rows.json")
#: The SAME reader serves the independents' closure lane (jacksonville_fl_places_policy_pages_001). Both of its write
