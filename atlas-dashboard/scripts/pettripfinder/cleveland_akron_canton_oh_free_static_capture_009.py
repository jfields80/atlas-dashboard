"""PTF-CLEVELAND-AKRON-CANTON-HARDENED-REVALIDATION-001 -- Phase 9 / 10.

Free FIRST-PARTY static capture through the canonical lanes:

    acquisition.direct_http_capture.run_attempt   (one HTTPS GET, the same
        gates the paid lanes use: denial markers -> page health -> identity
        read + assessment -> policy locator -> reader; artifacts persisted as
        rendered.html / page-text.txt / policy-block.txt / locator.json)
    acquisition.market_observation_store.observation_for   (reader ->
        publication_grade.assess -> ptf-market-observation-store record)

No vendor, no browser, no price. Every target is captured at most ONCE per
run; a rerun reuses the attempt on disk. Targets come from the phase-7 lane
FREE_STATIC_QUALIFIED, from a phase-8 routing file, or -- for the phase-10
live contradiction audit -- from the live authority itself (``--live``).

Nothing is written to authority. Output is one report of observations and
classifications keyed on the page's own sha256.
"""
...
BRANDS = [
    ...
    ("WYNDHAM", r"wyndham|baymont|days inn|super 8|ramada|travelodge|la quinta|microtel|howard johnson|hawthorn|americinn"),
...
def text_bound_read(attempt_root: Path, crow, brand: str):
    """Fallback over a document the canonical gate DECLINED (kept under
    declined-01/). The page's own text is bound to the census row on street
    number + postal or telephone, and only then is the policy block located
    and read. Identity here is TEXT-bound, not structured-data-bound: the
    result is a candidate for attended confirmation, never a clean row."""
    ...
    out = OrderedDict([("street_number_agrees", street_ok), ("postal_agrees", postal_ok), ("phone_agrees", phone_ok), ("text_bound", bound),
    ...
    try:
        ...
        out["reader"] = OrderedDict([("found", True), ("walk", walk), ("block_chars", len(hit.text)), ("pets_allowed", result.extraction.get("pets_allowed")),
                                     ("pets_allowed_quote", (getattr(reading, "pets_allowed_quote", "") or "")[:300]), ("extraction", result.extraction),
                                     ("withheld", dict(result.withheld)), ("evidence_quotes", [e.get("quote", "")[:300] for e in result.evidence][:6]),
...
def targets_from_args(args, census, policy, exclusions, routing):
    targets = []
    if args.lane_from_007:
        rebuild = read_json(os.path.join(REPORTS, f"{MARKET_ID.replace('-', '_')}_unresolved_rebuild_007.json"))
        for r in rebuild["rows"]:
            if r["lane"] in args.lane_from_007 and r.get("url"):
                targets.append(OrderedDict([("identity_key", r["identity_key"]), ("url", r["url"]), ("origin", "PHASE_7:" + r["lane"])]))
    if args.routes:
        ...
        for r in doc.get("routes_recovered") or doc.get("rows") or []:
            if r.get("identity_key") and r.get("url") and r.get("status", "ROUTING_CONFIRMED") == "ROUTING_CONFIRMED":
                targets.append(OrderedDict([("identity_key", r["identity_key"]), ("url", r["url"]), ("origin", "PHASE_8")]))
    if args.live:
        for p in policy:
            targets.append(OrderedDict([("identity_key", p["identity_key"]), ("url", p.get("source_url") or ""), ("origin", "LIVE_PET_FRIENDLY")]))
        for e in exclusions:
            targets.append(OrderedDict([("identity_key", ptf_identity_key(e["canonical_name"])), ("url", e.get("official_url") or e.get("source_url") or ""), ("origin", "LIVE_VERIFIED_NO_PETS")]))
    # de-duplicate on (identity_key, url)
    seen = set()
    out = []
    for t in targets:
...
def build(args) -> OrderedDict:
    ...
    targets = targets_from_args(args, census, policy, exclusions, routing)
    ...
    for i, t in enumerate(targets):
        ...
        target = BC.CaptureTarget(
            slug=slug, hotel=name, requested_url=t["url"], property_code=DEDUP.property_code({"official_url": t["url"]}),
            market_id=MARKET_ID, normalized_name=key, identity_key=key, street_identity=(crow or {}).get("street_identity", ""),
            expected_postal_code=((crow or {}).get("postal_code") or "")[:5], expected_street=(crow or {}).get("address", ""),
            expected_phone=(crow or {}).get("phone", ""), expected_locality=(crow or {}).get("city", ""), identity_brand=brand,
        ...
        if record_path.is_file() and not args.refetch:
            ...
            requests += 1
            a = as_plain(attempt)
            rec = OrderedDict([("outcome", a.get("outcome")), ("final_url", a.get("final_url")), ("title", a.get("title")), ("detail", a.get("detail")),
                               ("body_chars", a.get("body_chars")), ("identity", a.get("identity")), ("payload_keys", sorted((payload or {}).keys()) if isinstance(payload, dict) else None),
        ...
        row = OrderedDict([
            ("identity_key", key), ("canonical_name", name), ("brand", brand), ("origin", t["origin"]), ("requested_url", t["url"]),
            ("live_state", "PET_FRIENDLY_LIVE" if key in policy_by_key else "VERIFIED_NO_PETS_LIVE" if key in excl_keys else "UNRESOLVED_OR_NEW"),
            ("outcome", rec.get("outcome")), ("final_url", rec.get("final_url")), ("title", (rec.get("title") or "")[:160]), ("detail", (rec.get("detail") or "")[:300]),
            ("identity_assessment", rec.get("identity")), ("page_sha256", sha256_file(html_path)), ("artifact_dir", str(attempt_dir.relative_to(_DASH)) if attempt_dir.is_dir() else ""),
        ])
        if rec.get("outcome") == "VALID" and (attempt_dir / "policy-block.txt").is_file():
            result = {"identity_key": key, "canonical_name": name, "brand": brand, "corridor": (crow or {}).get("corridor", ""), "source_url": t["url"],
                      "outcome": "VALID", "final_url": rec.get("final_url") or t["url"], "artifact_dir": str(attempt_dir),
                      "identity_confirmed": bool((rec.get("identity") or {}).get("confirmed", True)), "locator_strategy": ""}
            try:
                ...
                row["observation"] = OrderedDict([
                    ("extraction", ext), ("evidence", ((obs or {}).get("observation") or {}).get("evidence")),
                    ("withheld_fields", (obs or {}).get("withheld_fields")), ("publication_grade", grade), ("refusal_reason", refusal),
                    ("reader_provenance", (obs or {}).get("reader_provenance")), ("membrane", (obs or {}).get("membrane")),
                ])
            ...
            row["classification"] = {"POLICY_NOT_FOUND": "SOURCE_SILENT_STATIC", "UNHYDRATED": "NEEDS_ATTENDED_RENDER", "ACCESS_DENIED": "ACCESS_BLOCKED_PLAIN_CLIENT",
                                     "IDENTITY_MISMATCH": "IDENTITY_MISMATCH", "BLANK_PAGE": "NEEDS_ATTENDED_RENDER", "NAVIGATION_FAILED": "TRANSPORT_FAILED",
                                     "UNEXPECTED_PAGE": "UNEXPECTED_PAGE", "CAPTURE_FAILED": "TRANSPORT_FAILED"}.get(oc, "OTHER:" + oc)
        # text-bound fallback over a document the canonical gate declined
        ...
        # live comparison (phase 10)
        if row["live_state"] != "UNRESOLVED_OR_NEW":
            cls = row["classification"]
            if cls == "CLEAN_PET_FRIENDLY_CANDIDATE" or cls == "PET_FRIENDLY_READ_NOT_PUBLICATION_GRADE":
                ...
                if row["live_state"] == "PET_FRIENDLY_LIVE":
                    ...
                    fee_new = ext.get("pet_fee") if isinstance(ext.get("pet_fee"), int) else (ext.get("pet_fee") or {}).get("amount_minor") if isinstance(ext.get("pet_fee"), dict) else None
                ...
                row["live_audit"] = "CURRENTLY_CORRECT_TEXT_BOUND" if row["live_state"] == "PET_FRIENDLY_LIVE" else "WRONG_LIVE_POLICY_CANDIDATE_TEXT_BOUND"
            elif cls == "NO_PETS_READ_TEXT_BOUND":
                row["live_audit"] = "CURRENTLY_CORRECT_TEXT_BOUND" if row["live_state"] == "VERIFIED_NO_PETS_LIVE" else "WRONG_LIVE_POLICY_CANDIDATE_TEXT_BOUND"
            elif cls in ("IDENTITY_TEXT_BOUND_POLICY_SILENT",):
    ...
    return OrderedDict([
        ("schema", SCHEMA), ("work_order", WORK_ORDER), ("phase", args.phase_label), ("market_id", MARKET_ID), ("run_id", run_id),
        ("as_of", time.strftime("%Y-%m-%d", time.gmtime())),
        ("lane", "direct_http (first-party static; no vendor, no browser, no price)"),
        ("paid_provider_calls", 0), ("usd_spent", 0.0), ("free_http_requests_this_run", requests), ("targets", len(targets)),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default=f"{MARKET_ID.replace('-', '_')}_free_static_009")
    ap.add_argument("--out", default=os.path.join(REPORTS, f"{MARKET_ID.replace('-', '_')}_free_static_capture_009.json"))
    ap.add_argument("--phase-label", default="9 -- zero-cost policy capture (static)")
    ap.add_argument("--lane-from-007", nargs="*", default=None)
