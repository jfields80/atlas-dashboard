"""PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 9A: the Florida DBPR public-lodging REGISTRY lane.
...
units. That makes the registry the census backbone for Tampa Bay: a licensed HOTEL (rank HOTL) or MOTEL
...
-------------------------------------------------
  ...
  TAPT   transient apartment        a census LEAD only when its own name reads as a hotel / suites /
                                    ...
                                    single units dominate this class around Tampa Bay's beaches)
...
Inputs (downloaded by this order, gitignored, hashed here):
  data/tampa_fl_v2/dbpr/hrlodge1.csv .. hrlodge7.csv
Output:
  launch_packages/pettripfinder/markets/reports/tampa_fl_v2_dbpr_lane_001.json
"""
...
MARKET_ID = "tampa-fl"
SRC_DIR = os.path.join(_DASH, "data", "tampa_fl_v2", "dbpr")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports", "tampa_fl_v2_dbpr_lane_001.json")
...
RETRIEVED_AT = "2026-09-15T20:53:00Z"
...
OBSERVED_COUNTIES = {"Hillsborough", "Pinellas", "Pasco", "Manatee", "Sarasota", "Polk", "Hernando"}
...
#: Refused neighbours this lane OBSERVES row by row (the order's "careful evaluation" towns next to the
#: admitted corridors). Orlando (a separate, already-built market) is named and refused by the geography but
#: not enumerated here: its licences are not Tampa leads, and listing them would count another market's
#: inventory as discovered.
OBSERVED_OUTSIDE = ("Sarasota", "Bradenton", "Anna Maria Island", "Lakeland", "Spring Hill", "Lithia")
