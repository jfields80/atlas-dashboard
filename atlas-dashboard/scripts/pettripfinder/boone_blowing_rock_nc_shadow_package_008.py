"""PTF-BOONE-BLOWING-ROCK-NC-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Production deployment is blocked at Netlify, and Fayetteville (founder-authorized, not
yet deployed), then Jacksonville, Greenville and Atlanta (source-ready, not registered) must become
the live parent before Outer Banks can be released.
...
to touch and that would conflict with Fayetteville's, Jacksonville's, Greenville's and Atlanta's own registration of the
same documents.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/boone-blowing-rock-nc/launch_package/``) from the proposed
  ...
  3. runs the FAST lane (rules A-O) over it, which builds the joining market's
     ...
     current live parent (Asheville) -- no whole-site candidate is assembled;
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/boone-blowing-rock-nc/``: never ``markets/packages/`` or
...
WHAT BINDS TO THE ASHEVILLE PARENT, AND HOW IT IS RECONCILED
------------------------------------------------------------
...
Fayetteville (and later Jacksonville, Greenville and Atlanta) is live, rule N FAILS this package on purpose; the registration
...
REPORT = PKG / "markets" / "reports" / "boone_blowing_rock_nc_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "boone_blowing_rock_nc_proposed_authority_002.json"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the CURRENT live release (Asheville, deploy 6aa60a3e...). That is what this "
         "package was validated against. Once Fayetteville is live, FAST rule N fails this package BY DESIGN; the "
         "registration order -- after Fayetteville, Jacksonville, Greenville AND Atlanta are live -- re-seals the same committed inputs against that parent as a NEW "
