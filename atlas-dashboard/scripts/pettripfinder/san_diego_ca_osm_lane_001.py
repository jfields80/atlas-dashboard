"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 7: the OpenStreetMap lane.

Reads the Geofabrik Southern California extract (ODbL, (c) OpenStreetMap contributors) and keeps every
``tourism=hotel / motel / guest_house / hostel / chalet / apartment`` element -- node, way or multipolygon
relation -- inside this market's OBSERVATION box, which reaches north past the Orange and Riverside County lines
(San Clemente, Temecula), east to Borrego Springs and south across the border into Tijuana, so the boundary audit
counts what it refuses instead of being blind to it.

WHY AN EXTRACT, AND WHY NOT OVERPASS
------------------------------------
No California extract was owned by this repository. The order names Overpass as a lane, so ONE Overpass query
over the observation box was attempted first against all three public instances (overpass-api.de,
overpass.kumi.systems, overpass.private.coffee): every one answered 504 Gateway Timeout on 2026-09-25. The
Geofabrik SoCal extract (``socal-260924.osm.pbf``, 670,521,236 bytes) was then downloaded ONCE, free, and is
read locally; it is the same instrument every earlier market used (Florida, Georgia, the Carolinas).
...
Output:
  launch_packages/pettripfinder/markets/reports/san_diego_ca_osm_lane_001.json
"""
...
PBF = os.path.join(_DASH, "data", "osm_extracts", "socal-latest.osm.pbf")
...
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "san_diego_ca_osm_lane_001.json")

#: guest_house, hostel, chalet and apartment are kept too: San Diego's beach cottages, Mission Beach vacation
#: rentals and serviced-apartment operators are often tagged that way, and the census must SEE a rental to refuse
#: it by decision.
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ("pbf", "data/osm_extracts/socal-latest.osm.pbf"),
            ("url", "https://download.geofabrik.de/north-america/us/california/socal-260924.osm.pbf"),
            ("pbf_md5", md5.hexdigest()),
            ("pbf_bytes", os.path.getsize(PBF)),
            ("pbf_snapshot",
             "downloaded once by this order on 2026-09-25 (Geofabrik socal-260924, Last-Modified 2026-09-25T04:30:48Z, "
             "670,521,236 bytes) after all three public Overpass instances answered 504 to the one box query"),
            ("overpass_attempts", [["https://overpass-api.de/api/interpreter", "504 Gateway Timeout"],
                                   ["https://overpass.kumi.systems/api/interpreter", "504 Gateway Timeout"],
        ...
        ("paid_provider_calls", 0), ("free_http_requests", 4),
