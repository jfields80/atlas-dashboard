"""PTF-TOLEDO-OH-DEPLOYMENT-AND-LAUNCH-AUTHORIZATION-003 -- the founder's Toledo launch decision.

    python -m scripts.pettripfinder.toledo_oh_launch_participation_003
    python -m scripts.pettripfinder.toledo_oh_launch_participation_003 --write

This is the ONE lever that moves a market from source-ready to live. Until it
flips, Toledo's 17 published profiles exist in authority and reach no reader;
PTF-TOLEDO-OH-PROMOTION-AND-APPLICATION-002 proved exactly that by reassembling
the bundle with participation off and reproducing the live digest byte for byte.

THIS RECORDS A DECISION, IT DOES NOT MAKE ONE
---------------------------------------------
The founder authorized the flip in this order, in the words "YES -- authorize
Toledo participation" and "Toledo is authorized to move from NOT LIVE -> LIVE".
...
WHY THE PROMOTION PACKET'S BUNDLE SHA IS NOT THE LAUNCH BUNDLE SHA
------------------------------------------------------------------
The promotion packet pins bundle ca59f371..., which is the PRE-FLIP bundle --
ten markets, 786 profiles, Toledo contributing nothing. A launch cannot deploy
that bundle, because deploying it would change nothing. The flip below is what
makes Toledo's routes exist, and the bundle assembled after it is necessarily a
different one. That is the Cincinnati shape exactly: the promotion order
prepares the candidate a flip WOULD produce, and the launch order flips and
reassembles. The founder was shown both digests and bound the authorization to
the reassembled candidate.
...
---------------------------------------------
The Cincinnati record this one supersedes is appended to the lineage with its
...
NO OTHER MARKET MOVES. Detroit is assemblable at 121 published and stays NOT
authorized: the founder's decision named Toledo and nothing else, and a launch
record that quietly carried a second market would be the worst kind of defect
this file could hold. Fort Wayne and Lexington are not registered at all.
"""
...
PROMOTION_ORDER = "PTF-TOLEDO-OH-PROMOTION-AND-APPLICATION-002"
SOURCE_COMMIT = "c9971727ecf255e7173db69468e8c0bebaa4055b"

PACKAGE = _REPO_ROOT / "launch_packages" / "pettripfinder"
PATH = LP.PARTICIPATION_PATH

REASON = (
    "Founder authorizes Toledo to participate in the next production assembly, "
    "joining as the ELEVENTH market with 17 pet-friendly profiles. Every other "
    "market's decision is unchanged, and detroit-ann-arbor-mi remains NOT "
    "authorized for launch even though it is assemblable at 121 published -- the "
    "decision named Toledo and nothing else. %s built this market from zero to a "
    "54-identity census, promoted 17 published and 9 verified-no-pets, proved the "
    "promotion moved no byte of the live bundle while participation stayed off, "
    "and prepared the exact candidate a flip would produce. This row consumes "
    "that preparation. The founder was shown that the promotion packet pins the "
    "PRE-FLIP bundle and that a launch necessarily produces a different digest, "
    "and bound the authorization to the reassembled candidate."
    % PROMOTION_ORDER)
...
def _basis() -> Dict:
    ...
    return OrderedDict((
        ...
        ("founder_identity_holds",
         "Founder ruling TOLEDO-R3 holds BOTH policy reads at 10667 / 10667B "
         "Fremont Pike, where address_key collapses a building letter and two "
         "Wyndham brands state opposite policies. Neither is published under the "
         "merged identity and this launch does not resolve the split."),
...
def flip(write: bool = False) -> Dict:
    ...
    for market in document["markets"]:
        ...
        market["note"] = (
            "17 pet-friendly profiles and 9 verified-no-pets over a 54-identity "
            "census built from zero; release contract verifies with zero "
            ...
            "show_in_sitemap=false, exactly as live Louisville and Cincinnati "
            "are -- those flags govern the hub's place in navigation, not whether "
            "the market's profile URLs are indexed, and its 21 routes do enter "
            "sitemap.xml. 2 of its 13 corridors reach their own publication "
            "minimum and the rest are suppressed, which is a threshold and not a "
            "gap." % (PROMOTION_ORDER, WORK_ORDER))
    if hit != 1:
