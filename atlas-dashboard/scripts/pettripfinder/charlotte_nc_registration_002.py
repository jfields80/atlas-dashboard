"""PTF-CHARLOTTE-NC-ZERO-TO-LIVE-BENCHMARK-001 -- Phase 8: register Charlotte.
...
from, and validates both against the contracts that own them:
  ...
  1. the committed POLICY PACKAGE  launch_packages/pettripfinder/hotel_policy_facts_charlotte-nc.json
  ...
  2. the PROPOSED AUTHORITY        launch_packages/pettripfinder/charlotte_nc_proposed_authority_002.json
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/charlotte-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "charlotte_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census", "charlotte-nc.json")
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_charlotte-nc.json")
AUTHORITY_OUT = os.path.join(PKG, "charlotte_nc_proposed_authority_002.json")
...
def build():
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order registers Charlotte without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Greater Charlotte, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Charlotte's committed policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Charlotte's authority as this order proposes it. Registration makes the market "
