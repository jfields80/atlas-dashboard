"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phases 16-20: the clean set (cloned from the Savannah GA helper).
...
REFUSAL (for a no-pets record), read from a surface this order named per family:
    ...
    WYNDHAM    the client-rendered .pet-policy-desc element
    INDEPENDENT the property's own website, verbatim, with the page's sha256
...
NEGATION SAFETY (PHASE 18). The shared reader is negation-blind on some refusals: measured in this order,
"Is the Holiday Inn pet-friendly? We do not allow pets at this hotel." and "This is a non-pet friendly hotel." read as
ACCEPTANCES and pass the first-party gate as pet-friendly. Every candidate PET-FRIENDLY quote is therefore checked
against an explicit human-readable refusal pattern first; a quote that contains one is held as NEGATION_HOLD and never
published as acceptance. A refusal the shared reader cannot read at all ("We do not allow pets", "Pets are
prohibited") is held as NEGATION_HOLD_UNREADABLE_REFUSAL rather than published as verified-no-pets. The shared reader
is not modified.

Nothing here fetches. Nothing here publishes. Output is market-local.

Output:
  launch_packages/pettripfinder/markets/reports/orlando_fl_v2_clean_authority_001.json
"""
...
#: PHASE 18: explicit human-readable refusals. Checked on every candidate pet-friendly quote BEFORE the shared gate.
_NEGATION = re.compile(
    r"(\bnot\s+(?:a\s+)?pet[- ]friendly\b|\bnon[- ]pet[- ]friendly\b|\bpet[- ]free\b|\b(?:do|does|will)\s+not\s+(?:allow|accept|permit)\s+(?:any\s+)?(?:pets?|dogs?|animals?)"
    r"|\bpets?\s+(?:are|is)\s+(?:not\s+(?:allowed|permitted|accepted)|prohibited|forbidden)\b|\bno\s+pets?\b(?!\s+fee)"
    r"|\bpets?\s+not\s+allowed\b|\bservice\s+(?:animals?|dogs?|pets?)\s+only\b|\bonly\s+service\s+(?:animals?|dogs?)\b"
    r"|\bcannot\s+accommodate\s+pets\b|\bnot\s+pet\s+friendly\b|\bstrictly\s+prohibited\b)", re.I)
...
SURFACE = {
    ...
    "IHG": "the brand's own FAQ answer to 'Can I bring my pet to X?'",
    "INDEPENDENT": "the property's own website (the page whose sha256 is recorded)",
    "MY_PLACE": "the brand's own property page, pet paragraph",
    "RED_ROOF": "the property's own page, Pet Policy sentence",
    "EXTENDED_STAY": "the property's own page, Pet Policy block",
    "WYNDHAM": "the client-rendered .pet-policy-desc element on the property's own page",
    "CHOICE": "the property page's own 'Pets' item (label and general policy words)",
    "HYATT": "the property page's own pet module",
    "BEST_WESTERN": "the property page's own FAQ answer to 'Q. Is the X Pet Friendly?'",
    "WOODSPRING": "the property's own page, Pet Policy block",
    "SONESTA": "the property's own page, the bounded policy container the shared reader located",
    "LOEWS": "the property's own page, the bounded policy container the shared reader located",
    "UNIVERSAL": "the property's own page, the bounded policy container the shared reader located",
    "DRURY": "the property's own page, Pet Policy block",
}
...
def _keys_for(h):
    ...
    Raleigh buildings hold two separately bookable hotels each, and a
...
def rows_from_attended(doc):
    for r in doc.get("rows", []):
        ...
        yield OrderedDict([
            ("lane", r.get("lane") or "ATTENDED_BROWSER"), ("brand", r.get("brand")),
            ...
            ("first_party_conflict", r.get("first_party_conflict")),
            ("in_market", r.get("in_market")),
            ("document_sha256_kind", r.get("document_sha256_kind")),
        ])
...
def build():
    ...
    for r in reads:
        ...
        if r.get("in_market") is False and r.get("identity_confirmed"):
            reject("OUTSIDE_MARKET_READ", "the page's own postal code is outside the corridor registry")
            continue
        ...
        if h.get("classification") != "TRUE_HOTEL_IDENTITY":
            ...
            continue
        if r.get("first_party_conflict"):
            reject("FIRST_PARTY_CONFLICT",
                   "the property's own site states both acceptance and refusal: %s. Two "
                   "first-party statements that disagree publish neither"
                   % r["first_party_conflict"][:400])
            continue
        if not quote.strip():
            ...
            continue
        # A SERVICE-ANIMAL SENTENCE IS NOT A REFUSAL. Two Raleigh DoubleTrees
        ...
        if (ext.get("pets_allowed") is False
            ...
            continue
        if "pets_allowed" not in ext and _NEGATION.search(quote):
            reject("NEGATION_HOLD_UNREADABLE_REFUSAL",
                   "the page's own words state a refusal (%r) that no parser read as a fact; held, never published"
                   % _NEGATION.search(quote).group(0))
            continue
        ...
        # PHASE 18 NEGATION SAFETY, before the shared gate is consulted.
        if ext.get("pets_allowed") is True and _NEGATION.search(quote):
            reject("NEGATION_HOLD",
                   "the quoted first-party words contain an explicit refusal (%r) while the parsed facts read an "
                   "acceptance; the shared reader is negation-blind on this form, so acceptance is NOT published"
                   % _NEGATION.search(quote).group(0))
            continue
        # THE GATE'S OWN READING, applied here rather than discovered at FAST.
        # "Pets are not allowed. Only certified service dogs are allowed." is a
        # refusal to a person, but the shared reader does not read it as one, and
        # the first-party binding gate refuses what the reader cannot read. The
        # row is HELD with the gate's class instead of rewording the hotel's
        # sentence until the reader agrees.
        verdict, why = FPB.classify_quote(
            quote, kind=FPB.KIND_PET_FRIENDLY if ext.get("pets_allowed") else FPB.KIND_NO_PETS)
        if verdict != FPB.ELIGIBLE:
            if ext.get("pets_allowed") is False and _NEGATION.search(quote):
                reject("NEGATION_HOLD_UNREADABLE_REFUSAL",
                       "the page states a refusal in plain words (%r) that the shared gate's reader does not read as "
                       "operative (%s); held, never reworded and never published as verified-no-pets"
                       % (_NEGATION.search(quote).group(0), verdict))
            else:
                reject(verdict, "the first-party binding gate's reader does not read this quote as "
                                "operative: %s" % why[:200])
            continue
    ...
    # TWO PET-FRIENDLY HOTELS IN ONE BUILDING (first measured in Atlanta). Hilton Garden Inn and Homewood
    ...
    # a pet-friendly hotel that shares its building with a VERIFIED_NO_PETS hotel (TownePlace Suites and
    ...
    from scripts.pettripfinder.orlando_fl_v2_census_reconciliation_001 import directional_conflict
    # (from the Savannah registration order) a record is released ONLY when a ruling that validates
    ...
    # stays held -- which, by founder order, includes every street-directional collision.
    ...
    for r in co_located:
        ...
        mine = r["identity_signals"].get("address_on_page") or ""
        k = address_key(mine, (r["identity_signals"].get("postal_code") or "")[:5])
        peers = [p for p in co_located if p is not r and address_key(p["identity_signals"].get("address_on_page") or "",
                 (p["identity_signals"].get("postal_code") or "")[:5]) == k]
        if peers and all(directional_conflict(mine, p["identity_signals"].get("address_on_page") or "") for p in peers):
            # two DIFFERENT buildings the shared address_key cannot tell apart ("201 E. Bay St." and
            # "201 West Bay Street"). The shared listing builder would dedupe them to one row exactly as it does a
            # dual-brand building, and the shared key may not be changed here -- a factory blocker, recorded.
            row["classification"] = "ADDRESS_KEY_DIRECTIONAL_COLLISION"
            row["why"] = ("a second pet-friendly hotel on the opposite directional of the same street and house number "
                          "(%r vs %r) collapses to the same shared address_key; the shared listing builder would dedupe "
                          "the two distinct buildings, so both are held until the shared identity layer distinguishes "
                          "street directionals or a registration-order resolution records the pair. The operative quote is "
                          "retained: %r" % (mine, peers[0]["identity_signals"].get("address_on_page"), r["operative_quote"][:160]))
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "orlando_fl_v2_clean_authority_001.json"))
