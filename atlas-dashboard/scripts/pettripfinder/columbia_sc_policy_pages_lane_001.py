"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phase 11B: the independents' own policy pages.

Columbia independents (like Charleston's) publish their pet policy on a subpage (``/pet-policy``,
...
``data/acquisition/columbia_sc_policy_pages_001/``).
...
recorded in ``markets/staging/columbia-sc/raw_captures/brand_pages.json``
with the sha256 of the document they were read from, and the capture helper binds
them.

Output:
  launch_packages/pettripfinder/markets/reports/columbia_sc_policy_pages_lane_001.json
"""
...
from scripts.pettripfinder import columbia_sc_brand_inventory_001 as B  # noqa: E402
from scripts.pettripfinder import columbia_sc_static_lane_001 as S      # noqa: E402
...
STATIC = os.path.join(REPORTS, "columbia_sc_static_lane_001.json")
OUT = os.path.join(REPORTS, "columbia_sc_policy_pages_lane_001.json")
DOCS = os.path.join(_DASH, "data", "acquisition", "columbia_sc_policy_pages_001")
LINK_CAP = 6
...
EXTRA_TARGETS = [
    ...
    # InTown Suites' own Columbia property pages (the brand's own city page lists three).
    ("https://www.intownsuites.com/extended-stay-hotels/south-carolina/columbia/broad-river/", "intown suites columbia west"),
    ("https://www.intownsuites.com/extended-stay-hotels/south-carolina/columbia/columbiana/", "intown suites columbia northwest"),
]
