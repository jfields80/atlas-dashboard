"""PTF-MIAMI-FL-HARDENED-SOURCE-READY-001 -- Phase 12F: Extended Stay America, the plain-client lane.

Extended Stay America's own Miami city page (https://www.extendedstayamerica.com/hotels/fl/miami) answers a
...
persisted by sha256 under data/acquisition/miami_fl_esa_001/.

Also reads the Miami Lakes, Doral and Coral Gables city pages directly, since a single Miami city-page walk may not
surface every suburban Miami-Dade property.

Outputs:
  launch_packages/pettripfinder/markets/staging/miami-fl/raw_captures/esa_rows.json
  launch_packages/pettripfinder/markets/reports/miami_fl_esa_lane_001.json
"""
...
B.DOCS = os.path.join(_DASH, "data", "acquisition", "miami_fl_esa_001")
CITY_PAGES = [
    "https://www.extendedstayamerica.com/hotels/fl/miami",
    "https://www.extendedstayamerica.com/hotels/fl/miami-lakes",
    "https://www.extendedstayamerica.com/hotels/fl/doral",
    "https://www.extendedstayamerica.com/hotels/fl/coral-gables",
]
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "miami-fl", "raw_captures", "esa_rows.json")
REPORT_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports", "miami_fl_esa_lane_001.json")
OUT_OF_MARKET_TOWNS = ("fort-lauderdale", "hollywood", "davie", "plantation", "sunrise", "miramar",
                       "pembroke-pines", "coral-springs", "deerfield-beach", "boca-raton", "pompano-beach",
                       "west-palm-beach", "dania-beach", "hallandale-beach", "weston", "tamarac", "lauderhill")
...
def main():
    ...
    with open(REPORT_OUT, "w", encoding="utf-8", newline="\n") as fh:
        json.dump(OrderedDict([
            ("schema", "ptf-brand-first-party-lane/1.0"), ("work_order", WORK_ORDER), ("market_id", "miami-fl"),
