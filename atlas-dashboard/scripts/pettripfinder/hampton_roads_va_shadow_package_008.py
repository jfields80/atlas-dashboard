"""PTF-HAMPTON-ROADS-VA-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Hampton Roads is not at the front of the serialized release queue. The live index this branch's lineage carries
is Atlanta (23 markets / 1500 profiles / 1744 routes); later markets (Richmond, Charleston) went live on their own
branches, and other source-ready markets are queued ahead of Hampton Roads.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/hampton-roads-va/launch_package/``) from the proposed
  ...
  3. runs the FAST lane (rules A-O) over it, which builds the joining market's
     ...
     live parent this lineage carries (Atlanta) -- no whole-site candidate is assembled;
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/hampton-roads-va/``: never ``markets/packages/`` or
...
WHAT BINDS TO THE PARENT, AND HOW IT IS RECONCILED
------------------------------------------------------------
...
REPORT = PKG / "markets" / "reports" / "hampton_roads_va_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "hampton_roads_va_proposed_authority_002.json"
SEALED_AT = "2026-09-14T16:00:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the live release this branch lineage carries (Atlanta, deploy 6aa7f125...; production has since "
         "moved to Charleston on another branch). That is what this "
         "package was validated against. Once any later market is live, FAST rule N fails this package BY DESIGN; the "
         "registration order -- when Hampton Roads reaches the front of the release queue -- re-seals the same committed inputs against that parent as a NEW "
