"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- Phase 10/11: the Wyndham first-party lane (plain client).
...
``data/acquisition/outer_banks_nc_wyndham_001/``. The pet quote is read from the
...
Output:
  data/acquisition/outer_banks_nc_attended/wyndham_rows.json   (the capture helper's input shape)
  launch_packages/pettripfinder/markets/reports/outer_banks_nc_wyndham_lane_001.json
"""
...
from scripts.pettripfinder import outer_banks_nc_brand_inventory_001 as B  # noqa: E402
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "outer-banks-nc", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "outer_banks_nc_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "outer_banks_nc_wyndham_001")
