"""PTF-ATLANTA-GA-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Atlanta's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/atlanta-ga/launch_package/hotel_policy_facts_atlanta-ga.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/atlanta-ga/atlanta_ga_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-ATLANTA-GA-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
markets (Fayetteville, Jacksonville, then Greenville) must deploy first. This order therefore writes NOTHING at
...
against the then-current live parent (after Fayetteville, Jacksonville and Greenville).

WHY A LATER ORDER MAY REGISTER WITHOUT A ROW-BY-ROW FOUNDER SIGNATURE
------------------------------------------------------------------
Registration is not publication. Writing `markets/atlanta-ga.json`, an
...
STAGING = os.path.join(PKG, "markets", "staging", "atlanta-ga")
#: REGISTERED by PTF-ATLANTA-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root, as every registered market.
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_atlanta-ga.json")
AUTHORITY_OUT = os.path.join(PKG, "atlanta_ga_proposed_authority_002.json")
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Atlanta without a row-by-row
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Atlanta's REGISTERED policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Atlanta's authority as the source-ready order proposed it, registered by PTF-ATLANTA-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Registration makes the market "
