"""PTF-AUGUSTA-GA-V2-REGISTRATION-AND-FOUNDER-PACKET-003 -- Augusta's release contract.
...
TEMPLATE = os.path.join(_DASH, "deploy", "netlify", "release_contracts", "savannah-ga.json")
...
def main(argv=None) -> int:
    ...
    partition = _load(os.path.join(REPORTS, "augusta_ga_final_partition_007.json"))
    template = _load(TEMPLATE)

    held = Counter(i["final_state"] for i in partition["items"]
                   if i["final_state"] not in ("PUBLISHED_PET_FRIENDLY", "VERIFIED_NO_PETS"))
    ...
    doc = OrderedDict([
        ...
        ("release_name_prefix", "prod-augusta"),
        ("description",
         "Deterministic release-gate contract for the PetTripFinder Augusta market through %s. "
        ...
        ("deployment_authorization", OrderedDict([
            ...
            ("means",
             ...
             "decision exists for Augusta at %d published profiles."
        ...
        ("identity_census", OrderedDict([
            ...
            ("note",
             "%d registered identities, built from zero by PTF-AUGUSTA-GA-HARDENED-V2-SOURCE-READY-001 "
             "under existing authorized Firecrawl capacity (89 credits, $0 new spend), registered by %s. "
             "Membership is this market's own corridor postal partition "
             "(census_membership_basis CORRIDOR_REGISTRY): 11 corridors over 15 postal codes -- Downtown / "
             "Medical District, Washington Road / I-20 / Martinez / Belair, Gordon Highway, Fort Eisenhower, "
             "AGS Airport / South Augusta / Hephzibah (CORE); Grovetown, Evans / Harlem (CORRIDOR); Thomson, "
             "Wrens, Waynesboro, Lincolnton (FRINGE). South Carolina (North Augusta, Aiken, Edgefield, "
             "Graniteville, Clearwater) is OUTSIDE and preserved for future north-augusta-sc / aiken-sc "
             "standalone markets; 0 South Carolina profiles are in this census. Rows the identity graph did "
             "not admit are recorded, not dropped: %s."
        ...
        ("reconciliation", OrderedDict(
            ...
            + [("note",
                ...
                "named in augusta_ga_final_partition_007.json with its exact class -- "
                "AWAITING_OFFICIAL_URL, AWAITING_POLICY_OBSERVATION, AWAITING_ROUTING_REVIEW "
                "(includes the Hilton capability wall) and AWAITING_CONTRADICTION_RESOLUTION "
                "(a plain-reading refusal the shared first-party reader would not mechanically "
                "confirm -- held, never guessed). No gate was lowered to preserve a count."
                % (derived.published_hotel_profiles, recon["verified_no_pets"], census["count"],
                   len(partition["items"]) - derived.published_hotel_profiles - recon["verified_no_pets"],
                   hold_summary))]
        )),
        ("policy_package", OrderedDict([
            ...
            ("note",
             ...
             "the sha256 of the document it was read from. Every Firecrawl-sourced record here "
             "carries a real paid_reservation bound to the shared cross-run paid-attempt ledger "
             "(ptf_paid_attempt_ledger_001.json); 89 Firecrawl credits were spent under existing "
             "authorized capacity, $0 new spend, $0 new provider authorization."),
        ])),
        ...
        ("routes", OrderedDict([
            ...
            ("note",
             ...
             "(Washington Road only, at this census); the rest are suppressed, which is a "
             "threshold and not a gap. The market and its corridors are hidden from navigation "
             "and the sitemap (show_in_navigation=false, show_in_sitemap=false) until a launch "
             "order says otherwise." % (derived.route_mode, derived.market_slug,
