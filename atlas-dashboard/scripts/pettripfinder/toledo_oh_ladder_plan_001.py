"""PTF-TOLEDO-OH-NEW-MARKET-001 -- Phase 11 preflight: what the ladder says next.
...
Output:
  launch_packages/pettripfinder/markets/reports/toledo_oh_ladder_plan_001.json
"""
...
CAPTURE = os.path.join(REPORTS, "toledo_oh_free_static_capture_001.json")
...
def ledger_state():
    ...
    return OrderedDict([
        ("paid_attempt_ledger", OrderedDict([
            ...
            ("toledo_attempts", sum(1 for r in paid_rows if r.get("market_id") == MARKET_ID)),
        ...
        ("discovery_attempt_ledger", OrderedDict([
            ...
            ("toledo_attempts", sum(1 for r in disc_rows if r.get("market_id") == MARKET_ID)),
            ("markets_present", sorted({r.get("market_id") for r in disc_rows if r.get("market_id")})),
        ])),
        ("double_buy_risk",
         "NONE. Neither shared ledger carries a single toledo-oh row, so no page in this market "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "toledo_oh_ladder_plan_001.json"))
