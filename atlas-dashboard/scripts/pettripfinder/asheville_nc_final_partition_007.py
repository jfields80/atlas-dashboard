"""PTF-ASHEVILLE-NC-NORMAL-PRODUCTION-001 -- the final partition.

Every registered Asheville identity, in EXACTLY ONE state, with exactly one next
...
------------------------------------
  ...
  ACCESS_BLOCKED              the surface refused to serve us. Choice (a blank
                              attended page), Best Western, Red Roof and
                              Radisson (sitemap 403) each refused this client
                              on this run, as did several independent motels'
                              own sites (HTTP 403, 500 or no DNS answer).
...
Output: launch_packages/pettripfinder/asheville_nc_final_partition_007.json
"""
...
OUT = os.path.join(PKG, "asheville_nc_final_partition_007.json")
...
REFUSALS = {
    ...
    "CHOICE": "a sitemap timeout and a blank attended property page", "RADISSON": "a sitemap 403 and a domain the operator's session does not navigate",
}
#: Independent properties whose OWN site refused this client on this run.
SITE_REFUSALS = {
    "the asheville inn": "HTTP 403 to a plain client", "plantation motel": "HTTP 403 to a plain client",
    "the monte vista hotel": "HTTP 403 to a plain client",
    "the mountaineer inn": "no DNS answer for its probed domain", "mount vue": "no DNS answer for its probed domain",
    "thunderbird motel": "no DNS answer for its probed domain", "rock haven terrace motel": "no DNS answer for its probed domain",
    "whispering pines motel": "no DNS answer for its probed domain", "the log cabin motor court": "no DNS answer for its probed domain",
}
...
def build():
    ...
    routing = _load(os.path.join(REPORTS, "asheville_nc_routing_001.json"), {})
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-ASHEVILLE-NC-NORMAL-PRODUCTION-001 "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-13"),
        ("what_this_is",
         "Every registered Asheville identity in exactly one disposition state. A terminal state "
