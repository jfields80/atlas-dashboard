"""PTF-RALEIGH-NC-FINAL-FRESH-CITY-PROOF-001 -- the two shared registries.
...
resolving the registry. Registering Raleigh therefore extends it by exactly
...
DAYTON, a market Raleigh never touched.
...
not a decision, so Raleigh gets a row saying, in the document's own
...
Output: launch_packages/pettripfinder/markets/reports/raleigh_nc_participation_registration_010.json
"""
...
REASON = (
    "Raleigh is registered, releasable and NOT authorized. This write records that "
    ...
    "The founder-authorized set is unchanged at thirteen markets; the founder's Raleigh "
    "decision, if it comes, is a separate write.")
OUT = (_DASH / "launch_packages" / "pettripfinder" / "markets" / "reports"
       / "raleigh_nc_participation_registration_010.json")
...
CLOSURE_NOTE = (
    ...
    "contract by name, so registering Raleigh extends it by exactly two paths. Until "
    "they are declared the bundle cache calls them undeclared reads and publishes every "
    "bundle UNTRUSTED -- including bundles of markets Raleigh never touched, which is "
...
def wire_closure(*, write: bool):
    """Declare Raleigh's two shared data inputs. Returns what it added."""
