"""PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 -- Phases 13 and 14: the clean set.
...
Output:
  launch_packages/pettripfinder/markets/reports/hickory_nc_clean_authority_001.json
"""
...
CENSUS = os.path.join(PKG, "identity_census_proposed", "hickory-nc.json")
ATTENDED = os.path.join(REPORTS, "hickory_nc_attended_capture_001.json")
STATIC = os.path.join(REPORTS, "hickory_nc_free_static_lane_001.json")
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "hickory_nc_clean_authority_001.json"))
