"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- the Choice / IHG property pages the Firecrawl route-discovery lane found.

Choice's own sitemap timed out to the plain client and IHG's answered 403, so neither family's routes reached the
census by name: a discovered route names a FLAG and a city ("/florida/fort-lauderdale/sleep-inn-hotels/fl123"), never the
...
Outputs:
  launch_packages/pettripfinder/markets/staging/fort-lauderdale-fl/raw_captures/brand_page_rows.json
  launch_packages/pettripfinder/markets/reports/fort_lauderdale_fl_brand_page_reads_001.json
"""
...
DISCOVERY = os.path.join(REPORTS, "fort_lauderdale_fl_firecrawl_discovery_001.json")
DOCS = os.path.join(_DASH, "data", "acquisition", "fort_lauderdale_fl_brand_page_reads_001")
RAW_OUT = os.path.join(PKG, "markets", "staging", "fort-lauderdale-fl", "raw_captures", "brand_page_rows.json")
REPORT_OUT = os.path.join(REPORTS, "fort_lauderdale_fl_brand_page_reads_001.json")
CAP = 34
FLOOR = 400
#: The market's own towns, as a Choice or IHG route spells them. A route naming another town is another market's.
IN_MARKET = ("fort-lauderdale", "lauderdale-by-the-sea", "wilton-manors", "oakland-park", "lauderdale-lakes",
             "lauderhill", "tamarac", "north-lauderdale", "dania-beach", "hollywood",
             "hallandale-beach", "west-park", "pompano-beach", "lighthouse-point",
             "hillsboro-beach", "deerfield-beach", "plantation", "davie", "cooper-city",
             "southwest-ranches", "sunrise", "weston", "pembroke-pines", "miramar",
             "coral-springs", "coconut-creek", "margate", "parkland")
...
def main(argv=None):
    ...
    with open(RAW_OUT, "w", encoding="utf-8", newline="\n") as fh:
        ...
        fh.write("\n")
    report = OrderedDict([
        ("schema", "ptf-brand-page-reads/1.0"), ("work_order", WORK_ORDER), ("market_id", "fort-lauderdale-fl"),
