"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- Phase 10: BringFido gap reconciliation (identity only, never policy).
...
vacation-rental, duplicate, review) -- so each lead lands in exactly one class:
  ...
  TRUE_MISSING           a lodging-establishment name no census node carries (worked by
                         west_palm_beach_fl_competitor_additions_001 through an authoritative identity lane)

A name match PROPOSES an identity; it never decides one. BringFido's pet claims are never read.

Output:
  launch_packages/pettripfinder/markets/reports/west_palm_beach_fl_competitor_reconciliation_001.json
"""
...
from scripts.pettripfinder import west_palm_beach_fl_census_reconciliation_001 as CR  # noqa: E402
...
_STOP = re.compile(r"\b(hotel|hotels|inn|suites?|by|the|and|resort|resorts|motel|at|of|a|an|hilton|marriott|"
                   r"collection|autograph|tapestry|ascend|curio|tribute|palm beach|west palm|boca|delray|boynton|jupiter|wellington|riviera|lantana|sunrise|weston|miramar|fl|florida|spa|beach)\b", re.I)
...
def build():
    ...
    return OrderedDict([
        ("schema", "ptf-competitor-reconciliation/1.0"), ("work_order", WORK_ORDER), ("market_id", "west-palm-beach-fl"),
