"""PTF-NASHVILLE-TN-NEW-MARKET-001 -- Phases 14 to 19, in one pass.
...
Outputs:
  launch_packages/pettripfinder/markets/reports/nashville_tn_evidence_audit_001.json
  launch_packages/pettripfinder/markets/reports/nashville_tn_clean_authority_001.json
  launch_packages/pettripfinder/markets/reports/nashville_tn_founder_packet_001.json
  launch_packages/pettripfinder/markets/reports/nashville_tn_paid_readiness_001.json
  launch_packages/pettripfinder/markets/reports/nashville_tn_shadow_market_001.json
"""
...
CONTRACT = _first_existing(os.path.join(PKG, "markets", "nashville-tn.json"),
                           os.path.join(PKG, "markets", "proposed", "nashville-tn.json"))
CENSUS = _first_existing(os.path.join(PKG, "identity_census", "nashville-tn.json"),
                         os.path.join(PKG, "identity_census_proposed", "nashville-tn.json"))
RECON = os.path.join(REPORTS, "nashville_tn_census_reconciliation_001.json")
ROUTING = os.path.join(REPORTS, "nashville_tn_routing_001.json")
STATIC = os.path.join(REPORTS, "nashville_tn_free_static_capture_001.json")
FIRECRAWL = os.path.join(REPORTS, "nashville_tn_firecrawl_pass_001.json")
ATTENDED = os.path.join(REPORTS, "nashville_tn_attended_capture_001.json")
LADDER = os.path.join(REPORTS, "nashville_tn_ladder_plan_001.json")
GAPS = os.path.join(REPORTS, "nashville_tn_competitor_gap_matrix_001.json")
CITY_PAGES = os.path.join(REPORTS, "nashville_tn_brand_city_pages_001.json")
SITEMAPS = os.path.join(REPORTS, "nashville_tn_brand_sitemaps_001.json")
...
_CHAIN_WORDS = {
    ...
    "wyndham", "royal", "relax", "sunset", "crown",
    # Nashville's own layer: full-service and soft brands no prior market had
    # in this quantity.
    "aloft", "element", "moxy", "westin", "sheraton", "conrad", "curio",
    "autograph", "tribute", "motto", "tempo", "gaylord", "omni", "loews",
    "graduate", "thompson", "joseph", "harpeth", "hutton", "fraye", "untitled",
    "printing", "noelle", "bobby", "dream", "union", "station",
}
...
def audit_read(row, census_row):
    ...
    if census_row is None:
        problems.append(OrderedDict([
            ...
            ("why", "this read is not bound to an admitted Nashville identity")]))
    else:
...
def clean_reads():
    ...
    # Resolve a conflict to the CENSUS IDENTITY, not to a display name. Toledo
    # had one building claimed by "Days Inn by Wyndham Perrysburg/Toledo" and
    # "Super 8 Perrysburg Toledo"; a read captured as "Days Inn Perrysburg"
    # carries neither string, so a display-name test held one read on that
    # building and published the other. Both are reads on one unresolved
    # identity and both must be held.
...
def founder_packet(census_doc, recon, routing, audited, gaps, city_pages):
    ...
    # The identity reviews are ONE grouped item per REASON, never one per row.
    # A market this size produces 83 of them, and a packet that asked the
    # founder 83 separate questions would be the row-by-row approval loop the
    # order forbids. Each group states the rule that produced it, the count, and
    # a sample; the rows themselves are in the census, which the founder can
    # read, and every one of them is already excluded from the clean inventory.
    reviews = [r for r in non_admitted if r["classification"] == "IDENTITY_REVIEW_REQUIRED"]
    by_reason = OrderedDict()
    for r in reviews:
        reason = re.sub(r"\b\d+(\.\d+)?\b", "N", r["classification_reason"] or "")
        reason = reason.split(";")[0].split("(")[0].strip()[:150]
        by_reason.setdefault(reason, []).append(r)
    for reason, rows in by_reason.items():
        named = [r for r in rows if r["canonical_name"]]
        items.append(item(
            "A_IDENTITY",
            "%d identities held for review: %s" % (len(rows), reason),
            OrderedDict([
                ("rule_that_held_them", reason),
                ("rows", len(rows)),
                ("with_a_name", len(named)),
                ("lanes", sorted({l for r in rows for l in r["lanes"]})),
                ("sample", [OrderedDict([("name", r["canonical_name"]),
                                         ("street", r["street"]),
                                         ("city", r["city"]),
                                         ("postal_code", r["postal_code"])])
                            for r in rows[:15]]),
                ("where_the_rest_are",
                 "every row is in identity_census_proposed/nashville-tn.json under "
                 "non_admitted with its own reason and its own evidence")]),
            "rule on the GROUP: admit these on a stated rule, or leave them held",
            "leave held. The evidence does not decide any of them, and a guess "
            "publishes a wrong page. None is a promotion blocker: all %d are "
            "already excluded from the clean inventory and from the census count."
            % len(rows),
            "up to %d rows admitted or dropped" % len(rows),
            "none while held", "no route is issued while held",
            "REVERSIBLE -- every row stays in the proposed census either way", "NO"))

    # A merge the graph refused to make silently. Most of these are the SAME
    # hotel under two spellings of its own name -- "Hampton Inn & Suites
    # Nashville-Airport" and a bare "Hampton" from the map source -- which the
    # merge absorbs and which is no question at all. Only a conflict whose claimants carry
    ...
    # B -- geography and fringe. Derived from the contract's OWN held corridors
    # and the rows that landed in their postal codes, so the founder is shown
    # the inventory each ruling would admit rather than an assertion about it.
    contract = _load(CONTRACT, {}) or {}
    held_rows_by_zip = {}
    for r in non_admitted:
        if r["classification"] != "OUTSIDE_MARKET":
            continue
        held_rows_by_zip.setdefault((r.get("postal_code") or "")[:5], []).append(r)
    for corridor in contract.get("corridors", []):
        held_zips = corridor.get("_held_postal_codes") or []
        if not held_zips:
            continue
        rows = [r for z in held_zips for r in held_rows_by_zip.get(z, [])]
        recommend = corridor["slug"] in ("cool-springs-franklin", "mount-juliet-i40-east",
                                         "hendersonville")
        items.append(item(
            "B_GEOGRAPHY", "%s -- a HELD corridor claiming no postal code" % corridor["name"],
            OrderedDict([
                ("held_postal_codes", held_zips),
                ("identities_found_in_those_postal_codes", len(rows)),
                ("identities", [OrderedDict([("name", r["canonical_name"]),
                                             ("street", r["street"]),
                                             ("city", r["city"]),
                                             ("postal_code", r["postal_code"])])
                                for r in rows[:30]]),
                ("how_it_is_held",
                 "the corridor exists in the proposed contract with included_postal_codes "
                 "EMPTY, so it claims nothing and admits nothing. Its hotels were "
                 "discovered and identified and are classified OUTSIDE_MARKET on the "
                 "postal code they state -- deliberately, so this ruling rests on an "
                 "inventory that was looked at"),
                ("what_the_operators_themselves_call_it", corridor.get("description", "")),
            ]),
            "admit the corridor by moving its held postal codes into "
            "included_postal_codes, or leave it excluded",
            ("ADMIT is defensible: the operators' own published property names call these "
             "properties Nashville, and the corridor is on a primary interstate approach. "
             "This order recommends the founder rule on it explicitly rather than letting "
             "silence decide."
             if recommend else
             "EXCLUDE. This order recommends exclusion: the area sits in another county's "
             "own lodging orbit and the market publishes comfortably without it."),
            "%d identities in or out" % len(rows),
            "%d rows in or out of the clean inventory" % len(rows),
            "1 corridor route in or out",
            "REVERSIBLE -- a one-line move of the postal codes; the corridor and the rows "
            "already exist either way", "NO"))
    items.append(item(
        "B_GEOGRAPHY", "Brentwood and Goodlettsville, ADMITTED by this order",
        OrderedDict([
            ("brentwood_evidence",
             "contiguous with Davidson County at I-65; the Maryland Farms lodging node; and "
             "FIVE separate first-party market claims in Marriott's own published property "
             "names -- bnaab AC Hotel NASHVILLE Brentwood, bnasb Sheraton NASHVILLE "
             "Brentwood, bnabr Courtyard NASHVILLE Brentwood, bnabt TownePlace Suites "
             "NASHVILLE Brentwood, bnawo SpringHill Suites NASHVILLE Brentwood"),
            ("goodlettsville_evidence",
             "postal code 37072 STRADDLES the Davidson/Sumner county line, so part of it is "
             "Metropolitan Nashville-Davidson ground; Marriott names bnagv Courtyard "
             "NASHVILLE Goodlettsville and bnatg TownePlace Suites NASHVILLE Goodlettsville"),
            ("what_this_evidence_is_not",
             "an operator's marketing name is evidence of the MARKET it serves, never of "
             "the identity of the building; every one of these rows is still admitted on "
             "the postal code its own page states")]),
        "confirm both, or move either to a hold",
        "confirm. Both are contiguous, both sit on a primary approach, and both are named "
        "Nashville by their own operators",
        "the identities admitted in 37027 and 37072",
        "their rows in the clean inventory", "2 corridors",
        "REVERSIBLE -- the corridors' postal codes move back out in one line", "NO"))
    ...
    refused = sorted(f for f, v in (_load(SITEMAPS, {}) or {}).get("families", {}).items()
                     if v.get("verdict") == "SITEMAP_REFUSED_TO_THIS_CLIENT")
    items.append(item(
        "C_CATEGORY", "%d brand families refused this client outright" % len(refused),
        OrderedDict([
            ("families", refused),
            ("what_was_measured", "each one declared a sitemap in its OWN robots.txt and "
                                  "then refused that very sitemap to the same client seconds "
                                  "later; the city page refused too"),
            ("what_it_is_not", "a statement about any building. It is not evidence that a "
                               "property of these brands closed, moved or stopped taking "
                               "pets. Their rows are routed and read on other rungs, or "
                               "they stay unresolved and are reported as such"),
            ("what_it_costs", "these families are the reason the Firecrawl and attended "
                              "rungs exist for this market")]),
        "note only",
        "note. No ruling needed; recorded so a later order re-probes rather than inherits, "
        "because a refusal is a fact about a moment",
    ...
    items.append(item(
        "F_CROSS_MARKET", "Nashville against the eleven live markets and Detroit",
        OrderedDict([("scanned", "every committed identity census in the package"),
                     ("collisions_found", 0),
                     ("distance_to_the_nearest_committed_market",
                      "Louisville, Kentucky begins at 37.9 N; this market ends at 36.42 N, "
                      "more than 1.4 degrees of latitude away. A collision here would be a "
                      "defect, not a boundary question")]),
        ...
        "carries a Nashville-area address",
        "none", "none", "none", "REVERSIBLE", "NO"))
    ...
    return OrderedDict([
        ...
        ("bright_data", OrderedDict([
            ...
            ("measured_rate", "NOT MEASURED FOR THIS MARKET -- no Bright Data call has been made "
                              "for Nashville, so no rate may be quoted from another market's run"),
        ...
        ("places_paid_identity_discovery", OrderedDict([
            ...
            ("what_they_are", "map-source rows with coordinates inside the market and no street "
                              "address"),
            ("existing_paid_attempts_for_nashville", 0),
...
def build():
    ...
    shadow = OrderedDict([
        ...
        ("artifacts", OrderedDict([
            ...
            ("proposed_census",
             "launch_packages/pettripfinder/identity_census_proposed/nashville-tn.json"),
            ("discovery_config",
             "scripts/pettripfinder/discovery/config/nashville_tn.json"),
            ("routing", "launch_packages/pettripfinder/markets/reports/nashville_tn_routing_001.json"),
            ("clean_authority",
             "launch_packages/pettripfinder/markets/reports/nashville_tn_clean_authority_001.json"),
            ("founder_packet",
             "launch_packages/pettripfinder/markets/reports/nashville_tn_founder_packet_001.json"),
        ...
        ("promotion_ready_because", OrderedDict([
            ...
            ("geography_deterministic_or_held",
             "the postal partition is deterministic -- 31 admitted ZIPs, each claimed by "
             "exactly one corridor -- and the five fringe corridors that claim NO postal "
             "code are explicitly HELD, so no held row is admitted and none is silently "
             "excluded either"),
        ...
        ("required_before_promotion", [
            "the founder rules on the %d HELD fringe corridors -- Franklin/Cool Springs, "
            "Mount Juliet, Hendersonville, Smyrna/La Vergne and Lebanon (packet group B). "
            "Each is a one-line move of postal codes and each is fully reversible."
            % len([c for c in (_load(CONTRACT, {}) or {}).get("corridors", [])
                   if c.get("_held_postal_codes")]),
            "the founder confirms Brentwood (37027) and Goodlettsville (37072), which THIS "
            "order admitted on first-party evidence rather than assumption (packet group B)",
            "the founder rules on %d rebrand successors and %d merge conflicts, each one "
            "address carrying two brand identities (packet group A)"
            % (len([r for r in census_doc.get("non_admitted", [])
                    if r["classification"] == "SAME_IDENTITY_REBRAND_SUCCESSOR"]),
               len({tuple(sorted(c.get("claimed_by") or []))
                    for c in (recon or {}).get("merge_conflicts", [])})),
            "PTF-NASHVILLE-TN-PROMOTION-AND-APPLICATION-002 INTEGRATES THE THEN-CURRENT "
            "DEPLOYED CANONICAL LINEAGE BEFORE ANY OF THIS IS PROMOTED. This order is pinned "
            "to d335c50, which was the deployed tip when it started; production serves eleven "
            "live markets and Detroit holds the serialized launch lane, so the lineage may "
        ...
        ("not_blockers_because",
         ...
         "which NAME a building trades under, or whether a fringe corridor is in scope at "
         "all, and every affected row is already excluded from the clean inventory. A "
         "ruling can only ADD rows to this market; none can invalidate one it already has."),
...
def main(argv=None) -> int:
    ...
    for name, doc in (("nashville_tn_evidence_audit_001.json", audit),
                      ("nashville_tn_clean_authority_001.json", authority),
                      ("nashville_tn_founder_packet_001.json", packet),
                      ("nashville_tn_paid_readiness_001.json", paid),
                      ("nashville_tn_shadow_market_001.json", shadow)):
