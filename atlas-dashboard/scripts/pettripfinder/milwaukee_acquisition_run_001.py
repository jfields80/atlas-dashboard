...
Spend is measured from the vendor's own account balance, read live before the
run and again after every property, never from the router's per-attempt
estimate. The estimate exists to stop one runaway property; it is not billing
and is never reported as billing.
...
class SpendMeter:
    """Account-balance drawdown, which is the only figure that covers BOTH
    zones. The Browser API and the Web Unlocker bill to different zones, so a
    single zone's month-to-date cost would understate a run that used both.
    Balance is account-wide and therefore cannot miss a lane.
    """
    ...
    def spent_usd_minor(self) -> Optional[int]:
        """Balance drawdown since baseline. ``None`` when it cannot be read --
        which callers must treat as a stop, never as zero."""
        if not (self.baseline and self.latest):
            return None
        a, b = self.baseline.balance_usd_minor, self.latest.balance_usd_minor
        if a is None or b is None:
            return None
        # A top-up mid-run would show as negative drawdown; clamp at zero and
        # let the zone figure corroborate rather than reporting a refund.
        return max(0, a - b)

    def zone_delta_usd_minor(self) -> Optional[int]:
        if not (self.baseline and self.latest):
            return None
        a, b = self.baseline.cost_month_usd_minor, self.latest.cost_month_usd_minor
        if a is None or b is None:
            return None
        return max(0, b - a)
...
async def run(*, max_properties: Optional[int], resume: bool,
    ...
    meter = SpendMeter()
    pre = preflight(meter, cap_usd_minor=HARD_CAP_USD_MINOR)
    ...
    for row in pending:
        ...
        record = CORPUS.BenchmarkRecord(
            ...
            brand=row["brand"], url=row["official_url"])
        target = CORPUS.target_for(record)
...
def main(argv=None) -> int:
    ...
    out = Path(args.report) if args.report else (
        REPORTS / ("%s_acquisition_run_001.json" % MARKET))
