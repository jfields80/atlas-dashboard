"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 13, the Firecrawl rung (cloned from the Orlando FL V2 harness).
...
-----------------------------------------
  ...
  DISCOVERED    Choice property routes read from the brand's own city pages through Firecrawl
                (san_diego_ca_firecrawl_discovery_001, when it exists), in-market towns only, that no static read
...
Output:
  launch_packages/pettripfinder/markets/reports/san_diego_ca_firecrawl_pass_001.json
"""
...
STATIC_REPORT = os.path.join(REPORTS, "san_diego_ca_free_static_capture_001.json")
DISCOVERY = os.path.join(REPORTS, "san_diego_ca_firecrawl_discovery_001.json")
PROBE_PER_FAMILY = 4
#: SAN DIEGO: independents are the market's largest unresolved family, so EVERY escalatable independent static failure
...
_IN_MARKET_CHOICE_TOWNS = ("san-diego", "la-jolla", "coronado", "del-mar", "solana-beach", "encinitas",
                           "cardiff", "carlsbad", "oceanside", "chula-vista", "national-city", "imperial-beach",
                           "la-mesa", "el-cajon", "santee", "lakeside", "lemon-grove", "spring-valley", "poway",
                           "escondido", "san-marcos", "vista", "san-ysidro", "bonita", "rancho-santa-fe")
SPACING_SECONDS = 2.0
...
def cohort(static_report):
    ...
    extra = []
    # SAN DIEGO: the DISCOVERED Choice / IHG routes are bought ONCE, by san_diego_ca_brand_page_reads_001.
    # Planning them again here would pay twice for one page -- the standing rule is pay once per page ever.
    disc = {"routes": []}
    for r in sorted(disc.get("routes", []), key=lambda x: x["route"]):
        town = r["route"].split("/california/", 1)[-1].split("/", 1)[0] if "/california/" in r["route"] else ""
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="san_diego_ca_firecrawl_001")
    ap.add_argument("--cap-attempts", type=int, default=150)
    ap.add_argument("--floor-credits", type=int, default=400)
    ap.add_argument("--out", default=os.path.join(REPORTS, "san_diego_ca_firecrawl_pass_001.json"))
