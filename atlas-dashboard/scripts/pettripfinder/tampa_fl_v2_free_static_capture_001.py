"""PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 12C, free first-party static capture (router rung DIRECT_STATIC_FETCH).

Cloned from the Orlando FL V2 helper.

One HTTPS GET per route through the canonical gates, for two cohorts:

  ROUTED          every admitted Tampa Bay identity that carries a first-party route. The question is the PET
                  POLICY.
...
Output:
  launch_packages/pettripfinder/markets/reports/tampa_fl_v2_free_static_capture_001.json
"""
...
def targets():
    ...
    for r in routing["routes"]:
        if not r.get("url"):
            continue
        # TAMPA: a Wyndham route is read by the brand's own property service in the plain-client Wyndham lane
        # (tampa_fl_v2_wyndham_lane_001); fetching the client-rendered overview here would read nothing.
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS,
                                                  "tampa_fl_v2_free_static_capture_001.json"))
    ap.add_argument("--run-id", default="tampa_fl_v2_free_static_001")
