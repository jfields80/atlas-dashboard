"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 12C, free first-party static capture (router rung DIRECT_STATIC_FETCH).

Cloned from the Orlando FL V2 helper.

One HTTPS GET per route through the canonical gates, for two cohorts:

  ROUTED          every admitted San Diego identity with a first-party route. The question is the PET
                  POLICY.
  IDENTITY_FILL   the brand-roster / registry properties whose existence and route the brand's own inventory
                  states but which no source gave an address. The question is first the ADDRESS -- a property
                  code selects, the page admits -- and only then the policy.

The lane is ``acquisition.direct_http_capture.run_attempt``: denial markers -> page health -> identity read and
assessment -> policy locator -> reader, with every artifact persisted under
``data/acquisition/<run>/<slug>/attempt-01/``. ``market_observation_store.observation_for`` then grades the read.

No vendor, no browser, no price. Each target is captured at most ONCE per run; a rerun reuses the attempt on
disk. Nothing is written to any authority.

WHAT THIS RUN WILL NOT DO
--------------------------
It will not treat a route as confirmed because a directory stated it. Every row carries the identity it claims
to serve, and the identity assessment on the page's own address, phone or property code decides. It will not
read a policy off a page whose identity did not confirm, and it will not infer acceptance from a fee, a weight,
a count or an amenity chip: that judgement belongs to the reader and the publication grade, both of which this
module only reports.

Output:
  launch_packages/pettripfinder/markets/reports/san_diego_ca_free_static_capture_001.json
"""
...
BRANDS = [
    ("MARRIOTT", r"marriott|courtyard by marriott|courtyard marriott|courtyard san diego|courtyard by|courtyard(?= (san diego|downtown|airport|old town|mission|liberty|carlsbad|oceanside|north|south))|residence inn|springhill|fairfield|towneplace|ac hotel|"
...
def text_bound_read(attempt_root: Path, row, brand: str):
    ...
    if not declined.is_file() or row is None:
        return None
    # A declined artifact can go unreadable between the gate writing it and this reader opening it
    # (Windows filesystem / scanner interference was observed once on this run). A capture that cannot be
    # re-read is NOT silently treated as a read: it returns None, and the row keeps its gate outcome.
    try:
    ...
    try:
        ...
        text = ZCR.full_document_text(html)
...
def targets():
    ...
    for r in routing["routes"]:
        if not r.get("url"):
            continue
        # MIAMI: a Wyndham route is read by the brand's own property service in the plain-client Wyndham lane
        # (san_diego_ca_wyndham_lane_001); fetching the client-rendered overview here would read nothing.
        if "wyndhamhotels.com" in r["url"]:
            continue
        # SAN DIEGO: a Marriott route is NOT probed by the plain client. Akamai's per-window read quota tightens
        # across a run and is shared with the attended browser on this machine, so a plain probe that can only be
        # refused would spend the browser lane's reads. Marriott is read in the attended-browser lane only.
        if "marriott.com" in r["url"]:
            continue
    ...
    for r in routing.get("identity_fill_routes", []):
        if "wyndhamhotels.com" in r["url"] or "marriott.com" in r["url"]:
            continue
        out.append(OrderedDict([
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS,
                                                  "san_diego_ca_free_static_capture_001.json"))
    ap.add_argument("--run-id", default="san_diego_ca_free_static_001")
