"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phase 17: the FIRST-PARTY READS, assembled for the census.
...
-------------------------------------------------------------
All 74 attended-browser reads are refused here for ONE reason: this order's browser transcription recorded each
page's ADDRESS but not the page's own NAME, and the shared identity-key contract refuses an empty name outright
("an empty key would match every other empty key"). Rather than invent a name -- from the census row, from the
route slug, or from the brand -- the rows are refused and counted. The cost is bounded and known: those reads
are Marriott, Hilton, Hyatt, Best Western, Red Roof and Motel 6, whose premises the census already holds from
the state licence, the brands' own inventories and the bureaus, and whose POLICY reaches the adjudicator by
brand property code through browser_closure_rows.json, untouched by this document. What is lost is their
ROUTE, which matters only for the Hyatt Studios row whose bureau route is a vanity domain. A later order that
records page titles in the browser lane recovers all 74 at no new cost.
...
Output:
  launch_packages/pettripfinder/markets/reports/jacksonville_fl_policy_reads_001.json
"""
...
OUT = os.path.join(REPORTS, "jacksonville_fl_policy_reads_001.json")

_ZIP = re.compile(r"\b(\d{5})(?:-\d{4})?\b")
_STATE = re.compile(r"\b(FL|Florida|GA|Georgia)\b", re.I)
...
def _split_address_line(line):
    ...
    for p in parts[1:]:
        if _STATE.search(p) and not region:
            m = _STATE.search(p)
            region = "FL" if m.group(1).lower().startswith("f") else "GA"
...
def build():
    ...
    for r in (bp.get("rows") if isinstance(bp, dict) else bp) or []:
        ...
        _r = _row("FIRECRAWL_BRAND_PAGE", r.get("family"), r.get("u"), r.get("final_url"),
                         r.get("n"), r.get("st"), r.get("ci"), "FL", r.get("z"), r.get("ph"),
    ...
    for path, lane in ((os.path.join(REPORTS, "jacksonville_fl_firecrawl_pass_001.json"), "FIRECRAWL"),
                       (os.path.join(REPORTS, "jacksonville_fl_free_static_capture_001.json"),
                        "DIRECT_STATIC_FETCH")):
        for r in (_load(path, {}) or {}).get("rows", []):
            ...
            _r = _row(lane, r.get("family"), r.get("requested_url"), r.get("final_url"),
                             sig.get("name_on_page"), street, sig.get("locality"), sig.get("region") or "FL",
    ...
    for fname in ("policy_pages_rows.json", "closure_static_rows.json"):
        for r in (_load(os.path.join(STAGING, fname), {}) or {}).get("rows", []):
            ...
            _r = _row("DIRECT_STATIC_FETCH", "INDEPENDENT", r.get("url"), r.get("final_url"),
                             r.get("name") or r.get("canonical_name"), street, r.get("city"), "FL",
                             r.get("postal_code"), r.get("phone"), "",
