"""PTF-PIEDMONT-TRIAD-NC-NORMAL-PRODUCTION-001 -- the final partition.

Every registered Piedmont Triad identity, in EXACTLY ONE state, with exactly one next
...
Output: launch_packages/pettripfinder/piedmont_triad_nc_final_partition_007.json
"""
...
OUT = os.path.join(PKG, "piedmont_triad_nc_final_partition_007.json")
...
REFUSALS = {
    "BEST_WESTERN": "a structured petsAllowed boolean only", "ESA": "HTTP 403",
    "RED_ROOF": "HTTP 403", "MOTEL6": "read timeout", "HYATT": "HTTP 403",
    "CHOICE": "a sitemap timeout and a blank attended page", "RADISSON": "HTTP 403",
}
...
def build():
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-PIEDMONT-TRIAD-NC-NORMAL-PRODUCTION-001 "
                                         "acquisition passes: owned evidence, brand city pages, "
                                         "brand sitemaps, OSM extract, one static read and the "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-12"),
        ("what_this_is",
         "Every registered Piedmont Triad identity in exactly one disposition state. A terminal state "
