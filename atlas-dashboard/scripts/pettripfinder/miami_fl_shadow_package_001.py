"""PTF-MIAMI-FL-HARDENED-SOURCE-READY-001 -- Phase 26: the SHADOW sealed package (cloned from the Orlando FL V2 helper).

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
------------------------------------------------------------------
Miami is not at the front of the serialized release queue: the current verified live release is Augusta
(deploy 6aaeb3b7, 30 markets / 2165 profiles / 2477 routes, source 96564199). The generic lane would reissue shared, parent-sensitive state this
order is forbidden to touch. So this helper:

  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/miami-fl/launch_package/``) from the proposed market document, the proposed census, the
  ...
  4. writes the package, the receipt and a report ONLY under ``markets/staging/miami-fl/``.
...
REPORT = PKG / "markets" / "reports" / "miami_fl_shadow_package_001.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "miami_fl_proposed_authority_001.json"
PARTITION_SRC = STAGING / "launch_package" / "miami_fl_final_partition_001.json"
SEALED_AT = "2026-09-20T02:00:00Z"
...
def stage_tree():
    ...
    # the partition already lives under LP from miami_fl_final_partition_001; nothing to copy
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         ...
         "DESIGN; the registration order -- when Miami reaches the front of the release queue -- re-seals the "
