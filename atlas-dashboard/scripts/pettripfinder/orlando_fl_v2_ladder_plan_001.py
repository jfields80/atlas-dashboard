"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 12D/13 preflight: what the ladder says next.
...
Output:
  launch_packages/pettripfinder/markets/reports/orlando_fl_v2_ladder_plan_001.json
"""
...
CAPTURE = os.path.join(REPORTS, "orlando_fl_v2_free_static_capture_001.json")
...
def ledger_state():
    ...
    return OrderedDict([
        ("paid_attempt_ledger", OrderedDict([
            ...
            ("orlando_attempts", sum(1 for r in paid_rows if r.get("market_id") == MARKET_ID)),
        ...
        ("discovery_attempt_ledger", OrderedDict([
            ...
            ("orlando_attempts", sum(1 for r in disc_rows if r.get("market_id") == MARKET_ID)),
            ("markets_present", sorted({r.get("market_id") for r in disc_rows if r.get("market_id")})),
        ])),
        ("double_buy_risk",
         "checked mechanically: see orlando_attempts in each ledger above; this order WRITES neither ledger."),
    ])
...
def build(args) -> OrderedDict:
    ...
    return OrderedDict([
        ...
        ("phase", "12D preflight -- the ladder's decision, before any credit is spent"),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "orlando_fl_v2_ladder_plan_001.json"))
