"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phases 21 and 26: actionability and coverage readiness.
...
Output:
  launch_packages/pettripfinder/markets/reports/fort_lauderdale_fl_actionability_001.json
"""
...
OUT = os.path.join(R, "fort_lauderdale_fl_actionability_001.json")
...
CLASSES = OrderedDict([
    ...
    ("ROUTING_HOLD", (
        "AUTHORIZED_ROUTER_EXHAUSTED",
        "No first-party route exists to read. The brand inventories, the Visit Lauderdale roster, the map "
        "and Google Places route discovery (174 requests on existing capacity) between them found no website "
...
def build():
    clean = _load(os.path.join(R, "fort_lauderdale_fl_clean_authority_001.json"), {}) or {}
    acc = _load(os.path.join(R, "fort_lauderdale_fl_source_ready_accounting_001.json"), {}) or {}
    browser = _load(os.path.join(R, "fort_lauderdale_fl_browser_lane_001.json"), {}) or {}
    recon = _load(os.path.join(R, "fort_lauderdale_fl_competitor_reconciliation_001.json"), {}) or {}
    shadow = _load(os.path.join(R, "fort_lauderdale_fl_shadow_package_001.json"), {}) or {}
