"""PTF-RALEIGH-NC-FINAL-FRESH-CITY-PROOF-001 -- the modern release lane.

Takes the RECOVERED Raleigh authority through the redesigned lane
ATLAS-THROUGHPUT-003 to 008 built, and writes what each stage proved:
    ...
    6  FINAL CANDIDATE                    release_index.compose (live + Raleigh)
...
The candidate is composed with Raleigh PARTICIPATING, because a pre-flip
...
PTF-RALEIGH-NC-FINAL-FRESH-CITY-PROOF-001 inherits. Composing an
...
# Raleigh has NO paid ledger to ingest. Every published row came through a
...
def joining_delta(inputs) -> "OrderedDict":
    """Raleigh's intended delta: a market that JOINS, and nothing else moves.
    ...
    market that is live on both sides of the comparison. Raleigh is on neither
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=str(REPORTS / "raleigh_nc_release_lane_005.json"))
    ...
    # No published Raleigh row came through a paid lane, so there is no
    ...
    # exists to prevent:
    #
    #   (a) what does REGISTERING Raleigh cost?  The change surface of this
    ...
    #
    #   (b) what does a ROUTINE Raleigh release cost?  The fast lane's receipt
    ...
    #       Raleigh, which this order proposes and does not apply.
    t = time.monotonic()
