"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phases 15-19: policy adjudication over one identity per row.
...
Output:
  launch_packages/pettripfinder/markets/reports/jacksonville_fl_clean_authority_001.json
"""
...
from scripts.pettripfinder.jacksonville_fl_census_reconciliation_001 import canonical_street  # noqa: E402
...
STATIC = os.path.join(REPORTS, "jacksonville_fl_free_static_capture_001.json")
FIRECRAWL = os.path.join(REPORTS, "jacksonville_fl_firecrawl_pass_001.json")
#: The retry pass (after the census street restatement); a retry row supersedes the first pass's row for its key.
FIRECRAWL_RETRY = os.path.join(REPORTS, "jacksonville_fl_firecrawl_pass_002.json")
...
FIRECRAWL_PROBE2 = os.path.join(REPORTS, "jacksonville_fl_firecrawl_pass_003.json")
...
OUT = os.path.join(REPORTS, "jacksonville_fl_clean_authority_001.json")
...
def build_evidence_index(census_hotels):
    ...
    # bound to its own premises by jacksonville_fl_browser_closure_002 (property code, or the brand page's own name with
...
BROWSER_REACHED = {
    ...
    "BEST_WESTERN": "both reachable Best Western properties were read first-party on bestwestern.com this "
                    ...
                    "hotel-search page and the brand's own results for that city do not list it -- not because "
...
BROWSER_OUTCOME_STATE = {
    ...
    # NORTHEAST FLORIDA -- the outcomes this market's own attended-browser lane records. Phase 11 rule 7: a page this
...
def build():
    PLACES_BY_KEY.clear()
    SITES_BY_KEY.clear()
    for r in (_load(os.path.join(REPORTS, "jacksonville_fl_places_route_discovery_001.json"), {}) or {}).get("rows", []):
