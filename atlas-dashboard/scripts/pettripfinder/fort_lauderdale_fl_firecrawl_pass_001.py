"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phase 13, the Firecrawl rung (cloned from the Orlando FL V2 harness).
...
-----------------------------------------
  ...
  DISCOVERED    Choice property routes read from the brand's own city pages through Firecrawl
                (fort_lauderdale_fl_firecrawl_discovery_001, when it exists), in-market towns only, that no static read
...
Output:
  launch_packages/pettripfinder/markets/reports/fort_lauderdale_fl_firecrawl_pass_001.json
"""
...
STATIC_REPORT = os.path.join(REPORTS, "fort_lauderdale_fl_free_static_capture_001.json")
DISCOVERY = os.path.join(REPORTS, "fort_lauderdale_fl_firecrawl_discovery_001.json")
PROBE_PER_FAMILY = 4
#: BROWARD: independents are the market's largest unresolved family, so EVERY escalatable independent static failure
...
_IN_MARKET_CHOICE_TOWNS = ("fort-lauderdale", "lauderdale-by-the-sea", "wilton-manors", "oakland-park", "lauderdale-lakes",
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="fort_lauderdale_fl_firecrawl_001")
    ap.add_argument("--cap-attempts", type=int, default=150)
    ap.add_argument("--floor-credits", type=int, default=400)
    ap.add_argument("--out", default=os.path.join(REPORTS, "fort_lauderdale_fl_firecrawl_pass_001.json"))
