"""PTF-ATLANTA-GA-PARALLEL-SOURCE-READY-001 -- Phase 10/11: the free static first-party lane.

For every census identity the brand lanes left unresolved, the official website
...
addressed by sha256, under ``data/acquisition/atlanta_ga_static_001/``, and this
...
recorded in ``atlanta_ga_attended/brand_pages.json`` with the document sha256 they
...
Output:
  launch_packages/pettripfinder/markets/reports/atlanta_ga_static_lane_001.json
"""
...
OUT = os.path.join(REPORTS, "atlanta_ga_static_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "atlanta_ga_static_001")
...
BROWSER_ONLY = ("marriott.com", "hilton.com", "ihg.com", "choicehotels.com", "hyatt.com",
                "bestwestern.com", "redroof.com", "motel6.com", "extendedstayamerica.com",
...
def targets():
    ...
    unresolved = {u["identity_key"] for u in clean["unresolved_census_identities"]}
    out = OrderedDict()
    for h in census["hotels"]:
        if h["identity_key"] not in unresolved:
            continue
