"""PTF-HAMPTON-ROADS-VA-PARALLEL-SOURCE-READY-001 -- Phase 4A: the OpenStreetMap lane.

Reads the local Geofabrik Virginia extract (ODbL, (c) OpenStreetMap
...
Output:
  launch_packages/pettripfinder/markets/reports/hampton_roads_va_osm_lane_001.json
"""
...
PBF = os.path.join(_DASH, "data", "osm_extracts", "virginia-latest.osm.pbf")
CONFIG = os.path.join(_DASH, "scripts", "pettripfinder", "discovery", "config",
                      "hampton_roads_va.json")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "hampton_roads_va_osm_lane_001.json")

#: guest_house is kept too: Richmond's historic inns and B&Bs are often tagged guest_house, and the census
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ("pbf", "data/osm_extracts/virginia-latest.osm.pbf"),
            ...
            ("pbf_snapshot",
             "the 2026-09-14 Geofabrik Virginia download made by this order; the upstream file may since "
