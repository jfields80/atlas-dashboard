"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- Phase 26: the SHADOW sealed package (cloned from the Orlando FL V2 helper).

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
------------------------------------------------------------------
West Palm Beach is not at the front of the serialized release queue: the current verified live release is FORT
LAUDERDALE (deploy 6ab1a035c7ef3b14a23a4ec6, 32 markets / 2,375 profiles / 2,646 release-index routes / 2,711
served routes, source e82120cc, resolved mechanically in Phase 1). The generic lane would reissue shared,
parent-sensitive state this order is forbidden to touch. So this helper:

  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/west-palm-beach-fl/launch_package/``) from the proposed market document, the proposed census, the
  ...
  4. writes the package, the receipt and a report ONLY under ``markets/staging/west-palm-beach-fl/``.
...
REPORT = PKG / "markets" / "reports" / "west_palm_beach_fl_shadow_package_001.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "west_palm_beach_fl_proposed_authority_001.json"
PARTITION_SRC = STAGING / "launch_package" / "west_palm_beach_fl_final_partition_001.json"
SEALED_AT = "2026-09-22T08:00:00Z"
...
def stage_tree():
    ...
    # the partition already lives under LP from west_palm_beach_fl_final_partition_001; nothing to copy
