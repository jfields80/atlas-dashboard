"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 26: STAGE Orlando's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/orlando-fl/launch_package/hotel_policy_facts_orlando-fl.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/orlando-fl/orlando_fl_v2_proposed_authority_001.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-MARKET-GA-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
markets stand ahead of it in the serialized release queue (the current live release is Savannah). This order therefore writes NOTHING at
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/orlando-fl.json`, an
...
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_orlando-fl.json")
AUTHORITY_OUT = os.path.join(STAGING, "orlando_fl_v2_proposed_authority_001.json")
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages the market without a row-by-row
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Orlando's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Orlando's authority as this order proposes it, STAGED and not registered. Registration makes the market "
