"""PTF-HAMPTON-ROADS-VA-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Richmond's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/hampton-roads-va/launch_package/hotel_policy_facts_hampton-roads-va.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/hampton-roads-va/hampton_roads_va_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-HAMPTON-ROADS-VA-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/hampton-roads-va.json`, an
...
CLEAN = os.path.join(REPORTS, "hampton_roads_va_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census_proposed", "hampton-roads-va.json")
STAGING = os.path.join(PKG, "markets", "staging", "hampton-roads-va")
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_hampton-roads-va.json")
AUTHORITY_OUT = os.path.join(STAGING, "hampton_roads_va_proposed_authority_002.json")
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Richmond without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages Richmond without a row-by-row
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Richmond's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Richmond's authority as this order proposes it, STAGED and not registered. Registration makes the market "
