"""PTF-ORLANDO-FL-PARALLEL-SOURCE-READY-001 -- Orlando, FL shadow source-ready market build.

Deterministic: reads only committed, market-owned raw captures and rulings under
``launch_packages/pettripfinder/markets/staging/orlando-fl/`` and writes only
Orlando-owned outputs (staging launch package + report). No shared registry,
participation, release contract, global authority or live state is touched.

Pipeline
  1. leads      DBPR licences, Hilton inventory, owned Marriott codes, Visit Orlando, OSM
  ...
  6. evidence   orlando_fl_policy_evidence_001 (durable pages + shared readers)
  7. outputs    census, partition, market config, authority shards, evidence ledger, accounting

Run:
    python -m scripts.pettripfinder.orlando_fl_market_build_001 [--out-root DIR]
"""
...
from scripts.pettripfinder import orlando_fl_identity_rules_001 as R  # noqa: E402
from scripts.pettripfinder import orlando_fl_policy_evidence_001 as EV  # noqa: E402

WORK_ORDER = "PTF-ORLANDO-FL-PARALLEL-SOURCE-READY-001"
...
REFUSING_BRAND_DOMAINS = ("marriott.com", "ihg.com", "hyatt.com", "choicehotels.com", "bestwestern.com", "wyndhamhotels.com",
                          "redroof.com", "motel6.com", "omnihotels.com", "radissonhotels.com", "fourseasons.com", "druryhotels.com",
                          "hilton.com", "holidayinn.com", "ritzcarlton.com")
CLIENT_RENDERED_DOMAINS = ("disneyworld.disney.go.com", "universalorlando.com")
...
def build_leads():
    ...
    for r in _load("dbpr_lodging_inscope_001.json")["licences"]:
        g = r.get("geocode") or {}
        if g.get("match_type") in ("NOMINATIM:city", "NOMINATIM:place"):
            g = {}
    ...
    for r in _load("hilton_inventory_001.json")["hotels"]:
        leads.append(dict(src="HILTON", sid=r["property_code"], name=r["name"], street=r["street"] or "", city=r["city"] or "", zip=(r["postal"] or "")[:5],
                          lat=r["lat"], lng=r["lng"], url=r["url"] or "", phone=r["phone"] or "", extra={"brand_code": r["brand_code"], "open": r.get("open")}))
    for r in _load("visitorlando_hotels_001.json")["docs"]:
        leads.append(dict(src="VISITORLANDO", sid=str(r["recid"]), name=r["title"], street=r.get("address1") or "", city=r.get("city") or "", zip=(r.get("zip") or "")[:5],
                          lat=r.get("latitude"), lng=r.get("longitude"), url=r.get("weburl") or "", phone=r.get("phone") or "", extra={"listing": r.get("absolute_url")}))
    for e in _load("osm_overpass_lodging_001.json")["elements"]:
        t = e["tags"]
        if not t.get("name"):
            continue
    ...
    for code, slug in _load("marriott_owned_harvest_codes_001.json")["codes"].items():
...
def resolve(all_leads):
    in_box = [l for l in all_leads if l["lat"] is None or (27.9 < l["lat"] < 29.2 and -82.2 < l["lng"] < -80.8)]
    DB = [l for l in in_box if l["src"] == "DBPR"]
    HI = [l for l in in_box if l["src"] == "HILTON"]
    MA = [l for l in in_box if l["src"] == "MARRIOTT_HARVEST" and (l["sid"].startswith("mco") or l["sid"] == "dabdl")]
    OT = [l for l in in_box if l["src"] in ("VISITORLANDO", "OSM") and (l["src"] != "OSM" or l["extra"].get("tourism"))]
    ...
    city_tokens = ("kissimmee", "celebration", "davenport", "clermont", "ocoee", "sanford", "lake mary", "altamonte", "winter park", "winter haven", "leesburg", "deland", "lake nona")
    for mh in MA:
...
def apply_identity_rulings(identities):
    rulings = json.loads((STAGING / "identity_rulings_001.json").read_text(encoding="utf-8"))
    by_anchor = {i["anchor"]: i for i in identities}
    applied = []
    for b in rulings["bind_brand_code_to_licence"]:
    ...
    for c in rulings["classify_identity"]:
...
def best_name(ident, lead_by_key, shared_leads=frozenset()):
    ...
    order = [l for l in ident["leads"] if l["src"] == "HILTON"] + [l for l in ident["leads"] if l["src"] == "VISITORLANDO"] + [l for l in ident["leads"] if l["src"] == "OSM"]
    if ident["kind"] == "BRAND_CODE":
        slug_name = ident["leads"][0]["name"]
        order = sorted(order, key=lambda l: (-round(R.name_sim(slug_name, l["name"]), 3), {"HILTON": 0, "VISITORLANDO": 1, "OSM": 2}[l["src"]], l["key"]))
...
def classify_all(identities, lead_by_key):
    ...
    for ident in identities:
        ...
        order = {"HILTON": 0, "VISITORLANDO": 1, "OSM": 2, "DBPR": 3}
        coord = sorted([p for p in pts if p.get("lat")], key=lambda p: (order[p["src"]], p["key"]))
        addrp = sorted([p for p in pts if p.get("street")], key=lambda p: ({"DBPR": 0, "HILTON": 1, "VISITORLANDO": 2, "OSM": 3}[p["src"]], p["key"]))
...
def route_for(row):
    ...
    for code in ident["brand_codes"]:
        for l in ident["leads"]:
            if l["src"] in ("HILTON", "MARRIOTT_HARVEST") and l["url"]:
                urls.append(l["url"])
    for l in sorted(ident["leads"], key=lambda l: ({"VISITORLANDO": 0, "OSM": 1}.get(l["src"], 2), l["key"])):
        u = (l["url"] or "").strip()
        if u and "doubleclick" not in u and "h-rez.com" not in u and u not in urls:
    ...
    for u in urls:
        ...
        if any(host.endswith(d) for d in REFUSING_BRAND_DOMAINS + CLIENT_RENDERED_DOMAINS):
            if any(rx.search(u.lower()) for rx in PROPERTY_URL_RX) or any(host.endswith(d) for d in CLIENT_RENDERED_DOMAINS):
...
def partition_state(row, evidence, url_status):
    ...
    if "FETCH_FAILED" in decisions:
        return enums.AWAITING_ROUTING_REPLACEMENT, "Bound brand page returned HTTP 404 at capture", url, route_class, ev, "ROUTING"
    ...
    if route_class == "BRAND_PROPERTY_URL":
        if any(host.endswith(d) for d in CLIENT_RENDERED_DOMAINS):
            return enums.AWAITING_ATTENDED_CAPTURE, "Official resort page is a client-rendered shell to a plain client; attended capture required", url, route_class, ev, "ATTENDED"
        return enums.ACCESS_BLOCKED, "Brand property page refuses a plain client (403 / timeout) this run; attended capture was not permitted on this origin", url, route_class, ev, "ACCESS"
...
def main(argv=None) -> int:
    ...
    leads = build_leads()
    import os, pickle
    cache = os.environ.get("ORLANDO_RESOLVE_CACHE")
    if cache and os.path.exists(cache):
        identities, decisions, ambiguous, lead_by_key = pickle.load(open(cache, "rb"))
    else:
        identities, decisions, ambiguous, lead_by_key = resolve(leads)
        if cache:
            pickle.dump((identities, decisions, ambiguous, lead_by_key), open(cache, "wb"))
    ...
    url_status = _load("lane_attempts_001.json")["url_last_status"]
    ...
    for r in admitted:
        ...
        lic = [lead_by_key[k] for k in ident["licenses"]]
        carried = {
            "orlando_anchor": ident["anchor"], "identity_kind": ident["kind"], "geography_class": r["geo"], "geography_rule": r["geo_rule"],
        ...
        census_rows.append(census_row(
            identity_key=key, canonical_name=name, slug=slugify(name), market_id=MARKET_ID, city=r["city"] or "Orlando", state="FL",
            postal_code=r["postal"], identity_state=identity_state, lodging_state=lodging, policy_state=policy,
            source="PTF-ORLANDO-FL lanes: " + ", ".join(r["sources"]), source_id=ident["anchor"], address=r["street"],
            phone=(lic[0]["phone"] if lic else ""), corridor=r["corridor"] or "", assignment_basis=enums.BASIS_EXPLICIT if r["corridor"] else "",
            assignment_value=r["corridor"] or "", collision_state=collision, observed_at=AS_OF, provenance="orlando_fl_market_build_001",
            official_url=url or "", carried=carried))
        partition_items.append(partition_item(
            identity_key=key, canonical_name=name, slug=slugify(name), city=r["city"] or "Orlando", state="FL", postal_code=r["postal"],
    ...
    census_doc = census_document(MARKET_ID, census_rows, captured_at=AS_OF,
                                 note="Greater Orlando shadow census (%s). Backbone: Florida DBPR active public-lodging licences (HOTL/MOTL); brand, CVB and OSM lanes bind names, codes, URLs and coordinates. Timeshare, vacation-rental, resort-residence, non-hotel, restricted and outside identities are accounted in the accounting report, not admitted." % WORK_ORDER,
                                 source_authorities=["orlando_fl_market_build_001", "raw_captures/dbpr_lodging_inscope_001.json", "raw_captures/hilton_inventory_001.json",
                                                     "raw_captures/visitorlando_hotels_001.json", "raw_captures/osm_overpass_lodging_001.json", "raw_captures/marriott_owned_harvest_codes_001.json"])
    census_doc["work_order"] = WORK_ORDER
    partition_doc = partition_document(MARKET_ID, partition_items, as_of=AS_OF,
                                       note="Orlando shadow partition. PUBLISHED_PET_FRIENDLY / VERIFIED_NO_PETS only where a durable first-party page was read by the shared readers; every other row carries one honest blocker.",
                                       source_authorities=["orlando_fl_market_build_001", "orlando_fl_policy_evidence_001"])
    ...
    for order, (cid, label, klass) in enumerate(R.CORRIDORS, start=1):
        ...
        corridors.append({
            ...
            "title": "Pet-Friendly Hotels in %s | PetTripFinder Orlando" % label,
            "meta_description": "Verified pet-friendly hotels in %s, Greater Orlando, Florida, with pet policies read from each hotel's own official website." % label,
            "description": "%s corridor of Greater Orlando (%s); assignment is explicit per hotel from coordinates and city (orlando_fl_identity_rules_001.assign)." % (label, klass),
    ...
    market_config = {
        "schema": "ptf-market/1.1", "market_id": MARKET_ID, "market_name": "Orlando – Greater Orlando / Central Florida", "market_slug": MARKET_ID,
        "state_name": "Florida", "state_code": "FL", "primary_state_code": "FL", "states": ["FL"], "primary_city": "Orlando", "country_code": "US",
        "title": "Pet-Friendly Hotels in Orlando, Florida | PetTripFinder",
        "meta_description": "Verified pet-friendly hotels across Greater Orlando -- Downtown, MCO, International Drive, Universal, SeaWorld, Lake Buena Vista, Walt Disney World and Kissimmee.",
        "introductory_copy": "", "navigation_label": "Orlando, FL", "show_in_navigation": False, "show_in_sitemap": False,
    ...
    out["partition"] = write_json(lp / ("orlando_fl_final_partition_001.json"), partition_doc)
    ...
    out["evidence_ledger"] = write_json(lp / "orlando_fl_policy_evidence_ledger_001.json", {
        "schema": "ptf-orlando-fl-policy-evidence-ledger/1.0", "market_id": MARKET_ID, "work_order": WORK_ORDER, "count": len(ledger_rows),
    ...
    out["accounting"] = write_json(reports_out / "orlando_fl_source_ready_accounting_001.json", acct)
    out["identity_resolution"] = write_json(staging_out / "orlando_fl_identity_resolution_001.json", {
        "schema": "ptf-orlando-fl-identity-resolution/1.0", "market_id": MARKET_ID,
...
def accounting(rows, census_meta, ambiguous, decisions, applied, evidence_records, market_config):
    ...
    disney = [m for m in census_meta if m["corridor"] in ("walt-disney-world", "disney-springs", "flamingo-crossings", "lake-buena-vista")]
    universal = [m for m in census_meta if m["corridor"] == "universal-orlando"]
    kiss = [m for m in census_meta if m["corridor"] in ("kissimmee-us-192-maingate", "kissimmee-downtown-east-us-192", "celebration")]
    return {
        "schema": "ptf-orlando-fl-source-ready-accounting/1.0", "market_id": MARKET_ID, "work_order": WORK_ORDER, "as_of": AS_OF,
        ...
        "theme_park_audit": {
            "DISNEY_AREA_QUALIFYING_HOTELS": len(disney), "DISNEY_AREA_PET_FRIENDLY": sum(1 for m in disney if m["state"] == enums.PUBLISHED_PET_FRIENDLY),
            "UNIVERSAL_AREA_QUALIFYING_HOTELS": len(universal), "UNIVERSAL_AREA_PET_FRIENDLY": sum(1 for m in universal if m["state"] == enums.PUBLISHED_PET_FRIENDLY),
            "KISSIMMEE_QUALIFYING_HOTELS": len(kiss), "KISSIMMEE_PET_FRIENDLY": sum(1 for m in kiss if m["state"] == enums.PUBLISHED_PET_FRIENDLY),
            ...
            "no_unit_in_census_proof": "every census row carries a DBPR HOTL/MOTL licence (or is a MIXED_RESORT_HOLD with lodging_state NEEDS_REVIEW); no TAPT/CNDO/DWEL unit licence, numbered villa unit, timeshare or vacation-rental identity is admitted -- checked by FAST O4.",
        },
