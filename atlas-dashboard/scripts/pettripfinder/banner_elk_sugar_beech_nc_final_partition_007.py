"""PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 -- the final partition.

Every census Banner Elk - Sugar - Beech identity, in EXACTLY ONE state, with exactly one next
...
Output: launch_packages/pettripfinder/markets/staging/banner-elk-sugar-beech-nc/launch_package/banner_elk_sugar_beech_nc_final_partition_007.json
...
OUT = os.path.join(PKG, "markets", "staging", "banner-elk-sugar-beech-nc", "launch_package",
                   "banner_elk_sugar_beech_nc_final_partition_007.json")

#: A family this order MEASURED as refusing a plain client on this run, with the
#: status it returned. Used only to explain an ACCESS_BLOCKED row. No census row of this
#: market carries a brand whose surface refused (Marriott's one property was read).
REFUSALS = {}
#: Independent properties whose OWN site refused this client on this run.
SITE_REFUSALS = {
    "top of the beech inn": "a plain-client 403 on the operator's beechmountaininns.com and no JavaScript permission for "
                            "that domain in the attended browser; the inn's own beech-mountain.org pages state its "
                            "address and no pet policy",
}
#: Buildings whose own page SERVED but states no operative, bindable policy, by census name.
POLICY_SILENT = {
    "little main street inn & suites": "the inn's own reservation page (v2.reservationkey.com, linked from its site) says "
                                       "'Pets: please call for pet reservations. There is a $20 per night fee for each "
                                       "pet.' -- FEE_ONLY to the shared reader, and that page states no street and "
                                       "postal code; held, never reworded",
    ...
    "huskins court motel & cottages": "the motel's own site (two small pages) served with no pet wording and states no "
                                      "street address; identity rests on the map row and the site's name and phone",
}
...
def build():
    ...
    routing = _load(os.path.join(REPORTS, "banner_elk_sugar_beech_nc_routing_001.json"), {})
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 "
                                         "acquisition passes: owned Boone evidence, brand city pages, "
                                         "brand sitemaps, OSM extract, the Banner Elk / Beech Mountain / Avery / "
                                         "High Country / Sugar Mountain rosters, the independents' own "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-14"),
        ("what_this_is",
         "Every census Banner Elk - Sugar - Beech identity in exactly one disposition state. A terminal state "
