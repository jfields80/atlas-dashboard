"""PTF-RALEIGH-NC-FINAL-FRESH-CITY-PROOF-001 -- Phase 8: register Raleigh.
...
from, and validates both against the contracts that own them:
  ...
  1. the committed POLICY PACKAGE  launch_packages/pettripfinder/hotel_policy_facts_raleigh-nc.json
  ...
  2. the PROPOSED AUTHORITY        launch_packages/pettripfinder/raleigh_nc_proposed_authority_002.json
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/raleigh-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "raleigh_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census", "raleigh-nc.json")
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_raleigh-nc.json")
AUTHORITY_OUT = os.path.join(PKG, "raleigh_nc_proposed_authority_002.json")
OBSERVED_AT = "2026-09-11"
CAPTURED_AT = "2026-09-11T19:30:00+00:00"
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order registers Raleigh without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Greater Raleigh, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Raleigh's committed policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Raleigh's authority as this order proposes it. Registration makes the market "
