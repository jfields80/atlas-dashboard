"""PTF-CHARLESTON-SC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002 -- Charleston's same-address-key rulings.

The source-ready order held two same-premises pairs:

  * 406 Sigma Drive, 29486 -- Hilton Garden Inn Summerville (Hilton chsgigi) and Homewood Suites by Hilton
                              Summerville (Hilton chssmhw): a dual-brand building with one switchboard, both
                              pet-friendly (held CO_LOCATION_RULING_REQUIRED);
  * 560 King Street, 29403 -- Hyatt House Charleston/Historic District (Hyatt chsxh) and The Lowline Hotel (Hyatt
                              chszh): held in the census as SAME_CAMPUS_DISTINCT_ENTITY because the shared
                              co_located_distinct proof reads no Hyatt route code.

A ruling is DATA under the existing publication-guard contract (same_campus_distinct_entity: one address key,
distinct entities), the shape Charlotte, Raleigh, Atlanta and Richmond recorded. No code, schema or rule changes.
...
The Hyatt pair cannot meet the third condition (the shared proof returns INSUFFICIENT for hyatt.com URLs), and its
census rows are not admitted, so no ruling is written for it; both stay held. Nothing merges by display name, phone or
row order. A ruling waives the STREET-IDENTITY match and nothing else and asserts nothing about either property's
policy.

Output: launch_packages/pettripfinder/identity_resolutions.json  (rows appended, --write)
        launch_packages/pettripfinder/markets/reports/charleston_sc_co_location_003.json
"""
...
from scripts.pettripfinder.charleston_sc_census_reconciliation_001 import directional_conflict  # noqa: E402

WORK_ORDER = "PTF-CHARLESTON-SC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002"
...
OUT = os.path.join(REPORTS, "charleston_sc_co_location_003.json")
    ...
    for key, recs in by_street.items():
        ...
        row = OrderedDict((
            ...
            ("evidence",
             ...
             "PTF-CHARLESTON-SC-PARALLEL-SOURCE-READY-001 for a registration-order ruling; this resolution waives the "
