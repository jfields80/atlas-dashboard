"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phase 12C, free first-party static capture (router rung DIRECT_STATIC_FETCH).

Cloned from the Orlando FL V2 helper.

One HTTPS GET per route through the canonical gates, for two cohorts:

  ROUTED          every admitted Northeast Florida identity with a first-party route. The question is the PET
                  POLICY.
...
Output:
  launch_packages/pettripfinder/markets/reports/jacksonville_fl_free_static_capture_001.json
"""
...
BRANDS = [
    ("MARRIOTT", r"marriott|courtyard by marriott|courtyard marriott|courtyard jacksonville|courtyard amelia|courtyard by|courtyard(?= (jacksonville|downtown|airport|butler|orange|beach|amelia|flagler|mayo|north|south))|residence inn|springhill|fairfield|towneplace|ac hotel|"
...
def text_bound_read(attempt_root: Path, row, brand: str):
    ...
    if not declined.is_file() or row is None:
        return None
    # NORTHEAST FLORIDA: a declined artifact can go unreadable between the gate writing it and this reader opening it
...
def targets():
    ...
    for r in routing["routes"]:
        ...
        # (jacksonville_fl_wyndham_lane_001); fetching the client-rendered overview here would read nothing.
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS,
                                                  "jacksonville_fl_free_static_capture_001.json"))
    ap.add_argument("--run-id", default="jacksonville_fl_free_static_001")
