"""PTF-NASHVILLE-TN-NEW-MARKET-001 -- Phase 11, the Firecrawl rung.

The Cincinnati phase-11 harness re-pointed at ``nashville-tn``. The cohort is
computed by ``ladder.plan_cohort`` from this order's committed static report and
is never typed in: 17 rows across IHG (4), Choice (5) and Wyndham (8), the three
families the committed route table sends to Firecrawl on a measured decision.
Marriott and Hilton are a measured capability wall and are not candidates at any
budget -- 87 rows in this market's ladder plan carry
FIRECRAWL_KNOWN_CAPABILITY_WALL, and every one of them was already read by the
attended browser for nothing.
...
exactly this shape, and this cohort is smaller than every one it was set on:
Indianapolis FIRECRAWL-RECOVERY-021 attempted 36 rows for 17 credits, Toledo
NEW-MARKET-001 attempted 19 for 19, and Cincinnati HARDENED-REVALIDATION-001
attempted its cohort under a 7-credit cap. SEVENTEEN attempts against a
459-credit plan balance is under four percent of it, and no USD is involved at
any point. Anything larger would have been ONE grouped founder question rather
than a decision this order made for itself.
...
Output:
  launch_packages/pettripfinder/markets/reports/nashville_tn_firecrawl_pass_001.json
"""
...
STATIC_REPORT = os.path.join(REPORTS, "nashville_tn_free_static_capture_001.json")
SPACING_SECONDS = 2.0
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="nashville_tn_firecrawl_001")
    ap.add_argument("--cap-attempts", type=int, default=19)
    ap.add_argument("--floor-credits", type=int, default=400)
    ap.add_argument("--out", default=os.path.join(REPORTS, "nashville_tn_firecrawl_pass_001.json"))
