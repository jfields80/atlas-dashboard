"""PTF-CHARLESTON-SC-PARALLEL-SOURCE-READY-001 -- Phase 4A: the OpenStreetMap lane.
...
Output:
  launch_packages/pettripfinder/markets/reports/charleston_sc_osm_lane_001.json
"""
...
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "charleston_sc_osm_lane_001.json")

#: guest_house is kept too: Charleston's historic inns are often tagged guest_house, and the census
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ...
            ("pbf_snapshot",
             "the 2026-09-10 Geofabrik South Carolina download, reused (hard link) from the Charlotte NC "
             "run's local copy; the upstream file may since have been republished, so this md5 is "
             "the snapshot's own"),
