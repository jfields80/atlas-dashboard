"""PTF-CHARLESTON-SC-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.

Cloned from the Boone - Blowing Rock NC census helper (itself from Outer Banks NC and Atlanta GA).
Independent discovery lanes, none authoritative on its own:
  ...
  OSM_OVERPASS        tourism=hotel / motel / guest_house / apartment / chalet elements
                      inside the observation box, read from the local Geofabrik South
                      Carolina extract (ODbL).
  BRAND_INVENTORY     the committed national Marriott harvest (CHS-coded routes);
                      Marriott's South Carolina hotel sitemap page; Hilton's own South
                      Carolina city pages (property cards stating each hotel's address);
                      Wyndham's, Drury's and WoodSpring's own sitemaps.
  DESTINATION_ORGANIZATION  every Lodging listing of the Charleston Area CVB, with the
                      bureau's own categories (Hotels, Bed & Breakfasts, Historic Inns,
                      Extended Stay, Beach Resorts, Cottages, Vacation Rentals).
...
-----------------------
The market contract's postal partition (charleston_sc_geography_001). Kiawah Island
and Seabrook Island (by municipality inside 29455) are OUTSIDE and every such row
carries the future-market reason so the later kiawah-seabrook-sc order starts from it.

VACATION RENTALS, CONDOS, HISTORIC HOMES AND TIMESHARES ARE NOT HOTELS
--------------------------------------------------------------------
Charleston, its beaches and its resort islands carry large vacation-rental,
historic-home, condo, villa and property-management inventories. None is admitted: a row whose only typing
evidence is the bureau's "Vacation Rentals" category, a campground or RV park, or
a map apartment / chalet tag is NON_LODGING; named rental companies, apartment
inventory and venues are refused BY NAME with the reason
(charleston_sc_nonhotel_rulings_001).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/charleston-sc.json
  launch_packages/pettripfinder/markets/reports/charleston_sc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/charleston_sc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import charleston_sc_geography_001 as GEO  # noqa: E402
...
#: REGISTERED by PTF-CHARLESTON-SC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002; the source-ready census in identity_census_proposed/ is history.
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "charleston-sc.json"),
                                os.path.join(PKG, "markets", "proposed", "charleston-sc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "charleston_sc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "charleston_sc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "charleston_sc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "charleston_sc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "charleston_sc_destination_roster_001.json")
#: CHARLESTON: the market-local reading of the static lane's own-site JSON-LD addresses (identity only).
STATIC_LANE = os.path.join(REPORTS, "charleston_sc_static_lane_001.json")
POLICY_PAGES_LANE = os.path.join(REPORTS, "charleston_sc_policy_pages_lane_001.json")
...
from scripts.pettripfinder.charleston_sc_nonhotel_rulings_001 import (  # noqa: E402
...
HOTEL_BUREAU_SUBCATEGORIES = {"Hotels", "Bed & Breakfasts", "Historic Inns", "Extended Stay"}
...
_WYNDHAM = re.compile(
    r"wyndhamhotels\.com/([a-z0-9-]+)/([a-z-]+)-south-carolina/([a-z0-9-]+)/overview",
    re.I)
...
def read_brand_city_pages():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if state_code(card.get("state")) not in ("SC",):
            continue
        if (card.get("postal_code") or "")[:3] not in ("294", "299"):
            continue
...
def read_brand_state_sitemap():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if not ((r.get("property_code") or "").lower().startswith("chs") or places_in(r.get("brand_title") or "")):
            continue
...
_PROPERTY_ROUTE = re.compile(
    r"wyndhamhotels\.com/(?![a-z]{2}-[a-z]{2}/)[a-z0-9-]+/[a-z-]+-south-carolina/[a-z0-9-]+/overview$",
    re.I)
...
from scripts.pettripfinder.charleston_sc_static_rulings_001 import (  # noqa: E402
...
def read_identity_only_pages():
    ...
    for name, street, city, postal, phone, url, _sha, note in IDENTITY_ONLY_PAGES:
        out.append(observation(
            "PROPERTY_PAGE_IDENTITY_ONLY", 1, url, name, street=street, city=city, region="SC",
    ...
    for url, sha, addrs in docs:
        ...
        for a in addrs:
            ...
            out.append(observation(
                ...
                region=state_code(a.get("region") or "SC") or "SC", postal=postal,
...
_NOT_OWN_SITE = ("marriott.com", "hilton.com", "ihg.com", "choicehotels.com", "hyatt.com", "bestwestern.com",
                 "wyndhamhotels.com", "charlestoncvb.com", "tripadvisor", "expedia", "booking.com", "vrbo", "airbnb",
...
_IN_MARKET_MUNICIPALITIES = {
    "charleston", "north charleston", "mount pleasant", "mt pleasant", "summerville", "goose creek", "ladson",
    "hanahan", "folly beach", "isle of palms", "sullivans island", "johns island", "james island", "daniel island",
    "west ashley",
}
#: The Kiawah Island / Seabrook Island places, preserved for the future market.
_FUTURE_SUBMARKET_PLACES = {"kiawah island", "kiawah", "seabrook island"}
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {
    "moncks corner", "awendaw", "mcclellanville", "ravenel", "hollywood", "meggett", "wadmalaw island", "edisto",
    "edisto island", "edisto beach", "walterboro", "beaufort", "hilton head", "hilton head island", "bluffton",
    "ridgeland", "hardeeville", "yemassee", "georgetown", "pawleys island", "myrtle beach", "st george",
    "saint george", "harleyville", "ridgeville", "santee", "bonneau", "huger", "saint stephen", "st stephen",
}
...
_DIRECTIONALS = {
    ...
    # CHARLESTON area words that tell same-brand hotels apart (Courtyard Waterfront / Historic District / Airport)
    "airport", "historic", "district", "riverview", "waterfront", "coliseum", "convention", "patriots", "point",
    "northwoods", "university", "ashley", "phosphate", "nexton", "towne", "centre", "medical", "i526", "i26",
    "harbor", "riverfront", "tanger", "king", "meeting", "market", "battery",
}
...
def _route_numbers(street):
    ...
    toks = re.sub(r"[^a-z0-9 ]", " ", (street or "").lower()).split()
    if len(toks) < 2 or not re.search(r"\b(sc|ga|hwy|highway|us|sr|route)\b", " ".join(toks[1:])):
        return ()
...
_WYNDHAM_URL_CITY = re.compile(r"wyndhamhotels\.com/[a-z0-9-]+/([a-z-]+)-south-carolina/", re.I)
...
#: evidence alone.
ADMITTED_STATES = frozenset({"SC", "SOUTH CAROLINA"})
...
CORRIDOR_CITIES = {
    ...
    "upper-peninsula": {"charleston"},
    "west-ashley": {"charleston", "west ashley"},
    # North Charleston hotels are often addressed "Charleston, SC 29406"; the ZIP decides.
    "north-charleston": {"north charleston", "charleston", "hanahan", "ladson"},
    "chs-airport-convention": {"north charleston", "charleston"},
    "mount-pleasant": {"mount pleasant", "mt pleasant", "charleston"},
    "daniel-island": {"charleston", "daniel island"},
    "james-island": {"charleston", "james island"},
    "summerville": {"summerville", "ladson", "north charleston"},
    "goose-creek": {"goose creek", "charleston", "north charleston"},
    # the I-26 exit 203 hotels are addressed Ladson or North Charleston inside 29456
    "ladson": {"ladson", "north charleston", "summerville"},
    "folly-beach": {"folly beach"},
    "isle-of-palms": {"isle of palms", "sullivans island"},
    "johns-island": {"johns island", "charleston"},
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and (_subcats & NON_HOTEL_BUREAU_SUBCATEGORIES) and not any(
        ...
        return NON_LODGING, ("the Charleston Area CVB files this listing under %s -- a vacation-rental "
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and _CAMPGROUND_NAME.search(node.name or "") and not any(
            str(o.get("lane", "")).startswith(("PROPERTY_PAGE", "BRAND_INVENTORY")) for o in node.observations):
        return NON_LODGING, ("the Charleston Area CVB files this listing only under %s and its own name is a campground, RV "
    ...
    if not node.postal and _pins and all(float(t) < 32.645 and float(g) < -80.02 for t, g in _pins):
        return OUTSIDE_MARKET, ("FUTURE_SUBMARKET %s -- no postal code of its own, and every source pins it "
                                "on Kiawah / Seabrook Island (%.4f, %.4f)" % (GEO.FUTURE_SUBMARKET, float(_pins[0][0]),
                                                                              float(_pins[0][1])))
    if not node.postal and _pins and all(float(t) > 33.12 or float(g) < -80.26 or float(g) > -79.69
                                         for t, g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it beyond the admitted "
                                "corridors (Moncks Corner north, Ravenel / Edisto west or Awendaw east; %.4f, %.4f)"
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Charleston "
...
_COVERED_FLAGS = (
    ("HILTON", r"\b(hampton|hilton|home2|homewood|embassy suites|doubletree|spark by hilton|tru by hilton|tempo|signia)\b"),
...
def brand_not_listed(node):
    ...
    for fam, rx in _COVERED_FLAGS:
        if re.search(rx, node.name or "", re.I):
            return ("a %s row naming a %s flag (%r) that no read of that brand's own Charleston-area inventory "
...
SOURCE_AUTHORITIES = [
    "OpenStreetMap via the Geofabrik South Carolina extract (2026-09-10 snapshot), reduced to this market's "
    ...
    "national Marriott harvest, CHS-coded routes)",
    "https://www.marriott.com/en-us/hotel-sitemap/usa-south-carolina-hotel-sitemap (the brand's own South Carolina "
    "hotel sitemap page)",
    "https://www.hilton.com/en/locations/usa/south-carolina/<city>/ (South Carolina city pages and Charleston "
    "sub-pages, with property cards)",
    "https://www.wyndhamhotels.com/sitemap.xml, https://www.druryhotels.com/sitemap.xml and "
    "https://www.woodspring.com/sitemap.xml (the brands' own property shards); Wyndham overview pages and the "
    "brand's own property service",
    "https://www.charlestoncvb.com every Lodging category page, the bureau's listings sitemap and every lodging "
    "listing (Charleston Area CVB, plain client)",
    "the brands' own property pages read in the attended browser: IHG, Hyatt, Choice, Best Western, Red Roof",
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-CHARLESTON-SC-PARALLEL-SOURCE-READY-001 Charleston census, built from zero "
         "under the modern factory and registered to identity_census/ by PTF-CHARLESTON-SC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Every row carries "
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Charleston authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "charleston-sc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "charleston_sc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "charleston_sc_competitor_gap_matrix_001.json"))
