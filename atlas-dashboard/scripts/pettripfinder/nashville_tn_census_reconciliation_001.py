"""PTF-NASHVILLE-TN-NEW-MARKET-001 -- Phases 5, 6 and 8: one identity graph.

Independent discovery lanes ran for Nashville, none of them authoritative on
its own:

  OSM_OVERPASS        hotel and motel nodes inside this market's twenty-nine
                      discovery cells, read from OpenStreetMap under the ODbL.
  BRAND_INVENTORY     Marriott's BNA* roster, OWNED at zero requests from the
                      committed Dayton harvest, plus every brand roster this
                      order could read from the brand's OWN city page.
  CVB_PARTNER         the official Nashville destination organisation (tier 2).
  COMPETITOR_LEAD     BringFido's Greater Nashville city pages (tier 4, leads
                      only, never policy).
...
-----------------------
The proposed market contract's postal partition, and nothing else. This market
holds five fringe areas -- Franklin/Cool Springs, Mount Juliet, Hendersonville,
Smyrna/La Vergne and Lebanon -- as corridors that claim NO postal code, so
their hotels are discovered, identified and then classified OUTSIDE_MARKET with
their ZIP recorded. That is the point: a founder rules on an inventory that was
looked at, and the ruling is a one-line ZIP move.

Nothing here fetches. Nothing here writes outside Nashville artifacts. Nothing
...
Outputs:
  ...
  launch_packages/pettripfinder/markets/reports/nashville_tn_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/nashville_tn_competitor_gap_matrix_001.json
"""
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "nashville-tn.json"),
                                os.path.join(PKG, "markets", "proposed", "nashville-tn.json"))

CANDIDATES = os.path.join(_DASH, "data", "discovery", "nashville_tn_free_001", "candidates",
                          "nashville-tn_candidates.json")
STATIC_CAPTURE = os.path.join(REPORTS, "nashville_tn_free_static_capture_001.json")
FIRECRAWL_PASS = os.path.join(REPORTS, "nashville_tn_firecrawl_pass_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "nashville_tn_attended_capture_001.json")
OWNED = os.path.join(REPORTS, "nashville_tn_owned_evidence_001.json")
CITY_PAGES = os.path.join(REPORTS, "nashville_tn_brand_city_pages_001.json")
LEADS = os.path.join(REPORTS, "nashville_tn_lead_sources_001.json")
...
NOT_LODGING_NAMES = {
    "blackbird studios", "ocean way nashville", "rca studio b", "historic rca studio b",
    "the tracking room", "sound stage studios", "welcome to 1979 recording studio",
    "nashville film studios", "soundcheck nashville", "hotel management company",
    "the studio nashville", "studio bank",
}
...
STR_PLACEHOLDERS = {"nashville airbnb rentals", "vrbo nashville", "airbnb nashville",
                    "vrbo rentals", "nashville vacation rentals", "airbnb rentals"}
...
class Node:
    ...
    def absorb(self, obs):
        ...
        # The longest name wins: "Courtyard by Marriott Nashville at Opryland"
...
_WYNDHAM = re.compile(r"wyndhamhotels\.com/([a-z0-9-]+)/([a-z-]+)-tennessee/([a-z0-9-]+)/overview", re.I)
...
#: Nashville hotels were classified OUTSIDE_MARKET by exactly that before this
...
_SLUG_NOISE = {"hotels", "hotel", "overview", "index", "en", "us", "tn", "tennessee"}
...
def read_brand_city_pages():
    ...
    Toledo only had Hilton on this rung. Nashville is a far larger market and
...
def read_cvb():
    ...
    partner. Nashville's destination organisations are probed the same way and
    ...
    doc = _load(LEADS, {}) or {}
    out = []
    for org, block in (doc.get("official_destination_sources") or {}).items():
        for url in block.get("lodging_leads") or []:
...
def read_property_pages():
    ...
    allowed to turn a brand-roster row into a Nashville identity.
...
#: market's own name is here on purpose -- "nashville" is in most of these names
...
_GENERIC_TOKENS = {
    ...
    "extended", "stay", "america", "select", "s", "nashville",
    # PARENT COMPANIES. One source writes "Residence Inn by MARRIOTT Nashville
    # Vanderbilt" and the next writes "Residence Inn Nashville Vanderbilt";
    ...
    # what names the hotel. Nashville adds the soft-brand collection words --
...
_IN_MARKET_MUNICIPALITIES = {
    "brentwood", "goodlettsville", "madison", "antioch", "hermitage", "old hickory",
    "donelson", "bellevue", "belle meade", "berry hill", "forest hills", "oak hill",
    "joelton", "whites creek", "opryland", "germantown", "inglewood",
}
...
#: Marriott's BNA prefix, which is the Nashville AIRPORT code and reaches
#: Clarksville, Cookeville, Columbia, Murfreesboro, Manchester, Tullahoma and
#: two Kentucky towns -- every one of them observed in the owned roster.
_OUT_OF_MARKET_PLACES = {
    ...
    "franklin", "cool springs", "mount juliet", "mt juliet", "hendersonville",
    "smyrna", "la vergne", "lavergne", "lebanon",
    # Excluded outright. Every one of these is reached by a BNA-prefixed
    # Marriott property code, which is exactly why a code never admits a row.
    "murfreesboro", "clarksville", "cookeville", "columbia", "manchester",
    "tullahoma", "hopkinsville", "bowling green", "gallatin", "spring hill",
    "dickson", "springfield", "nolensville", "thompsons station", "white house",
    "millersville", "ashland city", "kingston springs", "pegram", "portland",
    "shelbyville", "lewisburg", "mcminnville", "dale hollow", "berry farms",
}
#: "Nashville" is not a distinguishing word inside a hotel NAME -- most of this
...
_ALL_PLACES = _IN_MARKET_MUNICIPALITIES | _OUT_OF_MARKET_PLACES | {"nashville"}
#: Area and directional words. They are the difference between two hotels of the
#: same brand in the same town: this market has SIX Marriott properties whose
#: names differ only in the area -- Nashville AIRPORT, DOWNTOWN, VANDERBILT WEST
#: END, GREEN HILLS, METROCENTER and OPRYLAND -- so the area word is the whole
#: difference between them.
_DIRECTIONALS = {
    "north", "south", "east", "west", "northwest", "northeast", "southwest", "southeast",
    "downtown", "midtown", "uptown", "airport", "university", "vanderbilt", "gulch",
    "sobro", "broadway", "opryland", "metrocenter", "metro", "center", "green",
    "hills", "hillsboro", "village", "belmont", "music", "row", "valley", "yards",
    "capitol", "district", "convention", "briley", "rivergate", "maryland", "farms",
    "hickory", "hollow", "harding", "charlotte", "pike", "bank", "stadium",
    "riverfront", "waterfront", "mall", "campus", "medical", "brentwood",
}
...
def specific_municipality(places):
    ...
    named = {p for p in places if p != "nashville"}
    return named or ({"nashville"} if "nashville" in places else set())
...
def explicit_index(cfg):
    ...
    corridor. Two Nashville districts need this -- Opryland shares 37214 with
    the airport, and the West End spine shares 37203 with the Gulch -- and the
    lists are derived from the street each property's own page states.
    """
    out = {}
    for c in cfg.corridors:
...
def classify(node, zips, name_counts, street_counts):
    ...
    if node.postal and node.postal[:5] not in zips:
        return OUTSIDE_MARKET, ("postal code %s is claimed by no corridor of the proposed "
                                "Nashville contract. Five fringe areas -- Franklin/Cool Springs, "
                                "Mount Juliet, Hendersonville, Smyrna/La Vergne and Lebanon -- "
...
def build():
    ...
    # fully-addressed, identity-confirmed Nashville hotels because a BringFido
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-NASHVILLE-TN-NEW-MARKET-001 proposed Nashville census. It lives in "
        ...
        ("source_authorities", [
            "OpenStreetMap via the public Overpass API (ODbL, "
            ...
            "https://www.hilton.com/en/locations/usa/tennessee/<city>/ (sixteen city pages)",
            ...
            "https://www.druryhotels.com/locations/nashville-tn",
            "https://www.woodspring.com/extended-stay-hotels/locations/tennessee/nashville",
    ...
    report = OrderedDict([
        ...
        ("first_party_naming", OrderedDict([
            ("what_it_is",
             ...
             "Nashville row named 'Hampton' collides across markets and the seed assembler "
        ...
        ("city_backfill", OrderedDict([
            ("what_it_is",
             ...
             "is named 'Nashville' by its own operator."),
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Nashville authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(PROPOSED_CENSUS_DIR, "nashville-tn.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "nashville_tn_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "nashville_tn_competitor_gap_matrix_001.json"))
