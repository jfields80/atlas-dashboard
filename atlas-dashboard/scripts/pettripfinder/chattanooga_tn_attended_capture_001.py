"""PTF-CHATTANOOGA-TN-NEW-MARKET-001 -- Phase 13, the attended browser pass.
...
(PTF-FIRECRAWL-HARD-LANES-003) and both refuse this project's plain client -- on
this order's own static pass, 23 Marriott and 20 Hilton routes all returned
ACCESS_DENIED. This is the only free lane that reads them.
...
nowhere else:
  ...
  HILTON    ``__NEXT_DATA__`` carries a ``petsInfo`` node -- petsAllowed,
            ...
            prose that can say pets were allowed. That is a review, not a
            policy, and the walk is anchored on the node's own key set so it can
            never be read.
...
Each brand is batched by SAME-ORIGIN FETCH from one tab already on the brand's
own origin -- the Dayton precedent -- rather than by one navigation per hotel.

A PROPERTY CODE SELECTS; THE PAGE ADMITS
----------------------------------------
This pass is what settles that. Marriott's ``cha`` prefix reaches Cleveland TN,
Dalton GA, Calhoun GA, Ellijay GA and Rome GA, and Hilton's ``cha`` CTYHOCN
prefix reaches the same region. Every such row is captured and classified on the
address its OWN page states -- not on a guess, and not by omitting it. A Georgia
address is a Phase 6 border hold, not a market membership.

No paid provider. No USD. No credential is entered and no bot check is answered:
every page must serve a plain same-origin fetch from the operator's own session.

Output:
  launch_packages/pettripfinder/markets/reports/chattanooga_tn_attended_capture_001.json
"""
...
CONTRACT = os.path.join(PKG, "markets", "proposed", "chattanooga-tn.json")
...
def build(rows_path):
    ...
    return OrderedDict([
        ...
        ("phase", "13 -- attended browser pass over the Marriott and Hilton wall"),
        ("as_of", CAPTURED_AT),
        ...
        ("a_code_selects_the_page_admits",
         "Marriott's cha prefix reaches Cleveland TN, Dalton GA, Calhoun GA, Ellijay GA and "
         "Rome GA, and Hilton's cha CTYHOCN prefix reaches the same region. Every such row is "
         "captured and classified on the address its OWN page states."),
        ("rows_not_read", src.get("not_read") or []),
        ("counts", OrderedDict([
            ("pages_read", len(out)),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS,
                                                  "chattanooga_tn_attended_capture_001.json"))
