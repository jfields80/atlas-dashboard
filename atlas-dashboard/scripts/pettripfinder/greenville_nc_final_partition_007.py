"""PTF-GREENVILLE-NC-PARALLEL-SOURCE-READY-001 -- the final partition.

Every census Greenville identity, in EXACTLY ONE state, with exactly one next
...
Output: launch_packages/pettripfinder/markets/staging/greenville-nc/launch_package/greenville_nc_final_partition_007.json
...
OUT = os.path.join(PKG, "markets", "staging", "greenville-nc", "launch_package",
                   "greenville_nc_final_partition_007.json")
...
REFUSALS = {
    "CHOICE": "a sitemap timeout and choicehotels.com not being a permitted navigation in the attended session",
    "MOTEL6": "a plain-client timeout on motel6.com and no permitted attended navigation",
    "BEST_WESTERN": "a sitemap 403 and a property route that redirects the attended session to the brand's search page",
    "RADISSON": "a sitemap 403 and an empty 2,833-byte shell for the Country Inn Winterville page in the attended session",
}
...
POLICY_SILENT = {
    "affordable suites extended stay hotel in greenville, nc":
        "the brand's own property page served the attended session and states only a 'Pet-Friendly Rooms' "
        "amenity chip -- never operative",
    "intown suites extended stay greenville":
        "the property's own page (intownsuites.com .../greenville-nc/arlington-blvd/) answers 'No pets allowed at "
        "InTown Suites Greenville NC' beside the street 2111 W Arlington Blvd, but states no postal code, so the "
        "read cannot bind by the page's own street identity; a page that states its postal code (or a founder "
        "ruling on the binding) is needed before a verified-no-pets record publishes",
    "the 5th street inn":
        "the inn's own site served its home page (LocalBusiness JSON-LD 1105 E 5th St 27858) with no pet wording",
}
...
def build():
    ...
    clean = _load(os.path.join(REPORTS, "greenville_nc_clean_authority_001.json"))
    routing = _load(os.path.join(REPORTS, "greenville_nc_routing_001.json"), {})
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-GREENVILLE-NC-PARALLEL-SOURCE-READY-001 "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-13"),
        ("what_this_is",
         "Every census Greenville identity in exactly one disposition state. A terminal state "
