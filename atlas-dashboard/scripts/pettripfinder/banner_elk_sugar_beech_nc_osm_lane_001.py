"""PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 -- Phase 4A: the OpenStreetMap lane.
...
Output:
  launch_packages/pettripfinder/markets/reports/banner_elk_sugar_beech_nc_osm_lane_001.json
"""
...
CONFIG = os.path.join(_DASH, "scripts", "pettripfinder", "discovery", "config",
                      "banner_elk_sugar_beech_nc.json")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "banner_elk_sugar_beech_nc_osm_lane_001.json")

#: guest_house is kept too: High Country inns are often tagged guest_house, and the census
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ...
            ("pbf_snapshot",
             "the Geofabrik North Carolina download the Boone - Blowing Rock run read, reused (hard link) from "
             "that run's local copy; the upstream file has since been republished, so this md5 is the "
             "snapshot's own"),
