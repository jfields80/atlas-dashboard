...
def _prefixed(market: MarketConfig) -> MarketConfig:
    ...
    return MarketConfig(**dict(
        ...
        # PTF-GEOGRAPHY-NORMALIZATION-001: carried through unchanged. This view
        # differs from its market in ROUTE MODE only; re-deriving its geography
        # here would let a report describe a market that does not exist.
        primary_state_code=market.primary_state_code, states=market.states,
...
def build_route_migration_md(market: MarketConfig) -> str:
    ...
    lines = [
        "# %s route migration report (ptf-market/1.1)" % market.market_id,
        "",
