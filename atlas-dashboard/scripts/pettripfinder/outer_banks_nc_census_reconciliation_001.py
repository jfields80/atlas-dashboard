"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.

Cloned from the Atlanta GA census helper. Independent discovery lanes, none
authoritative on its own:
  ...
  DESTINATION_ORGANIZATION  every Lodging listing of the Outer Banks Visitors Bureau,
                      with the bureau's own sub-category (Hotels & Motels, Bed &
                      Breakfasts, Condos/Townhouses, Vacation Rentals, ...).
...
-----------------------
The market contract's postal partition (outer_banks_nc_geography_001). Hatteras
Island and Ocracoke postal codes are OUTSIDE and every such row carries the
future-submarket reason so the later hatteras-ocracoke-nc order starts from it.

VACATION RENTALS, CONDOS AND TIMESHARES ARE NOT HOTELS
-----------------------------------------------------
Outer Banks lodging is dominated by vacation houses, condo complexes, realty
rental programmes and vacation-ownership resorts. None is admitted: a row whose
only evidence types it as a condo / townhouse, a vacation rental, a realty office
or a campground is NON_LODGING; named timeshare resorts and cottage collections
are refused BY NAME with the reason (TIMESHARE_AND_RENTAL_REFUSALS).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/outer-banks-nc.json
  launch_packages/pettripfinder/markets/reports/outer_banks_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/outer_banks_nc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import outer_banks_nc_geography_001 as GEO  # noqa: E402
...
#: REGISTERED by PTF-OUTER-BANKS-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002; the source-ready census in identity_census_proposed/ is history.
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "outer-banks-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "outer-banks-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "outer_banks_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "outer_banks_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "outer_banks_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "outer_banks_nc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "outer_banks_nc_destination_roster_001.json")
...
from scripts.pettripfinder.outer_banks_nc_nonhotel_rulings_001 import NOT_LODGING_WHY, LODGING_UNCONFIRMED  # noqa: E402
...
NON_HOTEL_BUREAU_SUBCATEGORIES = {
    # the bureau's own spellings, as its listing pages print them
    "Vacation Rentals", "Campgrounds / RV Parks", "Condos / Townhouses", "Condos/Townhouses",
    "Cottage Courts",
...
from scripts.pettripfinder.outer_banks_nc_static_rulings_001 import IDENTITY_ONLY_PAGES, COMPETITOR_LEADS  # noqa: E402
...
_IN_MARKET_MUNICIPALITIES = {
    "kitty hawk", "southern shores", "duck", "kill devil hills", "colington", "nags head", "manteo",
    "roanoke island", "wanchese", "corolla", "carova", "manns harbor", "point harbor", "powells point",
    "harbinger",
}
_OUT_OF_MARKET_PLACES = {
    "rodanthe", "waves", "salvo", "avon", "buxton", "frisco", "hatteras", "ocracoke", "grandy",
    "jarvisburg", "coinjock", "barco", "moyock", "currituck", "stumpy point", "east lake", "columbia",
    "engelhard", "elizabeth city", "edenton", "hertford", "virginia beach", "chesapeake", "norfolk",
}
_ALL_PLACES = _IN_MARKET_MUNICIPALITIES | _OUT_OF_MARKET_PLACES
_DIRECTIONALS = {
    "north", "south", "east", "west", "northwest", "northeast", "southwest", "southeast",
    "oceanfront", "soundfront", "soundside", "oceanside", "beach", "beachfront", "pier", "outer",
    "banks", "obx", "island", "village", "harbor", "marina", "waterfront", "milepost", "mp",
    "bypass", "trail", "area", "near", "blvd", "road", "rd", "center", "downtown",
}
...
CORRIDOR_CITIES = {
    "kitty-hawk-southern-shores-duck": {"kitty hawk", "southern shores", "duck"},
    "kill-devil-hills": {"kill devil hills", "colington"},
    "nags-head": {"nags head"},
    "manteo-roanoke-island": {"manteo", "roanoke island"},
    "wanchese": {"wanchese"},
    "corolla": {"corolla", "carova"},
    "manns-harbor": {"manns harbor"},
    "point-harbor": {"point harbor", "powells point", "harbinger"},
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    if normalize_name(node.city) in _OUT_OF_MARKET_PLACES:
        _fs = normalize_name(node.city) in {"rodanthe", "waves", "salvo", "avon", "buxton", "frisco",
                                            "hatteras", "ocracoke"}
    ...
    _subcats = {o.get("bureau_subcategory") for o in node.observations if o.get("bureau_subcategory")}
    if _subcats and _subcats <= NON_HOTEL_BUREAU_SUBCATEGORIES and not any(
            str(o.get("lane", "")).startswith(("PROPERTY_PAGE", "BRAND_INVENTORY")) for o in node.observations):
        return NON_LODGING, ("the Outer Banks Visitors Bureau files this listing under %s -- a vacation-rental, "
                             "condo or campground category, not a hotel establishment; refused under the "
                             "order's vacation-rental filter" % sorted(_subcats))
    ...
    if lanes == {"OSM_OVERPASS"} and all(o.get("non_hotel_name") for o in node.observations) and not \
        ...
        # OUTER BANKS: "Inn" is exempt as well. The Tranquil House Inn, Cypress House Inn and
        # Cameron House Inn are inns with bookable rooms whose names merely contain "house";
        # a genuine rental house named "Inn" is refused BY NAME in the rulings module.
    ...
    # HATTERAS ISLAND / OCRACOKE BY PIN. A row with NO postal code of its own that every
    # source pins south of Oregon Inlet (latitude < 35.78) cannot be an admitted Outer Banks
    # identity whatever else is unknown about it; it is recorded for the future submarket.
    # Refusal only: a pin never ADMITS a building.
    _pins = [o.get("lat") for o in node.observations if o.get("lat") is not None]
    if not node.postal and _pins and all(float(p) < 35.78 for p in _pins):
        return OUTSIDE_MARKET, ("FUTURE_SUBMARKET %s -- no postal code of its own, and every source pins it "
                                "south of Oregon Inlet (Hatteras Island / Ocracoke, lat %.4f)"
                                % (GEO.FUTURE_SUBMARKET, float(_pins[0])))
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Outer Banks "
...
SOURCE_AUTHORITIES = [
    ...
    "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (21 city pages, with property cards)",
    ...
    "https://www.ihg.com/north-carolina-united-states (the brand's own North Carolina page, attended same-origin)",
    "https://www.outerbanks.org/sitemap.xml and every /listing/ page it names (Outer Banks Visitors Bureau)",
...
def build():
    ...
    # OUTER BANKS: a building no property page named keeps the name the Visitors Bureau
    # lists it under ("Comfort Inn South Oceanfront") rather than a bare map label
    # ("Comfort Inn"), so competitor leads can meet it and a bare label never collides.
    for n in nodes:
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 Outer Banks census, built from zero "
         "under the modern factory and registered to identity_census/ by PTF-OUTER-BANKS-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Every row carries "
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Outer Banks authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "outer-banks-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "outer_banks_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "outer_banks_nc_competitor_gap_matrix_001.json"))
