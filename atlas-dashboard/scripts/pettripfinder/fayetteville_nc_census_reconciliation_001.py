"""PTF-FAYETTEVILLE-NC-NORMAL-PRODUCTION-001 -- Phases 4-8: one identity graph.

Independent discovery lanes ran for Fayetteville, none of them
authoritative on its own:
  ...
  BRAND_INVENTORY     Marriott's FAY* roster OWNED at zero requests from the
...
-----------------------
The market contract's postal partition, and nothing else: downtown / Haymount,
Cross Creek / Skibo, FAY airport, North Fayetteville, the I-95 central exits /
Eastover, Spring Lake / Fort Liberty and Hope Mills (CORE), Wade (CORRIDOR),
Raeford (FRINGE). A FAY property code reaches Southern Pines, Lumberton, Dunn and
Rockingham as readily as Fayetteville; every such row is classified on its own
postal code. On-post Fort Liberty lodging is MILITARY_GOVERNMENT_NONPUBLIC and is
never admitted.
...
Outputs:
  launch_packages/pettripfinder/identity_census/fayetteville-nc.json
  launch_packages/pettripfinder/markets/reports/fayetteville_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/fayetteville_nc_competitor_gap_matrix_001.json
"""
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "fayetteville-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "fayetteville-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "fayetteville_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "fayetteville_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "fayetteville_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "fayetteville_nc_attended_capture_001.json")
...
#: Phase 3. On-post Fort Liberty lodging: it requires installation access (a DoD
#: credential or a sponsored visitor pass) and is not bookable by the general
#: traveling public under normal conditions. Matched on the full normalised name.
MILITARY_NONPUBLIC_NAMES = {
    "ihg army hotels in forrestal hall and delmont house":
        "IHG Army Hotels on Fort Liberty (Forrestal Hall / Delmont House): on-post Army lodging reached "
        "only through an installation gate with a DoD credential or sponsored pass",
    "airborne inn lodging":
        "Airborne Inn, the on-post Army Lodging facility on Fort Liberty: transient quarters behind the "
        "installation gate, not a public commercial hotel",
    "landmark inn":
        "Landmark Inn, 1208 Glider Street, Fort Bragg 28307: on-post Army lodging behind the installation "
        "gate, not a public commercial hotel",
}
...
def read_property_pages():
    ...
    allowed to turn a brand-roster or map row into a Fayetteville identity."""
...
#: Phase 6. Competitor directory leads, as a web search result page summarised the
#: BringFido / Booking / Tripadvisor Fayetteville pet-friendly lists in this order
#: (the attended session may not navigate bringfido.com this run). NAMES ONLY -- a
#: lead proposes and never decides, and its pet claims are never read. Each is
#: reconciled in the gap matrix.
_LEAD_SRC = "web search result: pet friendly hotels Fayetteville NC Fort Liberty (BringFido / Booking / Tripadvisor summaries)"
COMPETITOR_LEADS = [
    ("Embassy Suites by Hilton Fayetteville Fort Bragg", _LEAD_SRC),
    ("Hampton Inn & Suites Fayetteville", _LEAD_SRC),
    ("Home2 Suites by Hilton Fayetteville Fort Liberty", _LEAD_SRC),
    ("Tru by Hilton Fayetteville I-95", _LEAD_SRC),
    ("Red Roof Inn & Suites Fayetteville - Fort Bragg", _LEAD_SRC),
    ("Motel 6 Fayetteville NC Fort Liberty Area", _LEAD_SRC),
    ("Studio 6 Fayetteville NC Fort Bragg Area", _LEAD_SRC),
    ("Travelodge by Wyndham Fayetteville", _LEAD_SRC),
]
...
_IN_MARKET_MUNICIPALITIES = {
    "fayetteville", "spring lake", "hope mills", "eastover", "vander", "wade", "raeford",
}
_OUT_OF_MARKET_PLACES = {
    "southern pines", "pinehurst", "aberdeen", "sanford", "lumberton", "dunn", "erwin", "godwin",
    "clinton", "benson", "laurinburg", "rockingham", "saint pauls", "st pauls", "smithfield", "selma",
    "lillington", "elizabethtown", "pembroke", "red springs", "newton grove", "goldsboro",
    "white lake", "wadesboro", "parkton", "stedman",
}
...
_DIRECTIONALS = {
    ...
    # Fayetteville's same-brand pairs differ only in these: "Fort Bragg" / "Fort Liberty"
    # vs "I-95", "Cross Creek", "Skibo", "Cliffdale", "Owen", "Bordeaux", "Exit 49".
    "fort", "bragg", "liberty", "ft", "95", "cross", "creek", "skibo", "cliffdale", "owen", "dr",
    "exit", "49", "west", "bordeaux", "cedar", "rowan",
}
...
#: Wyndham flags with a Fayetteville-area route that redirected to the brand's city
#: search in this order's attended pass (see the capture report): Ramada (2),
#: Wingate, Super 8 Spring Lake, Days Inn Fayetteville and two Baymonts. The rule only
#: ever reaches a MAP-ONLY row -- a building whose own live page was read is not a
#: map-only row -- so the live Super 8, Days Inn and Baymont pages are untouched.
RETIRED_WYNDHAM_FLAGS = ("super 8", "days inn", "ramada", "wingate", "baymont")
...
def classify(node, zips, name_counts, street_counts):
    ...
    # Fayetteville routes for Ramada, Wingate, Super 8 Spring Lake, Days Inn and Baymont now
    ...
    if lanes == {"OSM_OVERPASS"} and any(
        ...
        return IDENTITY_REVIEW_REQUIRED, (
            "a map row under a flag (%s) whose own Fayetteville-area route on the brand's site is "
    ...
    if node.postal and node.postal[:5] not in zips:
        return OUTSIDE_MARKET, ("postal code %s is claimed by no corridor of the Fayetteville "
                                "contract; Fort Liberty on-post (28307 / 28308 / 28310), Southern "
                                "Pines, Pinehurst, Aberdeen, Sanford, Lumberton, Dunn, Stedman and "
                                "Clinton are refused by name in the geography"
                                % node.postal[:5])
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-FAYETTEVILLE-NC-NORMAL-PRODUCTION-001 Fayetteville census, built from zero "
        ...
        ("source_authorities", [
            ...
            "https://www.wyndhamhotels.com/hotels/fayetteville-north-carolina (the brand's own search)",
            ...
            "https://www.extendedstayamerica.com/hotels/nc/fayetteville (the brand's own city page) and its five property pages",
            "each property's own page (attended browser pass and plain static reads)",
            "a web search result page summarising competitor pet-friendly lists (competitor leads; names only)",
        ]),
        ("count", len(confirmed)),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "fayetteville-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "fayetteville_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "fayetteville_nc_competitor_gap_matrix_001.json"))
