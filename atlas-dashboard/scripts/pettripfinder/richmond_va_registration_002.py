"""PTF-RICHMOND-VA-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Richmond's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/richmond-va/launch_package/hotel_policy_facts_richmond-va.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/richmond-va/richmond_va_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-RICHMOND-VA-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/richmond-va.json`, an
...
STAGING = os.path.join(PKG, "markets", "staging", "richmond-va")
#: REGISTERED by PTF-RICHMOND-VA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root, as every registered market.
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_richmond-va.json")
AUTHORITY_OUT = os.path.join(PKG, "richmond_va_proposed_authority_002.json")
...
def build():
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Richmond's REGISTERED policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Richmond's authority as the source-ready order proposed it, registered by PTF-RICHMOND-VA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Registration makes the market "
