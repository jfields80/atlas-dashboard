"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- the Choice / IHG property pages the Firecrawl route-discovery lane found.

Choice's own sitemap timed out to the plain client and IHG's answered 403, so neither family's routes reached the
census by name: a discovered route names a FLAG and a city ("/florida/jacksonville/sleep-inn-hotels/fl123"), never the
...
Outputs:
  ...
  launch_packages/pettripfinder/markets/reports/jacksonville_fl_brand_page_reads_001.json
"""
...
DISCOVERY = os.path.join(REPORTS, "jacksonville_fl_firecrawl_discovery_001.json")
DOCS = os.path.join(_DASH, "data", "acquisition", "jacksonville_fl_brand_page_reads_001")
RAW_OUT = os.path.join(PKG, "markets", "staging", "jacksonville-fl", "raw_captures", "brand_page_rows.json")
REPORT_OUT = os.path.join(REPORTS, "jacksonville_fl_brand_page_reads_001.json")
CAP = 34
FLOOR = 400
#: The market's own towns, as a Choice or IHG route spells them. A route naming another town is another market's.
IN_MARKET = ("jacksonville", "jacksonville-beach", "atlantic-beach", "neptune-beach", "mayport",
             "ponte-vedra", "ponte-vedra-beach", "sawgrass", "nocatee", "orange-park", "fleming-island",
             "middleburg", "green-cove-springs", "oakleaf", "fernandina-beach", "amelia-island", "yulee",
             "callahan", "hilliard", "baldwin", "saint-johns", "st-johns", "fruit-cove", "julington-creek")
...
def main(argv=None):
    ...
    with open(RAW_OUT, "w", encoding="utf-8", newline="\n") as fh:
        ...
        fh.write("\n")
    report = OrderedDict([
        ("schema", "ptf-brand-page-reads/1.0"), ("work_order", WORK_ORDER), ("market_id", "jacksonville-fl"),
