"""PTF-JACKSONVILLE-NC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Jacksonville's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/jacksonville-nc/launch_package/hotel_policy_facts_jacksonville-nc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/jacksonville-nc/jacksonville_nc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-JACKSONVILLE-NC-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
market (Fayetteville) must deploy first. This order therefore writes NOTHING at
...
against the Fayetteville-live parent.

WHY A LATER ORDER MAY REGISTER WITHOUT A ROW-BY-ROW FOUNDER SIGNATURE
------------------------------------------------------------------
Registration is not publication. Writing `markets/jacksonville-nc.json`, an
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Jacksonville without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages Jacksonville without a row-by-row
