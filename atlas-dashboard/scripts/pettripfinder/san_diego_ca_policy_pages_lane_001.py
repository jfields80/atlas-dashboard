"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 12G: independents' own policy / FAQ / pet pages (plain client).
...
The sentences are the candidate quote; facts are read by the shared reader in san_diego_ca_policy_reads_001 and
judged by the first-party gate and the negation guard in the clean authority. Documents persisted by sha256 under
data/acquisition/san_diego_ca_policy_pages_001/.

Output:
  launch_packages/pettripfinder/markets/staging/san-diego-ca/raw_captures/policy_pages_rows.json
"""
...
B.DOCS = os.path.join(_DASH, "data", "acquisition", "san_diego_ca_policy_pages_001")
PKG = os.path.join(_DASH, "launch_packages", "pettripfinder")
ROUTING = os.path.join(PKG, "markets", "reports", "san_diego_ca_routing_001.json")
RAW = os.path.join(PKG, "markets", "staging", "san-diego-ca", "raw_captures")
OUT = os.path.join(RAW, "policy_pages_rows.json")
#: The SAME reader serves the independents' closure lane (san_diego_ca_places_policy_pages_001). Both of its write
