"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phases 15-19: policy adjudication over one identity per row.
...
Output:
  launch_packages/pettripfinder/markets/reports/fort_lauderdale_fl_clean_authority_001.json
"""
...
from scripts.pettripfinder.fort_lauderdale_fl_census_reconciliation_001 import canonical_street  # noqa: E402
...
STATIC = os.path.join(REPORTS, "fort_lauderdale_fl_free_static_capture_001.json")
FIRECRAWL = os.path.join(REPORTS, "fort_lauderdale_fl_firecrawl_pass_001.json")
#: The retry pass (after the census street restatement); a retry row supersedes the first pass's row for its key.
FIRECRAWL_RETRY = os.path.join(REPORTS, "fort_lauderdale_fl_firecrawl_pass_002.json")
...
FIRECRAWL_PROBE2 = os.path.join(REPORTS, "fort_lauderdale_fl_firecrawl_pass_003.json")
...
OUT = os.path.join(REPORTS, "fort_lauderdale_fl_clean_authority_001.json")
...
def build_evidence_index(census_hotels):
    ...
    # bound to its own premises by fort_lauderdale_fl_browser_closure_002 (property code, or the brand page's own name with
...
def build():
    PLACES_BY_KEY.clear()
    SITES_BY_KEY.clear()
    for r in (_load(os.path.join(REPORTS, "fort_lauderdale_fl_places_route_discovery_001.json"), {}) or {}).get("rows", []):
