"""PTF-BOONE-BLOWING-ROCK-NC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Boone - Blowing Rock's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/boone-blowing-rock-nc/launch_package/hotel_policy_facts_boone-blowing-rock-nc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/boone-blowing-rock-nc/boone_blowing_rock_nc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-BOONE-BLOWING-ROCK-NC-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
markets (Fayetteville, Jacksonville, Greenville, then Atlanta) must deploy first. This order therefore writes NOTHING at
...
against the then-current live parent (after Fayetteville, Jacksonville, Greenville and Atlanta).

WHY A LATER ORDER MAY REGISTER WITHOUT A ROW-BY-ROW FOUNDER SIGNATURE
------------------------------------------------------------------
Registration is not publication. Writing `markets/boone-blowing-rock-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "boone_blowing_rock_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census", "boone-blowing-rock-nc.json")
STAGING = os.path.join(PKG, "markets", "staging", "boone-blowing-rock-nc")
#: REGISTERED by PTF-BOONE-BLOWING-ROCK-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root, as every registered market.
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_boone-blowing-rock-nc.json")
AUTHORITY_OUT = os.path.join(PKG, "boone_blowing_rock_nc_proposed_authority_002.json")
OBSERVED_AT = "2026-09-13"
CAPTURED_AT = "2026-09-13T19:00:00+00:00"
...
def build():
    ...
    package = OrderedDict((
        ("market", "Boone \u2013 Blowing Rock, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Boone - Blowing Rock's REGISTERED policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Boone - Blowing Rock's authority as the source-ready order proposed it, registered by PTF-BOONE-BLOWING-ROCK-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Registration makes the market "
