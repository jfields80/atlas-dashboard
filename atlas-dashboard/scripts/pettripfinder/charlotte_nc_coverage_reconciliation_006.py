"""PTF-CHARLOTTE-NC-ZERO-TO-LIVE-BENCHMARK-001 -- Phases 15 and 16.
...
Output: launch_packages/pettripfinder/markets/reports/charlotte_nc_coverage_006.json
"""
...
OUT = os.path.join(REPORTS, "charlotte_nc_coverage_006.json")
...
def build():
    ...
    clean = _load(os.path.join(REPORTS, "charlotte_nc_clean_authority_001.json"))
    routing = _load(os.path.join(REPORTS, "charlotte_nc_routing_001.json"), {})
    gap = _load(os.path.join(REPORTS, "charlotte_nc_competitor_gap_matrix_001.json"), {})
    package = _load(os.path.join(PKG, "hotel_policy_facts_%s.json" % MARKET_ID))
    static = _load(os.path.join(REPORTS, "charlotte_nc_free_static_lane_001.json"), {})
    sitemaps = _load(os.path.join(REPORTS, "charlotte_nc_brand_sitemaps_001.json"), {})
    ...
    families_unresolved = OrderedDict((
        ...
        ("BEST_WESTERN_ESA_REDROOF_MOTEL6_HYATT_SONESTA", OrderedDict((
            ...
            ("why", "each refused this project's plain client on this run -- Best Western 403, "
                    ...
                    "never evidence that the family has no Charlotte property."),
    ...
    doc = OrderedDict((
        ...
        ("launch_quality", OrderedDict((
            ...
            ("why",
             ...
             "publication set covers Uptown, South End, the airport and Billy Graham Parkway, "
             "Arrowood and Steele Creek, SouthPark, Ballantyne, University City, Northlake, "
             "Pineville, Matthews, Lake Norman, Concord, Gastonia and Fort Mill -- every major "
             "traveller cluster in the metro. It is NOT complete: %d admitted identities remain "
             "unread, and three brand families are unresolved for named, recorded reasons."
             % (published, len(covered), len(admitting),
                sum(1 for c in corridors if c["corridor_page_publishes"]),
                len(census["hotels"]) - resolved)),
        ))),
        ("identities", rows),
    ))
    return doc
