"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- the Choice / IHG property pages the Firecrawl route-discovery lane found.

Choice's own sitemap timed out to the plain client and IHG's answered 403, so neither family's routes reached the
census by name: a discovered route names a FLAG and a city ("/california/san-diego/sleep-inn-hotels/ca123"), never the
...
Outputs:
  launch_packages/pettripfinder/markets/staging/jacksonville-fl/raw_captures/brand_page_rows.json
  launch_packages/pettripfinder/markets/reports/san_diego_ca_brand_page_reads_001.json
"""
...
DOCS = os.path.join(_DASH, "data", "acquisition", "san_diego_ca_brand_page_reads_001")
RAW_OUT = os.path.join(PKG, "markets", "staging", "san-diego-ca", "raw_captures", "brand_page_rows.json")
REPORT_OUT = os.path.join(REPORTS, "san_diego_ca_brand_page_reads_001.json")
CAP = 66
FLOOR = 400
IN_MARKET = ("san-diego", "la-jolla", "coronado", "del-mar", "solana-beach", "encinitas", "cardiff", "carlsbad",
             "oceanside", "chula-vista", "national-city", "imperial-beach", "la-mesa", "el-cajon", "santee",
             "lakeside", "lemon-grove", "spring-valley", "poway", "escondido", "san-marcos", "vista", "san-ysidro",
             "bonita", "rancho-santa-fe")
...
_ADDR_LINE = re.compile(
    ...
    r",\s*([A-Za-z .'\-]{3,28}),\s*CA,?\s*(\d{5})\b", re.I)
...
def town_of(route):
    m = re.search(r"choicehotels\.com/california/([a-z-]+)/", route) or \
...
def main(argv=None):
    ...
    with open(RAW_OUT, "w", encoding="utf-8", newline="\n") as fh:
        ...
        fh.write("\n")
    report = OrderedDict([
        ("schema", "ptf-brand-page-reads/1.0"), ("work_order", WORK_ORDER), ("market_id", "san-diego-ca"),
