"""PTF-INDIANAPOLIS-ATTENDED-CAPTURE-PASS2-001.

Drives the recommended 10-row NON-HILTON subset of
``indianapolis_capture_ready_queue_002.json`` through the official
CaptureRunner. Writes captures only under the gitignored worker tree.
...
QUEUE_PATH = (
    ...
    / "indianapolis_capture_ready_queue_002.json"
)
BATCH_DIR = (
    _REPO_ROOT / "data" / "worker_runs" / "pettripfinder"
    / "indianapolis-attended-capture-002"
)
WORK_ORDER = "PTF-INDIANAPOLIS-ATTENDED-CAPTURE-PASS2-001"
BATCH_ID = "indianapolis-pass2-001"

# Recommended first 10 from the 18-row ready queue, Hilton excluded.
SELECTED = (
...
def build_queue() -> CaptureQueue:
    ...
    if missing:
        raise SystemExit("selected hotels missing from queue 002: %s" % missing)
    chosen = [by_id[k] for k in SELECTED]
    if any(e.brand in HILTON for e in chosen):
        raise SystemExit("Hilton hotel leaked into the Pass-2 selection")
    if len(chosen) != 10:
        raise SystemExit("expected 10 selected hotels, got %d" % len(chosen))
    return CaptureQueue(
...
def main() -> int:
    ...
    (BATCH_DIR / "pass2_runner_summary.json").write_text(
