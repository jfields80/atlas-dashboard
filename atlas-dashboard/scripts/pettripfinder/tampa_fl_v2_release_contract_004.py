"""PTF-TAMPA-FL-V2-REGISTRATION-AND-FOUNDER-PACKET-004 -- Tampa's release contract.
...
WORK_ORDER = "PTF-TAMPA-FL-V2-REGISTRATION-AND-FOUNDER-PACKET-004"
MARKET_ID = "tampa-fl"
PKG = os.path.join(_DASH, "launch_packages", "pettripfinder")
REPORTS = os.path.join(PKG, "markets", "reports")
OUT = os.path.join(_DASH, "deploy", "netlify", "release_contracts", "%s.json" % MARKET_ID)
TEMPLATE = os.path.join(_DASH, "deploy", "netlify", "release_contracts", "orlando-fl.json")
...
def main(argv=None) -> int:
    ...
    clean = _load(os.path.join(REPORTS, "tampa_fl_v2_clean_authority_001.json"))
    template = _load(TEMPLATE)

    held = {c: n for c, n in clean["counts"].items()
            if c not in ("CLEAN_PET_FRIENDLY", "CLEAN_VERIFIED_NO_PETS")}
    ...
    doc = OrderedDict([
        ...
        ("release_name_prefix", "prod-tampa"),
        ("description",
         "Deterministic release-gate contract for the PetTripFinder Tampa Bay market through %s. "
        ...
        ("deployment_authorization", OrderedDict([
            ...
            ("means",
             ...
             "decision exists for Tampa at %d published profiles."
        ...
        ("identity_census", OrderedDict([
            ...
            ("note",
             "%d registered identities, built from zero by PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 and "
             "closed by PTF-TAMPA-FL-V2-COVERAGE-CLOSURE-002 and PTF-TAMPA-FL-V2-TERMINAL-HOLD-CLOSURE-003 "
             "at $0 USD net new provider spend (existing Firecrawl plan credits, Google Places and "
             "browser capacity only), registered by %s. Membership is this market's own corridor "
             "postal partition (census_membership_basis CORRIDOR_REGISTRY): a multi-core Tampa Bay "
             "market -- Tampa, St. Petersburg and Clearwater/the Gulf beaches treated as ONE market, "
             "not three -- of 19 corridors over 70 postal codes: Downtown Tampa/Riverwalk, Ybor City, "
             "Westshore/TPA Airport/Rocky Point, Busch Gardens/USF, Brandon and East Tampa/I-75 (CORE); "
             "Downtown St. Petersburg, St. Petersburg, Clearwater, Clearwater Beach, St. Pete Beach/"
             "Treasure Island, Madeira Beach/Redington/Indian Rocks Beach and Largo (CORRIDOR); Wesley "
             "Chapel/New Tampa, South Hillsborough, Plant City, Tarpon Springs/Dunedin, mid-Pinellas and "
             "Safety Harbor/Oldsmar/Palm Harbor (FRINGE). Sarasota/Bradenton, Anna Maria Island, Lakeland "
             "and Spring Hill/Brooksville are observed and refused. Vacation-rental, timeshare and "
             "resort-residence rows are excluded; DBPR condominium, dwelling and non-transient apartment "
             "licences never enter the graph. Rows the identity graph did not admit are recorded, not "
             "dropped: %s."
        ...
        ("reconciliation", OrderedDict(
            ...
            + [("note",
                ...
                "publishes nothing. %d rows did not publish (%s); every one is "
                "named in tampa_fl_v2_clean_authority_001.json with its class. No gate was "
                "lowered to preserve a count."
                % (derived.published_hotel_profiles, recon["verified_no_pets"], census["count"],
                   held_total, hold_summary))]
        )),
        ("policy_package", OrderedDict([
            ...
            ("note",
             ...
             "lane: this market cost $0 USD net new spend, existing plan credits only."),
        ])),
