"""PTF-HAMPTON-ROADS-VA-PARALLEL-SOURCE-READY-001 -- Phase 10/11: the free static first-party lane.
...
addressed by sha256, under ``data/acquisition/hampton_roads_va_static_001/``, and this
...
recorded in ``hampton_roads_va_attended/brand_pages.json`` with the document sha256 they
...
Output:
  launch_packages/pettripfinder/markets/reports/hampton_roads_va_static_lane_001.json
"""
...
OUT = os.path.join(REPORTS, "hampton_roads_va_static_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "hampton_roads_va_static_001")
...
BROWSER_ONLY = ("marriott.com", "hilton.com", "ihg.com", "choicehotels.com", "hyatt.com",
                "bestwestern.com", "redroof.com", "motel6.com", "extendedstayamerica.com", "reservations.com", "visitvirginiabeach.com", "visithampton.com", "visitportsmouthva.com",
...
def targets():
    ...
    out = OrderedDict()
    # Rows held for identity review, name-only or same-campus are read too (their own site may state the address);
    # OUTSIDE and NON_LODGING rows are never targeted.
    rows = census["hotels"] + [h for h in census["non_admitted"]
                               if h["classification"] in ("IDENTITY_REVIEW_REQUIRED", "NAME_ONLY_UNRESOLVED",
                                                          "SAME_CAMPUS_DISTINCT_ENTITY")]
    for h in rows:
