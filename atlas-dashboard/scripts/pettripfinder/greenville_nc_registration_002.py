"""PTF-GREENVILLE-NC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Greenville's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/greenville-nc/launch_package/hotel_policy_facts_greenville-nc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/greenville-nc/greenville_nc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-GREENVILLE-NC-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
markets (Fayetteville, then Jacksonville) must deploy first. This order therefore writes NOTHING at
...
against the then-current live parent (after Fayetteville and Jacksonville).

WHY A LATER ORDER MAY REGISTER WITHOUT A ROW-BY-ROW FOUNDER SIGNATURE
------------------------------------------------------------------
Registration is not publication. Writing `markets/greenville-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "greenville_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census", "greenville-nc.json")
STAGING = os.path.join(PKG, "markets", "staging", "greenville-nc")
#: REGISTERED by PTF-GREENVILLE-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root, as every registered market.
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_greenville-nc.json")
AUTHORITY_OUT = os.path.join(PKG, "greenville_nc_proposed_authority_002.json")
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Greenville without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages Greenville without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Greenville, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Greenville's REGISTERED policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Greenville's authority as the source-ready order proposed it, registered by PTF-GREENVILLE-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Registration makes the market "
