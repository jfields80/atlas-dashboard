...
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_greenville-nc.json")
AUTHORITY_OUT = os.path.join(STAGING, "greenville_nc_proposed_authority_002.json")
...
def build():
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Greenville's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Greenville's authority as this order proposes it, STAGED and not registered. Registration makes the market "
