"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- Phase 4A: the OpenStreetMap lane.
...
Output:
  launch_packages/pettripfinder/markets/reports/outer_banks_nc_osm_lane_001.json
"""
...
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "outer_banks_nc_osm_lane_001.json")

#: guest_house is kept too: Outer Banks inns are often tagged guest_house, and the census
#: must SEE a bed-and-breakfast or rental house to refuse it by decision.
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ...
            ("pbf_snapshot",
             "the 2026-09-10 Geofabrik North Carolina download, reused (hard link) from the Greenville / "
             "Jacksonville runs' local copy; the upstream file has since been republished, so this md5 is "
             "the snapshot's own"),
            ...
            ("kept", "tourism in (hotel, motel, guest_house)"),
        ])),
