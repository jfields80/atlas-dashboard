"""PTF-ATLANTA-GA-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.
...
yet deployed), then Jacksonville and Greenville (source-ready, not registered) must become
the live parent before Atlanta can be released.
...
to touch and that would conflict with Fayetteville's, Jacksonville's and Greenville's own registration of the
same documents.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/atlanta-ga/launch_package/``) from the proposed
  ...
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/atlanta-ga/``: never ``markets/packages/`` or
...
Fayetteville (and later Jacksonville and Greenville) is live, rule N FAILS this package on purpose; the registration
...
REPORT = PKG / "markets" / "reports" / "atlanta_ga_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "atlanta_ga_proposed_authority_002.json"
SEALED_AT = "2026-09-13T19:30:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         ...
         "registration order -- after Fayetteville, Jacksonville AND Greenville are live -- re-seals the same committed inputs against that parent as a NEW "
