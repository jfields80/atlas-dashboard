"""PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 26: the SHADOW sealed package (cloned from the Orlando FL V2 helper).

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
------------------------------------------------------------------
Tampa is not at the front of the serialized release queue: the current verified live release is Orlando
(28 markets / 2003 profiles / 2299 routes). The generic lane would reissue shared, parent-sensitive state this
order is forbidden to touch. So this helper:

  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/tampa-fl/launch_package/``) from the proposed market document, the proposed census, the
  ...
  4. writes the package, the receipt and a report ONLY under ``markets/staging/tampa-fl/``.
...
REPORT = PKG / "markets" / "reports" / "tampa_fl_v2_shadow_package_001.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "tampa_fl_v2_proposed_authority_001.json"
PARTITION_SRC = STAGING / "launch_package" / "tampa_fl_final_partition_v2_001.json"
SEALED_AT = "2026-09-15T22:00:00Z"
...
def stage_tree():
    ...
    # the partition already lives under LP from tampa_fl_v2_final_partition_001; nothing to copy
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the CURRENT live release (Orlando, deploy 6aa9dad7...). That is what this "
         "package was validated against. Once any later market is live, FAST rule N fails this package BY "
         "DESIGN; the registration order -- when Tampa reaches the front of the release queue -- re-seals the "
