"""PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.

Cloned from the Boone - Blowing Rock NC census helper (Outer Banks, Atlanta lineage). Independent discovery lanes, none
authoritative on its own:
  ...
  DESTINATION_ORGANIZATION  every "Places to Stay" listing of Visit Hickory Metro
                      (visithickorync.com, the Hickory Metro CVB) with every
                      sub-category the bureau gives it (Hotel, Extended Stay, Bed
                      and Breakfast, Campgrounds & RV, Retreat Center, ...).
...
-----------------------
The market contract's postal partition (hickory_nc_geography_001). Lenoir (future
submarket), Morganton, Statesville, Taylorsville, Lincolnton and Denver are OUTSIDE by name.

APARTMENTS, CLUBS, RETREATS AND RENTALS ARE NOT HOTELS
-----------------------------------------------------
The Hickory evidence includes apartment communities the map tags tourism=apartment,
campgrounds and RV resorts the bureau files as lodging, a religious retreat centre,
a country club and its townhomes, and a cottage-rental company. None is admitted: a
row whose only evidence types it as a campground, retreat or venue is NON_LODGING,
and named non-hotels are refused BY NAME with the reason
(hickory_nc_nonhotel_rulings_001).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/hickory-nc.json
  launch_packages/pettripfinder/markets/reports/hickory_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/hickory_nc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import hickory_nc_geography_001 as GEO  # noqa: E402
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "hickory-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "hickory-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "hickory_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "hickory_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "hickory_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "hickory_nc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "hickory_nc_destination_roster_001.json")
...
from scripts.pettripfinder.hickory_nc_nonhotel_rulings_001 import NOT_LODGING_WHY, LODGING_UNCONFIRMED  # noqa: E402
...
NON_HOTEL_BUREAU_SUBCATEGORIES = {
    # the bureau's own spellings, as its listing service prints them (visithickorync.com)
    "Campgrounds & RV", "Retreat Center", "Museums & Historical Sites", "Wedding",
    # spellings earlier bureaus used, kept defensively
    "Condos / Villas", "RV/Campground", "Camping / RV Resorts", "Unique Venue", "Event Venue",
    "Wedding Venue", "Agritourism", "Equestrian", "Farm", "Unique Meeting Facility",
    # defensive spellings of the same categories
    "Vacation Rental Companies", "Campgrounds", "RV Parks", "Real Estate", "Timeshares",
}
#: PINEHURST: the bureau files a listing under several categories ("Gather » Meeting Facilities" beside
#: "Lodging » Hotels / Inns"). The vacation-rental rule reads EVERY sub-category the bureau gives a
...
#: HICKORY: Visit Hickory Metro's "Places to Stay" hotel-type sub-categories.
HOTEL_BUREAU_SUBCATEGORIES = {"Hotel", "Extended Stay", "Bed and Breakfast"}
...
from scripts.pettripfinder.hickory_nc_static_rulings_001 import (  # noqa: E402
...
#: Bureau listings whose website is a brand short link for a property whose own page this order READ. The
#: listing joins that read by its route (the bureau also prints the house number 10024 in its ZIP field).
DESTINATION_ROWS_SUPERSEDED_BY_A_READ = {}

#: PINEHURST RESORT. The bureau lists the resort ("Pinehurst Resort", Resorts) and its flagship hotel ("The Carolina
#: Hotel", Resorts) at ONE street, 80 Carolina Vista Drive -- the hotel's own street. The premises is the hotel
#: building; the resort is its operator. The identity is named for the hotel. Keyed on the ZIP-free street identity.
NAME_RULINGS = {}

#: Name-only MAP rows bound to a building by ruling: the map label names the hotel, carries no address, and its
#: pin sits on that building. (osm element, ZIP-free street identity prefix, why)
MAP_NAME_ONLY_BINDINGS = []
...
REBRAND_HOLDS = {
    "1607|church fairgrove": (
        "1607 Fairgrove Church Road, Conover 28613: the bureau lists ONE listing 'Comfort Inn & Main Stay Suites' "
        "(website choicehotels.com nc408); competitor and search summaries describe two Choice hotels here "
        "(Comfort Inn Conover-Hickory and MainStay Suites Conover-Hickory, 'Building A'); the map still names La "
        "Quinta Inn & Suites, whose Wyndham route is RETIRED. Choice's own pages served this client a blank shell, so "
        "whether the premises is one hotel or a dual-brand pair, and under which flag(s), cannot be proven"),
}
...
def read_destination_roster():
    ...
    for r in doc.get("listings", []):
        _sup = DESTINATION_ROWS_SUPERSEDED_BY_A_READ.get(r["listing_url"])
        if _sup:
            r = dict(r, website=_sup[0])
...
def merge_destination_rows(nodes):
    ...
    for n in nodes:
        ...
        hits = [h for h in targets if (h.street.split() or [""])[0] == num
                and (address_key(h.street, "") == key or _canon_street_words(h.street, n.street)
...
def read_regional_visitor_center():
    """A regional visitor-centre roster: names only, tier 2, never policy (none in this market)."""
...
def merge(observations):
    ...
    for obs in observations:
        ...
        # PINEHURST: a bureau listing typed ONLY as condos / villas / campground never joins a building by street.
        # "The Carolina Villas" and "The Condos at Pinehurst" print the Carolina Hotel's street.
        if obs.get("lane") == "DESTINATION_ORGANIZATION":
            _subs = set(obs.get("bureau_all_subcategories") or [])
            if _subs and not (_subs & HOTEL_BUREAU_SUBCATEGORIES) and (_subs & NON_HOTEL_BUREAU_SUBCATEGORIES):
                cand_keys = [k for k in cand_keys if not k.startswith("street:")]
        # HICKORY: a listing refused BY NAME as not lodging ("The Townhomes at Rock Barn") never joins a building by
        # street -- the townhomes print the Lodge at Rock Barn's street.
        if normalize_name(obs.get("name") or "") in NOT_LODGING_NAMES:
            cand_keys = [k for k in cand_keys if not k.startswith("street:")]
...
_GENERIC_TOKENS = {
    ...
    # Marketing vocabulary names no chain ("Hickory Downtown", "I-40", "Lenoir-Rhyne University area").
    "golf", "club", "spa", "historic", "near", "area", "i", "40", "i40", "university", "airport",
    "downtown", "valley",
}
...
_IN_MARKET_MUNICIPALITIES = {
    "hickory", "conover", "newton", "long view", "longview", "mountain view", "claremont", "granite falls",
    "sawmills", "hildebran", "icard", "maiden", "catawba", "hudson",
    "conover hickory area",  # Wyndham's own city segment for the Ramada ("conover-hickory-area-north-carolina")
}
#: Lenoir is preserved as a future submarket (lenoir-nc).
_FUTURE_SUBMARKET_PLACES = {"lenoir"}
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {
    "morganton", "valdese", "drexel", "connelly springs", "rutherford college", "statesville", "troutman",
    "taylorsville", "stony point", "hiddenite", "lincolnton", "iron station", "vale", "cherryville", "denver",
    "sherrills ford", "terrell", "mooresville", "boone", "blowing rock", "collettsville", "marion", "shelby",
    "gastonia", "charlotte", "cornelius", "huntersville", "wilkesboro", "north wilkesboro", "banner elk",
}
...
def _bureau_non_hotel_only(node):
    """PINEHURST: True when every observation is a bureau listing typed ONLY as a condo / villa / campground /
    venue category. Pinehurst Resort's 'The Carolina Villas' and 'The Condos at Pinehurst' print the Carolina
    Hotel's street with no ZIP; folding them by street would carry villa inventory into the hotel's identity."""
    subs = set()
    for o in node.observations:
        if o.get("lane") != "DESTINATION_ORGANIZATION":
            return False
        subs |= set(o.get("bureau_all_subcategories") or [])
    return bool(subs) and not (subs & HOTEL_BUREAU_SUBCATEGORIES) and bool(subs & NON_HOTEL_BUREAU_SUBCATEGORIES)
...
def merge_zipless(nodes):
    ...
    for n in without:
        if _bureau_non_hotel_only(n):
            continue
        key = address_key(n.street, "")
        # HICKORY: "1125 13th Avenue Drive Southeast" (map) is "1125 13th Ave Dr SE" (bureau): the same house number
        # and the same distinctive street words once quadrant spellings and suffixes are dropped.
        hits = [h for h in with_zip
                if (address_key(h.street, "") == key
                    or ((h.street.split() or [""])[0] == (n.street.split() or [""])[0]
                        and _canon_street_words(h.street, n.street)))
...
def _canon_words(street):
    ...
    # PINEHURST: a plural is typography. AmeriVu's own page writes "1408 North Sandhill Blvd" where the bureau
    # and the map write "1408 N Sandhills Boulevard"; the house number, the ZIP and the single-match rule still apply.
    return {(t[:-1] if len(t) > 4 and t.endswith("s") else t) for t in toks if t not in _STREET_DROP and not t.isdigit()}
...
CORRIDOR_CITIES = {
    "hickory": {"hickory", "long view", "longview", "mountain view"},
    "conover": {"conover"},
    "newton": {"newton"},
    "claremont": {"claremont"},
    "granite-falls-sawmills": {"granite falls", "sawmills"},
    "hildebran-icard": {"hildebran", "icard"},
    "maiden": {"maiden"},
    "catawba": {"catawba"},
    "hudson": {"hudson"},
}
...
def _pin_in_admitting_cell(lat, lng):
    """True when a pin lies inside any ADMITTING cell of the geography (refusal only; a pin never admits)."""
    for _s, _m, _l, clat, clng, radius, admitting in GEO.CELLS:
        if admitting and _km(lat, lng, clat, clng) * 1000.0 <= radius:
            return True
    return False
...
def classify(node, zips, name_counts, street_counts):
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and (_subcats & NON_HOTEL_BUREAU_SUBCATEGORIES) and not any(
        ...
        return NON_LODGING, ("the destination organisation files this listing under %s -- a "
                             "condo / villa, campground, venue or farm category, not a hotel establishment; "
                             "refused under the order's golf-villa / vacation-rental filter" % sorted(_subcats))
    ...
    if not node.street and not node.phone and node.lat is not None and not _pin_in_admitting_cell(node.lat, node.lng):
        return OUTSIDE_MARKET, ("a name-only map row whose pin (%.5f, %.5f) lies outside every admitting observation "
                                "cell of the geography; recorded, never admitted" % (node.lat, node.lng))
    ...
    if node.postal and node.postal[:5] not in zips:
        _fs = (" FUTURE_SUBMARKET %s --" % GEO.FUTURE_SUBMARKET) if GEO.is_future_submarket(node.postal) else ""
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Hickory "
...
SOURCE_AUTHORITIES = [
    ...
    "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (18 Hickory-area city pages, with property cards)",
    ...
    "https://www.visithickorync.com listing service, every 'Places to Stay' sub-category (Hickory Metro "
...
def build():
    ...
    for n in nodes:
        if not n.postal:
            ...
            if stated and all(o.get("lane") in ("DESTINATION_ORGANIZATION", "COMPETITOR_LEAD", "REGIONAL_VISITOR_CENTER")
                              for o in n.observations):
                n.postal = stated[0]
    # HICKORY: the map prints "1125 13th Avenue Drive Southeast" with no ZIP beside the bureau's zipless row for the
    # same building; once the roster's stated ZIP is carried, the map row has one building to join.
    nodes, zipless_merged_2, zipless_ambiguous_2 = merge_zipless(nodes)
    zipless_merged += zipless_merged_2
    zipless_ambiguous += zipless_ambiguous_2
    ...
    for n in nodes:
        ...
        if bureau:
            n.name = bureau[0]
    name_rulings_applied = []
    for n in nodes:
        _sk = address_key(n.street, "") if n.street else ""
        for _prefix, (_name, _why) in NAME_RULINGS.items():
            if _sk and _sk.startswith(_prefix) and not _bureau_non_hotel_only(n):
                name_rulings_applied.append(OrderedDict([("was", n.name), ("now", _name), ("why", _why)]))
                n.name = _name
    map_bindings_applied = []
    for _el, _prefix, _why in MAP_NAME_ONLY_BINDINGS:
        src = [n for n in nodes if not n.street and any(o.get("osm_element") == _el for o in n.observations)]
        dst = [n for n in nodes if n.street and address_key(n.street, "").startswith(_prefix)
               and not _bureau_non_hotel_only(n)]
        if len(src) == 1 and len(dst) == 1:
            for o in src[0].observations:
                o = OrderedDict(o)
                o["binding"] = "MAP_LABEL_BOUND_BY_RULING"
                o["binding_reason"] = _why
                dst[0].observations.append(o)
            nodes = [n for n in nodes if n is not src[0]]
            map_bindings_applied.append(OrderedDict([("osm_element", _el), ("into", dst[0].name), ("why", _why)]))
    ...
    # HICKORY: a pair is counted among buildings this market could ADMIT (an admitted or unstated ZIP). The Sleep
    # Inns of Statesville and Morganton and the Affordable Suites of Mooresville are outside the market by their own
    # ZIPs and are not "another node" for the Hickory and Conover buildings.
    name_counts = Counter((normalize_name(n.name), "") for n in nodes
                          if (n.street or n.phone) and (not n.postal or n.postal[:5] in zips))
    ...
    # PINEHURST: one identity key is one listing ACROSS markets. A row whose only names are bare flags ("Econo-Lodge",
    # the map's "Econo Lodge & Suites") and whose key a REGISTERED market's census already holds cannot be admitted
    # under that key; with no first-party page to name it (Choice served a bot shell), it is held, never renamed by
    # invention. The registered censuses are read, never written.
    registered_keys = {}
    for _f in sorted(os.listdir(os.path.join(PKG, "identity_census"))):
        if _f.endswith(".json"):
            for _h in (_load(os.path.join(PKG, "identity_census", _f), {}) or {}).get("hotels", []):
                registered_keys.setdefault(_h.get("identity_key"), _f[:-5])
    cross_market_collisions = []
    for r in admitted:
        other = registered_keys.get(r["identity_key"])
        if other and other != MARKET_ID:
            r["classification"] = IDENTITY_REVIEW_REQUIRED
            r["classification_reason"] = (
                "CROSS_MARKET_IDENTITY_KEY_COLLISION -- the key %r is already a registered identity of %s; no "
                "first-party page in this order names the building more specifically, and a name is never invented"
                % (r["identity_key"], other))
            r["identity_state"] = "IDENTITY_UNRESOLVED"
            r["lodging_state"] = "NEEDS_REVIEW"
            cross_market_collisions.append(OrderedDict([("identity_key", r["identity_key"]), ("registered_in", other)]))
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 Hickory - Newton - Conover census, built from zero "
    ...
    report = OrderedDict([
        ...
        ("name_rulings_applied", name_rulings_applied),
        ("cross_market_identity_key_collisions", cross_market_collisions),
        ("map_name_only_bindings_applied", map_bindings_applied),
        ("destination_rows_superseded_by_a_read",
         OrderedDict((k, v[1]) for k, v in DESTINATION_ROWS_SUPERSEDED_BY_A_READ.items())),
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Hickory authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "hickory-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "hickory_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "hickory_nc_competitor_gap_matrix_001.json"))
