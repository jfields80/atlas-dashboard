"""PTF-AUGUSTA-GA-V2-STALE-PARENT-RECHECK-004 -- supersedes
augusta_ga_founder_authorization_packet_003.json, which was bound to the
Savannah-live parent (deploy 6aa93de5) and went stale the moment Orlando-fl's
independent branch deployed live (deploy 6aa9dad7, 28/2003/2299) and was
merged into this branch. This document is bound to that current parent
instead. Every digest and count here is READ from a committed or
lane-written artifact -- nothing is typed. UNSIGNED:
...
commit, Orlando's own tip, or this branch's pre-merge tip) -- because any
diff that reaches back far enough to explain Orlando's presence necessarily
crosses Orlando's own deployment event (deployment_records /
deployment_authorizations / launch_participation.json), which the narrow
registration classes structurally exclude by design (they prove ONE
market's data-only registration, never a deployment). This is a genuine
limitation of comparing two independently-progressing branch timelines with
a point-in-time diff tool, not a defect introduced by Augusta's own data.
Release-prep safety here rests instead on the direct mechanical proofs
listed in `readiness_gate` and `production_delta_audit` below, each read
from a real committed artifact, not inferred from the classifier's verdict.
"""
...
WORK_ORDER = "PTF-AUGUSTA-GA-V2-STALE-PARENT-RECHECK-004"
SUPERSEDES_WORK_ORDER = "PTF-AUGUSTA-GA-V2-REGISTRATION-AND-FOUNDER-PACKET-003"
MARKET_ID = "augusta-ga"
PKG = os.path.join(_DASH, "launch_packages", "pettripfinder")
REPORTS = os.path.join(PKG, "markets", "reports")
OUT = os.path.join(REPORTS, "augusta_ga_founder_authorization_packet_004.json")
...
def main() -> int:
    ...
    classify = _load(os.path.join(REPORTS, "augusta_ga_registration_classification_004.json"))
    prior_packet = _load(os.path.join(REPORTS, "augusta_ga_founder_authorization_packet_003.json"))
    ...
    # The automated narrow classifier is NOT applicable here -- record that fact
    # explicitly rather than reporting a class it never actually granted.
    ...
    classifier_bases_tried = OrderedDict([
        ...
        ("709e84b3 (this branch's pre-merge tip)",
         "the diff crosses orlando-fl's actual deployment event (deployment_records, "
    ...
    readiness_gate = OrderedDict([
        ...
        ("FULL_REGRESSION_REQUIRED_classifier_output", classify["FULL_REGRESSION_REQUIRED"]),
        ("full_regression_required_reason",
         "caused by inherited Orlando live deployment/participation history in the branch diff, "
         "not by a newly introduced Augusta shared-factory change"),
        ...
        ("CANDIDATE_REPRODUCTION_METHOD",
         "3 independent builds compared byte-identical: two internal to one seal invocation, "
         "one fully separate clean git worktree checkout"),
    ...
    doc = OrderedDict([
        ...
        ("supersedes", OrderedDict([
            ...
            ("packet_path",
             "launch_packages/pettripfinder/markets/reports/augusta_ga_founder_authorization_packet_003.json"),
            ...
            ("reason",
             "packet 003 was bound to the Savannah-live parent (deploy 6aa93de5, 27/1802/2081); "
             "Orlando-fl deployed live independently on its own branch (deploy 6aa9dad7, 28/2003/2299) "
             "and was merged into this branch (ae8e0133), making packet 003's parent binding stale. "
             "Nothing was authorized against packet 003; it is superseded, not edited."),
        ])),
        ...
        ("what_this_explicitly_does_not_claim", [
            ...
            "was YES, for the structural reason recorded above, and no broad regression was run to "
            "resolve it.",
            "Does NOT assert a full whole-site sitemap rebuild was performed; only the release-index "
            "candidate (profiles, hotel/corridor/market routes) was independently composed and "
            "reproduced.",
        ]),
        ("would_become",
         "Augusta, Georgia joins production as the TWENTY-NINTH market with %d pet-friendly hotel "
         ...
         "current 28 live markets plus Augusta)."
        ...
        ("authorized_by", None),
        ("authorized_at", None),
        ("source_commit", None),  # filled in after this commit lands; see note field
        ("source_commit_note",
        ...
        ("parent_release", OrderedDict([
            ...
            ("rechecked_before_composition",
             ["Phase 1/3 (this order, pre-merge state)", "immediately after the Orlando merge",
        ...
        ("what_is_not_claimed", [
            ...
            "change set (see readiness_gate.AUTOMATED_NARROW_CLASSIFICATION); safety is established "
            "by the direct mechanical proofs in readiness_gate and production_delta_audit instead.",
            ...
            "rate, and that assessment is unchanged by registration or this reseal.",
        ]),
    ])
    ...
    print("AUTOMATED_NARROW_CLASSIFICATION:", readiness_gate["AUTOMATED_NARROW_CLASSIFICATION"])
    return 0
