"""PTF-LEXINGTON-KY-PROMOTION-AND-NEW-LANE-LAUNCH-PREP-003 -- the modern release lane.

Takes the newly registered Lexington market through the redesigned lane
ATLAS-THROUGHPUT-003 to 008 built, and writes what each stage proved:
    ...
    6  FINAL CANDIDATE                    release_index.compose (live + Lexington)
...
The candidate is composed with Lexington PARTICIPATING, because a pre-flip
candidate can never carry the digest a launch would produce -- the lesson
PTF-TOLEDO-OH-DEPLOYMENT-AND-LAUNCH-AUTHORIZATION-003 paid for. Composing an
...
def joining_delta(inputs) -> "OrderedDict":
    """Lexington's intended delta: a market that JOINS, and nothing else moves.
    ...
    market that is live on both sides of the comparison. Lexington is on neither
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=str(REPORTS / "lexington_ky_release_lane_003.json"))
    ap.add_argument("--work", default=str(_DASH / "data" / "lexington_release_lane_003"))
    ap.add_argument("--base", default="fa08b22",
    ...
    # exists to prevent:
    #
    #   (a) what does REGISTERING Lexington cost?  The change surface of this
    ...
    #
    #   (b) what does a ROUTINE Lexington release cost?  The fast lane's receipt
    ...
    #       Lexington, which this order proposes and does not apply.
    t = time.monotonic()
    ...
    # eleven markets are neither reused nor rebuilt -- they are not built at
    ...
    doc = OrderedDict((
        ...
        ("candidate", OrderedDict((
            ...
            ("parent_route_count", len({r for i in live[0].markets.values() if i.participating
                                        for r in i.routes})),
            ("lexington_routes", sorted(candidate.markets[MARKET_ID].routes)),
        ))),
        ("cache_reuse", reuse),
        ("timings", timings),
    ))
