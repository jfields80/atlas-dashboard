"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- Phases 14, 15 and 16: the attended-browser lane.
...
Two shapes of first-party policy block were read, and each is quoted in the page's OWN label vocabulary:
  ...
  HILTON  serves TWO templates in this market, and their labels differ. Both were verified verbatim here:
          HOTEL_INFO_LIST on ``/hotel-info/`` (witness pbicppy, Canopy West Palm Beach Downtown, whose
          ...
          POLICIES_ACCORDION on ``/amenities/`` (witness pbiwphh, Hilton West Palm Beach, whose
...
-------------------------------
An amenity chip ("Pet-Friendly" in Hilton's own "Hotel Amenities" list -- which pbiwphh carries ALONGSIDE its
...
Output (the shape west_palm_beach_fl_clean_authority_001 consumes):
  launch_packages/pettripfinder/markets/staging/west-palm-beach-fl/raw_captures/browser_closure_rows.json
  launch_packages/pettripfinder/markets/reports/west_palm_beach_fl_browser_lane_001.json
"""
...
REPORT_OUT = os.path.join(REPORTS, "west_palm_beach_fl_browser_lane_001.json")

#: The Hilton PETS-block labels, verified verbatim on THIS market's own properties -- one witness per TEMPLATE,
#: because Hilton serves two and their labels differ:
#:   pbiwphh (Hilton West Palm Beach)          POLICIES_ACCORDION on /amenities/ (/hotel-info/ redirects there)
#:   pbicppy (Canopy West Palm Beach Downtown) HOTEL_INFO_LIST on /hotel-info/ (/amenities/ 404s)
#: Re-verified here rather than inherited. Had this order assumed one vocabulary, every row read from the other
#: template would have been transcribed in labels its own page never used.
_HILTON_LABEL_WITNESSES = ("pbiwphh", "pbicppy")
...
def compose_quote(row):
    ...
    #
    #   HOTEL_INFO_LIST  (/hotel-info/, e.g. pbicppy Canopy West Palm Beach Downtown) renders the PETS block as
    #                    a definition list: "Pets allowed:" / "Yes", "Non-refundable fee:" / "$75.00",
    #                    "Max weight:" / "75 lbs", "Max size:" / "large", "Pet policy:" / "<text>".
    #   POLICIES_ACCORDION (/amenities/, e.g. pbiwphh Hilton West Palm Beach, where /hotel-info/ REDIRECTS)
...
def bind(row, census_hotels):
    ...
    #    page itself states. Needed because this market has two different hotels at house number 1800 in 33401
    #    (Best Western Palm Beach Lakes on Palm Beach Lakes Boulevard and Courtyard West Palm Beach Airport on
    #    Centrepark Drive East), and some brands -- Best Western among them -- print their address WITHOUT a
    #    postal code, so rule 2 cannot fire and rule 3 sees an ambiguous pair and correctly refuses.
    #
