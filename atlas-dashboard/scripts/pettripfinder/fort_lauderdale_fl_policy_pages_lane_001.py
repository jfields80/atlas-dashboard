"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phase 12G: independents' own policy / FAQ / pet pages (plain client).

A property's own home page often says nothing about pets while its own "Pet Policy", "Dog Policy", "Policies" or
"FAQ" page does. For every admitted census identity routed to an INDEPENDENT first-party site (not a brand host),
this lane:
...
The sentences are the candidate quote; facts are read by the shared reader in fort_lauderdale_fl_policy_reads_001 and
judged by the first-party gate and the negation guard in the clean authority. Documents persisted by sha256 under
data/acquisition/fort_lauderdale_fl_policy_pages_001/.

Output:
  launch_packages/pettripfinder/markets/staging/fort-lauderdale-fl/raw_captures/policy_pages_rows.json
"""
...
B.DOCS = os.path.join(_DASH, "data", "acquisition", "fort_lauderdale_fl_policy_pages_001")
PKG = os.path.join(_DASH, "launch_packages", "pettripfinder")
ROUTING = os.path.join(PKG, "markets", "reports", "fort_lauderdale_fl_routing_001.json")
RAW = os.path.join(PKG, "markets", "staging", "fort-lauderdale-fl", "raw_captures")
OUT = os.path.join(RAW, "policy_pages_rows.json")
#: The SAME reader serves the independents' closure lane (fort_lauderdale_fl_places_policy_pages_001). Both of its write
...
# minified sites write an unquoted href; both spellings are read
