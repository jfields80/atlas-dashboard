"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phase 4A: the OpenStreetMap lane.

Reads the local Geofabrik South Carolina extract (ODbL, (c) OpenStreetMap
...
at over an hour on the 428 MB North Carolina extract (the 356 MB Georgia extract is read the same way) before it died without writing an index.
...
Output:
  launch_packages/pettripfinder/markets/reports/columbia_sc_osm_lane_001.json
"""
...
PBF = os.path.join(_DASH, "data", "osm_extracts", "south-carolina-latest.osm.pbf")
...
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "columbia_sc_osm_lane_001.json")

#: guest_house is kept too: Columbia's historic inns and B&Bs are often tagged guest_house, and the census
#: must SEE a bed-and-breakfast or cabin to refuse it by decision.
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ("pbf", "data/osm_extracts/south-carolina-latest.osm.pbf"),
            ...
            ("pbf_snapshot",
             "the 2026-09-10 Geofabrik South Carolina download (copied byte-for-byte from the Charleston SC worktree's data/osm_extracts); the upstream file may since "
             "have been republished, so this md5 is the snapshot's own"),
