"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- the final partition.

Every census Outer Banks identity, in EXACTLY ONE state, with exactly one next
...
Output: launch_packages/pettripfinder/markets/staging/outer-banks-nc/launch_package/outer_banks_nc_final_partition_007.json
...
#: REGISTERED by PTF-OUTER-BANKS-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root.
OUT = os.path.join(PKG, "outer_banks_nc_final_partition_007.json")
...
REFUSALS = {
    "CHOICE": "a plain-client sitemap timeout, and in the attended session every choicehotels.com Outer Banks city "
              "listing returned a 2,223-byte behavioural bot-challenge shell (the brand's bot defence; never bypassed)",
}
#: Independent properties whose OWN site refused this client on this run.
SITE_REFUSALS = {
    "burrus house inn waterfront suites": "a plain-client 403 on burrushouse.com, and the domain is not a permitted "
                                          "navigation in the attended session",
    "white doe inn bed and breakfast": "a plain-client 403 on whitedoeinn.com, and the domain is not a permitted "
                                       "navigation in the attended session",
    "island house of wanchese b&b": "its website domain (islandhouse-bb.com) does not resolve",
}
#: Buildings whose own page SERVED but states no operative, bindable policy, by census name.
POLICY_SILENT = {
    "first colony inn": "the inn's own home page, amenities and FAQ pages served (attended) with no pet wording",
    "cypress house inn": "the inn's own home page served (Hotel JSON-LD 500 N. Virginia Dare Trail 27948) with no pet wording",
    "colington creek inn": "the inn's own home page served (Hotel JSON-LD 1293 Colington Road 27948) with no pet wording",
    "cavalier by the sea": "the motel's own home page served (cavalierbythesea.com) with no pet wording and links to no policy page",
    "the roanoke bungalow": "the inn's own home page served (roanokebungalow.com) with no pet wording and links to no policy page",
    "the manteo house": "the inn's own home page served with no pet wording; /policies is a 404",
    "driftin' sands motel": "the motel's own home page lists 'Pets are allowed upon request' in its amenity list only -- an "
                            "amenity item, never operative (the shared reader: AMENITY_CHIP_ONLY)",
    "dare haven motel": "the motel's own home page says 'families and pets are always welcome!' in a marketing paragraph "
                        "that states no street and postal code together; not operative and not bindable",
    "islander motel": "the motel's own page says 'NO PETS' on its rate card, but states only a P.O. Box (605) and no "
                      "street of its own, so the read cannot bind by the page's own street identity",
    "fin n' feather waterside inn": "the operator's KEES page says 'This property is a pet-friendly motel so feel free to "
                                    "bring pets' beside the street 7740 South Virginia Dare Trail but states no postal "
                                    "code, so the read cannot bind by the page's own street identity",
    "sandspur ocean cottages": "the property's own guest-info page states a dog policy ('Dogs are permitted in all units') "
                               "but no street and postal code, so the read cannot bind by the page's own street identity",
    "nags head beach inn": "the operator's KEES page answers 'The units at the Nags Head Beach Inn are not pet-friendly; "
                           "however, KEES Vacations has plenty of other vacation rentals that do allow pets', which the "
                           "shared reader does not read as a refusal, and states no postal code",
}
...
def build():
    ...
    clean = _load(os.path.join(REPORTS, "outer_banks_nc_clean_authority_001.json"))
    routing = _load(os.path.join(REPORTS, "outer_banks_nc_routing_001.json"), {})
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 "
                                         "acquisition passes: owned evidence, brand city pages, "
                                         "brand sitemaps, OSM extract, the county tourism roster, the independents' own "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-13"),
        ("what_this_is",
         "Every census Outer Banks identity in exactly one disposition state. A terminal state "
