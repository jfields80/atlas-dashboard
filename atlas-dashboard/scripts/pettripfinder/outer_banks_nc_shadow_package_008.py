"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/outer-banks-nc/launch_package/``) from the proposed
  ...
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/outer-banks-nc/``: never ``markets/packages/`` or
...
REPORT = PKG / "markets" / "reports" / "outer_banks_nc_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "outer_banks_nc_proposed_authority_002.json"
