"""PTF-RICHMOND-VA-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.
...
Independent discovery lanes, none authoritative on its own:
  ...
  DESTINATION_ORGANIZATION  every lodging listing of Visit Richmond VA (Richmond Region
                      Tourism), with the bureau's own sub-categories.
...
-----------------------
The market contract's postal partition (richmond_va_geography_001). Petersburg,
...
-----------------------------------------------------------------------------------
Richmond carries substantial apartment-style lodging, corporate housing, short-term
rentals, historic-home rentals and student housing. None is admitted: a row whose only
...
(richmond_va_nonhotel_rulings_001).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/richmond-va.json
  launch_packages/pettripfinder/markets/reports/richmond_va_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/richmond_va_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import richmond_va_geography_001 as GEO  # noqa: E402
...
OSM_LANE = os.path.join(REPORTS, "richmond_va_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "richmond_va_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "richmond_va_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "richmond_va_destination_roster_001.json")
#: CHARLESTON: the market-local reading of the static lane's own-site JSON-LD addresses (identity only).
STATIC_LANE = os.path.join(REPORTS, "richmond_va_static_lane_001.json")
POLICY_PAGES_LANE = os.path.join(REPORTS, "richmond_va_policy_pages_lane_001.json")
...
from scripts.pettripfinder.richmond_va_nonhotel_rulings_001 import (  # noqa: E402
...
def read_brand_city_pages():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if state_code(card.get("state")) not in ("VA",):
            continue
        if (card.get("postal_code") or "")[:3] not in ("230", "231", "232", "238"):
            continue
...
def read_brand_state_sitemap():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if not ((r.get("property_code") or "").lower().startswith("ric") or places_in(r.get("brand_title") or "")):
            continue
...
from scripts.pettripfinder.richmond_va_static_rulings_001 import (  # noqa: E402
...
_NOT_OWN_SITE = ("marriott.com", "hilton.com", "ihg.com", "choicehotels.com", "hyatt.com", "bestwestern.com",
                 "wyndhamhotels.com", "visitrichmondva.com", "tripadvisor", "expedia", "booking.com", "vrbo", "airbnb",
                 ...
                 "hrs.com", "zenhotels.com", "hotels.com", "agoda", "trip.com",
                 # a franchise site that presents Red Roof Inn and HomeTowne Studios (two properties) as one hotel
                 "airporthotelrichmond.com")
...
ROSTER_LISTINGS_OF_A_READ_PROPERTY = {
    "courtyard richmond downtown": (
        "MARRIOTT", "ricrl", "Visit Richmond lists 'Courtyard Richmond Downtown' at 1320 East Cary Street, the building "
        "the Courtyard (ricrl) shares with the Residence Inn (ricrt); a street alone cannot say which hotel the listing is"),
    "residence inn richmond downtown": (
        "MARRIOTT", "ricrt", "Visit Richmond lists 'Residence Inn Richmond Downtown' at 1320 East Cary Street, the building "
        "the Residence Inn (ricrt) shares with the Courtyard (ricrl)"),
    "hampton inn richmond west": (
        "HILTON", "ricwehx", "Visit Richmond lists 'Hampton Inn Richmond-West' at 10800 West Broad Street; Hilton's own page "
        "writes the same address 'Innsbrook-10800 W. Broad St.', which the street key cannot meet"),
}
...
_IN_MARKET_MUNICIPALITIES = {
    "richmond", "henrico", "glen allen", "short pump", "sandston", "highland springs", "midlothian", "chesterfield",
    "north chesterfield", "n chesterfield", "bon air", "chester", "mechanicsville", "ashland", "moseley", "varina",
    "innsbrook",
}
#: The Petersburg / Colonial Heights / Hopewell places, preserved for the future market.
_FUTURE_SUBMARKET_PLACES = {"colonial heights", "petersburg", "hopewell", "prince george", "fort lee",
                            "fort gregg adams", "south chesterfield", "ettrick"}
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {
    "doswell", "powhatan", "goochland", "oilville", "rockville", "hanover", "new kent", "providence forge", "quinton",
    "lanexa", "barhamsville", "williamsburg", "fredericksburg", "charlottesville", "farmville", "emporia",
    "south hill", "stony creek", "ruther glen", "west point", "amelia", "dinwiddie", "manakin sabot", "maidens",
    "toano", "stafford", "spotsylvania", "carson", "disputanta",
}
...
_DIRECTIONALS = {
    ...
    # RICHMOND area words that tell same-brand hotels apart (Courtyard Downtown / Airport / West / Northwest / Scott's
    # Addition; Residence Inn Midtown / West End / Northwest; Hampton Inn Richmond-South / -North / -Airport)
    "airport", "medical", "university", "end", "midtown", "glenside", "innsbrook", "parham", "chippenham", "commerce",
    "broad", "scotts", "addition", "willow", "lawn", "i95", "i64", "i295", "vcu", "museum", "district",
    "convention", "capitol", "riverfront", "ric", "atlee", "hull", "notch", "mall", "towne", "centre",
    "southside", "northside", "westend", "gaskins", "arena", "southpark",
}
...
CORRIDOR_CITIES = {
    "downtown": {"richmond", "henrico"},
    "vcu-fan-museum-district": {"richmond"},
    "west-end": {"richmond", "henrico", "glen allen"},
    "short-pump": {"glen allen", "richmond", "henrico", "short pump"},
    "innsbrook-glen-allen": {"glen allen", "richmond", "henrico", "innsbrook"},
    "ric-airport-sandston": {"sandston", "richmond", "henrico", "highland springs", "varina"},
    "i-95-north": {"richmond", "glen allen", "henrico"},
    "i-95-south": {"richmond", "north chesterfield", "n chesterfield", "chesterfield"},
    "midlothian-chesterfield": {"midlothian", "richmond", "north chesterfield", "n chesterfield", "chesterfield",
                                "bon air", "moseley"},
    "mechanicsville": {"mechanicsville", "richmond"},
    "ashland": {"ashland", "glen allen"},
    "chester": {"chester", "richmond", "chesterfield"},
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and (_subcats & NON_HOTEL_BUREAU_SUBCATEGORIES) and not any(
        ...
        return NON_LODGING, ("Visit Richmond VA files this listing under %s -- a rental / apartment "
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and _CAMPGROUND_NAME.search(node.name or "") and not any(
            str(o.get("lane", "")).startswith(("PROPERTY_PAGE", "BRAND_INVENTORY")) for o in node.observations):
        return NON_LODGING, ("Visit Richmond VA files this listing only under %s and its own name is a campground, RV "
    ...
    if not node.postal and _pins and all(float(t) < 37.28 or (float(t) < 37.34 and float(g) > -77.34) for t, g in _pins):
        return OUTSIDE_MARKET, ("FUTURE_SUBMARKET %s -- no postal code of its own, and every source pins it "
                                "in Colonial Heights / Petersburg / Hopewell (%.4f, %.4f)"
                                % (GEO.FUTURE_SUBMARKET, float(_pins[0][0]), float(_pins[0][1])))
    if not node.postal and _pins and all(float(t) > 37.80 or float(g) < -77.78 or float(g) > -77.20
                                         for t, g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it beyond the admitted "
                                "corridors (Doswell north, Powhatan / Goochland west or New Kent east; %.4f, %.4f)"
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Richmond "
...
BRAND_ROUTE_REFUSED = {
    "quality inn richmond airport": "https://www.choicehotels.com/virginia/sandston/quality-inn-hotels/va033 (403)",
}
...
def brand_not_listed(node):
    ...
    for fam, rx in _COVERED_FLAGS:
        if re.search(rx, node.name or "", re.I):
            return ("a %s row naming a %s flag (%r) that no read of that brand's own Richmond-area inventory "
...
SOURCE_AUTHORITIES = [
    ...
    "sitemap page)",
    "https://www.hilton.com/en/locations/usa/virginia/<city>/ (Virginia city pages and Richmond sub-pages, with "
    "property cards)",
    ...
    "https://www.visitrichmondva.com the bureau's own listing service, every lodging-category listing (Richmond "
    "Region Tourism, plain client)",
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Richmond authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "richmond-va.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "richmond_va_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "richmond_va_competitor_gap_matrix_001.json"))
