"""PTF-FORT-LAUDERDALE-FL-REGISTRATION-AND-STAGING-002 -- Fort Lauderdale's release contract.
...
WORK_ORDER = "PTF-FORT-LAUDERDALE-FL-REGISTRATION-AND-STAGING-002"
SOURCE_ORDER = "PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001"
...
def main(argv=None) -> int:
    ...
    clean = _load(os.path.join(REPORTS, "fort_lauderdale_fl_clean_authority_001.json"))
    ...
    doc = OrderedDict([
        ...
        ("release_name_prefix", "prod-fort-lauderdale"),
        ("description",
         "Deterministic release-gate contract for the PetTripFinder Greater Fort Lauderdale market through %s. "
        ...
        ("deployment_authorization", OrderedDict([
            ...
            ("means",
             ...
             "decision exists for Fort Lauderdale at %d published profiles."
        ...
        ("identity_census", OrderedDict([
            ...
            ("note",
             "%d registered identities, built from zero by %s on the Miami-live release-factory "
             "lineage and closed in that same order through the supported attended-browser lane "
             "(90 paced attempts, 81 first-party brand reads, no paid provider and no Akamai "
             "bypass), registered by %s. Membership is this market's own corridor postal partition "
             "(census_membership_basis CORRIDOR_REGISTRY): 18 corridors over 53 Broward County "
             "postal codes. Miami-Dade belongs to the LIVE miami-fl market and Palm Beach County "
             "and the Florida Keys are named, refused and preserved for their own future markets. "
             "Broward's postal codes straddle municipalities, so a corridor covers a shared code "
             "WHOLE and reports its towns as overlays; Port Everglades IS a corridor because ZIP "
             "33316 carries hotels no other corridor claims, and FLL is NOT because the airport "
             "owns no postal code. Individual vacation houses, condo units, rental-management "
             "portfolios and timeshare inventory are never admitted. Rows the identity graph did "
        ...
        ("reconciliation", OrderedDict(
            ...
            + [("note",
                ...
                "fort_lauderdale_fl_clean_authority_001.json with its class. No gate was lowered to "
                "preserve a count."
