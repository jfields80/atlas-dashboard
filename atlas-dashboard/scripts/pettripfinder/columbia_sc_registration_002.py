"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Columbia's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/columbia-sc/launch_package/hotel_policy_facts_columbia-sc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/columbia-sc/columbia_sc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/columbia-sc.json`, an
...
CLEAN = os.path.join(REPORTS, "columbia_sc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census_proposed", "columbia-sc.json")
STAGING = os.path.join(PKG, "markets", "staging", "columbia-sc")
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_columbia-sc.json")
AUTHORITY_OUT = os.path.join(STAGING, "columbia_sc_proposed_authority_002.json")
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Columbia without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages Columbia without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Columbia, South Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Columbia's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Columbia's authority as this order proposes it, STAGED and not registered. Registration makes the market "
