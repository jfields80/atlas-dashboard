"""PTF-FAYETTEVILLE-NC-NORMAL-PRODUCTION-001 -- the final partition.

Every registered Fayetteville identity, in EXACTLY ONE state, with exactly one next
...
------------------------------------
  ...
  ACCESS_BLOCKED              the surface refused to serve us. Choice (a
                              sitemap timeout and a blank attended page) and
                              Marriott's StudioRes route (a redirect loop)
...
Output: launch_packages/pettripfinder/fayetteville_nc_final_partition_007.json
"""
...
OUT = os.path.join(PKG, "fayetteville_nc_final_partition_007.json")
...
REFUSALS = {
    "BEST_WESTERN": "a sitemap 403 to a plain client",
    "CHOICE": "a sitemap timeout and a blank attended page for choicehotels.com/north-carolina/fayetteville/hotels",
    "RADISSON": "a sitemap 403",
}
...
POLICY_SILENT = {
    "studio 6 fayetteville, nc - fort bragg area":
        "the brand's own sitemap lists Studio 6 (293587) and Motel 6 (294393) at 3719 Bragg Boulevard; "
        "both pages served the attended session and state only a 'Pets Allowed' amenity chip and "
        "'Pets Stay Free Details', no postal code and no operative sentence",
    "red lion inn & suites fayetteville at cross creek mall":
        "a map-only row; third-party listings now name 562 Cross Creek Mall a Choice Quality Inn, and "
        "Choice refused this client, so no first-party page was read",
}
...
NAME_FAMILY = (
    ...
    ("CHOICE", ("comfort inn", "comfort suites", "quality inn", "sleep inn", "clarion",
                "econo lodge", "rodeway", "mainstay", "suburban studios", "cambria", "ascend hotel")),
...
def build():
    ...
    routing = _load(os.path.join(REPORTS, "fayetteville_nc_routing_001.json"), {})
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-FAYETTEVILLE-NC-NORMAL-PRODUCTION-001 "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-13"),
        ("what_this_is",
         "Every registered Fayetteville identity in exactly one disposition state. A terminal state "
