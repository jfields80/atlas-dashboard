"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 12C, free first-party static capture (router rung DIRECT_STATIC_FETCH).

Cloned from the Nashville TN helper.

One HTTPS GET per route through the canonical gates, for two cohorts:

  ROUTED          every admitted Orlando identity that carries a first-party
                  route. The question is the PET POLICY.
  IDENTITY_FILL   the brand-roster / registry properties whose existence and route the
...
Output:
  launch_packages/pettripfinder/markets/reports/orlando_fl_v2_free_static_capture_001.json
"""
...
BRANDS = [
    ...
    ("DRURY", r"drury"),
    # Nashville-specific families. This market has a full-service and
    # soft-brand layer no prior market carried: Omni and Loews operate their own
    # domains, Gaylord Opryland is a Marriott property on its own site, and the
    # Autograph / Tribute Portfolio / Curio / Tapestry collections sit inside
    # the Marriott and Hilton readers rather than beside them.
    ("OMNI", r"\bomni\b"),
    ("LOEWS", r"\bloews\b"),
    ...
    ("ROSEN", r"rosen"),
    ("DISNEY", r"disney'?s"),
    ("UNIVERSAL", r"universal'?s|cabana bay|endless summer|helios grand"),
]
...
def targets():
    ...
    for r in routing["routes"]:
        if not r.get("url"):
            continue
        # ORLANDO: a Wyndham route is read by the brand's own property service in the plain-client Wyndham lane
        # (orlando_fl_v2_wyndham_lane_001); fetching the client-rendered overview here would read nothing.
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS,
                                                  "orlando_fl_v2_free_static_capture_001.json"))
    ap.add_argument("--run-id", default="orlando_fl_v2_free_static_001")
