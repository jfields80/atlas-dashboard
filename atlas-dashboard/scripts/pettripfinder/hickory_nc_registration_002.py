"""PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Hickory - Newton - Conover's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/hickory-nc/launch_package/hotel_policy_facts_hickory-nc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/hickory-nc/hickory_nc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 runs while the release queue is drained separately
(Greenville is live). This order therefore writes NOTHING at
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/hickory-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "hickory_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census_proposed", "hickory-nc.json")
STAGING = os.path.join(PKG, "markets", "staging", "hickory-nc")
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_hickory-nc.json")
AUTHORITY_OUT = os.path.join(STAGING, "hickory_nc_proposed_authority_002.json")
OBSERVED_AT = "2026-09-14"
CAPTURED_AT = "2026-09-14T03:40:00+00:00"
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Hickory without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages Hickory without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Hickory \u2013 Newton \u2013 Conover, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Hickory - Newton - Conover's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Hickory - Newton - Conover's authority as this order proposes it, STAGED and not registered. Registration makes the market "
