"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Outer Banks' authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/outer-banks-nc/launch_package/hotel_policy_facts_outer-banks-nc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/outer-banks-nc/outer_banks_nc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/outer-banks-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "outer_banks_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census", "outer-banks-nc.json")
STAGING = os.path.join(PKG, "markets", "staging", "outer-banks-nc")
#: REGISTERED by PTF-OUTER-BANKS-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root, as every registered market.
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_outer-banks-nc.json")
AUTHORITY_OUT = os.path.join(PKG, "outer_banks_nc_proposed_authority_002.json")
...
def build():
    ...
    package = OrderedDict((
        ("market", "Outer Banks, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Outer Banks' REGISTERED policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Outer Banks' authority as the source-ready order proposed it, registered by PTF-OUTER-BANKS-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Registration makes the market "
