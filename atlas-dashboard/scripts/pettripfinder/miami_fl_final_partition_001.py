"""PTF-MIAMI-FL-HARDENED-SOURCE-READY-001 -- Phases 20-21: every census identity in exactly one disposition.

Cloned from the Orlando FL V2 helper, simplified because this market's clean-authority pass
(miami_fl_clean_authority_001) already computes one disposition per confirmed identity directly, with its
...
Output: launch_packages/pettripfinder/markets/staging/miami-fl/launch_package/miami_fl_final_partition_001.json
"""
...
def build():
    ...
    for h in census["hotels"]:
        ...
        if disp in _TERMINAL:
            ...
            row["resolved"] = False
            row["next_action_source"] = "PTF-MIAMI-FL-HARDENED-SOURCE-READY-001 acquisition router record"
    ...
    return OrderedDict((
        ...
        ("what_this_is", "Every admitted Greater Miami census identity in exactly one disposition state, with the "
