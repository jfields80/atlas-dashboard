"""PTF-RALEIGH-NC-FINAL-FRESH-CITY-PROOF-001 -- the co-located-hotel rulings.

Raleigh's airport and uptown corridors are full of DUAL-BRAND BUILDINGS: one
...
2220 West Tyvola Road is the sharp case. The Fairfield Inn & Suites Raleigh
Airport (cltfs) states "Pets Not Allowed" and becomes a VERIFIED_NO_PETS
exclusion. The Residence Inn by Marriott Raleigh Airport (cltwe) states "2
...
Output: launch_packages/pettripfinder/identity_resolutions.json  (rows appended)
        launch_packages/pettripfinder/markets/reports/raleigh_nc_co_location_003.json
"""
...
AUTHORITY = os.path.join(PKG, "raleigh_nc_proposed_authority_002.json")
RESOLUTIONS = os.path.join(PKG, "identity_resolutions.json")
OUT = os.path.join(REPORTS, "raleigh_nc_co_location_003.json")
...
def build():
    ...
    for key, rows in sorted(by_street.items()):
        ...
        # UNREVIEWED shared address. Raleigh has four such pairs -- Courtyard
    ...
    for key, a, b, _row in resolved:
        ...
        rows.append(OrderedDict((
            ...
            # unscoped. Leaving these unscoped made seven Raleigh rulings
