"""PTF-CHARLESTON-SC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Charleston's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/charleston-sc/launch_package/hotel_policy_facts_charleston-sc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/charleston-sc/charleston_sc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-CHARLESTON-SC-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
markets stand ahead of it in the serialized release queue (the current live release is Atlanta). This order therefore writes NOTHING at
...
against the then-current verified live parent.

WHY A LATER ORDER MAY REGISTER WITHOUT A ROW-BY-ROW FOUNDER SIGNATURE
------------------------------------------------------------------
Registration is not publication. Writing `markets/charleston-sc.json`, an
...
CLEAN = os.path.join(REPORTS, "charleston_sc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census", "charleston-sc.json")
STAGING = os.path.join(PKG, "markets", "staging", "charleston-sc")
#: REGISTERED by PTF-CHARLESTON-SC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root, as every registered market.
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_charleston-sc.json")
AUTHORITY_OUT = os.path.join(PKG, "charleston_sc_proposed_authority_002.json")
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Charleston without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages Charleston without a row-by-row
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Charleston's REGISTERED policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Charleston's authority as the source-ready order proposed it, registered by PTF-CHARLESTON-SC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Registration makes the market "
