...
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_charleston-sc.json")
AUTHORITY_OUT = os.path.join(STAGING, "charleston_sc_proposed_authority_002.json")
...
def build():
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Charleston's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Charleston's authority as this order proposes it, STAGED and not registered. Registration makes the market "
