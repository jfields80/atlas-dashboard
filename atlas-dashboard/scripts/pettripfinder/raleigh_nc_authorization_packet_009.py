"""PTF-RALEIGH-NC-FINAL-FRESH-CITY-PROOF-001 -- the founder authorization packet.
...
Output: launch_packages/pettripfinder/markets/reports/raleigh_nc_authorization_packet_009.json
"""
...
OUT = REPORTS / "raleigh_nc_authorization_packet_009.json"
...
CLOSURE = (_DASH / "launch_packages" / "pettripfinder" / "failure_closures"
           / "raleigh_nc_registration_011.json")
...
def main(argv=None):
    ...
    lane = _load(REPORTS / "raleigh_nc_release_lane_005.json")
    asm = _load(REPORTS / "raleigh_nc_candidate_assembly_008.json")
    cov = _load(REPORTS / "raleigh_nc_coverage_006.json")
    part = _load(PKG / "raleigh_nc_final_partition_007.json")
    census = _load(PKG / "identity_census" / "raleigh-nc.json")
    clean = _load(REPORTS / "raleigh_nc_clean_authority_001.json")
    ...
    doc = OrderedDict((
        ...
        ("would_become",
         "Raleigh, North Carolina joins production as the FOURTEENTH market with 106 "
        ...
        ("cost", OrderedDict((
            ...
            ("note",
             "Raleigh was built from zero for nothing. Every published row came through a free "
        ...
        ("what_is_not_claimed", [
            "Raleigh is not complete: %d of its %d registered identities remain unread, and "
            "three brand families are unresolved for named reasons recorded in the partition."
            % (part["unresolved"], census["count"]),
            "No founder deployment decision exists for Raleigh at %d published profiles."
