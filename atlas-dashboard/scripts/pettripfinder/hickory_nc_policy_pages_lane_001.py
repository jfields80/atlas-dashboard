"""PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 -- Phase 11B: the independents' own policy pages.

Hickory-area independents publish their pet policy, when they publish one, on a subpage (``/pet-policy``,
...
``data/acquisition/hickory_nc_policy_pages_001/``).
...
recorded in ``markets/staging/hickory-nc/raw_captures/brand_pages.json``
with the sha256 of the document they were read from, and the capture helper binds
them.

Output:
  launch_packages/pettripfinder/markets/reports/hickory_nc_policy_pages_lane_001.json
"""
...
from scripts.pettripfinder import hickory_nc_static_lane_001 as S      # noqa: E402
...
STATIC = os.path.join(REPORTS, "hickory_nc_static_lane_001.json")
OUT = os.path.join(REPORTS, "hickory_nc_policy_pages_lane_001.json")
DOCS = os.path.join(_DASH, "data", "acquisition", "hickory_nc_policy_pages_001")
LINK_CAP = 6
