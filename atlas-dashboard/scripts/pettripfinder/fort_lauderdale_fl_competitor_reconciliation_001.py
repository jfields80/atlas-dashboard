"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phase 10: BringFido gap reconciliation (identity only, never policy).
...
vacation-rental, duplicate, review) -- so each lead lands in exactly one class:
  ...
  TRUE_MISSING           a lodging-establishment name no census node carries (worked by
                         fort_lauderdale_fl_competitor_additions_001 through an authoritative identity lane)

A name match PROPOSES an identity; it never decides one. BringFido's pet claims are never read.

Output:
  launch_packages/pettripfinder/markets/reports/fort_lauderdale_fl_competitor_reconciliation_001.json
"""
...
from scripts.pettripfinder import fort_lauderdale_fl_census_reconciliation_001 as CR  # noqa: E402
...
COMPETITOR = os.path.join(REPORTS, "fort_lauderdale_fl_competitor_challenge_001.json")
CLEAN = os.path.join(REPORTS, "fort_lauderdale_fl_clean_authority_001.json")
OUT = os.path.join(REPORTS, "fort_lauderdale_fl_competitor_reconciliation_001.json")

_STOP = re.compile(r"\b(hotel|hotels|inn|suites?|by|the|and|resort|resorts|motel|at|of|a|an|hilton|marriott|"
                   r"collection|autograph|tapestry|ascend|curio|tribute|lauderdale|hollywood|pompano|deerfield|dania|hallandale|plantation|davie|sunrise|weston|miramar|fl|florida|spa|beach)\b", re.I)
...
_OTHER_FLORIDA = re.compile(r"\b(vero beach|tallahassee|jacksonville|pensacola|destin|cocoa beach|port canaveral|"
                            ...
                            r"boca raton|fort lauderdale|hollywood|hallandale|daytona|st\.? augustine|sarasota|"
...
def build():
    ...
    return OrderedDict([
        ("schema", "ptf-competitor-reconciliation/1.0"), ("work_order", WORK_ORDER), ("market_id", "fort-lauderdale-fl"),
