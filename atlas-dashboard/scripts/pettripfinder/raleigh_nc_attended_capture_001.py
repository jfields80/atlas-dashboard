"""PTF-RALEIGH-NC-FINAL-FRESH-CITY-PROOF-001 -- the attended browser pass.

The Marriott, Hilton and IHG wall, walked in a real Chrome session with the
operator present. All three refuse this project's plain client, so this is the
only free lane that reads them.

HOW THE READ IS BOUND
---------------------
No family is read from rendered innerText. Each publishes the operative policy
as structured data, and the attended session takes it from there and nowhere
else:
  ...
  HILTON    ``__NEXT_DATA__`` carries a ``petsInfo`` node -- petsAllowed,
            ...
            prose; a review is not a policy, and the walk is anchored on the
            node's own key set so it can never be read.
  ...
  IHG       the property FAQ's own answer to "Can I bring my pet to X?". The
            pets-allowed AMENITY ICON several of these pages also carry
            establishes nothing and is never read.

Each brand was batched by SAME-ORIGIN FETCH from one tab already on the brand's
own origin -- the Dayton and Lexington precedent, one navigation per ORIGIN --
rather than by one navigation per property.

A PROPERTY CODE SELECTS; THE PAGE ADMITS
----------------------------------------
This pass is what settles that, and Raleigh is the market where it matters
most. Marriott's RDU prefix is the Raleigh-DURHAM airport code; dozens of
properties are named "...Raleigh-Durham... Research Triangle Park..." and stand
in Durham County, and Hilton's North Carolina city pages reach the same ring.
Every such row is captured and classified on the postal code its OWN page
states -- not on a guess, and not by omitting it.
...
CONTRACT = os.path.join(PKG, "markets", "raleigh-nc.json")
...
#: The corridor registry Phase 2 authored, read directly when the market shard
#: does not exist yet. The attended pass runs BEFORE registration -- that is the
#: point of it, since the addresses it reads are what the census is built from --
#: so it must be able to say IN_MARKET / OUTSIDE_MARKET from the registry alone.
#: The two documents carry the SAME postal partition; registration copies it.
REGISTRY = os.path.join(REPORTS, "raleigh_nc_corridor_registry_001.json")
...
def zips_of_contract():
    ...
    reg = _load(REGISTRY)
    return {z: c["corridor_id"] for c in reg["corridors"]
...
def build(rows_path):
    ...
    return OrderedDict([
        ...
        ("a_code_selects_the_page_admits",
         "Marriott's CLT prefix is the Raleigh Douglas AIRPORT code and the committed owned "
         "roster shows it reaching Shelby (cltby), Statesville (cltfv), Hickory (cltht), "
         "Salisbury (cltsb), Mooresville, Monroe and Gastonia. Raleigh adds a hazard Nashville "
         "did not have: the market name itself is ambiguous. CHARLOTTESVILLE, VIRGINIA shares "
         "the first eleven letters, Raleigh is a town in MICHIGAN and a county seat in "
         "VIRGINIA, and Marriott files a Fort Mill SOUTH CAROLINA property under a raleigh "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS,
                                                  "raleigh_nc_attended_capture_001.json"))
