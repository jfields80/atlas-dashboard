"""PTF-AUGUSTA-GA-POLICY-EVIDENCE-CLOSURE-002 -- 15 FAST-equivalent checks.

Supersedes augusta_ga_fast_checks_001.py now that real policy evidence
exists: checks 12 and 14 are updated (rebuild via build_002; terminal rows
must be evidence-backed rather than absent), and a 16th check proves the
exclusion shard's VERIFIED_NO_PETS count matches the partition's.

Run AFTER augusta_ga_market_build_002:

    python -m scripts.pettripfinder.augusta_ga_fast_checks_002
"""
...
def main() -> int:
    ...
    facts_doc = _load(POLICY_FACTS_PATH)
    facts_by_key = {f["identity_key"]: f for f in facts_doc["facts"]}
    ...
    def c6():
        ...
        assert len(records) == len(exclusions_doc["exclusions"]), (len(records), len(exclusions_doc["exclusions"]))
        for r in exclusions_doc["exclusions"]:
            assert r.get("record_hash") == HE.record_hash(r)
            assert r.get("approval_hash") == HE.approval_hash(r)
            assert r.get("evidence_quote"), r["canonical_name"]
    ...
    def c12():
        import hashlib
        import subprocess
        env_paths = [CENSUS_PATH, PARTITION_PATH, EXCLUSIONS_SHARD_PATH, POLICY_FACTS_PATH]
        before = {p: hashlib.sha256(p.read_bytes()).hexdigest() for p in env_paths}
        subprocess.run([sys.executable, "-m", "scripts.pettripfinder.augusta_ga_market_build_002"],
    ...
    def c14():
        """Every terminal partition row is backed by a durable evidence record with a
        non-empty operative quote whose classification agrees with the row's final_state,
        and no census row claims a policy_state without one."""
        for item in partition_doc["items"]:
            if item["final_state"] not in (enums.PUBLISHED_PET_FRIENDLY, enums.VERIFIED_NO_PETS):
                continue
            fact = facts_by_key.get(item["identity_key"])
            assert fact is not None, item["identity_key"]
            assert fact["operative_quote"], item["identity_key"]
            expected = ("VERIFIED_PET_FRIENDLY" if item["final_state"] == enums.PUBLISHED_PET_FRIENDLY
                       else "VERIFIED_NO_PETS")
            assert fact["classification"] == expected, (item["identity_key"], fact["classification"])
        for row in census_doc["hotels"]:
            if row["policy_state"] == enums.POLICY_NOT_VERIFIED:
                continue
            assert row["policy_state"] in (enums.POLICY_OBSERVED, enums.VERIFIED_NO_PETS), row
    ...
    def c15():
        ...
        assert facts_doc["market_id"] == MARKET_ID

    def c16():
        partition_no_pets = sum(1 for i in partition_doc["items"] if i["final_state"] == enums.VERIFIED_NO_PETS)
        assert len(exclusions_doc["exclusions"]) == partition_no_pets, (
            len(exclusions_doc["exclusions"]), partition_no_pets)
    ...
    check(6, "hotel_exclusions.validate + hash re-derivation + non-empty evidence_quote", c6)
    ...
    check(14, "every terminal partition row is evidence-backed and classification-consistent", c14)
    check(15, "market_id consistent across all 6 documents", c15)
    check(16, "exclusion shard count == partition VERIFIED_NO_PETS count", c16)
