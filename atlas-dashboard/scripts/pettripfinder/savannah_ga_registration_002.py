"""PTF-SAVANNAH-GA-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Savannah's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/savannah-ga/launch_package/hotel_policy_facts_savannah-ga.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/savannah-ga/savannah_ga_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-SAVANNAH-GA-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
markets stand ahead of it in the serialized release queue (the current live release is Jacksonville). This order therefore writes NOTHING at
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/savannah-ga.json`, an
...
STAGING = os.path.join(PKG, "markets", "staging", "savannah-ga")
#: REGISTERED by PTF-SAVANNAH-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root, as every registered market.
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_savannah-ga.json")
AUTHORITY_OUT = os.path.join(PKG, "savannah_ga_proposed_authority_002.json")
OBSERVED_AT = "2026-09-14"
CAPTURED_AT = "2026-09-14T06:00:00+00:00"
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Savannah without a row-by-row
    ...
    package = OrderedDict((
        ...
        ("as_of", OBSERVED_AT),
        ("note",
         "Savannah's REGISTERED policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Savannah's authority as the source-ready order proposed it, registered by PTF-SAVANNAH-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Registration makes the market "
