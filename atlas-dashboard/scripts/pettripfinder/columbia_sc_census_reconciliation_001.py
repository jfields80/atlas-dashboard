"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.
...
Independent discovery lanes, none authoritative on its own:
  ...
  OSM_OVERPASS        tourism=hotel / motel / guest_house / apartment / chalet elements
                      inside the observation box, read from the local Geofabrik South Carolina
                      extract (ODbL).
  BRAND_INVENTORY     the committed national Marriott harvest (CAE-coded routes);
                      Marriott's South Carolina hotel sitemap page; Hilton's own South Carolina
                      city pages (property cards stating each hotel's address);
                      Wyndham's, Drury's and WoodSpring's own sitemaps.
  DESTINATION_ORGANIZATION  every lodging listing of Experience Columbia SC (the Columbia CVB's
                      Simpleview listing service), with the bureau's own sub-categories.
...
-----------------------
The market contract's postal partition (columbia_sc_geography_001). Camden / Lugoff, Sumter,
Orangeburg, Newberry, rural Gaston / Swansea and the Fort Jackson installation are OUTSIDE by name.

APARTMENTS, CORPORATE HOUSING, STUDENT HOUSING AND SHORT-TERM RENTALS ARE NOT HOTELS
-----------------------------------------------------------------------------------
Columbia carries student apartments around USC, corporate housing, extended-stay apartments,
lake-house rentals, campgrounds and Fort Jackson military lodging. None is admitted: a row whose only
...
(columbia_sc_nonhotel_rulings_001).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/columbia-sc.json
  launch_packages/pettripfinder/markets/reports/columbia_sc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/columbia_sc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import columbia_sc_geography_001 as GEO  # noqa: E402
...
GEOGRAPHY_HOLDS = {
    "holiday inn express and suites columbia downtown the vista": (
        "IHG's own Hotel JSON-LD for caedt (attended browser) states '901 Washington Street' in The Vista beside postal code "
        "29229, which the corridor registry places in Sandhills / Clemson Road ten miles away; the street and the ZIP are two "
        "first-party facts that cannot both be right, and the ZIP alone never decides which"),
}
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "columbia-sc.json"),
                                os.path.join(PKG, "markets", "proposed", "columbia-sc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "columbia_sc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "columbia_sc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "columbia_sc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "columbia_sc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "columbia_sc_destination_roster_001.json")
#: CHARLESTON: the market-local reading of the static lane's own-site JSON-LD addresses (identity only).
STATIC_LANE = os.path.join(REPORTS, "columbia_sc_static_lane_001.json")
POLICY_PAGES_LANE = os.path.join(REPORTS, "columbia_sc_policy_pages_lane_001.json")
...
from scripts.pettripfinder.columbia_sc_nonhotel_rulings_001 import (  # noqa: E402
...
HOTEL_BUREAU_SUBCATEGORIES = {"Hotels", "Bed & Breakfasts", "Historic Inns", "Extended Stay", "Downtown Hotel",
                              ...
                              # COLUMBIA: Experience Columbia SC's own hotel-type sub-categories
                              "Preferred Hotels", "Hotels-Limited Service", "Hotel Meeting Space"}
NON_HOTEL_BUREAU_SUBCATEGORIES |= {"Beach Vacation Homes", "Condo Rentals", "Camping & Cabins", "Campground",
                                   "Group Lodging", "Camping"}
...
_MILITARY_LODGING = re.compile(
    r"\b(navy gateway|ngis|army hotels?|air force inns?|afi\b|transient (?:quarters|lodging)|"
    r"unaccompanied housing|bachelor (?:officer|enlisted) quarters|military lodging|fort jackson lodging|"
    r"palmetto lodge|ihg palmetto|fort jackson hotel|mcentire|distinguished visitors? quarters|navy lodges?)\b", re.I)
...
def read_brand_city_pages():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        # COLUMBIA: 290xx Lexington / Blythewood / Chapin / Elgin / Irmo / Cayce, 291xx West Columbia, 292xx Columbia.
        if (card.get("postal_code") or "")[:3] not in ("290", "291", "292"):
            continue
...
def read_brand_state_sitemap():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if not ((r.get("property_code") or "").lower().startswith(("cae",)) or places_in(r.get("brand_title") or "")):
            continue
...
from scripts.pettripfinder.columbia_sc_static_rulings_001 import (  # noqa: E402
...
_NOT_OWN_SITE = ("marriott.com", "hilton.com", "ihg.com", "choicehotels.com", "hyatt.com", "bestwestern.com",
                 "wyndhamhotels.com", "experiencecolumbiasc.com", "lakemurraycountry.com", "tripadvisor", "expedia", "booking.com",
...
_GENERIC_TOKENS = {
    ...
    "sc", "south", "carolina", "bed", "breakfast", "b",
...
_IN_MARKET_MUNICIPALITIES = {
    "columbia", "west columbia", "cayce", "lexington", "irmo", "blythewood", "chapin", "elgin", "forest acres",
    "arcadia lakes", "ballentine", "springdale", "pine ridge", "red bank", "south congaree", "oak grove",
}
#: No future submarket is preserved by this order.
_FUTURE_SUBMARKET_PLACES = set()
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {
    "camden", "lugoff", "sumter", "shaw afb", "dalzell", "orangeburg", "santee", "st matthews", "newberry",
    "prosperity", "little mountain", "pomaria", "winnsboro", "ridgeway", "batesburg", "leesville",
    "batesburg leesville", "gilbert", "gaston", "swansea", "pelion", "eastover", "hopkins", "gadsden", "aiken",
    "north augusta", "augusta", "florence", "rock hill", "charlotte", "summit",
}
...
_DIRECTIONALS = {
    ...
    # COLUMBIA area words that tell same-brand hotels apart (Fairfield Downtown / Harbison / Northeast / Lexington /
    # West Airport; Hampton Inn Downtown / Airport / Harbison / Northeast / Clemson Road / Killian Road)
    "airport", "medical", "university", "usc", "vista", "harbison", "jackson", "ft", "fort", "greystone", "killian",
    "clemson", "notch", "two", "gervais", "i26", "i20", "i77", "i126", "exit", "sandhills", "parklane", "bush",
    "river", "st", "andrews", "garners", "ferry", "dutch", "square", "knox", "abbott", "sunset", "historic",
    "district", "state", "capital", "fashion", "mall", "columbiana", "piney", "grove", "arena", "stadium", "zoo",
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    # COLUMBIA: Lugoff and Camden lie east of lng -80.76 (Elgin's I-20 exit 82 cluster is near -80.84); Gaston, Swansea
    # and Pelion south of lat 33.86 (Cayce's southern edge is near 33.90); Ridgeway / Winnsboro north of lat 34.26
    # (Blythewood tops out near 34.22); Little Mountain, Leesville and Gilbert west of lng -81.40 (Chapin is -81.35).
    # A pin never ADMITS a building.
    if not node.postal and _pins and all(float(g) > -80.76 or float(t) < 33.86 or float(t) > 34.26 or float(g) < -81.40
                                         for t, g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it beyond the admitted "
                                "corridors (Lugoff / Camden east, Gaston / Swansea south, Ridgeway north or Little "
                                "Mountain / Leesville west; %.4f, %.4f)" % (float(_pins[0][0]), float(_pins[0][1])))
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Columbia "
...
_COVERED_FLAGS = (
    ...
    # COLUMBIA: Red Roof's own sitemap was read in full (rri090, rri623).
    ("RED_ROOF", r"\b(red roof)\b"),
)
...
def brand_not_listed(node):
    ...
    for fam, rx in _COVERED_FLAGS:
        if re.search(rx, node.name or "", re.I):
            return ("a %s row naming a %s flag (%r) that no read of that brand's own Columbia inventory "
...
SOURCE_AUTHORITIES = [
    ...
    "national Marriott harvest, CAE-coded routes)",
    "https://www.marriott.com/en-us/hotel-sitemap/usa-south-carolina-hotel-sitemap (the brand's own South Carolina "
    "hotel sitemap page)",
    "https://www.hilton.com/en/locations/usa/south-carolina/<city>/ (Columbia-area city pages and sub-pages, with "
    "property cards)",
    ...
    "https://www.experiencecolumbiasc.com (the Columbia CVB's Simpleview listing service, plain client)",
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 Columbia census, built from zero "
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Columbia authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "columbia-sc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "columbia_sc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "columbia_sc_competitor_gap_matrix_001.json"))
