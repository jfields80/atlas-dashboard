"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 10: BringFido gap reconciliation (identity only, never policy).
...
vacation-rental, duplicate, review) -- so each lead lands in exactly one class:
  ...
  TRUE_MISSING           a lodging-establishment name no census node carries (worked by
                         san_diego_ca_competitor_additions_001 through an authoritative identity lane)

A name match PROPOSES an identity; it never decides one. BringFido's pet claims are never read.

Output:
  launch_packages/pettripfinder/markets/reports/san_diego_ca_competitor_reconciliation_001.json
"""
...
from scripts.pettripfinder import san_diego_ca_census_reconciliation_001 as CR  # noqa: E402
...
_STOP = re.compile(r"\b(hotel|hotels|inn|suites?|by|the|and|resort|resorts|motel|at|of|a|an|hilton|marriott|"
                   r"collection|autograph|tapestry|ascend|curio|tribute|san diego|la jolla|coronado|del mar|carlsbad|oceanside|"
                   r"mission valley|mission bay|gaslamp|downtown|old town|point loma|pacific beach|mission beach|"
                   r"ocean beach|chula vista|escondido|encinitas|solana beach|ca|california|spa|beach)\b", re.I)
...
#: Places outside this market that BringFido's radius widening pulls onto small-city pages.
_OTHER_FLORIDA = re.compile(r"\b(temecula|murrieta|menifee|lake elsinore|pechanga|riverside|palm springs|palm desert|"
                            r"san clemente|dana point|san juan capistrano|laguna|mission viejo|irvine|anaheim|"
                            r"newport beach|huntington beach|costa mesa|los angeles|long beach|fallbrook|"
                            r"valley center|pala|pauma|julian|ramona|borrego|alpine|jamul|tijuana|rosarito|"
                            r"ensenada|baja|mexico|el centro|yuma)\b", re.I)
_TIMESHARE = re.compile(r"\b(vacation club|grand vacations|hgv|marriott vacation|timeshare|residence club|"
                        r"club wyndham|worldmark|grand pacific|welk|carlsbad seapointe)\b", re.I)
...
def build():
    ...
    return OrderedDict([
        ("schema", "ptf-competitor-reconciliation/1.0"), ("work_order", WORK_ORDER), ("market_id", "san-diego-ca"),
