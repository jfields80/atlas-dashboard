"""PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 -- Phases 10-14: the first-party policy reads.

Cloned from the Boone - Blowing Rock NC capture helper (itself from Outer Banks NC). Families and surfaces:
  ...
  WYNDHAM   plain-client reads (``banner_elk_sugar_beech_nc_wyndham_lane_001``): the overview page
...
``data/acquisition/banner_elk_sugar_beech_nc_attended/``; each payload file's canonical JSON
...
Output:
  launch_packages/pettripfinder/markets/reports/banner_elk_sugar_beech_nc_attended_capture_001.json
"""
...
RAW = os.path.join(PKG, "markets", "staging", "banner-elk-sugar-beech-nc", "raw_captures")
REGISTRY = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_corridor_registry_001.json")
OUT = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_attended_capture_001.json")
CAPTURED_AT = "2026-09-14"

#: OWNED EVIDENCE. A payload row carried byte-for-byte from another market's COMMITTED raw
#: capture keeps that capture's own date and names the committed payload it was cut from
#: (file, commit, and that whole payload's browser-verified canonical digest).
OWNED_PAYLOADS = OrderedDict([
    ("marriott_rows.json", OrderedDict([
        ("captured_at", "2026-09-13"),
        ("from", "launch_packages/pettripfinder/markets/staging/boone-blowing-rock-nc/raw_captures/marriott_rows.json"),
        ("from_commit", "7adbafd194d7d3ce687b455789681678a20672ba"),
        ("from_payload_digest", "6fec8b392e976107c148743933fa41c95aa2ae7dfbfa625474db2269c47e3a97"),
        ("rows_carried", ["tribc-courtyard-sugar-mountain-banner-elk"]),
        ("why", "the Boone - Blowing Rock attended pass read Courtyard Sugar Mountain Banner Elk (Sugar Mountain "
                "28604) and preserved it for this market; the row is carried verbatim, never re-read or reworded"),
    ])),
])
...
PAYLOAD_DIGESTS = OrderedDict([
    ("hilton_rows.json", "4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945"),
    ("marriott_rows.json", "4b219d98e4c5fdb100d55c388fe263548550145836c851a46ae5c119efc08397"),
    ("ihg_rows.json", "4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945"),
])
...
def _row(brand, lane, code, req, final, sig, sha, nbytes, surface, quote, ext, zips, method, conflict=None,
    ...
    return OrderedDict([
        ...
        ("requested_url", req), ("final_url", final), ("captured_at", captured_at or CAPTURED_AT),
...
def build():
    ...
    for r in _load(os.path.join(RAW, "marriott_rows.json"), []):
        ...
        out.append(_row("MARRIOTT", "ATTENDED_BROWSER", code, u, u, sig, r.get("h"), r.get("b"),
                        ...
                        captured_at=OWNED_PAYLOADS.get("marriott_rows.json", {}).get("captured_at")))
