"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phase 8D-8M: the official destination lodging rosters
(cloned from the Hampton Roads VA destination-roster helper).

WHAT THIS IS
------------
Experience Columbia SC (the Columbia Convention & Visitors Bureau) runs a Simpleview CRM site. This lane reads
the site's OWN listing service -- the same ``plugins_listings_listings/find`` call its lodging pages make in the
browser -- once, for every listing whose categories carry the bureau's lodging category, with the public simple
token the site itself issues. Crawl-delay honoured between calls.

Every response is persisted by its own sha256, and every listing's sub-categories are
recorded verbatim so the census can refuse vacation-rental, apartment, campground and group
...
Output:
  launch_packages/pettripfinder/markets/reports/columbia_sc_destination_roster_001.json
  data/acquisition/columbia_sc_destination_001/<sha256>.bin
"""
...
from scripts.pettripfinder import columbia_sc_brand_inventory_001 as BI  # noqa: E402
...
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "columbia_sc_destination_roster_001.json")
RAW = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "columbia-sc", "raw_captures")
BI.DOCS = os.path.join(_DASH, "data", "acquisition", "columbia_sc_destination_001")
...
#: (bureau, site, lodging catid) -- the lodging category the bureau's own listing data carries. Experience Columbia SC
#: (the Columbia Convention & Visitors Bureau, Simpleview) files every lodging listing under catid 22 ("Hotels"),
#: measured on its own /hotels/ page widget (``listingcats: "22"``).
SIMPLEVIEW_BUREAUS = [
    ("Experience Columbia SC", "https://www.experiencecolumbiasc.com", "22"),
]

#: Browser-read rosters: (bureau, site, lodging catid, committed payload, its page-computed sha256). None yet.
BROWSER_BUREAUS = [
]
...
def _compact_to_doc(site, r):
    ...
    return OrderedDict([
        ("recid", rid), ("title", title), ("url", site + path), ("address1", street), ("city", city), ("state", "SC"),
...
def build():
    ...
    return OrderedDict([
        ...
        ("phase", "8D-8M -- the official Columbia visitors bureau lodging roster"),
