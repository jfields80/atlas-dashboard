...
What it does do is REGISTER a market, and Regression V2 classifies that
FULL_REGRESSION_REQUIRED = YES on its own grounds: a DEPLOYMENT_CHANGE
(``deploy/netlify/launch_participation.json``, the new release contract,
``bundle_cache_closure.json``) and an AUTHORITY_CHANGE (the generated global
exclusions, the new policy package, the new registered census). Registering a
market happens once per market, and exactly one broad run was executed for it.
...
def main(argv=None) -> int:
    ...
    mine = [n for n in (split["CAUSED_BY_THIS_ORDER"]["nodes"]
                        + split["IMPORTED_SUITE_FALSIFIED_BY_PROMOTION"]["nodes"])
            if n not in retired]
    ...
    doc = OrderedDict((
        ...
        ("what_this_audited",
         ...
         "measures on its own, and it is NOT a factory integration: the ATLAS-THROUGHPUT "
         "engineering was already on this lineage before this order started."),
        ...
        ("the_classifier_cannot_see_this", OrderedDict((
            ...
            ("IMPORTED_SUITE_FALSIFIED_BY_PROMOTION", OrderedDict((
                ("count", len(split["IMPORTED_SUITE_FALSIFIED_BY_PROMOTION"]["nodes"])),
                ("nodes", split["IMPORTED_SUITE_FALSIFIED_BY_PROMOTION"]["nodes"]),
            ))),
        ))),
        ("closure", OrderedDict((
            ("what_this_is",
    ...
    print("  imported suite        :",
          len(split["IMPORTED_SUITE_FALSIFIED_BY_PROMOTION"]["nodes"]))
