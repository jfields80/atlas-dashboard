"""PTF-ORLANDO-FL-PARALLEL-SOURCE-READY-001 -- 15 applicable FAST checks for the orlando-fl shadow package.
...
``market_authority``) plus Orlando-specific evidence and lodging-category
safety checks. Nothing shared is modified.

Run:
    python -m scripts.pettripfinder.orlando_fl_fast_checks_001 [--skip-rebuild]
"""
...
from scripts.pettripfinder.orlando_fl_market_build_001 import MARKET_ID, REPORTS_REL, STAGING_REL  # noqa: E402
from scripts.pettripfinder import orlando_fl_policy_evidence_001 as EV  # noqa: E402

LP = STAGING_REL / "launch_package"
OUTPUTS = [
    LP / "identity_census" / "orlando-fl.json",
    LP / "orlando_fl_final_partition_001.json",
    LP / "markets" / "orlando-fl.json",
    LP / "markets" / "authority" / "orlando-fl" / "identity_routing.json",
    LP / "markets" / "authority" / "orlando-fl" / "hotel_exclusions.json",
    LP / "markets" / "authority" / "orlando-fl" / "seed_businesses.csv",
    LP / "orlando_fl_policy_evidence_ledger_001.json",
    STAGING_REL / "orlando_fl_identity_resolution_001.json",
...
def main(argv=None) -> int:
    ...
    def c10():
        ...
        assert not (live / "orlando-fl.json").exists()
        assert not (live / "authority" / "orlando-fl").exists()
    ...
    def c12():
        ...
        with tempfile.TemporaryDirectory() as tmp:
            env = {k: v for k, v in os.environ.items() if k != "ORLANDO_RESOLVE_CACHE"}
            env["PYTHONIOENCODING"] = "utf-8"
            subprocess.run([sys.executable, "-m", "scripts.pettripfinder.orlando_fl_market_build_001", "--out-root", tmp],
    ...
    def c13():
        ...
        for line in out.splitlines():
            ...
            ok = (path.startswith("launch_packages/pettripfinder/markets/staging/orlando-fl/")
                  or path.startswith("launch_packages/pettripfinder/markets/reports/orlando_fl_")
                  or path.startswith("scripts/pettripfinder/orlando_fl_")
                  or path.startswith("../PTF-ORLANDO-FL-") or path.startswith("PTF-ORLANDO-FL-"))
            if not ok or line[:2].strip() not in ("??", "A", "M", "AM"):
                bad.append(line)
            if line[:2].strip() == "M" and "orlando" not in path:
                bad.append(line)
        assert not bad, bad[:10]
    ...
    def c15():
        for r in census_doc["hotels"]:
            ...
            assert not (ranks and ranks <= {"TAPT", "BNB", "CNDO", "DWEL"}), ("unit/non-hotel licence admitted", r["identity_key"])
    ...
    check(10, "orlando-fl absent from LIVE registry and sharded_market_ids()", c10)
    ...
    check(13, "git status touches only orlando-owned paths", c13)
