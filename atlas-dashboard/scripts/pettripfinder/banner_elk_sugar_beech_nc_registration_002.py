"""PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Banner Elk - Sugar - Beech's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/banner-elk-sugar-beech-nc/launch_package/hotel_policy_facts_banner-elk-sugar-beech-nc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/banner-elk-sugar-beech-nc/banner_elk_sugar_beech_nc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 runs while another founder-authorized
markets in the release queue must go live first. This order therefore writes NOTHING at
...
against the then-current live parent.

WHY A LATER ORDER MAY REGISTER WITHOUT A ROW-BY-ROW FOUNDER SIGNATURE
------------------------------------------------------------------
Registration is not publication. Writing `markets/banner-elk-sugar-beech-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "banner_elk_sugar_beech_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census_proposed", "banner-elk-sugar-beech-nc.json")
STAGING = os.path.join(PKG, "markets", "staging", "banner-elk-sugar-beech-nc")
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_banner-elk-sugar-beech-nc.json")
AUTHORITY_OUT = os.path.join(STAGING, "banner_elk_sugar_beech_nc_proposed_authority_002.json")
OBSERVED_AT = "2026-09-14"
CAPTURED_AT = "2026-09-14T15:00:00+00:00"
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages this market without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages this market without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Banner Elk \u2013 Sugar Mountain \u2013 Beech Mountain, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Banner Elk - Sugar - Beech's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Banner Elk - Sugar - Beech's authority as this order proposes it, STAGED and not registered. Registration makes the market "
