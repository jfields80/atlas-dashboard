...
# PTF-WORKERS-005 browser-rendered outcomes. All four are things we detected
# and stopped on; none is a state this system tries to get past.
FETCH_STATUS_CHALLENGE_DETECTED = "CHALLENGE_DETECTED"
FETCH_STATUS_LOGIN_REQUIRED = "LOGIN_REQUIRED"
FETCH_STATUS_CONSENT_GATED = "CONSENT_GATED"
FETCH_STATUS_RENDER_NONDETERMINISTIC = "RENDER_NONDETERMINISTIC"
...
FETCH_STATUS_VALUES = frozenset({
    ...
    FETCH_STATUS_CHALLENGE_DETECTED, FETCH_STATUS_LOGIN_REQUIRED,
    FETCH_STATUS_CONSENT_GATED, FETCH_STATUS_RENDER_NONDETERMINISTIC,
})
...
def classify_fetch_status(
    ...
    # PTF-WORKERS-005 render outcomes are checked first: they are more specific
    # than the transport reasons below and must not be flattened into
    # UNKNOWN_FETCH_FAILURE.
    if reason == C.REASON_CHALLENGE_DETECTED:
        return FETCH_STATUS_CHALLENGE_DETECTED
    if reason == C.REASON_LOGIN_REQUIRED:
        return FETCH_STATUS_LOGIN_REQUIRED
    if reason == C.REASON_CONSENT_GATED:
        return FETCH_STATUS_CONSENT_GATED
    if reason == C.REASON_RENDER_NONDETERMINISTIC:
        return FETCH_STATUS_RENDER_NONDETERMINISTIC
