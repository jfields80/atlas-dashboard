"""PTF-RICHMOND-VA-PARALLEL-SOURCE-READY-001 -- Phase 10/11: the Wyndham first-party lane (plain client).
...
``data/acquisition/richmond_va_wyndham_001/``. The pet quote is read from the
property-service document, so ``h``/``b`` on each row are that document's.

Routes selected: every en-us Virginia property route the brand's own sitemap
lists whose city segment is a Richmond-area or observed-neighbour place. A
route admits nothing -- the postal code the property service states decides.

Output:
  data/acquisition/richmond_va_attended/wyndham_rows.json   (the capture helper's input shape)
  launch_packages/pettripfinder/markets/reports/richmond_va_wyndham_lane_001.json
"""
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "richmond-va", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "richmond_va_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "richmond_va_wyndham_001")
...
_ROUTE = re.compile(r"^https://www\.wyndhamhotels\.com/(?![a-z]{2}-[a-z]{2}/)[a-z0-9-]+/([a-z-]+)-virginia/[a-z0-9-]+/overview$")
