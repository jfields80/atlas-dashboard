"""PTF-CHARLOTTE-NC-ZERO-TO-LIVE-BENCHMARK-001 -- the final partition.

Every registered Charlotte identity, in EXACTLY ONE state, with exactly one next
...
Output: launch_packages/pettripfinder/charlotte_nc_final_partition_007.json
"""
...
OUT = os.path.join(PKG, "charlotte_nc_final_partition_007.json")
...
def build():
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-CHARLOTTE-NC-ZERO-TO-LIVE-BENCHMARK-001 "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-10"),
        ("what_this_is",
         "Every registered Charlotte identity in exactly one disposition state. A terminal state "
