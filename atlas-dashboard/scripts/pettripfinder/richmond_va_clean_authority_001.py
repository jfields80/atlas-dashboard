"""PTF-RICHMOND-VA-PARALLEL-SOURCE-READY-001 -- Phases 13 and 14: the clean set.
...
Output:
  launch_packages/pettripfinder/markets/reports/richmond_va_clean_authority_001.json
"""
...
ATTENDED = os.path.join(REPORTS, "richmond_va_attended_capture_001.json")
STATIC = os.path.join(REPORTS, "richmond_va_free_static_lane_001.json")  # not produced in this market
...
def build():
    ...
    from scripts.pettripfinder.richmond_va_census_reconciliation_001 import directional_conflict
    # PTF-RICHMOND-VA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: a record is released ONLY when a ruling that validates
    ...
    for r in co_located:
        ...
        # RICHMOND: the same test against a VERIFIED_NO_PETS peer (Quality Inn & Suites Ashland, 107 N. Carter Road, beside
        # Holiday Inn Express & Suites Richmond North Ashland, 107 South Carter Road).
        np_peers = [p for p in nopets if address_key(p["identity_signals"].get("address_on_page") or "",
                    (p["identity_signals"].get("postal_code") or "")[:5]) == k]
        if not peers and np_peers and all(directional_conflict(mine, p["identity_signals"].get("address_on_page") or "")
                                          for p in np_peers):
            peers = np_peers
        if peers and all(directional_conflict(mine, p["identity_signals"].get("address_on_page") or "") for p in peers):
            # SAVANNAH: two DIFFERENT buildings the shared address_key cannot tell apart ("201 E. Bay St." and
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "richmond_va_clean_authority_001.json"))
