"""PTF-BOONE-BLOWING-ROCK-NC-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.

Cloned from the Outer Banks NC census helper (itself cloned from Atlanta GA). Independent discovery lanes, none
authoritative on its own:
  ...
  DESTINATION_ORGANIZATION  every "Hotels & Cabins" listing of Explore Boone (the
                      Boone TDA), with the bureau's own sub-category (Hotels & Motels,
                      Resorts & Inns, Bed & Breakfasts, Cabins Cottages & Condos,
                      Cabin Rental Company, RV & Campgrounds, ...).
...
-----------------------
The market contract's postal partition (boone_blowing_rock_nc_geography_001). Banner
Elk / Sugar Mountain / Seven Devils / Beech Mountain (28604) and the rest of Avery
County are OUTSIDE and every such row carries the future-submarket reason so the
later banner-elk-sugar-beech-nc order starts from it.
...
refused BY NAME with the reason (boone_blowing_rock_nc_nonhotel_rulings_001).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/boone-blowing-rock-nc.json
  launch_packages/pettripfinder/markets/reports/boone_blowing_rock_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/boone_blowing_rock_nc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import boone_blowing_rock_nc_geography_001 as GEO  # noqa: E402
...
#: REGISTERED by PTF-BOONE-BLOWING-ROCK-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002; the source-ready census in identity_census_proposed/ is history.
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "boone-blowing-rock-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "boone-blowing-rock-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "boone_blowing_rock_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "boone_blowing_rock_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "boone_blowing_rock_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "boone_blowing_rock_nc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "boone_blowing_rock_nc_destination_roster_001.json")
...
from scripts.pettripfinder.boone_blowing_rock_nc_nonhotel_rulings_001 import NOT_LODGING_WHY, LODGING_UNCONFIRMED  # noqa: E402
...
#: BOONE: Explore Boone files a listing under several sub-categories, and its PRIMARY one
...
from scripts.pettripfinder.boone_blowing_rock_nc_static_rulings_001 import (  # noqa: E402
...
MAP_ROWS_SUPERSEDED_BY_A_READ = {
    # The map draws TownePlace Suites Boone at 2240 Blowing Rock Road with the brand's own (legacy) route as its
    # website; the brand's own page for that route states 1110 Meadowview Drive, the street behind the strip.
    "way/949955548": ("https://www.marriott.com/en-us/hotels/hkyts-towneplace-suites-boone/overview/",
                      "TownePlace Suites Boone at 2240 Blowing Rock Road (map)"),
}
...
#: The Avery County submarket places, preserved for the future submarket.
_FUTURE_SUBMARKET_PLACES = {
    "banner elk", "sugar mountain", "seven devils", "beech mountain", "linville", "newland", "elk park",
    "crossnore", "pineola", "montezuma", "minneapolis", "plumtree",
}
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {
    ...
    "fleetwood", "glendale springs", "laurel springs", "collettsville", "patterson",
}
...
CORRIDOR_CITIES = {
    # Foscoe and the Seven Devils side of NC-105 carry Boone 28607 mail; the ZIP decides.
    "boone": {"boone", "foscoe", "seven devils"},
    "blowing-rock": {"blowing rock"},
    "valle-crucis-vilas": {"valle crucis", "vilas"},
    "deep-gap": {"deep gap"},
    "watauga-fringe": {"sugar grove", "zionville", "todd"},
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and (_subcats & NON_HOTEL_BUREAU_SUBCATEGORIES) and not any(
        ...
        return NON_LODGING, ("Explore Boone (the Boone TDA) files this listing under %s -- a cabin, condo, "
                             "rental-company or campground category, not a hotel establishment; refused under "
                             "the order's vacation-rental / cabin filter" % sorted(_subcats))
    ...
    # BANNER ELK / SUGAR / BEECH BY PIN. A row with NO postal code of its own that every source
    # pins west of the Seven Devils ridge (longitude < -81.80, the Avery side of NC-105) or north
    # of 36.36 (Ashe County) cannot be an admitted identity whatever else is unknown about it.
    # Refusal only: a pin never ADMITS a building.
    _pins = [(o.get("lat"), o.get("lng")) for o in node.observations
             if o.get("lat") is not None and o.get("lng") is not None]
    if not node.postal and _pins and all(float(g) < -81.80 for _t, g in _pins):
        return OUTSIDE_MARKET, ("FUTURE_SUBMARKET %s -- no postal code of its own, and every source pins it "
                                "on the Avery County side (Banner Elk / Sugar / Beech, lng %.4f)"
                                % (GEO.FUTURE_SUBMARKET, float(_pins[0][1])))
    if not node.postal and _pins and all(float(t) > 36.36 for t, _g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it in Ashe County "
                                "(West Jefferson / Jefferson, lat %.4f)" % float(_pins[0][0]))
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Boone - Blowing Rock "
...
SOURCE_AUTHORITIES = [
    ...
    "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (18 High Country city pages, with property cards)",
    ...
    "https://www.exploreboone.com listing service, every 'Hotels & Cabins' sub-category (Boone TDA)",
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-BOONE-BLOWING-ROCK-NC-PARALLEL-SOURCE-READY-001 Boone - Blowing Rock census, built from zero "
         "under the modern factory and registered to identity_census/ by PTF-BOONE-BLOWING-ROCK-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Every row carries "
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Boone - Blowing Rock authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "boone-blowing-rock-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "boone_blowing_rock_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "boone_blowing_rock_nc_competitor_gap_matrix_001.json"))
