"""PTF-TOLEDO-OH-NEW-MARKET-001 -- Phase 11, the Firecrawl rung.

The Cincinnati phase-11 harness re-pointed at ``toledo-oh``. The cohort is
computed by ``ladder.plan_cohort`` from this order's committed static report and
is never typed in: 19 rows across IHG, Choice and Wyndham, the three families
the committed route table sends to Firecrawl on a measured decision. Marriott
and Hilton are a measured capability wall and are not candidates at any budget.
...
exactly this shape, and this cohort is smaller than the ones it was set on:
Indianapolis FIRECRAWL-RECOVERY-021 attempted 36 rows for 17 credits, Cincinnati
HARDENED-REVALIDATION-001 attempted its cohort under a 7-credit cap. Nineteen
attempts against a 488-credit plan balance is under four percent of it, and no
USD is involved at any point.
...
STATIC_REPORT = os.path.join(REPORTS, "toledo_oh_free_static_capture_001.json")
SPACING_SECONDS = 2.0
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="toledo_oh_firecrawl_001")
    ap.add_argument("--cap-attempts", type=int, default=19)
    ap.add_argument("--floor-credits", type=int, default=400)
    ap.add_argument("--out", default=os.path.join(REPORTS, "toledo_oh_firecrawl_pass_001.json"))
