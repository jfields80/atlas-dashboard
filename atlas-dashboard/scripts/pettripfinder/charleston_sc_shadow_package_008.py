"""PTF-CHARLESTON-SC-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Charleston is not at the front of the serialized release queue: the current verified live release is
Atlanta (23 markets / 1500 profiles / 1744 routes), and other source-ready markets are queued ahead of it.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/charleston-sc/launch_package/``) from the proposed
  ...
  3. runs the FAST lane (rules A-O) over it, which builds the joining market's
     ...
     current live parent (Atlanta) -- no whole-site candidate is assembled;
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/charleston-sc/``: never ``markets/packages/`` or
...
WHAT BINDS TO THE ATLANTA PARENT, AND HOW IT IS RECONCILED
------------------------------------------------------------
...
REPORT = PKG / "markets" / "reports" / "charleston_sc_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "charleston_sc_proposed_authority_002.json"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the CURRENT live release (Atlanta, deploy 6aa7f125...). That is what this "
         "package was validated against. Once any later market is live, FAST rule N fails this package BY DESIGN; the "
         "registration order -- when Charleston reaches the front of the release queue -- re-seals the same committed inputs against that parent as a NEW "
