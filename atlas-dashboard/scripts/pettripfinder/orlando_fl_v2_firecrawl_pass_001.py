"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 13, the Firecrawl rung (cloned from the Nashville TN harness).

THREE COHORTS, ALL COMPUTED, NONE TYPED
---------------------------------------
  ROUTED        ``ladder.plan_cohort`` over this order's committed static report: every row whose static attempt failed
                on the CHANNEL and whose family the committed route table sends to Firecrawl (IHG, Choice, Wyndham).
  DISCOVERED    Choice property routes read from the brand's own city pages through Firecrawl
                (orlando_fl_v2_firecrawl_discovery_001), in-market towns only, that no static read answered. A property
                CODE selects; the page's own address admits -- these are identity-fill reads.
  PROBE         families the route table has NOT measured (independents, Loews / Universal, Red Roof, Motel 6, Gaylord,
                Rosen): the ladder marks them FIRECRAWL_UNMEASURED_FOR_FAMILY / probe-eligible. At most PROBE_PER_FAMILY
                rows each, chosen deterministically (sorted identity key).
  WALL_REPROBE  Marriott and Hilton are a measured capability wall (PTF-FIRECRAWL-HARD-LANES-003). ONE row each is
                re-probed as a diagnostic of whether the wall still stands; the result is recorded and is never used
                as policy evidence, because the router does not route those families here.

THE CAP IS ON ATTEMPTS; the credit floor is a second stop; the credit delta is the meter (one credit on success, zero
when every engine is refused). Existing plan credits only -- no new purchase, no USD.

Identity is never positional: ``ladder.bind_results`` binds each result to the request naming its identity key AND URL.
Nothing is written to authority, and neither shared ledger is written.

Output:
  launch_packages/pettripfinder/markets/reports/orlando_fl_v2_firecrawl_pass_001.json
"""
...
STATIC_REPORT = os.path.join(REPORTS, "orlando_fl_v2_free_static_capture_001.json")
DISCOVERY = os.path.join(REPORTS, "orlando_fl_v2_firecrawl_discovery_001.json")
PROBE_PER_FAMILY = 4
PROBE_FAMILIES = ("INDEPENDENT", "LOEWS", "UNIVERSAL", "RED_ROOF", "MOTEL6", "GAYLORD", "ROSEN")
WALL_REPROBE = ("MARRIOTT", "HILTON")
_IN_MARKET_CHOICE_TOWNS = ("orlando", "kissimmee", "davenport", "clermont", "apopka", "altamonte-springs",
                           "lake-buena-vista", "sanford", "ocoee", "celebration", "winter-garden", "st-cloud",
                           "lake-mary", "longwood", "casselberry", "winter-park", "maitland", "champions-gate",
                           "championsgate", "windermere")
SPACING_SECONDS = 2.0
...
def cohort(static_report):
    ...
    for d in sorted(decisions, key=lambda x: x.identity_key):
        ...
        if fam in PROBE_FAMILIES and d.firecrawl.reason == L.NOT_CANDIDATE_UNMEASURED:
            if per.get(fam, 0) >= PROBE_PER_FAMILY:
                continue
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="orlando_fl_v2_firecrawl_001")
    ap.add_argument("--cap-attempts", type=int, default=90)
    ap.add_argument("--floor-credits", type=int, default=400)
    ap.add_argument("--out", default=os.path.join(REPORTS, "orlando_fl_v2_firecrawl_pass_001.json"))
