"""PTF-WILMINGTON-NC-NORMAL-PRODUCTION-001 -- Phase 8: register Wilmington.
...
from, and validates both against the contracts that own them:
  ...
  1. the committed POLICY PACKAGE  launch_packages/pettripfinder/hotel_policy_facts_wilmington-nc.json
  ...
  2. the PROPOSED AUTHORITY        launch_packages/pettripfinder/wilmington_nc_proposed_authority_002.json
...
------------------------------------------------------------------
Registration is not publication. Writing `markets/wilmington-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "wilmington_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census", "wilmington-nc.json")
PACKAGE_OUT = os.path.join(PKG, "hotel_policy_facts_wilmington-nc.json")
AUTHORITY_OUT = os.path.join(PKG, "wilmington_nc_proposed_authority_002.json")
OBSERVED_AT = "2026-09-12"
CAPTURED_AT = "2026-09-12T22:45:00+00:00"
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order registers Wilmington without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order registers Wilmington without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Wilmington, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Wilmington's committed policy package. Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Wilmington's authority as this order proposes it. Registration makes the market "
