"""PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 -- Phase 10/11: the Wyndham first-party lane (plain client).
...
``data/acquisition/hickory_nc_wyndham_001/``. The pet quote is read from the
...
lists whose city segment is a Hickory-area or observed-neighbour place. A
route admits nothing -- the postal code the property service states decides.

Output:
  data/acquisition/hickory_nc_attended/wyndham_rows.json   (the capture helper's input shape)
  launch_packages/pettripfinder/markets/reports/hickory_nc_wyndham_lane_001.json
"""
...
from scripts.pettripfinder import hickory_nc_brand_inventory_001 as B  # noqa: E402
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "hickory-nc", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "hickory_nc_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "hickory_nc_wyndham_001")
...
PLACES = set(B.LEAD_TOKENS) | {"conover-hickory-area"}  # hyphenated city segments as the brand spells them
