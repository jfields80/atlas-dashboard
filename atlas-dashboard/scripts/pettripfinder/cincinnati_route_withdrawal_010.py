# -*- coding: utf-8 -*-
"""PTF-CINCINNATI-FREE-LANE-APPLICATION-010 Phase 7 -- withdraw answered routes.

    python -m scripts.pettripfinder.cincinnati_route_withdrawal_010 --write
...
It does not withdraw the held row. Studio 6 Extended Stay Fairfield was ruled
HOLD_FOR_IDENTITY_ADDRESS_CLARIFICATION and the founder said explicitly not
to withdraw its route: a withdrawn route is how a row stops being worked, and this one still
...
from scripts.pettripfinder.cincinnati_free_lane_application_010 import (  # noqa: E402
    HELD, MARKET_ID, PACKAGE, REPORTS, WORK_ORDER, _load, build)

AS_OF = "2026-08-31"
LEDGER = REPORTS / "cincinnati_free_lane_route_withdrawals_010.json"
...
def answered_identities():
    ...
    Derived from the committed artifacts rather than by re-running the
    application: once the package is written, build() refuses -- correctly --
    because its rows are now published. The set is the clean block plus every
    ruling that publishes; a held row contributes nothing and keeps its route.
    """
    from scripts.pettripfinder.cincinnati_free_lane_application_010 import (
        PENDING, RULINGS)
    pending = _load(PENDING)
    answered = {e["identity_key"]
                for name in ("CLEAN_PET_FRIENDLY", "CLEAN_VERIFIED_NO_PETS")
                for e in pending["buckets"][name]}
...
def run(write: bool) -> int:
    ...
    ledger = OrderedDict((
        ...
        ("why", "Every identity listed here entered Cincinnati authority in "
                "PTF-CINCINNATI-FREE-LANE-APPLICATION-010 -- as a published "
        ...
        ("not_withdrawn", OrderedDict((
            ("identity_key", HELD),
            ("why", "Ruled HOLD_FOR_IDENTITY_ADDRESS_CLARIFICATION. The founder ordered its "
