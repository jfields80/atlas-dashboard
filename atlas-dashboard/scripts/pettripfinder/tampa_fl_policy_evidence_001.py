"""PTF-TAMPA-FL-PARALLEL-SOURCE-READY-001 -- durable policy evidence for the tampa-fl shadow build.
...
Modeled on orlando_fl_policy_evidence_001.py (worker/ptf-orlando-fl-market-001).
...
def load_pages() -> Dict[str, dict]:
    pages = {}
    path = RAW / "evidence_pages_001.jsonl.gz"
    if not path.exists():
        return pages
...
def hilton_records(pages: Dict[str, dict]) -> List[dict]:
    inv_path = RAW / "hilton_inventory_001.json"
    if not inv_path.exists():
        return []
    inv = json.loads(inv_path.read_text(encoding="utf-8"))["hotels"]
    by_url = {}
    for h in inv:
        if h.get("url"):
    ...
    for url, rec in sorted(pages.items()):
        ...
        if m:
            ...
            # No <ul> card (e.g. a plain-text refusal like "Pets not allowed"
            # instead of a bulleted fee/weight list); fall back to the whole
            # container's text so the shared reader still sees the refusal.
            m2 = re.search(r'<h3 class="heading--base heading--sm">Pets</h3>(.*?)</div>\s*</div>', body, re.S)
            quote = html.unescape(" ".join(re.sub(r"<[^>]+>", "", m2.group(1)).split())) if m2 else ""
...
def ruling_records(pages: Dict[str, dict]) -> List[dict]:
    path = STAGING / "evidence_rulings_001.json"
    if not path.exists():
        return []
    rulings = json.loads(path.read_text(encoding="utf-8"))["rulings"]
    out = []
    for r in rulings:
        rec = pages[r["url"]]
        ...
        out.append(dict(anchor=r["anchor"], source_class=r.get("source_class", "PT1_FIRST_PARTY (property's own website)"),
