"""PTF-BANNER-ELK-SUGAR-BEECH-NC-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Banner Elk - Sugar - Beech is built while the release queue (Atlanta, Outer Banks,
Boone's neighbours and others) holds the live parent. The generic lane
``registration_release_lane register`` / ``seal --work-order`` would reissue
``launch_participation.json``, extend ``bundle_cache_closure.json``, write a pin
block into ``tests/pettripfinder/pins/market_state.json`` and require a registry
row, a shard, regenerated globals and a release contract -- every one of them
shared, parent-sensitive state this order is forbidden to touch.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/banner-elk-sugar-beech-nc/launch_package/``) from the proposed
  ...
  3. runs the FAST lane (rules A-O) over it, which builds the joining market's
     ...
     current live parent -- no whole-site candidate is assembled;
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/banner-elk-sugar-beech-nc/``: never ``markets/packages/`` or
...
WHAT BINDS TO THE LIVE PARENT, AND HOW IT IS RECONCILED
-------------------------------------------------------
``parent_live_state`` and ``intended_delta`` are body fields, so the digest
names the live release this tree verifies (the Jacksonville-live parent at the
branch base). A sealed package is never edited -- "a correction is a new package
with a new id" -- and FAST rule N refuses any package whose parent is not the
CURRENT live state. Once the parent moves (Atlanta is already live on its own
branch), rule N FAILS this package on purpose; the registration order re-seals
the same committed inputs against the then-current parent as a new id.
...
REPORT = PKG / "markets" / "reports" / "banner_elk_sugar_beech_nc_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "banner_elk_sugar_beech_nc_proposed_authority_002.json"
SEALED_AT = "2026-09-14T15:00:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the live release this tree verifies (%s). That is what this package was "
         "validated against. Once the live parent moves, FAST rule N fails this package BY DESIGN; the "
         "registration order re-seals the same committed inputs against the then-current parent as a NEW "
         "package id. Nothing here is a production candidate, and no whole-site build was assembled."
         % json.dumps(package["parent_live_state"], sort_keys=True)[:300]),
