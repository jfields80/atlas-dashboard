"""PTF-SAVANNAH-GA-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Savannah is not at the front of the serialized release queue: the current verified live release is
Jacksonville (22 markets), and other source-ready markets are queued ahead of it.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/savannah-ga/launch_package/``) from the proposed
  ...
  3. runs the FAST lane (rules A-O) over it, which builds the joining market's
     ...
     current live parent (Jacksonville) -- no whole-site candidate is assembled;
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/savannah-ga/``: never ``markets/packages/`` or
...
WHAT BINDS TO THE JACKSONVILLE PARENT, AND HOW IT IS RECONCILED
------------------------------------------------------------
...
REPORT = PKG / "markets" / "reports" / "savannah_ga_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "savannah_ga_proposed_authority_002.json"
SEALED_AT = "2026-09-14T06:30:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the CURRENT live release (Jacksonville, deploy 6aa77cb8...). That is what this "
         "package was validated against. Once any later market is live, FAST rule N fails this package BY DESIGN; the "
         "registration order -- when Savannah reaches the front of the release queue -- re-seals the same committed inputs against that parent as a NEW "
