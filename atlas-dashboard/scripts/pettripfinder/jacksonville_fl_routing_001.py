"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phase 11A: one first-party route per census identity.
...
census ADMITTED, the first-party route it will be read at and which source stated it, in this order:
  ...
  2. the website one of the THREE destination bureaus (Visit Jacksonville, the Amelia Island CVB,
     Florida's Historic Coast) links for the listing joined to this building -- all three publish the
     partner's OWN outbound website, which is a first-party route and not a directory page;
...
Nothing here fetches. Output:
  launch_packages/pettripfinder/markets/reports/jacksonville_fl_routing_001.json
"""
...
OUT = os.path.join(REPORTS, "jacksonville_fl_routing_001.json")

_NAME_FAMILY = [
    ("MARRIOTT", r"marriott|courtyard by marriott|courtyard marriott|courtyard jacksonville|courtyard amelia"
...
_NOT_A_PROPERTY_ROUTE = re.compile(
    ...
    r"miamiandbeaches\.com|visitlauderdale\.com|thepalmbeaches\.com|visitjacksonville\.com|ameliaisland\.com|floridashistoriccoast\.com|bringfido|airbnb|vrbo|priceline|agoda|trivago|orbitz)", re.I)
