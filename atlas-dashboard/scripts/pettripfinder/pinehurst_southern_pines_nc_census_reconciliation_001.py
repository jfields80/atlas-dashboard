"""PTF-PINEHURST-SOUTHERN-PINES-NC-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.
...
authoritative on its own:
  ...
  DESTINATION_ORGANIZATION  every lodging listing of the Pinehurst, Southern Pines,
                      Aberdeen Area CVB (homeofgolf.com), with every category the
                      bureau gives it (Hotels / Inns, Resorts, B&Bs, Condos / Villas,
                      RV/Campground, Unique Venue, ...).
...
-----------------------
The market contract's postal partition (pinehurst_southern_pines_nc_geography_001). Robbins,
Sanford, Fayetteville, Raeford, Rockingham and Laurinburg are OUTSIDE by name.

GOLF VILLAS, CONDOS AND RENTALS ARE NOT HOTELS
---------------------------------------------
Sandhills golf lodging includes resort villas and condos, golf-package operators'
cottages, rental-management portfolios and whole-property rentals. None is admitted:
a row whose only evidence types it as condos / villas, a campground or a venue is
NON_LODGING, and named golf-package operators and rentals are refused BY NAME with
the reason (pinehurst_southern_pines_nc_nonhotel_rulings_001). A resort complex is
decided by premises: the Carolina Hotel, the Holly Inn, the Manor and the Magnolia
Inn are separate hotel buildings; the resort's villas and condos are not hotels.
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/pinehurst-southern-pines-nc.json
  launch_packages/pettripfinder/markets/reports/pinehurst_southern_pines_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/pinehurst_southern_pines_nc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import pinehurst_southern_pines_nc_geography_001 as GEO  # noqa: E402
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "pinehurst-southern-pines-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "pinehurst-southern-pines-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "pinehurst_southern_pines_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "pinehurst_southern_pines_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "pinehurst_southern_pines_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "pinehurst_southern_pines_nc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "pinehurst_southern_pines_nc_destination_roster_001.json")
...
from scripts.pettripfinder.pinehurst_southern_pines_nc_nonhotel_rulings_001 import NOT_LODGING_WHY, LODGING_UNCONFIRMED  # noqa: E402
...
NON_HOTEL_BUREAU_SUBCATEGORIES = {
    # the bureau's own spellings, as its partner index prints them (homeofgolf.com)
...
HOTEL_BUREAU_SUBCATEGORIES = {"Hotels / Inns", "Resorts", "B&Bs"}
...
from scripts.pettripfinder.pinehurst_southern_pines_nc_static_rulings_001 import (  # noqa: E402
...
MAP_ROWS_SUPERSEDED_BY_A_READ = {
    # The map draws SpringHill Suites Pinehurst Southern Pines at "10024 US Highway 15-501, Southern Pines 28387" with
    # the brand's short link as its website; the brand's own page for that property (MARSHA faysh) states
    # "10024 US Highway 15/501, Pinehurst 28374". One building; the property's own address wins.
    "way/384211276": ("https://www.marriott.com/en-us/hotels/faysh-springhill-suites-pinehurst-southern-pines/overview/",
                      "SpringHill Suites Pinehurst Southern Pines at 10024 US Highway 15-501, Southern Pines 28387 (map)"),
}
...
DESTINATION_ROWS_SUPERSEDED_BY_A_READ = {
    "https://homeofgolf.com/directory/springhill-suites-by-marriott/":
        ("https://www.marriott.com/en-us/hotels/faysh-springhill-suites-pinehurst-southern-pines/overview/",
         "SpringHill Suites by Marriott, 10024 US Hwy 15-501, website marriott.com/faysh (bureau)"),
}
...
NAME_RULINGS = {
    "80|carolina vista|": ("The Carolina Hotel",
                          "the bureau's 'Pinehurst Resort' umbrella listing and its 'The Carolina Hotel' listing state "
                          "the same street (80 Carolina Vista Drive); the premises is the Carolina Hotel building, the "
                          "resort is its operator, and the Holly Inn, the Manor and the Magnolia Inn are separate "
                          "buildings with their own streets"),
}
...
MAP_NAME_ONLY_BINDINGS = [
    ("way/1333367058", "80|carolina vista|",
     "the map's 'The Carolina Hotel, Pinehurst' footprint (35.19625, -79.47352) is the Carolina Hotel building the "
     "bureau addresses at 80 Carolina Vista Drive; the label's only chain-free word is the generic 'carolina', so "
     "the name pass cannot bind it"),
]
...
_GENERIC_TOKENS = {
    ...
    # PINEHURST: Sandhills golf-resort marketing vocabulary names no chain ("Pine Needles Lodge & Golf Club",
    # "Talamore Golf Resort", "Legacy Golf Packages").
    "golf", "club", "packages", "spa", "historic", "village", "near", "area",
}
...
_IN_MARKET_MUNICIPALITIES = {
    "pinehurst", "southern pines", "aberdeen", "pinebluff", "whispering pines", "carthage", "vass", "cameron",
    "west end", "seven lakes", "foxfire", "jackson springs", "taylortown", "lakeview",
}
#: No neighbouring cluster is preserved as a future submarket by this order.
_FUTURE_SUBMARKET_PLACES = set()
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {
    "robbins", "eagle springs", "sanford", "broadway", "fayetteville", "spring lake", "fort bragg", "fort liberty",
    "hope mills", "raeford", "rockingham", "hamlet", "hoffman", "ellerbe", "laurinburg", "lumberton", "candor",
    "biscoe", "troy", "star", "lillington", "dunn", "pittsboro", "asheboro", "holly springs", "cary", "raleigh",
}
...
CORRIDOR_CITIES = {
    "pinehurst": {"pinehurst", "taylortown"},
    "southern-pines": {"southern pines"},
    "aberdeen": {"aberdeen"},
    "pinebluff": {"pinebluff"},
    "whispering-pines-carthage": {"whispering pines", "carthage"},
    "vass-cameron": {"vass", "cameron", "lakeview"},
    "seven-lakes-west-end": {"west end", "seven lakes", "foxfire", "jackson springs"},
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and (_subcats & NON_HOTEL_BUREAU_SUBCATEGORIES) and not any(
        ...
        return NON_LODGING, ("the Pinehurst, Southern Pines, Aberdeen Area CVB files this listing under %s -- a "
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Pinehurst - Southern Pines "
...
SOURCE_AUTHORITIES = [
    ...
    "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (17 Sandhills city pages, with property cards)",
    ...
    "https://homeofgolf.com/lodging/ partner-listing index, every lodging category (Pinehurst, Southern Pines, "
    "Aberdeen Area CVB)",
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-PINEHURST-SOUTHERN-PINES-NC-PARALLEL-SOURCE-READY-001 Pinehurst - Southern Pines census, built from zero "
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Pinehurst - Southern Pines authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "pinehurst-southern-pines-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "pinehurst_southern_pines_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "pinehurst_southern_pines_nc_competitor_gap_matrix_001.json"))
