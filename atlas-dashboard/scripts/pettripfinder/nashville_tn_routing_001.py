"""PTF-NASHVILLE-TN-NEW-MARKET-001 -- Phase 9, free routing.

For every confirmed Nashville identity, the cheapest route that a FIRST-PARTY
source states. Nothing here guesses a URL from a name.

Route sources, in ladder order:

  1. ROUTED_OFFICIAL_DESTINATION -- the route the official Nashville
     destination organisation links from a partner page. Toledo's CVB partner
     roster was the single most valuable rung in that market. Nashville's
     destination organisation refused this client on every host and path tried,
     so this rung yields nothing here and the market leans on rungs 2 and 3
     instead. The reader is kept, and the refusal is recorded, because a
     refusal is a fact about a moment and the next order re-probes it.
  2. ROUTED_OFFICIAL_SITEMAP -- a Marriott route from the committed Dayton
     harvest (OWNED, zero requests), a Hilton route from Hilton's own Nashville
     city pages, or a Sonesta, Omni, Loews, Drury or WoodSpring route from the
     brand's own sitemap or city page.
...
-------------------------------
Nashville's BNA property-code prefix reaches Clarksville, Cookeville, Columbia,
Murfreesboro, Manchester, Tullahoma, Hopkinsville KY and Bowling Green KY. Rows
...
Nothing here fetches. Output is Nashville-local.
...
#: NASHVILLE: it is the brand's town index, it names the wrong town, and
...
def cvb_routes():
    ...
    Nashville's destination organisations refused this client on every host and
    path this order tried, so this returns empty here. The reader stays because
    the refusal is a fact about a moment: the next order re-probes it, and if
    the roster serves, this rung outranks the brand sitemap exactly as it did
    for Toledo.
    """
    doc = _load(LEADS, {}) or {}
    out = {}
    for _org, block in (doc.get("official_destination_sources") or {}).items():
...
def brand_sitemap_routes():
    ...
    """
    doc = _load(os.path.join(REPORTS, "nashville_tn_brand_sitemaps_001.json"), {}) or {}
    out = {}
    for fam, block in (doc.get("families") or {}).items():
        for r in block.get("nashville_routes") or []:
...
def route_from_map_source(row):
    ...
    163 of 308 OpenStreetMap candidates carry a ``website_url``, and for several
    families -- IHG and Choice above all -- it is the only free route this order
    ever found: both refuse this client on their city page AND on the sitemap
    their own robots.txt declares.
    ...
    ``choicehotels.com/tennessee/white-house/quality-inn-hotels`` for a hotel
    that is in Nashville -- a list, and the wrong town), and the shared
...
def build():
    ...
    fill = []
    recon = _load(os.path.join(REPORTS, "nashville_tn_census_reconciliation_001.json"), {}) or {}
    ...
    return OrderedDict([
        ...
        ("a_route_is_a_proposal",
         ...
         "Nashville adds its own reason: the BNA property-code prefix reaches Clarksville, "
         "Cookeville, Columbia, Murfreesboro and two Kentucky towns, so a route selected by code "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "nashville_tn_routing_001.json"))
