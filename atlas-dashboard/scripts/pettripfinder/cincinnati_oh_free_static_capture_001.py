"""PTF-CINCINNATI-HARDENED-REVALIDATION-001 -- Phases 10 / 11 / 13.

Free FIRST-PARTY static capture through the canonical lanes, market-scoped to
Cincinnati. This is the Dayton phase-9 harness
(``cleveland_akron_canton_oh_free_static_capture_009.py``) re-pointed at
``cincinnati-oh`` and given a target source Cincinnati actually owns: its committed
...
def targets_from_args(args, census, policy, exclusions):
    """Cincinnati's target source is its own committed final partition, plus -- for
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="cincinnati_oh_free_static_001")
    ap.add_argument("--out", default=os.path.join(REPORTS, "cincinnati_oh_free_static_capture_001.json"))
