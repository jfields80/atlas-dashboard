"""PTF-WEST-PALM-BEACH-FL-REGISTRATION-AND-STAGING-002 -- West Palm Beach's release contract.
...
WORK_ORDER = "PTF-WEST-PALM-BEACH-FL-REGISTRATION-AND-STAGING-002"
SOURCE_ORDER = "PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001"
...
def main(argv=None) -> int:
    ...
    clean = _load(os.path.join(REPORTS, "west_palm_beach_fl_clean_authority_001.json"))
    ...
    doc = OrderedDict([
        ...
        ("release_name_prefix", "prod-west-palm-beach"),
        ("description",
         "Deterministic release-gate contract for the PetTripFinder The Palm Beaches market through %s. "
        ...
        ("deployment_authorization", OrderedDict([
            ...
            ("means",
             ...
             "decision exists for West Palm Beach at %d published profiles."
        ...
        ("identity_census", OrderedDict([
            ...
            ("note",
             "%d registered identities, built from zero by %s on the Fort-Lauderdale-live release-factory "
             "lineage and closed in that same order through the supported attended-browser lane "
             "(61 paced attempts, 49 first-party brand reads, no paid provider and no Akamai "
             "bypass), registered by %s. Membership is this market's own corridor postal partition "
             "(census_membership_basis CORRIDOR_REGISTRY): 15 corridors over 60 Palm Beach County "
             "postal codes. BROWARD belongs to the LIVE fort-lauderdale-fl market and MIAMI-DADE to "
             "the LIVE miami-fl market -- both already publish that inventory, so neither may be "
             "admitted here -- and Martin / St. Lucie and the Florida Keys are named, refused and "
             "preserved for their own future markets. Palm Beach County's postal codes straddle "
             "municipalities, so a corridor covers a shared code WHOLE and reports its towns as "
             "overlays; the Port of Palm Beach is NOT a corridor because it sits inside 33404, "
             "which riviera-beach-singer-island already claims in full, and PBI is NOT a corridor "
             "because the airport owns no postal code. Postal code 33469 (Tequesta) crosses the "
             "Martin County line and is covered whole, which is the one border case this market "
             "admits and names. Individual vacation houses, condo units, rental-management "
             "portfolios, equestrian-season rentals, member-only club lodging and timeshare "
             "inventory are never admitted. Rows the identity graph did not admit are recorded, "
             "not dropped: %s."
        ...
        ("reconciliation", OrderedDict(
            ...
            + [("note",
                ...
                "west_palm_beach_fl_clean_authority_001.json with its class. No gate was lowered to "
                "preserve a count."
