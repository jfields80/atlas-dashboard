"""PTF-ATLANTA-GA-PARALLEL-SOURCE-READY-001 -- the final partition.

Every census Atlanta identity, in EXACTLY ONE state, with exactly one next
...
Output: launch_packages/pettripfinder/markets/staging/atlanta-ga/launch_package/atlanta_ga_final_partition_007.json
...
#: REGISTERED by PTF-ATLANTA-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: the package root.
OUT = os.path.join(PKG, "atlanta_ga_final_partition_007.json")
...
REFUSALS = {
    "CHOICE": "a plain-client sitemap timeout, then -- after 37 Atlanta property pages were read in the attended "
              "session -- 403s on every Choice city listing and 671-byte shells on further property pages (the "
              "brand's bot defence; never bypassed)",
    "ESA": "a plain-client 403 on the sitemap and property pages, and JavaScript execution not permitted on "
           "extendedstayamerica.com in the attended session, so no hashed document could be captured",
    "RADISSON": "a plain-client sitemap timeout and an empty client-rendered shell for the Country Inn Smyrna page "
                "in the attended session",
    "RED_ROOF": "a plain-client 403 on the sitemap; no Atlanta property route was found by any free lane",
    "FOUR_SEASONS": "a plain-client 403; the attended page shows only a 'Pet Friendly Stays' label",
}
#: Independent properties whose OWN site refused this client on this run.
SITE_REFUSALS = {
    "economy hotel marietta": "a plain-client 403",
    "homestead studio suites": "a plain-client 403 (extendedstayhotels.com)",
    "barclay hotel atlanta downtown": "its website domain does not resolve",
    "best western plus hotel": "its website domain (suitesatroswell.com) does not resolve",
    "masters inn": "a connection timeout",
    "premier extended stay": "an invalid TLS certificate",
    "the brookhaven atlanta hotel": "an invalid TLS certificate",
}
#: Buildings whose own page SERVED but states no operative, bindable policy, by census name.
POLICY_SILENT = {
    "hotel clermont": "the hotel's own home page served (789 Ponce De Leon Ave N.E. 30306) with no pet wording; its /policies page is a 404",
    "hotel phoenix": "the hotel's own home page served with no pet wording",
    "emory conference center hotel": "the hotel's own home page served with no pet wording",
    "nobu hotel atlanta": "the hotel's own home page served with no pet wording",
    "reverb by hard rock downtown atlanta": "the hotel's own page states only a 'PET FRIENDLY HOTEL' label -- never operative",
...
def build():
    ...
    routing = _load(os.path.join(REPORTS, "atlanta_ga_routing_001.json"), {})
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-ATLANTA-GA-PARALLEL-SOURCE-READY-001 "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-13"),
        ("what_this_is",
         "Every census Atlanta identity in exactly one disposition state. A terminal state "
