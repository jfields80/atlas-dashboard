"""PTF-PINEHURST-SOUTHERN-PINES-NC-PARALLEL-SOURCE-READY-001 -- complete accounting.
...
Output:
  launch_packages/pettripfinder/markets/reports/pinehurst_southern_pines_nc_source_ready_accounting_009.json
"""
...
OUT = os.path.join(REPORTS, "pinehurst_southern_pines_nc_source_ready_accounting_009.json")
...
def build():
    ...
    partition = _load(os.path.join(STAGING, "launch_package", "pinehurst_southern_pines_nc_final_partition_007.json"))
    geography = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_geography_001.json"))
    capture = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_attended_capture_001.json"))
    clean = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_clean_authority_001.json"))
    recon = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_census_reconciliation_001.json"))
    gaps = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_competitor_gap_matrix_001.json"))
    osm = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_osm_lane_001.json"))
    brand = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_brand_inventory_001.json"))
    wyn = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_wyndham_lane_001.json"))
    roster = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_destination_roster_001.json"))
    shadow = _load(os.path.join(REPORTS, "pinehurst_southern_pines_nc_shadow_package_008.json"))
