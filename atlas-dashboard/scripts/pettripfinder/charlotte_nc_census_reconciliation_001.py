"""PTF-CHARLOTTE-NC-ZERO-TO-LIVE-BENCHMARK-001 -- Phases 5, 6 and 8: one identity graph.

Independent discovery lanes ran for Charlotte, none of them authoritative on
its own:

  OSM_OVERPASS        hotel and motel nodes inside this market's thirty-four
                      discovery cells, answered from md5-verified Geofabrik
                      North Carolina and South Carolina extracts under the ODbL.
  BRAND_INVENTORY     Marriott's CLT* roster, OWNED at zero requests from the
                      committed Dayton harvest, plus Hilton's CLT* roster read
                      from twenty-one of the brand's OWN city pages, plus every
                      other family's city-page and sitemap routes.
  CVB_PARTNER         the official Charlotte destination organisation (tier 2).
  COMPETITOR_LEAD     BringFido's Greater Charlotte city pages (tier 4, leads
                      only, never policy).
...
-----------------------
The market contract's postal partition, and nothing else. The CORE is a county
polygon -- the whole of Mecklenburg County, North Carolina -- plus three
contiguous corridors admitted on first-party brand naming: Concord on I-85
northeast, Belmont/Gastonia on I-85 southwest, and Fort Mill/Carowinds/Indian
Land on I-77 south across the SOUTH CAROLINA state line. This market holds four
fringe areas -- Rock Hill SC, Kannapolis, Mooresville/north Lake Norman, and
Monroe/Indian Trail/Waxhaw -- as corridors that claim NO postal code, so their
hotels are discovered, identified and then classified OUTSIDE_MARKET with their
ZIP recorded. That is the point: a founder rules on an inventory that was looked
at, and the ruling is a one-line ZIP move.

Charlotte is the first PetTripFinder market that spans two states, so the state
rule admits BOTH North Carolina and South Carolina, and a row whose own page
states any other state is OUTSIDE_MARKET on that evidence alone.

Nothing here fetches. Nothing here writes outside Charlotte artifacts. Nothing
here carries a pet policy: identity evidence never establishes one.

Outputs:
  launch_packages/pettripfinder/identity_census/charlotte-nc.json
  launch_packages/pettripfinder/markets/reports/charlotte_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/charlotte_nc_competitor_gap_matrix_001.json
"""
...
#: Charlotte is built under the MODERN factory and has no legacy shadow, so its
#: census is written where a registered market's census lives. There is no
#: pinned Charlotte census to overwrite: this order creates the file.
CENSUS_DIR = os.path.join(PKG, "identity_census")
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "charlotte-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "charlotte-nc.json"))

CANDIDATES = os.path.join(_DASH, "data", "discovery", "charlotte_nc_free_001", "candidates",
                          "charlotte-nc_candidates.json")
STATIC_CAPTURE = os.path.join(REPORTS, "charlotte_nc_free_static_capture_001.json")
STATIC_LANE = os.path.join(REPORTS, "charlotte_nc_free_static_lane_001.json")
FIRECRAWL_PASS = os.path.join(REPORTS, "charlotte_nc_firecrawl_pass_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "charlotte_nc_attended_capture_001.json")
OWNED = os.path.join(REPORTS, "charlotte_nc_owned_evidence_001.json")
CITY_PAGES = os.path.join(REPORTS, "charlotte_nc_brand_city_pages_001.json")
SITEMAPS = os.path.join(REPORTS, "charlotte_nc_brand_sitemaps_001.json")
...
NOT_LODGING_NAMES = {
    "hotel management company", "charlotte convention center",
    "charlotte regional visitors authority", "spectrum center", "bojangles coliseum",
    "carowinds", "charlotte motor speedway", "the speedway club", "u s national whitewater center",
    "discovery place", "nascar hall of fame", "billy graham library",
}
...
STR_PLACEHOLDERS = {"charlotte airbnb rentals", "vrbo charlotte", "airbnb charlotte",
                    "vrbo rentals", "charlotte vacation rentals", "airbnb rentals",
                    "lake norman vacation rentals", "airbnb lake norman"}
...
class Node:
    ...
    def absorb(self, obs):
        ...
        # already-read Charlotte hotels unjoinable to their own reads the
        ...
        # The longest name wins: "Courtyard by Marriott Charlotte SouthPark"
...
def read_osm():
    ...
    for r in rows:
        ...
        out.append(observation(
            ...
            # NEVER hardcoded. Charlotte spans two states and this market's own
...
#: Charlotte hotels were classified OUTSIDE_MARKET by exactly that before this
...
#: "north" and "south" are NOT noise in this market -- South End, North End,
#: Charlotte Airport North and South Tryon are all real distinguishing area
...
def read_brand_city_pages():
    """Hilton's coded roster, plus every other family's own city-page routes.

    Toledo only had Hilton on this rung. Charlotte is a far larger market and
    several families answered a plain client on their own city page, so each
    one's routes are carried as tier-1 ROUTING and IDENTITY evidence. None of
    them is policy, and none of them admits a property to the market: the route
    is captured so the property's OWN page can state its address.
    """
    ...
    for fam, block in (doc.get("brand_city_pages") or {}).items():
        base = block.get("final_url") or block.get("url") or ""
        m = re.match(r"(https?://[^/]+)", base)
        origin = m.group(1) if m else ""
        for route in block.get("property_routes") or []:
            full = route if route.startswith("http") else origin + route
            name = _name_from_route(full)
            if not name:
                continue
            out.append(observation(
                "BRAND_INVENTORY_CITY_PAGE", 1, full, name,
                brand=fam, route=full, merge_alias=normalize_name(name),
                admitted_by="ROUTE_READ_FROM_THE_BRAND_OWN_CITY_PAGE",
                seen_in=block.get("url", ""),
            ))
    return out
...
def read_brand_sitemaps():
    ...
    Rung 2 of the brand inventory. Nashville's reconciliation never read this
    report and its fifty-five sitemap routes reached the census only through the
    routing lane; Charlotte reads it here so a Wyndham property that appears in
    no other lane still opens an identity of its own. Like every roster row it
    NAMES a property and routes to it, and admits nothing: the postal code the
    property's own page states decides membership.
    """
    doc = _load(SITEMAPS, {}) or {}
    out = []
    for fam, block in (doc.get("families") or {}).items():
        for r in block.get("charlotte_routes") or []:
            route = r.get("route") or ""
            name = _name_from_route(route)
            if not name:
                continue
            out.append(observation(
                "BRAND_INVENTORY_SITEMAP", 1, route, name,
                brand=fam, route=route, merge_alias=normalize_name(name),
...
def read_cvb():
    ...
    partner. Charlotte's destination organisations are probed the same way and
...
def read_property_pages():
    ...
    allowed to turn a brand-roster row into a Charlotte identity.
...
def merge(observations):
    ...
    for obs in observations:
        ...
        # Charlotte has at least six: Sheraton Charlotte and Le Meridien share
...
#: market's own name is here on purpose -- "charlotte" is in most of these names
...
_GENERIC_TOKENS = {
    ...
    "extended", "stay", "america", "select", "s", "charlotte",
    # PARENT COMPANIES. One source writes "Residence Inn by MARRIOTT Charlotte
    # Vanderbilt" and the next writes "Residence Inn Charlotte Vanderbilt";
    ...
    # what names the hotel. Charlotte adds the soft-brand collection words --
...
_IN_MARKET_MUNICIPALITIES = {
    # Mecklenburg County towns and the districts the corridor registry names.
    "pineville", "matthews", "mint hill", "huntersville", "cornelius", "davidson",
    "ballantyne", "southpark", "south park", "northlake", "steele creek",
    "university city", "uptown", "south end", "dilworth", "myers park",
    "plaza midwood", "noda", "arrowood", "piper glen", "carolina place",
    "lake norman", "birkdale",
    # The three admitted corridors outside the county line.
    "concord", "harrisburg", "belmont", "gastonia", "cramerton",
    "fort mill", "tega cay", "indian land", "carowinds", "baxter village",
}
...
#: Marriott's CLT prefix, which is the Charlotte Douglas AIRPORT code and
...
_OUT_OF_MARKET_PLACES = {
    # HELD fringe areas: authored as corridors that claim no postal code, so a
    # name that states one of them is not naming an admitted building.
    "rock hill", "winthrop", "kannapolis", "china grove", "mooresville",
    "monroe", "indian trail", "stallings", "wesley chapel", "waxhaw",
    # Excluded outright.
    "salisbury", "statesville", "hickory", "shelby", "lincolnton", "denver",
    "albemarle", "chester", "lancaster", "troutman", "mount holly",
    "dallas", "bessemer city", "kings mountain", "cherryville", "lowell",
    "wadesboro", "marshville", "wingate", "landis", "mooresboro",
    "fort lawn", "clover", "york", "lake wylie", "van wyck", "great falls",
    "winston salem", "greensboro", "hendersonville", "asheville", "columbia",
}
#: "Charlotte" is not a distinguishing word inside a hotel NAME -- most of this
...
_ALL_PLACES = _IN_MARKET_MUNICIPALITIES | _OUT_OF_MARKET_PLACES | {"charlotte"}
...
#: SpringHill Suites whose names differ ONLY in the area word -- Charlotte
...
def specific_municipality(places):
    ...
    named = {p for p in places if p != "charlotte"}
    return named or ({"charlotte"} if "charlotte" in places else set())
...
#: The states this market admits. Charlotte is the first PetTripFinder market
...
#: evidence alone.
ADMITTED_STATES = frozenset({"NC", "SC", "NORTH CAROLINA", "SOUTH CAROLINA"})
...
def corridor_index():
    ...
    have written North Carolina onto four South Carolina corridors.
    """
...
def explicit_index(cfg):
    ...
    corridor. Charlotte's registry needs none at authoring time: every admitted
...
def distinct_brand_signatures(node):
    ...
    somebody last edited them, and Charlotte's airport corridor has churned:
...
def classify(node, zips, name_counts, street_counts):
    ...
    if node.region and node.region.upper() not in ADMITTED_STATES:
        return OUTSIDE_MARKET, ("the row's own state is %s; this market admits North Carolina "
                                "and South Carolina only -- its Fort Mill, Tega Cay and Indian "
                                "Land corridors are in South Carolina and its Rock Hill hold is "
                                "there too" % node.region)
    if node.postal and node.postal[:5] not in zips:
        return OUTSIDE_MARKET, ("postal code %s is claimed by no corridor of the Charlotte "
    ...
    if sk and street_counts.get(sk, 0) > 1:
        ...
        # brand family. Charlotte has seven such pairs -- Sheraton Charlotte and
...
def build():
    ...
    # fully-addressed, identity-confirmed Charlotte hotels because a BringFido
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-CHARLOTTE-NC-ZERO-TO-LIVE-BENCHMARK-001 Charlotte census. Charlotte was built "
        ...
        ("source_authorities", [
            "OpenStreetMap via md5-verified Geofabrik North Carolina and South Carolina "
            ...
            "https://www.druryhotels.com/locations/charlotte-nc",
            "https://www.woodspring.com/extended-stay-hotels/locations/north-carolina/charlotte",
    ...
    report = OrderedDict([
        ...
        ("first_party_naming", OrderedDict([
            ("what_it_is",
             ...
             "Charlotte row named 'Hampton' collides across markets and the seed assembler "
        ...
        ("city_backfill", OrderedDict([
            ("what_it_is",
             ...
             "is named 'Charlotte' by its own operator."),
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Charlotte authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "charlotte-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "charlotte_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "charlotte_nc_competitor_gap_matrix_001.json"))
