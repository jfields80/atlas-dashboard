"""PTF-CHATTANOOGA-TN-NEW-MARKET-001 -- Phases 11 and 14, free static capture.

One HTTPS GET per route through the canonical gates, for two cohorts:

  ROUTED          every confirmed Chattanooga identity that carries a
                  first-party route. The question is the PET POLICY.
  IDENTITY_FILL   every brand-roster property whose existence and route the
...
-------------------------
It will not treat a route as confirmed because a source stated it. Every row
...
reader and the publication grade, both of which this module only reports. A page
that states no pet policy is SOURCE_SILENT -- never a refusal, never an
acceptance.

Output:
  launch_packages/pettripfinder/markets/reports/chattanooga_tn_free_static_capture_001.json
"""
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS,
                                                  "chattanooga_tn_free_static_capture_001.json"))
    ap.add_argument("--run-id", default="chattanooga_tn_free_static_001")
