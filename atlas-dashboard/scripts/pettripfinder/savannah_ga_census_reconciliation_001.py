"""PTF-SAVANNAH-GA-PARALLEL-SOURCE-READY-001 -- Phases 5-9: one identity graph.
...
Independent discovery lanes, none authoritative on its own:
  ...
  OSM_OVERPASS        tourism=hotel / motel / guest_house / apartment / chalet elements
                      inside the observation box, read from the local Geofabrik Georgia
                      extract (ODbL).
  BRAND_INVENTORY     the committed national Marriott harvest (SAV-coded routes);
                      Marriott's Georgia hotel sitemap page; Hilton's own Georgia and
                      two South Carolina city pages (property cards stating each hotel's
                      address); Wyndham's, Drury's and WoodSpring's own sitemaps.
  DESTINATION_ORGANIZATION  every "Places to Stay" profile of Visit Savannah (the
                      Savannah Area CVB), with the bureau's own categories (Hotels &
                      Motels, Bed & Breakfasts, Historic Inns, Vacation Rentals,
                      Extended Stay).
...
-----------------------
The market contract's postal partition (savannah_ga_geography_001). Tybee Island
(31328) is OUTSIDE and every such row carries the future-market reason so the
later tybee-island-ga order starts from it.

VACATION RENTALS, CONDOS, HISTORIC HOMES AND TIMESHARES ARE NOT HOTELS
--------------------------------------------------------------------
Savannah and Tybee carry large vacation-rental, historic-home, condo and
property-management inventories. None is admitted: a row whose only typing
...
(savannah_ga_nonhotel_rulings_001).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/savannah-ga.json
  launch_packages/pettripfinder/markets/reports/savannah_ga_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/savannah_ga_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import savannah_ga_geography_001 as GEO  # noqa: E402
...
#: REGISTERED by PTF-SAVANNAH-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002; the source-ready census in identity_census_proposed/ is history.
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "savannah-ga.json"),
                                os.path.join(PKG, "markets", "proposed", "savannah-ga.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "savannah_ga_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "savannah_ga_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "savannah_ga_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "savannah_ga_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "savannah_ga_destination_roster_001.json")
#: SAVANNAH: the City of Pooler's own visitor roster, a second destination lane (same tier, same rules).
POOLER_ROSTER = os.path.join(REPORTS, "savannah_ga_pooler_roster_001.json")
...
from scripts.pettripfinder.savannah_ga_nonhotel_rulings_001 import NOT_LODGING_WHY, LODGING_UNCONFIRMED  # noqa: E402
...
HOTEL_BUREAU_SUBCATEGORIES = {"Hotels & Motels", "Bed & Breakfasts", "Historic Inns"}
...
def read_brand_city_pages():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if state_code(card.get("state")) not in ("GA", "SC"):
            continue
        if (card.get("postal_code") or "")[:3] not in ("313", "314", "304", "299"):
            continue
...
def read_brand_state_sitemap():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        if not ((r.get("property_code") or "").lower().startswith("sav") or places_in(r.get("brand_title") or "")):
            continue
...
from scripts.pettripfinder.savannah_ga_static_rulings_001 import (  # noqa: E402
...
def read_destination_roster():
    ...
    """
    listings = []
    for path in (DESTINATION, POOLER_ROSTER):
...
_GENERIC_TOKENS = {
    ...
    "ga", "georgia", "bed", "breakfast", "b",
    # SAVANNAH: marketing vocabulary that names no chain.
...
_IN_MARKET_MUNICIPALITIES = {
    "savannah", "pooler", "garden city", "port wentworth", "richmond hill", "thunderbolt",
    "wilmington island", "whitemarsh island", "skidaway island", "georgetown",
}
#: The Tybee Island places, preserved for the future market.
_FUTURE_SUBMARKET_PLACES = {"tybee island", "tybee"}
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {
    "hardeeville", "bluffton", "hilton head", "hilton head island", "beaufort", "ridgeland", "okatie",
    "rincon", "springfield", "guyton", "bloomingdale", "hinesville", "midway", "fort stewart", "statesboro",
    "brunswick", "st simons island", "jekyll island", "ellabell", "pembroke", "darien",
}
...
_DIRECTIONALS = {
    ...
    # SAVANNAH area words that tell same-brand hotels apart (Courtyard Downtown / Midtown / Airport)
    "midtown", "airport", "historic", "district", "riverfront", "gateway", "southside", "chatham", "i95",
}
...
def _route_numbers(street):
    ...
    toks = re.sub(r"[^a-z0-9 ]", " ", (street or "").lower()).split()
    if len(toks) < 2 or not re.search(r"\b(ga|hwy|highway|us|sr|route)\b", " ".join(toks[1:])):
        return ()
...
ROSTER_ROWS_SUPERSEDED_BY_A_READ = {
    "cottonwood suites and conference center": (
        "301b governor treutlen",
        "Visit Pooler lists 'Cottonwood Suites & Conference Center' at 301 Governor Treutlen Dr.; Wyndham's own property "
        "service states 'Clarks Inn Savannah Airport' at 301B Governor Treutlen Dr, 31322 (the brand's current flag)"),
}
...
CORRIDOR_CITIES = {
    ...
    "midtown": {"savannah", "thunderbolt"},
    "southside": {"savannah"},
    "sav-airport": {"savannah", "garden city"},
    # Pooler hotels on Pooler Parkway are often addressed "Savannah, GA 31322"; the ZIP decides.
    "pooler": {"pooler", "savannah", "bloomingdale"},
    "gateway-i95": {"savannah", "georgetown"},
    "port-wentworth": {"port wentworth", "savannah", "garden city"},
    "richmond-hill": {"richmond hill"},
    "eastern-islands": {"savannah", "wilmington island", "whitemarsh island", "skidaway island", "thunderbolt"},
}
...
def classify(node, zips, name_counts, street_counts):
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and (_subcats & NON_HOTEL_BUREAU_SUBCATEGORIES) and not any(
        ...
        return NON_LODGING, ("Visit Savannah (the Savannah Area CVB) files this listing under %s -- a vacation-rental "
    ...
    if _subcats and not (_subcats & HOTEL_BUREAU_SUBCATEGORIES) and _CAMPGROUND_NAME.search(node.name or "") and not any(
            str(o.get("lane", "")).startswith(("PROPERTY_PAGE", "BRAND_INVENTORY")) for o in node.observations):
        return NON_LODGING, ("Visit Savannah files this listing only under %s and its own name is a campground, RV "
    ...
    if lanes == {"OSM_OVERPASS"} and re.match(r"\s*STVR\b", node.name or ""):
        return NON_LODGING, ("a map row the map itself names 'STVR' (a permitted Chatham County short-term vacation "
    ...
    if not node.postal and _pins and all(float(g) > -80.90 for _t, g in _pins):
        return OUTSIDE_MARKET, ("FUTURE_SUBMARKET %s -- no postal code of its own, and every source pins it "
                                "on Tybee Island (lng %.4f)" % (GEO.FUTURE_SUBMARKET, float(_pins[0][1])))
    if not node.postal and _pins and all(float(t) > 32.22 or float(t) < 31.87 or float(g) < -81.42
                                         for t, g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it beyond the admitted "
                                "corridors (Rincon / Hardeeville north, Midway south or Pembroke west; %.4f, %.4f)"
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Savannah "
...
_COVERED_FLAGS = (
    ...
    ("IHG", r"\b(holiday inn|candlewood|staybridge|avid|atwell|hotel indigo|kimpton)\b"),
...
def brand_not_listed(node):
    ...
    for fam, rx in _COVERED_FLAGS:
        if re.search(rx, node.name or "", re.I):
            return ("a %s row naming a %s flag (%r) that no read of that brand's own Savannah-area inventory "
...
SOURCE_AUTHORITIES = [
    "OpenStreetMap via the Geofabrik Georgia extract (2026-09-13 snapshot), reduced to this market's "
    ...
    "national Marriott harvest, SAV-coded routes)",
    "https://www.marriott.com/en-us/hotel-sitemap/usa-georgia-hotel-sitemap (the brand's own Georgia hotel "
    "sitemap page)",
    "https://www.hilton.com/en/locations/usa/<state>/<city>/ (Georgia and South Carolina city pages and Savannah "
    ...
    "https://visitsavannah.com every 'Places to Stay' category page and profile (Savannah Area CVB, attended "
    "browser)",
    "https://www.visitpooler.com/stay/ (City of Pooler visitor roster, plain client)",
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-SAVANNAH-GA-PARALLEL-SOURCE-READY-001 Savannah census, built from zero "
         "under the modern factory and registered to identity_census/ by PTF-SAVANNAH-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Every row carries "
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Savannah authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "savannah-ga.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "savannah_ga_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "savannah_ga_competitor_gap_matrix_001.json"))
