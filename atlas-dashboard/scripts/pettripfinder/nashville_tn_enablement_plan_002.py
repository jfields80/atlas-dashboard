"""PTF-NASHVILLE-TN-PROMOTION-AND-NEW-LANE-LAUNCH-PREP-002 -- the enablement plan.
...
make to open the lane for NASHVILLE ALONE, reads the current value of every
...
Applying it would make Nashville deployable by the release coordinator BEFORE a
...
allowlist. It does not widen anything for Cincinnati, Toledo, Lexington,
Detroit, Chattanooga, Fort Wayne or any other market.
"""
...
OUT = os.path.join(REPORTS, "nashville_tn_production_enablement_plan_002.json")
...
def main(argv=None) -> int:
    ...
    changes = [
        ...
        OrderedDict((
            ...
            ("proposed", [MARKET_ID]),
            ("permits", "the fast path for Nashville and no other market"),
        )),
        OrderedDict((
            ("file", "launch_packages/pettripfinder/fast_release_activation.json"),
            ...
            ("permits", "the production assembler and deployer to consume a validated bundle "
                        ...
                        "DISABLED is also safe: Nashville is a market that has never been "
        ...
        OrderedDict((
            ...
            ("proposed", [MARKET_ID]),
            ("permits", "real production activation for Nashville and no other market"),
        )),
    ]
    ...
    doc = OrderedDict((
        ...
        ("market_id", MARKET_ID),
        ("what_this_is",
         "The exact flag change that would open the redesigned production lane for Nashville "
         "alone. It is NOT applied: applying it would make Nashville deployable by the release "
         "coordinator before a founder authorization exists, which is the ordering these flags "
         "were built to prevent."),
        ("applied", False),
        ("scope", "NASHVILLE_ONLY"),
        ("global_enablement_refused",
         "no flag is set to a value that admits every market. Both allowlists are empty today, "
         "so the coordinator refuses all fourteen registered markets including the twelve that "
        ...
        ("order_of_operations", [
            ...
            "2. the launch_participation row for nashville-tn flips to "
            "FOUNDER_AUTHORIZED_FOR_LAUNCH",
            "3. these flags are applied, narrowed to nashville-tn",
