"""PTF-SAVANNAH-GA-PARALLEL-SOURCE-READY-001 -- Phases 10-14: the first-party policy reads.
...
Cloned from the Boone - Blowing Rock NC capture helper (itself from Outer Banks NC and Atlanta GA). Families and surfaces:
  ...
  WYNDHAM   plain-client reads (``savannah_ga_wyndham_lane_001``): the overview page
...
``data/acquisition/savannah_ga_attended/``; each payload file's canonical JSON
...
Output:
  launch_packages/pettripfinder/markets/reports/savannah_ga_attended_capture_001.json
"""
...
RAW = os.path.join(PKG, "markets", "staging", "savannah-ga", "raw_captures")
REGISTRY = os.path.join(REPORTS, "savannah_ga_corridor_registry_001.json")
OUT = os.path.join(REPORTS, "savannah_ga_attended_capture_001.json")
...
PAYLOAD_DIGESTS = OrderedDict([
    ("hilton_rows.json", "62f284546def71a9cd2d3baff632257bedc2f9e91d77d3afa5beb2f9d7a31284"),
    ("marriott_rows.json", "e3a10bc9ea800f193f029b7416080ab05382958fc4d891b8487992bedff31b51"),
    ("ihg_rows.json", "827bac99754e7afd947b5743429cc2ed4cd6828ea60a6edf15477f55bdc28ed7"),
    ("choice_rows.json", "ed8995f9bc89a0bc98863df6d25c11348121acf9a3a494fa983009feccfe2ba7"),
    ("hyatt_rows.json", "51dec47f122f1011eb263607ade510d3ea7492f4d5d002d647e1c5510d199aa9"),
    ("bestwestern_rows.json", "17f5689ea984a6f6769e878fe715ced9e5c86a3eeae20477cec5cbed6d833509"),
])
...
def build():
    ...
    for r in _load(os.path.join(RAW, "marriott_rows.json"), []):
        ...
        sig = OrderedDict([("name_on_page", r.get("n")), ("address_on_page", r.get("st")),
                           ("postal_code", (r.get("z") or "")[:5]), ("locality", r.get("ci")),
                           ("region", "GA"), ("phone_on_page", r.get("ph")), ("property_code_on_page", code),
    ...
    for r in _load(os.path.join(RAW, "ihg_rows.json"), []):
        ...
        sig = OrderedDict([("name_on_page", r.get("n") or name), ("address_on_page", r.get("st")),
                           ("postal_code", (r.get("z") or "")[:5]), ("locality", r.get("ci")), ("region", "GA"),
    ...
    for r in _load(os.path.join(RAW, "choice_rows.json"), []):
        ...
        u = "https://www.choicehotels.com/georgia/%s/%s" % (r["path"], r["c"])
        sig = OrderedDict([("name_on_page", r.get("n")), ("address_on_page", r.get("st")),
                           ("postal_code", (r.get("z") or "")[:5]), ("locality", r.get("ci")), ("region", "GA"),
                           ("phone_on_page", r.get("ph")), ("property_code_on_page", r["c"]),
                           ("lat", _f(r.get("la"))), ("lng", _f(r.get("lo")))])
    ...
    for extra in _load(os.path.join(RAW, "brand_pages.json"), []) + BRAND_PAGES:
        sig = OrderedDict([("name_on_page", extra["name"]), ("address_on_page", extra["street"]),
                           ("postal_code", extra["postal"]), ("locality", extra["city"]), ("region", "GA"),
