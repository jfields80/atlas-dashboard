"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 12A: one first-party route per census identity.
...
census ADMITTED, the first-party route it will be read at and which source stated it, in this order:
  ...
  1. the route the brand's own published inventory carries for the row (owned Marriott harvest, Marriott's Florida
     sitemap page, Hilton's city-page card, the brand sitemaps that answered);
  2. the website the destination bureau (Visit Orlando) links for the listing joined to this building;
  3. the website the map source (OSM) tags on the building.

Guards (from Nashville / Toledo): a directory, social or OTA host is never a route; a brand LOCATOR INDEX is never a
route; a route on a host that serves a different chain than the hotel's own name is REJECTED; Marriott legacy /
fact-sheet shapes are repaired to the brand's current overview shape.

A route is bound to an identity by the source that stated it and is NOT confirmed until the page's own address, phone
or property code agrees -- that is the capture lane's job.

IDENTITY FILL: brand-roster rows the census could not place (a route and a name, no address) are routed too, so the
capture lane can read the address the property's OWN page states. A code selects; the page admits.

Nothing here fetches. Output:
  launch_packages/pettripfinder/markets/reports/orlando_fl_v2_routing_001.json
"""
...
_NOT_A_PROPERTY_ROUTE = re.compile(
    r"(google\.|facebook\.com|instagram\.com|tripadvisor|booking\.com|expedia|yelp\.com|hotels\.com|kayak\.|"
    r"visitorlando\.com|bringfido|airbnb|vrbo|priceline|agoda|trivago|orbitz)", re.I)
_EXTRA_NAME_FAMILY = [("ROSEN", r"\brosen\b"), ("DISNEY", r"\bdisney'?s\b"), ("UNIVERSAL", r"\buniversal'?s\b|cabana bay|endless summer")]
_EXTRA_HOST_FAMILY = [("ROSEN", r"rosenhotels\.com|rosen[a-z]*\.com"), ("DISNEY", r"disney\.go\.com"),
                      ("UNIVERSAL", r"universalorlando\.com|loewshotels\.com")]
...
def family_of_name(name):
    return _chain_of_name(name) or next((f for f, rx in _EXTRA_NAME_FAMILY if re.search(rx, name or "", re.I)), "")


def family_of_host(url):
    return _chain_of_host(url) or next((f for f, rx in _EXTRA_HOST_FAMILY if re.search(rx, url or "", re.I)), "")
...
def route_row(h, *, identity_fill=False):
    ...
    for c in candidates(h):
        ...
        if nf and hf and nf != hf and not ({nf, hf} <= {"UNIVERSAL", "LOEWS"}):
...
def build():
    ...
    for h in census.get("non_admitted", []):
        # a brand-roster row with a route and no address, or a registry / map row the census could not settle on
        # identity alone: reading its own page is what settles it
        if h.get("classification") not in ("NAME_ONLY_UNRESOLVED", "IDENTITY_REVIEW_REQUIRED"):
            continue
        if "BRAND_INVENTORY_DOES_NOT_LIST" in (h.get("classification_reason") or "") and not any(
                str(o.get("lane", "")).startswith("BRAND_INVENTORY") for o in h.get("evidence", [])):
            r = route_row(h, identity_fill=True)
        elif any(str(o.get("lane", "")).startswith("BRAND_INVENTORY") for o in h.get("evidence", [])):
            r = route_row(h, identity_fill=True)
