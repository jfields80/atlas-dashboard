"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- Phases 20-21: every census identity in exactly one disposition.

Cloned from the Orlando FL V2 helper, simplified because this market's clean-authority pass
(west_palm_beach_fl_clean_authority_001) already computes one disposition per confirmed identity directly, with its
...
Output: launch_packages/pettripfinder/markets/staging/west-palm-beach-fl/launch_package/west_palm_beach_fl_final_partition_001.json
"""
...
def build():
    ...
    for h in census["hotels"]:
        ...
        if disp in _TERMINAL:
            ...
            row["resolved"] = False
            row["next_action_source"] = "PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 acquisition router record"
    ...
    return OrderedDict((
        ...
        ("what_this_is", "Every admitted Palm Beach County census identity in exactly one disposition state, with the "
