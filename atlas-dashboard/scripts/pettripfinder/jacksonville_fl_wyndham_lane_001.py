"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phase 12B: the Wyndham first-party lane (plain client).
...
Both documents are persisted as returned, addressed by sha256, under ``data/acquisition/jacksonville_fl_wyndham_001/``.
...
Northeast Florida place. A route admits nothing -- the postal code the property service states
decides.

Output:
  launch_packages/pettripfinder/markets/staging/jacksonville-fl/raw_captures/wyndham_rows.json
  launch_packages/pettripfinder/markets/reports/jacksonville_fl_wyndham_lane_001.json
"""
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "jacksonville-fl", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "jacksonville_fl_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "jacksonville_fl_wyndham_001")
