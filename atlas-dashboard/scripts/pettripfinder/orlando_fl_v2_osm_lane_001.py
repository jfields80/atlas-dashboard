"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 9B: the OpenStreetMap lane.
...
at over an hour on the 428 MB North Carolina extract (the 626 MB Florida extract is read the same way) before it died without writing an index.
...
Output:
  launch_packages/pettripfinder/markets/reports/orlando_fl_v2_osm_lane_001.json
"""
...
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "orlando_fl_v2_osm_lane_001.json")

#: guest_house, chalet and apartment are kept too: Orlando's vacation-rental and resort-condo stock is often
#: tagged that way, and the census must SEE a rental to refuse it by decision.
...
def build():
    ...
    return OrderedDict([
        ...
        ("source", OrderedDict([
            ...
            ("pbf_snapshot",
             "the 2026-09-15 Geofabrik Florida download made by this order (656,440,205 bytes); the upstream "
