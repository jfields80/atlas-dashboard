"""PTF-NASHVILLE-TN-PROMOTION-AND-NEW-LANE-LAUNCH-PREP-002 -- the modern release lane.

Takes the newly registered Nashville market through the redesigned lane
ATLAS-THROUGHPUT-003 to 008 built, and writes what each stage proved:
    ...
    6  FINAL CANDIDATE                    release_index.compose (live + Nashville)
...
The candidate is composed with Nashville PARTICIPATING, because a pre-flip
...
PTF-NASHVILLE-TN-PROMOTION-AND-NEW-LANE-LAUNCH-PREP-002 inherits. Composing an
...
from scripts.pettripfinder.nashville_tn_paid_ledger_ingest_002 import (   # noqa: E402
    ledger_lookup, reservations_from_ledger)
...
def joining_delta(inputs) -> "OrderedDict":
    """Nashville's intended delta: a market that JOINS, and nothing else moves.
    ...
    market that is live on both sides of the comparison. Nashville is on neither
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out", default=str(REPORTS / "nashville_tn_release_lane_002.json"))
    ap.add_argument("--work", default=str(_DASH / "data" / "nashville_release_lane_002"))
    ap.add_argument("--base", default="a875032",
    ...
    # Every published Nashville row came through a paid lane, so rule O has a
    # real question to answer here. The reservation is read back out of the
    # shared paid-attempt ledger the ingestion wrote, which is the provenance
    # authority; the run report is not consulted twice.
    inputs.paid_reservations = reservations_from_ledger()
    ...
    receipt = FL.run_fast_lane(package, work_dir=Path(args.work), live=live,
                               participates=True, ledger_lookup=ledger_lookup())
    ...
    # exists to prevent:
    #
    #   (a) what does REGISTERING Nashville cost?  The change surface of this
    ...
    #
    #   (b) what does a ROUTINE Nashville release cost?  The fast lane's receipt
    ...
    #       Nashville, which this order proposes and does not apply.
    t = time.monotonic()
    ...
    doc = OrderedDict((
        ...
        ("candidate", OrderedDict((
            ...
            ("parent_route_count", len({r for i in live[0].markets.values() if i.participating
                                        for r in i.routes})),
            ("nashville_routes", sorted(candidate.markets[MARKET_ID].routes)),
        ))),
        ("cache_reuse", reuse),
        ("timings", timings),
    ))
