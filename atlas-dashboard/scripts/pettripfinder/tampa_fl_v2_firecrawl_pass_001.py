"""PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 13, the Firecrawl rung (cloned from the Orlando FL V2 harness).
...
-----------------------------------------
  ...
  DISCOVERED    Choice property routes read from the brand's own city pages through Firecrawl
                (tampa_fl_v2_firecrawl_discovery_001, when it exists), in-market towns only, that no static read
...
Output:
  launch_packages/pettripfinder/markets/reports/tampa_fl_v2_firecrawl_pass_001.json
"""
...
STATIC_REPORT = os.path.join(REPORTS, "tampa_fl_v2_free_static_capture_001.json")
DISCOVERY = os.path.join(REPORTS, "tampa_fl_v2_firecrawl_discovery_001.json")
PROBE_PER_FAMILY = 4
PROBE_FAMILIES = ("INDEPENDENT", "RED_ROOF", "MOTEL6", "RADISSON", "BEST_WESTERN")
WALL_REPROBE = ("MARRIOTT", "HILTON")
_IN_MARKET_CHOICE_TOWNS = ("tampa", "st-petersburg", "saint-petersburg", "clearwater", "clearwater-beach",
                          "brandon", "largo", "temple-terrace", "dunedin", "tarpon-springs", "palm-harbor",
                          "oldsmar", "safety-harbor", "seminole", "pinellas-park", "wesley-chapel",
                          "riverview", "plant-city", "st-pete-beach", "treasure-island", "madeira-beach")
SPACING_SECONDS = 2.0
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--run-id", default="tampa_fl_v2_firecrawl_001")
    ap.add_argument("--cap-attempts", type=int, default=90)
    ap.add_argument("--floor-credits", type=int, default=400)
    ap.add_argument("--out", default=os.path.join(REPORTS, "tampa_fl_v2_firecrawl_pass_001.json"))
