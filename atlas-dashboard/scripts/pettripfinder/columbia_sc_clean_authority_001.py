"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phases 13 and 14: the clean set.
...
Output:
  launch_packages/pettripfinder/markets/reports/columbia_sc_clean_authority_001.json
"""
...
CENSUS = os.path.join(PKG, "identity_census_proposed", "columbia-sc.json")
ATTENDED = os.path.join(REPORTS, "columbia_sc_attended_capture_001.json")
STATIC = os.path.join(REPORTS, "columbia_sc_free_static_lane_001.json")  # not produced in this market
...
def build():
    ...
    from scripts.pettripfinder.columbia_sc_census_reconciliation_001 import directional_conflict
    for r in co_located:
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "columbia_sc_clean_authority_001.json"))
