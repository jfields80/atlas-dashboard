"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phase 9A: the Florida DBPR public-lodging REGISTRY lane.
...
units. That makes the registry the census backbone for Greater Fort Lauderdale: a licensed HOTEL (rank HOTL) or MOTEL
...
-------------------------------------------------
  ...
  TAPT   transient apartment        a census LEAD only when its own name reads as a hotel / suites /
                                    ...
                                    single units dominate this class along the Hollywood Broadwalk and Fort Lauderdale Beach)
...
A licence proposes an identity on the state's record of the premises; it does not decide one. The brand's
own page or the property's own site still binds a route, a brand property code and the policy.
...
Inputs (downloaded by this order, gitignored, hashed here):
  data/fort_lauderdale_fl/dbpr/hrlodge1.csv .. hrlodge7.csv
Output:
  launch_packages/pettripfinder/markets/reports/fort_lauderdale_fl_dbpr_lane_001.json
"""
...
SRC_DIR = os.path.join(_DASH, "data", "fort_lauderdale_fl", "dbpr")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports", "fort_lauderdale_fl_dbpr_lane_001.json")
...
RETRIEVED_AT = "2026-09-20T20:21:00Z"
...
OBSERVED_COUNTIES = {"Broward", "Dade", "Miami-Dade", "Palm Beach", "Monroe"}
...
#: Refused neighbours this lane OBSERVES row by row: every OUTSIDE postal code the geography names (Miami and
#: Miami-Dade, Boca Raton, Delray Beach, Boynton Beach, West Palm / Palm Beach, the Keys). Miami-Dade / Palm
#: Beach / Monroe licences outside those named codes are COUNTED by county for the South Florida boundary
#: audit and never enumerated as leads.
...
_BOUNDARY_CITIES = ("MIAMI", "MIAMI BEACH", "AVENTURA", "SUNNY ISLES BEACH", "NORTH MIAMI BEACH", "HIALEAH",
...
def build():
    ...
    for i in range(1, 8):
        ...
        obs, _skipped = DBPR.parse_hrlodge_csv(
            text, observed_at="2026-09-20", retrieved_at=RETRIEVED_AT,
        ...
        for n, row in enumerate(reader, start=1):
            ...
            if county in GEO.OBSERVED_COUNTIES and rank0 in LEAD_RANKS + ("TAPT",):
                b = boundary.setdefault(county, OrderedDict([("future_market", GEO.OBSERVED_COUNTIES[county]),
                                                             ("hotel_rank_licences", 0), ("by_city", Counter())]))
            ...
            if county in GEO.OBSERVED_COUNTIES and z not in outside:
                continue
    ...
    return OrderedDict([
        ...
        ("south_florida_boundary_by_county", OrderedDict(
            (k, OrderedDict([("future_market", v["future_market"]), ("hotel_rank_licences", v["hotel_rank_licences"]),
                             ("by_city", OrderedDict(sorted(v["by_city"].items()))),
                             ("admitted_into_fort_lauderdale", 0)]))
