"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 26: the SHADOW sealed package (cloned from the Savannah GA helper).

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Orlando is not at the front of the serialized release queue: the current verified live release is
Savannah (27 markets / 1802 profiles / 2081 routes, deploy 6aa93de5), and other source-ready markets are queued ahead of it.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/orlando-fl/launch_package/``) from the proposed
  ...
  3. runs the FAST lane (rules A-O) over it, which builds the joining market's
     ...
     current live parent (Savannah) -- no whole-site candidate is assembled;
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/orlando-fl/``: never ``markets/packages/`` or
...
WHAT BINDS TO THE SAVANNAH-LIVE PARENT, AND HOW IT IS RECONCILED
------------------------------------------------------------
...
REPORT = PKG / "markets" / "reports" / "orlando_fl_v2_shadow_package_001.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "orlando_fl_v2_proposed_authority_001.json"
SEALED_AT = "2026-09-15T20:00:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         "parent_live_state names the CURRENT live release (Savannah, deploy 6aa93de5...). That is what this "
         "package was validated against. Once any later market is live, FAST rule N fails this package BY DESIGN; the "
         "registration order -- when Orlando reaches the front of the release queue -- re-seals the same committed inputs against that parent as a NEW "
