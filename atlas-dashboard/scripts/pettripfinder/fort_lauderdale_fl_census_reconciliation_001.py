"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phases 9, 11 and 24: one identity graph.

Cloned from the Miami FL census helper (itself from Tampa V2, Orlando V2, Savannah, Boone, Outer Banks and
Atlanta), with Broward County's vocabulary. Independent discovery lanes, none authoritative on its own:
  ...
  BRAND_INVENTORY     the committed national Marriott harvest (FLL-coded routes); Marriott's Florida hotel sitemap
  ...
  DESTINATION_ORGANIZATION  every lodging listing of Visit Lauderdale, Broward County's own CVB
                      (visitlauderdale.com), from the bureau's own public listings service.
...
-----------------------
The market contract's postal partition (fort_lauderdale_fl_geography_001). Refused neighbours carry their
reason; Miami-Dade names the EXISTING LIVE market miami-fl and Palm Beach / Monroe name a future standalone.

VACATION RENTALS, CONDOS, TIMESHARES AND RESORT RESIDENCES ARE NOT HOTELS
--------------------------------------------------------------------------
Broward County carries 4,569 DBPR condominium (CNDO) and 6,234 vacation-dwelling (DWEL) licences against
427 hotel / motel / B&B licences -- Hollywood Beach, the Galt Ocean Mile, Lauderdale-by-the-Sea and
Hallandale Beach hold most of them; they are counted by the registry lane and never enter this graph. A licence or map row whose own name
...
ATTENDED_PASS = os.path.join(REPORTS, "fort_lauderdale_fl_policy_reads_001.json")
DESTINATION = os.path.join(REPORTS, "fort_lauderdale_fl_destination_roster_001.json")
DBPR_LANE = os.path.join(REPORTS, "fort_lauderdale_fl_dbpr_lane_001.json")
...
from scripts.pettripfinder.fort_lauderdale_fl_nonhotel_rulings_001 import NOT_LODGING_WHY, LODGING_UNCONFIRMED, nonhotel_by_name  # noqa: E402
...
_SPELLING = (
    ...
    # BROWARD: the Fort Lauderdale grid is spelled both "SE 17 St" and "Southeast 17th Street"; the shared key drops
...
#: A house number followed directly by a bare numbered street ("2301 SE 17 St", Fort Lauderdale's own grid).
...
def read_brand_city_pages():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        # BROWARD: a card must state Florida AND a South Florida postal prefix (330 Broward/Dade south, 333 Broward, for the
...
def read_brand_sitemaps():
    ...
    seen_routes = set()
    _disc = (_load(os.path.join(REPORTS, "fort_lauderdale_fl_firecrawl_discovery_001.json"), {}) or {}).get("routes", [])
...
def read_property_pages():
    ...
    or map row into a Fort Lauderdale identity."""
...
from scripts.pettripfinder.fort_lauderdale_fl_static_rulings_001 import (  # noqa: E402
...
DUAL_BRAND_COMBINED = [
    # BROWARD: no DBPR licence, bureau listing or brand card in this market was found naming two hotels in one
    # row. The rule stays armed and is re-asserted by the partition contract -- a dual-brand building is TWO
    # hotels and is HELD for the split, never published as one (standing rule; proved on AC/Element Miami
    # Brickell and Eden Roc/Nobu Miami Beach in PTF-MIAMI-FL-HARDENED-SOURCE-READY-001).
]
...
DUPLICATE_OF = {
    "marriott fort lauderdale airport": (
        "DUPLICATE_LISTING -- a brand-roster row carrying the MARSHA code fllmp under the parcel's "
        "pre-development street name '166 Radiant Drive'. The brand's OWN page for fllmp, read this order "
        "(2026-09-21), states the premises as 166 N Compass Way, Dania Beach 33004, and the Florida DBPR "
        "licence HOT1620927 ('MARRIOTT FT. LAUDERDALE AIRPORT', 196 units) is at 166 N COMPASS WAY. The "
        "census row 'MARRIOTT FORT LAUDERDALE AIRPORT AT DANIA POINTE' already carries that premises"),
    "westin beach resort fort lauderdale": (
        "DUPLICATE_LISTING -- the SECOND tower of one establishment. Florida DBPR licenses the trade name "
        "'WESTIN BEACH RESORT FORT LAUDERDALE' twice on the same beachfront block: HOT1604688 at 303 N Fort "
        "Lauderdale Beach Blvd (223 units) and HOT1609861 at 321 (210 units). The brand's OWN page for MARSHA "
        "code flllw, read this order (2026-09-21), names 321 North Fort Lauderdale Beach Boulevard as the "
        "premises, and the census row 'WESTIN FORT LAUDERDALE BEACH RESORT' already carries it"),
    "residence inn fort lauderdale coconut creek": (
        "DUPLICATE_LISTING -- a brand-roster row carrying the MARSHA code fllgr with NO street of its own. "
        "The brand's OWN page for fllgr, read this order (2026-09-21), states the premises as 5730 North "
        "State Road 7, Coconut Creek 33073, which the census row 'RESIDENCE INN BY MARRIOTT FORT LAUDERDALE "
        "- COCONUT CREEK' already carries. A row with no addressable premises never publishes"),
}


def read_destination_roster():
    """Visit Lauderdale's lodging roster: tier-2 discovery and identity evidence.

    The roster report nests listings under ``bureaus``; every bureau's own ``listings`` are pooled here.
    """
...
PLACES_REPORT = os.path.join(REPORTS, "fort_lauderdale_fl_places_route_discovery_001.json")
...
def read_dbpr():
    """BROWARD: the Florida DBPR public-lodging licence registry -- tier 2 identity and eligibility evidence."""
...
_IN_MARKET_MUNICIPALITIES = {
    "fort lauderdale", "ft lauderdale", "lauderdale", "las olas", "downtown fort lauderdale",
    "fort lauderdale beach", "port everglades", "harbor beach", "cypress creek",
    "lauderdale by the sea", "sea ranch lakes", "galt ocean mile", "wilton manors", "oakland park",
    "lauderdale lakes", "lauderhill", "tamarac", "north lauderdale", "dania beach", "dania",
    "hollywood", "hollywood beach", "hallandale beach", "hallandale", "west park",
    "pompano beach", "pompano", "lighthouse point", "hillsboro beach", "deerfield beach",
    "plantation", "davie", "cooper city", "southwest ranches", "sunrise", "sawgrass", "weston",
    "pembroke pines", "miramar", "coral springs", "coconut creek", "margate", "parkland",
}
#: Refused places: the EXISTING LIVE miami-fl market's own municipalities, and the named future standalones.
_FUTURE_SUBMARKET_PLACES = {"miami", "miami beach", "south beach", "mid beach", "north beach", "brickell",
                            "downtown miami", "doral", "coral gables", "coconut grove", "miami springs",
                            "virginia gardens", "key biscayne", "aventura", "sunny isles", "sunny isles beach",
                            "bal harbour", "surfside", "bay harbor islands", "north miami", "north miami beach",
                            "north bay village", "hialeah", "hialeah gardens", "miami lakes", "kendall",
                            "south miami", "homestead", "florida city", "cutler bay", "miami gardens",
                            "opa locka", "sweetwater", "medley", "pinecrest", "palmetto bay", "miami shores",
                            "el portal", "west miami", "wynwood", "edgewater", "little havana", "fisher island",
                            "boca raton", "delray beach", "boynton beach", "west palm beach", "palm beach",
                            "palm beach gardens", "jupiter", "lake worth", "wellington",
...
_DIRECTIONALS = {
    ...
    # BROWARD area words that tell same-brand hotels apart (Courtyard Downtown / Airport & Cruise Port /
    # Plantation / Weston / Beach / Pompano / Coral Springs)
    "airport", "beach", "oceanfront", "bayfront", "bay", "port", "cruise", "everglades", "olas", "galt",
    "intracoastal", "marina", "harbor", "harbour", "waterfront", "pier", "causeway", "convention", "sawgrass",
    "mall", "stadium", "arena", "university", "medical", "hospital", "corporate", "executive", "turnpike",
    "cypress", "creek", "commercial", "plantation", "davie", "sunrise", "weston", "tamarac", "lauderhill",
    "pompano", "deerfield", "hollywood", "hallandale", "dania", "miramar", "pembroke", "pines", "springs",
    "creek", "coconut", "coral", "margate", "parkland", "broadwalk", "seabreeze", "birch", "cooper",
}
...
CORRIDOR_CITIES = {
    "downtown-las-olas": {"fort lauderdale", "ft lauderdale", "lauderdale", "las olas",
                          "downtown fort lauderdale"},
    "fort-lauderdale-beach": {"fort lauderdale", "ft lauderdale", "lauderdale", "fort lauderdale beach",
                              "las olas"},
    "port-everglades-17th-street": {"fort lauderdale", "ft lauderdale", "lauderdale", "port everglades",
                                    "harbor beach", "las olas"},
    "fll-airport-sr84": {"fort lauderdale", "ft lauderdale", "lauderdale", "dania beach", "dania"},
    "dania-beach": {"dania beach", "dania", "fort lauderdale", "ft lauderdale", "hollywood"},
    "hollywood-beach": {"hollywood", "hollywood beach", "hallandale", "hallandale beach"},
    "hollywood": {"hollywood", "pembroke pines", "davie", "cooper city", "hallandale beach"},
    "plantation-davie": {"plantation", "davie", "cooper city", "fort lauderdale", "ft lauderdale",
                         "lauderdale", "sunrise", "dania beach", "hollywood"},
    "hallandale-beach": {"hallandale beach", "hallandale", "hollywood"},
    "galt-ocean-lauderdale-by-the-sea": {"fort lauderdale", "ft lauderdale", "lauderdale",
                                         "lauderdale by the sea", "sea ranch lakes", "galt ocean mile",
                                         "pompano beach"},
    "oakland-park-wilton-manors": {"fort lauderdale", "ft lauderdale", "lauderdale", "oakland park",
                                   "wilton manors"},
    "cypress-creek": {"fort lauderdale", "ft lauderdale", "lauderdale", "oakland park", "wilton manors",
                      "lauderdale lakes", "cypress creek"},
    "pompano-beach": {"pompano beach", "pompano", "lighthouse point", "hillsboro beach", "deerfield beach",
                      "fort lauderdale", "ft lauderdale", "lauderdale by the sea"},
    "deerfield-beach": {"deerfield beach", "pompano beach"},
    "sunrise-tamarac-lauderhill": {"sunrise", "tamarac", "lauderhill", "lauderdale lakes", "plantation",
                                   "fort lauderdale", "ft lauderdale", "north lauderdale", "sawgrass"},
    "weston-southwest-ranches": {"weston", "southwest ranches", "davie", "cooper city", "fort lauderdale",
                                 "ft lauderdale", "lauderdale"},
    "pembroke-pines-miramar": {"pembroke pines", "miramar", "west park", "hollywood", "cooper city", "davie"},
    "coral-springs-coconut-creek": {"coral springs", "coconut creek", "margate", "parkland",
                                    "north lauderdale", "pompano beach", "tamarac"},
}
...
#: A map-only row under a flag whose own Broward-area route on the brand's site is RETIRED. Empty at authoring time.
...
def classify(node, zips, name_counts, street_counts):
    ...
    if lanes == {"OSM_OVERPASS"} and any(
        ...
        return IDENTITY_REVIEW_REQUIRED, (
            "a map row under a flag (%s) whose own Broward-area route on the brand's site is "
    ...
    # Broward / Miami-Dade county line (lat < 25.9575, the Hallandale Beach - Aventura line at County Line Road)
    # or NORTH of the Broward / Palm Beach county line (lat > 26.3400, the Deerfield Beach - Boca Raton line)
    # cannot be an admitted identity whatever else is unknown about it.
    _pins = [(o.get("lat"), o.get("lng")) for o in node.observations
             if o.get("lat") is not None and o.get("lng") is not None]
    if not node.postal and _pins and all(float(t) < 25.9575 or float(t) > 26.3400 for t, g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it beyond the admitted corridors "
                                "(Miami-Dade County south -- the LIVE miami-fl market -- or Palm Beach County "
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Greater Fort Lauderdale "
...
#: Brand families whose OWN Broward-area inventory this order read. Families whose inventory refused this
...
_COVERED_FLAGS = (
    ...
    ("MARRIOTT", r"\b(courtyard by marriott|courtyard marriott|courtyard fort lauderdale|courtyard ft lauderdale|courtyard oceanside|courtyard by|courtyard(?= (fort|ft|miami|downtown|airport|weston|plantation|coral))|residence inn|fairfield|springhill|towneplace|aloft|element|westin|sheraton|four points|ac hotel|marriott|moxy|gaylord|ritz)\b"),
...
def brand_not_listed(node):
    ...
    # BROWARD: the brand inventories this order could read are PARTIAL (Hilton city pages carry the twenty nearest
    ...
    for fam, rx in _COVERED_FLAGS:
        if re.search(rx, node.name or "", re.I):
            return ("a %s row naming a %s flag (%r) that no read of that brand's own Broward-area inventory "
...
SOURCE_AUTHORITIES = [
    ...
    "https://www.visitlauderdale.com listings (Visit Lauderdale's own public listings service, catid 49)",
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Greater Fort Lauderdale authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "fort-lauderdale-fl.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "fort_lauderdale_fl_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "fort_lauderdale_fl_competitor_gap_matrix_001.json"))
