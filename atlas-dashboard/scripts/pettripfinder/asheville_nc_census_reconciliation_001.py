"""PTF-ASHEVILLE-NC-NORMAL-PRODUCTION-001 -- Phases 4-8: one identity graph.

Independent discovery lanes ran for Asheville, none of them
authoritative on its own:
  ...
  BRAND_INVENTORY     Marriott's AVL* roster OWNED at zero requests from the
...
-----------------------
The market contract's postal partition, and nothing else: downtown, Biltmore /
South Asheville, Tunnel Road, West Asheville, North Asheville / Woodfin and
Arden / Fletcher / AVL (CORE), Candler, Weaverville and Swannanoa (CORRIDOR),
Black Mountain (FRINGE). An AVL property code reaches Hendersonville, Flat Rock,
Brevard, Waynesville and Cherokee as readily as Asheville; every such row is
classified on its own postal code.
...
Outputs:
  launch_packages/pettripfinder/identity_census/asheville-nc.json
  launch_packages/pettripfinder/markets/reports/asheville_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/asheville_nc_competitor_gap_matrix_001.json
"""
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "asheville-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "asheville-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "asheville_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "asheville_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "asheville_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "asheville_nc_attended_capture_001.json")
...
NOT_LODGING_NAMES = {
    "sensibilities day spa", "wrong way river lodge and cabins", "carolina bed breakfast",
    "grafton lodge bed and breakfast", "bison escape by buffalo creek vacations",
    "hare pin inn by buffalo creek vacations", "gaia arise farm", "springdale country club",
    "the yellow house on plott creek road", "river view cabins", "minnehaha falls guest house",
    "explore asheville", "asheville regional airport",
}
...
NOT_LODGING_WHY = {
    "sensibilities day spa": "a day spa inside Hilton Asheville Biltmore Park, tagged tourism=hotel on the map at the hotel's own street address",
    "wrong way river lodge and cabins": "VACATION_RENTAL / CAMPGROUND -- a campground with rental cabins (its own site is wrongwaycampground.com)",
    "carolina bed breakfast": "BED_AND_BREAKFAST -- a guest house without hotel operation; no official site read",
    "grafton lodge bed and breakfast": "BED_AND_BREAKFAST -- a guest house without hotel operation",
    "bison escape by buffalo creek vacations": "VACATION_RENTAL -- a property-management listing",
    "hare pin inn by buffalo creek vacations": "VACATION_RENTAL -- a property-management listing",
    "gaia arise farm": "VACATION_RENTAL -- a farm stay",
    "springdale country club": "a golf club tagged tourism=hotel on the map",
    "the yellow house on plott creek road": "VACATION_RENTAL -- a single rental house",
    "river view cabins": "VACATION_RENTAL -- rental cabins",
    "minnehaha falls guest house": "VACATION_RENTAL -- a single guest house",
}
NON_HOTEL_LEAD_TYPES = {"Campground"}
STR_PLACEHOLDERS = {"airbnb asheville", "vrbo asheville", "airbnb black mountain"}
...
def read_property_pages():
    ...
    allowed to turn a brand-roster or map row into a Asheville identity."""
...
IDENTITY_ONLY_PAGES = [
    ("Lantern Lodge Asheville", "700 Biltmore Ave", "Asheville", "28803", "(888) 786-3648",
     "https://www.lanternlodgeasheville.com/", "bf4b9d3a6487fd0037d2e806559324e9ad38ef0785058f63e725622ab9671424",
     "the map's 'The Residences at Biltmore' (700 Biltmore Avenue) now redirects to its own site as Lantern "
     "Lodge Asheville: a condo-hotel operated as one hotel with nightly suites and group booking under one "
     "name; the address is stated in the site footer of the static read"),
    ("Artful Way Hotel", "31 Artful Way", "Asheville", "28801", "828-617-1118",
     "https://www.artfulwayhotel.com/", "1a4d86dda866607e3a0b9b80b200854546aef4edc56fe9f17cc90ea5d3ae51ec",
     "the property's own site states its address and phone; its only animal sentences describe nearby "
     "pet-friendly patios and are not a policy"),
]

#: Phase 6. Competitor directory leads, as BringFido's Asheville lodging page
#: listed them in the attended session. NAMES ONLY -- a lead proposes and never
#: decides, and its pet claims are never read. Each is reconciled in the gap matrix.
COMPETITOR_LEADS = [
    ("Kimpton Hotel Arras", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("Aloft Asheville Downtown", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("1900 Inn on Montford", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("The Black Walnut Inn", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("Extended Stay America Premier Suites Asheville", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("Comfort Suites Outlet Center", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("Country Inn & Suites Asheville River Arts District", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("The Pines Cottages", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("The Restoration Asheville", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
    ("Americas Best Value Inn Asheville", "https://www.bringfido.com/lodging/city/asheville_nc_us/"),
]
...
def read_competitor():
    ...
    return [observation("COMPETITOR_LEAD", 4, src, name, merge_alias=normalize_name(name),
                        lead_source="BringFido (competitor directory, discovery only)")
...
def merge(observations):
    ...
    for obs in observations:
        ...
        _route = (obs.get("route") or "").split("?")[0].rstrip("/").lower()
        if _route and "wyndhamhotels.com/" in _route:
...
_IN_MARKET_MUNICIPALITIES = {
    "asheville", "biltmore", "biltmore village", "biltmore park", "arden", "fletcher", "candler",
    "weaverville", "swannanoa", "black mountain", "woodfin", "avl", "west asheville",
    "east asheville", "south asheville", "north asheville",
}
_OUT_OF_MARKET_PLACES = {
    "hendersonville", "flat rock", "mills river", "waynesville", "canton", "maggie valley",
    "mars hill", "marshall", "hot springs", "lake lure", "chimney rock", "bat cave", "brevard",
    "pisgah forest", "burnsville", "saluda", "tryon", "old fort", "clyde", "lake junaluska",
    "etowah", "fairview", "leicester", "cherokee", "morganton", "highlands", "marion",
    "forest city", "franklin", "gatlinburg", "sapphire", "cashiers",
}
...
#: Wyndham flags whose own Asheville-area routes redirected to the brand's city
#: search in this order's attended pass (see the capture report). Baymont is NOT
#: here: the brand's search still lists Baymont by Wyndham Asheville.
RETIRED_WYNDHAM_FLAGS = ("super 8", "days inn", "ramada", "travelodge", "microtel", "howard johnson", "wingate")
...
def classify(node, zips, name_counts, street_counts):
    ...
    # Asheville routes for Super 8, Days Inn, Ramada, Travelodge and Microtel now
    ...
    if lanes == {"OSM_OVERPASS"} and any(
        ...
        return IDENTITY_REVIEW_REQUIRED, (
            "a map row under a flag (%s) whose own Asheville-area route on the brand's site is "
    ...
    if node.postal and node.postal[:5] not in zips:
        return OUTSIDE_MARKET, ("postal code %s is claimed by no corridor of the Asheville "
                                "contract; Hendersonville, Flat Rock, Mills River, Waynesville, "
                                "Canton, Mars Hill, Brevard and Chimney Rock / Lake Lure are refused "
                                "by name in the geography"
                                % node.postal[:5])
...
def build():
    ...
    census = OrderedDict([
        ...
        ("work_order", WORK_ORDER), ("captured_at", "2026-09-12"),
        ("note",
         "PTF-ASHEVILLE-NC-NORMAL-PRODUCTION-001 Asheville census, built from zero "
        ...
        ("source_authorities", [
            ...
            "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (fifteen city pages)",
            "https://www.wyndhamhotels.com/sitemap.xml (the brand's own property shards) and "
            "https://www.wyndhamhotels.com/hotels/asheville-north-carolina (the brand's own search)",
            "https://www.ihg.com/destinations/us/en/united-states/north-carolina/<city>-hotels (attended same-origin)",
            "each property's own page (attended browser pass and the independents' own websites)",
            "BringFido Asheville lodging page (competitor leads; names only)",
        ]),
        ("count", len(confirmed)),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "asheville-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "asheville_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "asheville_nc_competitor_gap_matrix_001.json"))
