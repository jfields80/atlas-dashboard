"""PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 -- the final partition.

Every census Hickory - Newton - Conover identity, in EXACTLY ONE state, with exactly one next
...
Output: launch_packages/pettripfinder/markets/staging/hickory-nc/launch_package/hickory_nc_final_partition_007.json
...
OUT = os.path.join(PKG, "markets", "staging", "hickory-nc", "launch_package",
                   "hickory_nc_final_partition_007.json")
...
REFUSALS = {
    "CHOICE": "a plain-client sitemap timeout, and in the attended session choicehotels.com's Hickory property page "
              "(nc146 Sleep Inn) rendered an empty document (no title, no body text) after five seconds -- the brand's "
...
POLICY_SILENT = {
    "2nd street inn": "the inn's own home and booking pages (13 2nd Street NE, 28601) served with no pet wording",
    "the dragonfly inn": "the inn's own site (224 3rd Ave NW, 28601) served; its only pet words are the booking "
                         "engine's translation vocabulary ('Pets welcome' beside 'Pets not allowed'), never an "
                         "operative statement",
}
...
def build():
    ...
    routing = _load(os.path.join(REPORTS, "hickory_nc_routing_001.json"), {})
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-HICKORY-NC-PARALLEL-SOURCE-READY-001 "
                                         "acquisition passes: brand city pages, the Marriott state sitemap, "
                                         "brand sitemaps and property services, the OSM extract, the Visit Hickory "
                                         "Metro listing service, the independents' own sites and the attended "
                                         "browser pass")
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-14"),
        ("what_this_is",
         "Every census Hickory - Newton - Conover identity in exactly one disposition state. A terminal state "
