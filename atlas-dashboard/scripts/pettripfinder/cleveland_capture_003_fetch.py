...
def main() -> int:
    ...
    # The original run swept 74. Two of them -- Drury Plaza Hotel and Drury Inn
    # & Suites Beachwood -- resolved into published authority at
    # PTF-CLEVELAND-POLICY-CAPTURE-INTEGRATION-003 and left the unresolved set,
    # so this sweep is 72. Those two are not re-fetched here and do not need to
    # be: their citable URL and their capture's html/text sha256 are committed,
    # in the proposed-authority manifest and again on each published record, so
    # either one is reproducible on its own from a single direct fetch.
    assert len(targets) == 72, "expected 72 ROUTED_AWAITING_CAPTURE targets, found %d" % len(targets)
    names = [t["normalized_name"] for t in targets]
    assert len(set(names)) == 72, "target set is not 72 mutually unique identities"
