"""PTF-JACKSONVILLE-NC-PARALLEL-SOURCE-READY-001 -- Phases 4-8: one identity graph.

Independent discovery lanes ran for Jacksonville, none of them
authoritative on its own:
  ...
  BRAND_INVENTORY     Marriott's OAJ* roster OWNED at zero requests from the
                      committed national harvest; Hilton's roster read from the
                      brand's OWN North Carolina city pages; Wyndham property
                      routes read from its own sitemap; IHG codes read from the
                      brand's own destination pages.
  PROPERTY_PAGE       the address each property's OWN page states, from the
                      attended pass (Hilton, Marriott, Wyndham, IHG, Omni) and the
                      independent / resort / extended-stay properties' own websites.
...
-----------------------
The market contract's postal partition, and nothing else: Western Boulevard,
downtown / Marine Boulevard / US-17 / New River and Midway Park (CORE), Hubert
(CORRIDOR), Sneads Ferry, Swansboro and Richlands (FRINGE) -- plus, inside the two
shared postal codes 28460 and 28584, the property's own stated municipality: North
Topsail Beach, Cape Carteret and Cedar Point are OUTSIDE however their ZIP reads.
On-base Camp Lejeune / MCAS New River lodging is MILITARY_GOVERNMENT_NONPUBLIC and
is never admitted.

SHADOW UNTIL REGISTERED
-----------------------
Jacksonville is built while another market must go live first, so the census is
written to the zone's PROPOSED census path, never to identity_census/.

VACATION RENTALS ARE NOT HOTELS
-------------------------------
Individual vacation homes, cabins, condominium rental programmes, property-
management listings, bed-and-breakfast houses without hotel operation, day spas
and campgrounds are recorded NON_LODGING by name, never admitted. A condo-hotel
that operates as a hotel (a front desk, nightly rooms sold under one name) is a
lodging identity like any other.

Nothing here fetches. Nothing here carries a pet policy.

Outputs:
  launch_packages/pettripfinder/identity_census_proposed/jacksonville-nc.json
  launch_packages/pettripfinder/markets/reports/jacksonville_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/jacksonville_nc_competitor_gap_matrix_001.json
"""
...
from scripts.pettripfinder import jacksonville_nc_geography_001 as GEO  # noqa: E402
...
GEOGRAPHY_HOLDS = {
    "best western plus silver creek inn":
        "801 Cedar Point Boulevard sits in 28584, which Swansboro shares with Cedar Point and Cape "
        "Carteret. Only the map labels it Swansboro; the brand's own property page redirected this "
        "session to the brand's search page, so no first-party page stated the municipality. Cedar "
        "Point Boulevard is the Carteret County side of the White Oak River, which the geography "
        "refuses; membership is undecidable on this evidence",
}
...
#: REGISTERED by PTF-JACKSONVILLE-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002; the source-ready census in identity_census_proposed/ is history.
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "jacksonville-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "jacksonville-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "jacksonville_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "jacksonville_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "jacksonville_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "jacksonville_nc_attended_capture_001.json")
DESTINATION = os.path.join(REPORTS, "jacksonville_nc_destination_roster_001.json")
...
NOT_LODGING_NAMES = set()
#: Why each NON_LODGING name above is refused -- written down so a refusal is a
#: decision. Cabins, farm stays and rental houses are VACATION RENTALS.
NOT_LODGING_WHY = {}
NON_HOTEL_LEAD_TYPES = {"Campground"}
STR_PLACEHOLDERS = set()

#: Phase 3. On-base Camp Lejeune / MCAS New River lodging: it requires installation
#: access (a DoD credential or a sponsored visitor pass) and is not bookable by the
#: general traveling public under normal conditions. Matched on the full normalised name.
MILITARY_NONPUBLIC_NAMES = {
    "inns of the corps":
        "Inns of the Corps, the Marine Corps lodging facility inside Marine Corps Base Camp Lejeune "
        "(the map places it at 34.671 N, -77.340 W, behind the installation gate): transient quarters "
        "reached only with a DoD credential or sponsored pass, not a public commercial hotel",
}
...
def read_osm():
    ...
    for e in doc.get("elements", []):
        ...
        # writes "525 Marine Boulevard" beside a website that IS the Wyndham
        # route this order read, whose Hotel JSON-LD states "505 North Marine
        # Blvd". Keyed on its own street the map row would open a second
...
def read_property_pages():
    ...
    allowed to turn a brand-roster or map row into a Jacksonville identity."""
...
IDENTITY_ONLY_PAGES = [
    ("The Travelers Inn Hotel", "4315 New Bern Highway", "Jacksonville", "28546", "9104592575",
     "https://www.nctravelinn.com/", "722777b50f002fae789a658e0594a7b344f7d1fe2d58d055988a568280a09964",
     "the property's own site states its LocalBusiness "
     "address in JSON-LD; no page on the site states a pet policy (/faq 404, /policies 429)"),
    ("Affordable Suites of America", "35 McDaniel Drive", "Jacksonville",
     "28546", "(910) 347-2400", "https://www.affordablesuites.com/hotels/affordable-suites-jacksonville-nc/",
     "11fae4240a347b599b4e16902b0d99afff81d0c9759e435932bbc8f927268c1f",
     "the brand's own property page (attended; the plain client got 403) states its Hotel JSON-LD "
     "address; its only pet wording is a 'Pet-Friendly Rooms' amenity chip, which is never operative"),
    ("Comfort Suites near Camp Lejeune", "130 Workshop Lane", "Jacksonville", "28546", "(910) 346-8900",
     "https://www.comfortsuitesjacksonvillenc.com/",
     "bbd89489aaa2b22e203dba0671796b807cab14275602b7b6b84fd740fcf4efb0",
     "the property's own franchise site states its Hotel JSON-LD address (130 Workshop Ln, US 17 N); "
     "its FAQ answers 'No, pets are typically not allowed' -- a HEDGED statement, held, never a refusal"),
]

#: Phase 6. Competitor directory leads, as web search result pages summarised the
#: BringFido / Tripadvisor Jacksonville NC lists and a Western / Marine Boulevard hotel
#: search in this order. NAMES ONLY -- a lead proposes and never decides, and its pet
#: claims are never read. Each is reconciled in the gap matrix.
_LEAD_SRC = "web search result: pet friendly hotels Jacksonville NC (BringFido summaries) and a Western / Marine Boulevard hotel search"
COMPETITOR_LEADS = [
    ("Red Roof Inn & Suites Jacksonville NC", _LEAD_SRC),
    ("Home2 Suites by Hilton Jacksonville NC", _LEAD_SRC),
    ("My Place Hotel-Jacksonville/Camp Lejeune NC", _LEAD_SRC),
    ("Days Inn Jacksonville NC", _LEAD_SRC),
    ("Rodeway Inn & Suites Jacksonville Near Camp Lejeune", _LEAD_SRC),
    ("Affordable Suites of America", _LEAD_SRC),
    ("Studio 6 Suites Jacksonville, NC - Camp LeJeune", _LEAD_SRC),
    ("Motel 6 Jacksonville, NC", _LEAD_SRC),
    ("Quality Inn Jacksonville Near Camp Lejeune", _LEAD_SRC),
    ("Mainstay Suites Jacksonville near Camp Lejeune", _LEAD_SRC),
    ("Staybridge Suites Jacksonville-Camp Lejeune Area", _LEAD_SRC),
]
...
MAP_ROWS_SUPERSEDED_BY_A_READ = {
    "way/938691663": (
        "https://www.bwtopsailbeach.com/overview",
        "the map calls 1565 NC Highway 210 'Quality Inn & Suites Sneads Ferry - North Topsail Beach'; the "
        "property's own site (bwtopsailbeach.com, home-page address block 1565 NC Hwy 210, Sneads Ferry 28460) "
        "and the county tourism roster both name it Blue Water Inn - Best Western Signature Collection. A "
        "stale map flag is evidence of the building's history, not a second identity"),
}
...
REBRAND_HOLDS = {
    "2115 north marine": (
        "2115 North Marine Boulevard: the map (and a Best Western route that now redirects to the brand's "
        "search) calls it SureStay Plus Hotel by Best Western Jacksonville; the county tourism roster calls it "
        "Clarion Pointe Jacksonville (a Choice route, and Choice refused this client). Both carry the phone "
        "910-347-1900. No first-party page stated the current flag"),
    "474 western": (
        "474 Western Boulevard: the county tourism roster still lists Baymont Inn by Wyndham Camp Lejeune, "
        "whose own Wyndham route now redirects to the brand's city search (RETIRED); third-party listings and "
        "the operator's own studio6.com route name it Studio 6 Suites Jacksonville, NC - Camp LeJeune, and "
        "G6 refused this client. No first-party page stated the current flag"),
}
...
def read_destination_roster():
    ...
    Its ZIPs are loose (it prints 28546 for Marine Boulevard buildings whose own
    pages state 28540), so the stated ZIP is carried as ``stated_postal_code``
...
def merge(observations):
    ...
    for obs in observations:
        ...
        # Jacksonville extends the route key to every brand host, because a map
...
_IN_MARKET_MUNICIPALITIES = {
    "jacksonville", "midway park", "piney green", "half moon", "verona", "hubert",
    "sneads ferry", "swansboro", "richlands",
}
_OUT_OF_MARKET_PLACES = {
    "north topsail beach", "topsail beach", "surf city", "holly ridge", "hampstead",
    "cape carteret", "cedar point", "emerald isle", "atlantic beach", "morehead city", "newport",
    "maysville", "new bern", "havelock", "beulaville", "wilmington",
}
...
_DIRECTIONALS = {
    ...
    "arpt", "i", "40", "85", "sw", "ne", "nw", "se", "conv", "tunnel", "road", "rd", "outlet",
    "outlets", "outlet ctr", "ctr", "village", "square", "parkway", "pkwy", "blue", "ridge",
    # Jacksonville's same-brand names differ only in these: "Camp Lejeune" / "Near Camp
    # Lejeune" / "Camp Lejeune Area" vs "New River" vs a bare "Jacksonville NC".
    "camp", "lejeune", "near", "new", "river", "nc", "marine", "western", "blvd",
}
...
def attach_name_only(nodes, node_corridor_municipality=None):
    ...
    for s in soft:
        ...
        # Jacksonville, NC"): identical names cannot be told apart by overlap.
...
def merge_street_spelling(nodes):
    ...
    for n in nodes:
        if n in read or not n.street or not n.postal:
            continue
        # Jacksonville: a building only the map AND the county roster spell ("400
...
#: Wyndham flags with a Jacksonville-area route that redirected to the brand's city
#: search in this order's attended pass (see the capture report): Super 8 Jacksonville /
#: Camp Lejeune Area, Travelodge Jacksonville NC and Baymont Camp Lejeune. The rule only
#: ever reaches a MAP-ONLY row -- a building whose own live page was read is not a
#: map-only row -- so the live Days Inn and Microtel pages are untouched.
RETIRED_WYNDHAM_FLAGS = ("super 8", "travelodge", "baymont")
...
def classify(node, zips, name_counts, street_counts):
    ...
    if node.city and GEO.classify_postal("28460", node.city)[0] == "OUTSIDE":
        return OUTSIDE_MARKET, ("the row's own municipality %r is refused by the geography in every "
                                "admitted postal code: %s" % (node.city, GEO.classify_postal("28460", node.city)[2]))
    ...
    # Jacksonville routes for Super 8, Travelodge and Baymont now
    ...
    if lanes == {"OSM_OVERPASS"} and any(
        ...
        return IDENTITY_REVIEW_REQUIRED, (
            "a map row under a flag (%s) whose own Jacksonville-area route on the brand's site is "
    ...
    if node.postal and node.postal[:5] not in zips:
        return OUTSIDE_MARKET, ("postal code %s is claimed by no corridor of the Jacksonville "
                                "contract; Camp Lejeune / MCAS New River on-base (28542 / 28543 / "
                                "28545 / 28547), Topsail Island, the Crystal Coast, Maysville and "
                                "Wilmington are refused by name in the geography"
                                % node.postal[:5])
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-JACKSONVILLE-NC-PARALLEL-SOURCE-READY-001 Jacksonville census, built from zero "
         "under the modern factory and registered to identity_census/ by PTF-JACKSONVILLE-NC-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002. Every row carries "
        ...
        ("source_authorities", [
            ...
            "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (fourteen city pages)",
            "https://www.wyndhamhotels.com/sitemap.xml (the brand's own property shards) and "
            "https://www.wyndhamhotels.com/hotels/jacksonville-north-carolina (the brand's own search)",
            "https://www.ihg.com/destinations/us/en/united-states/north-carolina/<city>-hotels (attended same-origin)",
            "https://www.redroof.com/sitemap.xml (attended same-origin) and the property page it lists",
            "https://www.extendedstayamerica.com/hotels/nc/jacksonville (the brand's own city page) and its property page",
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Jacksonville authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "jacksonville-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "jacksonville_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "jacksonville_nc_competitor_gap_matrix_001.json"))
