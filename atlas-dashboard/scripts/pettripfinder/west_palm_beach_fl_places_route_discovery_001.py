"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- route discovery for unrouted census identities (Google Places Text
...
Two cohorts, one request each, capped:
  ...
  GAP_VERIFICATION  every BringFido TRUE_MISSING lead (west_palm_beach_fl_competitor_reconciliation_001). The question is
...
Output:
  launch_packages/pettripfinder/markets/reports/west_palm_beach_fl_places_route_discovery_001.json
"""
...
RECON = os.path.join(REPORTS, "west_palm_beach_fl_competitor_reconciliation_001.json")
OUT = os.path.join(REPORTS, "west_palm_beach_fl_places_route_discovery_001.json")
REQUEST_CAP = 400
...
def build(limit=REQUEST_CAP):
    ...
    for g in gap:
        if requests_made >= limit:
            break
        text = "%s West Palm Beach FL" % g["bringfido_name"]
        ...
        rows.append(rec)
    return OrderedDict([
        ("schema", "ptf-places-route-discovery/1.0"), ("work_order", WORK_ORDER), ("market_id", "west-palm-beach-fl"),
