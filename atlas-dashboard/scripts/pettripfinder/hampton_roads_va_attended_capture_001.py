"""PTF-HAMPTON-ROADS-VA-PARALLEL-SOURCE-READY-001 -- Phases 10-14: the first-party policy reads.

Cloned from the Charleston SC capture helper (itself from Savannah GA, Boone - Blowing Rock NC and Atlanta GA). Families and surfaces:
  ...
  CHOICE    same-origin fetch of the property page; the page's own "Pets" item.
  HYATT     same-origin fetch of the property page; the pet module.
  BEST_WESTERN  same-origin fetch of the property page; the page's own FAQ answer to
            "Q. Is the X Pet Friendly?" (its JSON-LD ``petsAllowed`` is false on every
            Savannah page read, including the one whose FAQ accepts dogs -- a template
            default, never read as a policy).
  WYNDHAM   plain-client reads (``hampton_roads_va_wyndham_lane_001``): the overview page
...
``data/acquisition/hampton_roads_va_attended/``; each payload file's canonical JSON
...
Output:
  launch_packages/pettripfinder/markets/reports/hampton_roads_va_attended_capture_001.json
"""
...
RAW = os.path.join(PKG, "markets", "staging", "hampton-roads-va", "raw_captures")
REGISTRY = os.path.join(REPORTS, "hampton_roads_va_corridor_registry_001.json")
OUT = os.path.join(REPORTS, "hampton_roads_va_attended_capture_001.json")
...
PAYLOAD_DIGESTS = OrderedDict([
    ("ihg_rows.json", "685f1cf0fe5b61a9f53f58c4f6d8fa82e870268143be29b62741034d2b032676"),
    ("hyatt_rows.json", "871d5a1c442c5a9ec13ad43de6f2e42580a8b8f12acaf164a3598be47e18a519"),
    ("hilton_rows.json", "0fb50af044dc10f9d46a9f16b207274a0578e799cf2b2bdf1e6e796229426e84"),
    ("marriott_rows.json", "bd94728d98db60c6a1122b911314d9c720ffcbf0cbe295d57e39ba0c4a8cd93e"),
    ("choice_rows.json", "6188061f2bdb576969edf08371b81b0178ab32a8008a7ace989f4b24ce700890"),
    ("bestwestern_rows.json", "06c94919ee07ad9aa720a8baf53342a9bf91f8e8731746de8cf78dda4fb3a612"),
    # HAMPTON ROADS: a second Choice pass after the 403 wall went stale served one more property page before walling again.
    ("choice_rows_002.json", "b81bf4adff2a17401b12775f527ee5a46ce0fd2884fcb8e0e764e13b6e134c2e"),
])
...
def parse_ihg(r):
    ...
    if re.match(r"\s*No,\s*pets\s+are\s+not\s+allowed", text, re.I):
        ...
        # HAMPTON ROADS: Holiday Inn Newport News - Hampton's description accepts pets without the word "pet" ("your four
        # legged companion. Bring up to 2, 50lb dogs or cats for 75USD ..."), which is an acceptance, not a service-animal-only
        # description; dogs / cats / four-legged-companion wording beside a charge counts as the hotel's own acceptance.
        if _desc and re.search(r"service animals?", _desc, re.I) and not re.search(
                r"\bwe welcome\b|\bpets?\b(?! related)[^.]{0,40}\b(?:welcome|allowed|accepted|fee)\b"
                r"|\bbring up to \d+[^.]{0,30}\b(?:dogs?|cats?)\b|four[- ]legged companion", _desc, re.I):
...
def parse_hyatt(r):
    ...
    if re.match(r"(Pets Are Welcome|We Are Pet Friendly)$", head, re.I):
        ext["pets_allowed"] = True
    elif re.match(r"We Are Dog Friendly$", head, re.I) and re.search(
            r"Your dog is welcome to stay|our pet-friendly [A-Za-z ]+ hotel\. Two dogs may be allowed", text, re.I):
        # RICHMOND: Hyatt Select Richmond / Chester's module reads "Bring your fluffy companions to our pet-friendly
        # Chester hotel. Two dogs may be allowed in a room ...".
...
def parse_bestwestern(r):
    ...
    if re.match(r"Pets are not accepted\.?$", text, re.I):
        ...
        # CHARLESTON: "The Pet Friendly rate is 30.00 USD per day." states cents.
        m = re.search(r"rate is (\d+(?:\.\d{2})?) USD per day", text, re.I)
        if m:
            ext["pet_fee"] = int(round(float(m.group(1)) * 100))
            ext["fee_currency"] = "USD"
            ext["fee_basis"] = "per_night"
...
def build():
    ...
    for r in _load(os.path.join(RAW, "marriott_rows.json"), []):
        ...
        sig = OrderedDict([("name_on_page", r.get("n")), ("address_on_page", r.get("st")),
                           ("postal_code", (r.get("z") or "")[:5]), ("locality", r.get("ci")),
                           ("region", "VA"), ("phone_on_page", r.get("ph")), ("property_code_on_page", code),
    ...
    for r in _load(os.path.join(RAW, "ihg_rows.json"), []):
        ...
        sig = OrderedDict([("name_on_page", r.get("n") or name), ("address_on_page", r.get("st")),
                           ("postal_code", (r.get("z") or "")[:5]), ("locality", r.get("ci")), ("region", "VA"),
    ...
    for r in _load(os.path.join(RAW, "choice_rows.json"), []) + _load(os.path.join(RAW, "choice_rows_002.json"), []):
        if r.get("s") != 200 or not r.get("st"):
            continue
        ext, quote, conflict = parse_choice(r)
        u = "https://www.choicehotels.com/virginia/%s/%s" % (r["path"], r["c"])
        sig = OrderedDict([("name_on_page", r.get("n")), ("address_on_page", r.get("st")),
                           ("postal_code", (r.get("z") or "")[:5]), ("locality", r.get("ci")), ("region", "VA"),
                           ("phone_on_page", r.get("ph")), ("property_code_on_page", r["c"]),
                           ("lat", _f(r.get("la"))), ("lng", _f(r.get("lo")))])
    ...
    for r in _load(os.path.join(RAW, "bestwestern_rows.json"), []):
        if r.get("outside") or r.get("s") != 200 or not r.get("st"):
            continue
        ext, quote, conflict = parse_bestwestern(r)
        # The JSON-LD name is an SEO title ("Best Western Central Inn | Savannah GA Hotel (Top Pick)");
        # the page's own FAQ question names the hotel.
        name = re.sub(r"^Q\.\s*Is the\s+|\s+Pet Friendly\?$", "", r.get("q") or "") or r.get("n")
        sig = OrderedDict([("name_on_page", name), ("address_on_page", r.get("st")),
                           ("postal_code", (r.get("z") or "")[:5]), ("locality", r.get("ci")), ("region", "VA"),
                           ("phone_on_page", r.get("ph")), ("property_code_on_page", r["c"]),
                           ("lat", _f(r.get("la"))), ("lng", _f(r.get("lo")))])
        out.append(_row("BEST_WESTERN", "ATTENDED_BROWSER", r["c"], r["u"], r["u"], sig, r.get("h"), r.get("b"),
                        "the property page's own FAQ answer to 'Q. Is the X Pet Friendly?'",
                        quote, ext, zips, "HOTEL_JSONLD_ADDRESS_AND_PROPERTY_CODE", conflict))
    ...
    for extra in _load(os.path.join(RAW, "brand_pages.json"), []) + BRAND_PAGES:
        sig = OrderedDict([("name_on_page", extra["name"]), ("address_on_page", extra["street"]),
                           ("postal_code", extra["postal"]), ("locality", extra["city"]), ("region", "VA"),
    ...
    return OrderedDict([
        ...
        ("phase", "10-14 -- first-party reads: Marriott, Hilton, IHG, Choice, Hyatt, Best Western (attended same-origin), Wyndham (plain client) and brand pages"),
        ("as_of", CAPTURED_AT),
