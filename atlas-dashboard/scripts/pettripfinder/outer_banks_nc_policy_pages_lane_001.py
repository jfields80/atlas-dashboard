"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- Phase 11B: the independents' own policy pages.

Outer Banks independents publish their pet policy on a subpage (``/pet-policy``,
...
``data/acquisition/outer_banks_nc_policy_pages_001/``).
...
recorded in ``markets/staging/outer-banks-nc/raw_captures/brand_pages.json``
with the sha256 of the document they were read from, and the capture helper binds
them.

Output:
  launch_packages/pettripfinder/markets/reports/outer_banks_nc_policy_pages_lane_001.json
"""
...
from scripts.pettripfinder import outer_banks_nc_static_lane_001 as S      # noqa: E402
...
STATIC = os.path.join(REPORTS, "outer_banks_nc_static_lane_001.json")
OUT = os.path.join(REPORTS, "outer_banks_nc_policy_pages_lane_001.json")
DOCS = os.path.join(_DASH, "data", "acquisition", "outer_banks_nc_policy_pages_001")
LINK_CAP = 6
...
EXTRA_TARGETS = [
    ("https://www.searanchresort.com/", "sea ranch resort"),
    ("https://obxmotorlodge.com/", "outer banks motor lodge"),
    ("http://www.cavalierbythesea.com", "cavalier by the sea"),
    ("https://www.innonpamlicosound.com/", "the inn on pamlico sound"),
]
