"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phase 9: the SHADOW sealed package.

WHY A SHADOW PACKAGE, AND NOT ``registration_release_lane seal``
----------------------------------------------------------------
Columbia is not at the front of the serialized release queue. The live index this branch's lineage carries
is Atlanta (23 markets / 1500 profiles / 1744 routes); later markets (Richmond, Charleston) went live on their own
branches, and other source-ready markets (Hampton Roads) are queued ahead of Columbia.
...
ownership template already owns ``markets/staging/<id>/``. So this helper:
  ...
  1. STAGES a repository-shaped launch-package tree inside the market's own zone
     (``markets/staging/columbia-sc/launch_package/``) from the proposed
  ...
  4. writes the package, the receipt and a report ONLY under
     ``markets/staging/columbia-sc/``: never ``markets/packages/`` or
...
REPORT = PKG / "markets" / "reports" / "columbia_sc_shadow_package_008.json"
PROPOSED_MARKET = PKG / "markets" / "proposed" / ("%s.json" % MARKET_ID)
PROPOSED_CENSUS = PKG / "identity_census_proposed" / ("%s.json" % MARKET_ID)
AUTHORITY = STAGING / "columbia_sc_proposed_authority_002.json"
SEALED_AT = "2026-09-15T02:00:00Z"
...
def seal(source_sha, work_dir, write=True):
    ...
    out.update(OrderedDict([
        ...
        ("parent_binding",
         ...
         "registration order -- when Columbia reaches the front of the release queue -- re-seals the same committed inputs against that parent as a NEW "
