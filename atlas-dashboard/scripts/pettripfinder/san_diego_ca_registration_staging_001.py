"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 26: STAGE San Diego's authority.
...
validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE  markets/staging/san-diego-ca/launch_package/hotel_policy_facts_san-diego-ca.json
     -- schema-1.3 facts, every published fact cited to the quote it rests on.
  2. the PROPOSED AUTHORITY     markets/staging/san-diego-ca/san_diego_ca_proposed_authority_001.json
...
package root, because the root is where a REGISTERED market's documents live and San Diego is registered
...
#: A TIERED FEE IS NOT ONE NUMBER. Hilton states "1-4nts $75, 5+nts $125 per stay" at most San Diego Hilton
...
def build():
    ...
    package = OrderedDict([
        ("market", "San Diego, California"), ("schema_version", PS.SCHEMA_VERSION), ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER), ("as_of", OBSERVED_AT),
        ("note", "San Diego / Coastal San Diego County's REGISTERED policy package. Every record is "
    ...
    authority = OrderedDict([
        ...
        ("what_this_is", "San Diego / Coastal San Diego County's authority as the source-ready order proposed it, REGISTERED "
