"""PTF-CLEVELAND-AKRON-CANTON-HARDENED-REVALIDATION-001 -- Phase 9 (attended lane).
...
document. No vendor, no price. Nothing is written to authority.
    ...
    python ...attended_capture_009b.py ingest --identity-key K --queue-id Q --payload payload.json [--interaction "..."]
    python ...attended_capture_009b.py summary
"""
...
RUN_ID = "cleveland-hardened-attended-001"
...
RESULTS = os.path.join(REPORTS, f"{MARKET_ID.replace('-', '_')}_attended_capture_009b.json")
BRANDS = [("MARRIOTT", r"marriott|courtyard|residence inn|springhill|fairfield|towneplace|ac hotel|aloft|westin|sheraton|moxy|element"),
          ...
          ("WYNDHAM", r"wyndham|baymont|days inn|super 8|ramada|travelodge|la quinta|microtel"), ("HYATT", r"hyatt"), ("MAGNUSON", r"magnuson")]
...
def ingest(args):
    ...
    if not rd.get("found"):
        rd2 = read(payload.get("pet_windows_hidden"), "hidden_text")
    ...
    if args.outcome_override:
        ...
        cls = "PET_FRIENDLY_STATED_ATTENDED" + ("" if rd.get("source") == "visible_text" else "_HIDDEN_TEXT")
    elif pa is False:
        cls = "NO_PETS_STATED_ATTENDED" + ("" if rd.get("source") == "visible_text" else "_HIDDEN_TEXT")
    ...
    record = OrderedDict([
        ...
        ("captured_at", artifact["captured_at"]), ("interaction", args.interaction or ""), ("identity_binding", binding), ("reader", rd), ("classification", cls),
