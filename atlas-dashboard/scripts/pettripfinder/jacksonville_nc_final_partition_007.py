"""PTF-JACKSONVILLE-NC-PARALLEL-SOURCE-READY-001 -- the final partition.

Every census Jacksonville identity, in EXACTLY ONE state, with exactly one next
...
------------------------------------
  ...
  ACCESS_BLOCKED              the surface refused to serve us. Choice (a
                              sitemap timeout and a blank attended page), G6
                              (Motel 6 / Studio 6: timeouts, not navigable) and
                              Best Western (property routes redirected to search)
                              refused this client on this run.
...
Output: launch_packages/pettripfinder/markets/staging/jacksonville-nc/launch_package/jacksonville_nc_final_partition_007.json
...
REFUSALS = {
    "CHOICE": "a sitemap timeout and a blank attended page for choicehotels.com/north-carolina/jacksonville/hotels",
    "MOTEL6": "a plain-client timeout on motel6.com and studio6.com and no permitted attended navigation",
    "BEST_WESTERN": "a sitemap 403 and property routes that redirect the attended session to the brand's search page",
}
#: Independent properties whose OWN site refused this client on this run.
SITE_REFUSALS = {
    "best rest inn": "a plain-client timeout on bestrest-inn.com, the website the county tourism roster gives",
}
#: Buildings whose own brand page SERVED but states no operative policy, read outside
#: the clean-authority join (no postal code on the page), by census name.
POLICY_SILENT = {
    "comfort suites near camp lejeune":
        "the property's own franchise site served (comfortsuitesjacksonvillenc.com) and its FAQ answers "
        "'Do you allow pets? No, pets are typically not allowed.' -- a HEDGED statement, which is not an "
        "operative refusal; its amenity list's 'No Pets Allowed' is a list item. Held; Choice's own page "
        "was not served",
    "quality inn jacksonville near camp lejeune":
        "the franchise site qijacksonville.com served an amenity-list line 'No Pets Allowed (ADA Defined "
        "Service Animals Allowed)' and a PETS line on service animals only -- a list item is not an operative "
        "sentence, and Choice's own page was not served",
    "affordable suites of america":
        "the brand's own property page served the attended session (the plain client got 403) and states only a "
        "'Pet-Friendly Rooms' amenity chip -- never operative",
    "the travelers inn hotel":
        "the property's own site served its home page with no pet wording; /faq returned 404 and /policies 429",
}
#: Held reads, by identity key, with the class the clean-authority helper gave.
HELD_CLASSES = ("SERVICE_ANIMAL_ONLY", "FIRST_PARTY_CONFLICT", "QUOTE_NOT_OPERATIVE")
...
def build():
    ...
    for h in census["hotels"]:
        ...
        if key in pf:
            ...
            row["next_action_source"] = ("PTF-JACKSONVILLE-NC-PARALLEL-SOURCE-READY-001 "
    ...
    doc = OrderedDict((
        ...
        ("as_of", "2026-09-13"),
        ("what_this_is",
         "Every census Jacksonville identity in exactly one disposition state. A terminal state "
