"""PTF-SAVANNAH-GA-PARALLEL-SOURCE-READY-001 -- Phases 13 and 14: the clean set.
...
Output:
  launch_packages/pettripfinder/markets/reports/savannah_ga_clean_authority_001.json
"""
...
ATTENDED = os.path.join(REPORTS, "savannah_ga_attended_capture_001.json")
STATIC = os.path.join(REPORTS, "savannah_ga_free_static_lane_001.json")  # not produced in this market
...
def build():
    ...
    # PTF-SAVANNAH-GA-REGISTER-RESEAL-AND-AUTHORIZATION-PREP-002: a record is released ONLY when a ruling that validates
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=os.path.join(REPORTS, "savannah_ga_clean_authority_001.json"))
