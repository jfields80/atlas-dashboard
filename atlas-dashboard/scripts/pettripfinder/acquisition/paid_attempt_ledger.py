...
#: takes a compatible name or a shared telephone to confirm, and two
#: disagreeing brand property codes refute it outright.
#:
...
class LedgerIndex:
    ...
    def _premises_confirmed(row: Mapping, record: Mapping, row_code: str) -> bool:
        ...
        positive confirmation: the same switchboard, or names that are
        compatible in the sense the census dedup already defines.
        """
        record_code = str(record.get("property_code") or "")
        if row_code and record_code and row_code != record_code:
            return False
        phone, other = normalized_phone(row), str(record.get("telephone") or "")
        if phone and other and phone == other:
            return True
        return DEDUP.names_compatible(_name(row), str(record.get("canonical_name") or ""))
...
def decide(row: Mapping, index: "LedgerIndex", *,
    ...
    ordered = sorted(attempts, key=lambda a: (str(a.get("attempted_at") or ""),
