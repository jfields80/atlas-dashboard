"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- Phase 7B: the OpenStreetMap lane.
...
Output:
  launch_packages/pettripfinder/markets/reports/west_palm_beach_fl_osm_lane_001.json
"""
...
CONFIG = os.path.join(_DASH, "scripts", "pettripfinder", "discovery", "config",
                      "west_palm_beach_fl.json")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "west_palm_beach_fl_osm_lane_001.json")

#: guest_house, chalet and apartment are kept too: Palm Beach County's vacation-rental and resort-condo stock,
#: especially on Singer Island, Highland Beach, the Delray and Boca oceanfronts and in Wellington's equestrian
#: season, is often tagged that way, and the census must SEE a rental to refuse it by decision.
...
def build():
    ...
    return OrderedDict([
        ...
        ("market_id", MARKET_ID),
        ("phase", "7B -- OpenStreetMap / Geofabrik lodging elements inside the observation box"),
        ("source", OrderedDict([
            ...
            ("pbf_snapshot",
             "the Geofabrik Florida extract already owned by this repository (downloaded by PTF-TAMPA-FL-HARDENED-V2, carried forward through the Miami and Fort Lauderdale builds) and reused unchanged as owned data (656,564,714 bytes); the upstream "
