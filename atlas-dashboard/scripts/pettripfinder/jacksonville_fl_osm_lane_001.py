"""PTF-JACKSONVILLE-FL-HARDENED-SOURCE-READY-001 -- Phase 9B: the OpenStreetMap lane.

Reads the local Geofabrik Florida extract (owned data, reused unchanged -- no new download, no provider call) (ODbL, (c) OpenStreetMap
contributors) and keeps every ``tourism=hotel`` / ``tourism=motel`` element --
node, way or multipolygon relation -- inside this market's OBSERVATION box, which reaches south to St. Augustine and
Palm Coast, west to Baker County and NORTH ACROSS THE GEORGIA STATE LINE to Kingsland and St. Marys, so the
boundary audit counts what it refuses instead of being blind to it.

WHY A TWO-PASS READ, NOT THE AREA-ASSEMBLY INDEX
------------------------------------------------
Orlando V2's run measured the shared index builder's pyosmium area-assembly pass at over an hour on the
similarly sized Florida extract before it died without writing an index. A lodging census needs a point
per building, not a polygon: pass one keeps tagged nodes inside the box and remembers the node references
of tagged ways (and of the outer ways of tagged relations); pass two reads only those node locations and
takes each way's centroid. Minutes, not hours.
...
Output:
  launch_packages/pettripfinder/markets/reports/jacksonville_fl_osm_lane_001.json
"""
...
PBF = os.path.join(_DASH, "data", "osm_extracts", "florida-latest.osm.pbf")
CONFIG = os.path.join(_DASH, "scripts", "pettripfinder", "discovery", "config",
                      "jacksonville_fl.json")
OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "reports",
                   "jacksonville_fl_osm_lane_001.json")

#: guest_house, chalet and apartment are kept too: Northeast Florida's vacation-rental and resort-condo stock --
#: Amelia Island Plantation's and the Omni resort's villa programmes (Nassau County holds 1,130 individually
#: licensed resort-condominium units), the Jacksonville Beach oceanfront towers, Sawgrass and Ponte Vedra's golf
#: villas -- is often tagged that way, and the census must SEE a rental to refuse it by decision.
LODGING = {"hotel", "motel", "guest_house", "chalet", "apartment"}
...
def build():
    ...
    return OrderedDict([
        ...
        ("market_id", MARKET_ID),
        ("phase", "9B -- OpenStreetMap / Geofabrik lodging elements inside the observation box"),
        ("source", OrderedDict([
            ("pbf", "data/osm_extracts/florida-latest.osm.pbf"),
            ...
            ("pbf_snapshot",
             "the Geofabrik Florida extract already owned by this repository (downloaded by PTF-TAMPA-FL-HARDENED-V2 and carried forward through the Miami, Fort Lauderdale and West Palm Beach builds) and reused unchanged as owned data (656,564,714 bytes); the upstream "
             "file may since have been republished, so this md5 is the snapshot's own"),
