"""PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 -- Phases 9, 11 and 24: one identity graph.

Cloned from the Orlando FL V2 census helper (itself from Savannah, Boone, Outer Banks and Atlanta), with Tampa
Bay's vocabulary. Independent discovery lanes, none authoritative on its own:
  ...
  BRAND_INVENTORY     the committed national Marriott harvest (TPA-coded routes); Marriott's Florida hotel sitemap
  ...
  DESTINATION_ORGANIZATION  every "Places to Stay" listing of Visit Tampa Bay's own Hotels & Resorts category.
...
-----------------------
The market contract's postal partition (tampa_fl_v2_geography_001). Refused neighbours carry their reason, and a
...
--------------------------------------------------------------------------
Tampa Bay, especially the Gulf beaches, carries thousands of DBPR condominium (CNDO) and vacation-dwelling (DWEL)
...
TIMESHARE / RESORT_RESIDENCE reason (tampa_fl_v2_nonhotel_rulings_001).
...
Outputs:
  launch_packages/pettripfinder/identity_census_proposed/tampa-fl.json
  launch_packages/pettripfinder/markets/reports/tampa_fl_v2_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/tampa_fl_v2_competitor_gap_matrix_001.json
"""
...
CONTRACT_PATH = os.path.join(PKG, "markets", "proposed", "tampa-fl.json")

OSM_LANE = os.path.join(REPORTS, "tampa_fl_v2_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "tampa_fl_v2_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "tampa_fl_v2_policy_reads_001.json")
DESTINATION = os.path.join(REPORTS, "tampa_fl_v2_destination_roster_001.json")
DBPR_LANE = os.path.join(REPORTS, "tampa_fl_v2_dbpr_lane_001.json")
...
from scripts.pettripfinder.tampa_fl_v2_nonhotel_rulings_001 import NOT_LODGING_WHY, LODGING_UNCONFIRMED, nonhotel_by_name  # noqa: E402
...
#: TAMPA: spellings the shared ``address_key`` keeps apart but that name one street ("N Dale Mabry Hwy" / "North
...
def read_brand_city_pages():
    ...
    for r in _brand_doc().get("leads", []):
        ...
        # TAMPA: a card must state Florida AND a Tampa Bay postal prefix (335/336/337/346-346-347-348 area codes
        # spanning Hillsborough/Pinellas/Pasco/Polk/Hernando) to be a lead of this box at all.
...
def read_brand_state_sitemap():
    ...
    for r in _brand_doc().get("leads", []):
        if r.get("lane") != "BRAND_STATE_SITEMAP_PAGE":
            continue
        if not ((r.get("property_code") or "").lower().startswith("tpa") or places_in(r.get("brand_title") or "")):
            continue
...
def read_brand_sitemaps():
    ...
    seen_routes = set()
    _disc = (_load(os.path.join(REPORTS, "tampa_fl_v2_firecrawl_discovery_001.json"), {}) or {}).get("routes", [])
...
def read_property_pages():
    ...
    or map row into a Tampa identity."""
...
from scripts.pettripfinder.tampa_fl_v2_static_rulings_001 import (  # noqa: E402
...
def read_destination_roster():
    """Visit Tampa Bay's Hotels & Resorts roster: tier-2 discovery and identity evidence.

    This market's roster report nests listings under ``bureaus`` (one entry per destination organisation
    probed, including Visit St. Pete-Clearwater's recorded ACCESS_BLOCKED); every bureau's own ``listings``
    are pooled here exactly as Orlando pooled its two bureaus (Visit Orlando + Experience Kissimmee).
    """
...
def _registry_title(text):
    ...
    for i, w in enumerate((text or "").split()):
        ...
        if i and lw in _SMALL_WORDS:
            words.append(lw)
        elif re.fullmatch(r"[a-z]{1,2}\d*|i+", lw) and lw in ("ac", "jw", "ii", "iii", "iv", "es", "fc", "usf", "tpa"):
...
def read_dbpr():
    """TAMPA: the Florida DBPR public-lodging licence registry -- tier 2 identity and eligibility evidence."""
...
_IN_MARKET_MUNICIPALITIES = {
    "tampa", "st petersburg", "saint petersburg", "clearwater", "clearwater beach", "brandon", "seffner",
    "ybor city", "largo", "madeira beach", "treasure island", "st pete beach", "saint pete beach",
    "indian rocks beach", "redington beach", "redington shores", "belleair beach", "belleair", "dunedin",
    "tarpon springs", "palm harbor", "oldsmar", "safety harbor", "seminole", "pinellas park", "gulfport",
    "wesley chapel", "new tampa", "riverview", "apollo beach", "ruskin", "sun city center", "wimauma",
    "plant city", "temple terrace", "westshore", "rocky point",
}
#: Refused places that preserve a named future standalone market.
_FUTURE_SUBMARKET_PLACES = {"sarasota", "bradenton", "anna maria", "bradenton beach", "longboat key",
                            "lakeland", "st petersburg fl" "spring hill", "brooksville", "orlando"}
_OUT_OF_MARKET_PLACES = _FUTURE_SUBMARKET_PLACES | {"lithia", "fishhawk"}
...
_PLACE_WORDS_KEPT_IN_CHAIN = {"garden", "park", "springs", "harbor", "beach"}
_DIRECTIONALS = {
    "north", "south", "east", "west", "northwest", "northeast", "southwest", "southeast",
    "village", "bypass", "area", "near", "blvd", "road", "rd", "center", "downtown",
    # TAMPA area words that tell same-brand hotels apart (Courtyard Downtown / Airport / Westshore / Busch Gardens)
    "airport", "westshore", "rocky", "point", "stadium", "riverwalk", "channelside", "ybor", "busch", "gardens",
    "usf", "university", "brandon", "beach", "gulf", "causeway", "downtown", "waterfront", "bayshore", "harbor",
    "marina", "pier", "north", "south", "central", "midtown",
}
...
CORRIDOR_CITIES = {
    ...
    "ybor-city": {"tampa"},
    "westshore-airport-rocky-point": {"tampa"},
    "busch-gardens-usf": {"tampa", "temple terrace"},
    "brandon": {"brandon", "seffner", "tampa"},
    "east-tampa-i75": {"tampa"},
    "st-petersburg-downtown": {"st petersburg", "saint petersburg"},
    "st-petersburg": {"st petersburg", "saint petersburg"},
    "clearwater-downtown": {"clearwater"},
    "clearwater-beach": {"clearwater", "clearwater beach"},
    "st-pete-beach-treasure-island": {"st pete beach", "saint pete beach", "treasure island"},
    "madeira-redington-indian-rocks": {"madeira beach", "redington beach", "redington shores",
                                       "indian rocks beach", "belleair beach", "belleair"},
    "largo": {"largo"},
    "wesley-chapel-new-tampa": {"wesley chapel", "new tampa", "tampa", "zephyrhills"},
    "south-hillsborough": {"riverview", "apollo beach", "ruskin", "sun city center", "wimauma"},
    "plant-city": {"plant city"},
    "tarpon-dunedin": {"tarpon springs", "dunedin"},
    "mid-pinellas": {"seminole", "pinellas park", "gulfport"},
    "safety-harbor-oldsmar-palm-harbor": {"safety harbor", "oldsmar", "palm harbor"},
}
...
#: A map-only row under a flag whose own Tampa-area route on the brand's site is RETIRED. Empty at authoring time.
...
def classify(node, zips, name_counts, street_counts):
    ...
    if lanes == {"OSM_OVERPASS"} and any(
        ...
        return IDENTITY_REVIEW_REQUIRED, (
            "a map row under a flag (%s) whose own Tampa-area route on the brand's site is "
    ...
    # Sunshine Skyway (lat < 27.50: Anna Maria / Bradenton), north of Wesley Chapel (lat > 28.28) or east of
    # Plant City (lng > -82.00: Lakeland) cannot be an admitted identity whatever else is unknown about it.
    _pins = [(o.get("lat"), o.get("lng")) for o in node.observations
             if o.get("lat") is not None and o.get("lng") is not None]
    if not node.postal and _pins and all(float(t) < 27.50 or float(t) > 28.28 or float(g) > -82.00 for t, g in _pins):
        return OUTSIDE_MARKET, ("no postal code of its own, and every source pins it beyond the admitted corridors "
                                "(Anna Maria / Bradenton south, Wesley Chapel north or Lakeland east; %.4f, %.4f)"
    ...
    if node.postal and node.postal[:5] not in zips:
        ...
        return OUTSIDE_MARKET, ("%s postal code %s is claimed by no corridor of the Tampa Bay "
...
#: Brand families whose OWN Tampa-area inventory this order read. Families whose inventory refused this
...
def brand_not_listed(node):
    ...
    for fam, rx in _COVERED_FLAGS:
        if re.search(rx, node.name or "", re.I):
            return ("a %s row naming a %s flag (%r) that no read of that brand's own Tampa-area inventory "
...
SOURCE_AUTHORITIES = [
    ...
    "Marriott harvest, TPA-coded routes)",
    ...
    "Suites, WoodSpring)",
    "https://www.visittampabay.com Places to Stay / Hotels & Resorts listings (Visit Tampa Bay's own Simpleview "
    "listing service, plain client, catid 5); Visit St. Pete-Clearwater answered every plain request with HTTP 403",
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 Tampa V2 census, built from zero under the current hardened "
...
def gap_matrix(rows, lanes):
    ...
    return OrderedDict([
        ...
        ("a_competitor_row_is_a_lead",
         "A competitor row cannot enter Tampa Bay authority without first-party identity "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "tampa-fl.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "tampa_fl_v2_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "tampa_fl_v2_competitor_gap_matrix_001.json"))
