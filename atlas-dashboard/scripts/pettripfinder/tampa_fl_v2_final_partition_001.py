"""PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 -- Phases 20-21: every census identity in exactly one disposition.

Cloned from the Orlando FL V2 helper, simplified because this market's clean-authority pass
(tampa_fl_v2_clean_authority_001) already computes one disposition per confirmed identity directly, with its
...
Output: launch_packages/pettripfinder/markets/staging/tampa-fl/launch_package/tampa_fl_final_partition_v2_001.json
"""
...
OUT = os.path.join(PKG, "markets", "staging", "tampa-fl", "launch_package", "tampa_fl_final_partition_v2_001.json")
...
def build():
    ...
    for h in census["hotels"]:
        ...
        if disp in _TERMINAL:
            ...
            row["resolved"] = False
            row["next_action_source"] = "PTF-TAMPA-FL-HARDENED-V2 acquisition router record"
    ...
    return OrderedDict((
        ...
        ("what_this_is", "Every admitted Tampa Bay V2 census identity in exactly one disposition state, with the "
