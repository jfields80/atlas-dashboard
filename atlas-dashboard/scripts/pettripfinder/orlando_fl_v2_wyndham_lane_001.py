"""PTF-ORLANDO-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 12B: the Wyndham first-party lane (plain client).
...
``data/acquisition/orlando_fl_v2_wyndham_001/``. The pet quote is read from the
property-service document, so ``h``/``b`` on each row are that document's.

Routes selected: every en-us Florida property route the brand's own sitemap
lists whose city segment is a Greater Orlando or careful-evaluation place. A
route admits nothing -- the postal code the property service states decides.

Output:
  launch_packages/pettripfinder/markets/staging/orlando-fl/raw_captures/wyndham_rows.json
  launch_packages/pettripfinder/markets/reports/orlando_fl_v2_wyndham_lane_001.json
"""
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "orlando-fl", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "orlando_fl_v2_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "orlando_fl_v2_wyndham_001")
...
PLACES = set(B.LOCALITIES)  # hyphenated city segments, e.g. lake-buena-vista
