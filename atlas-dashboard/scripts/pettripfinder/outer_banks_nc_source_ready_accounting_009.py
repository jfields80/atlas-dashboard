"""PTF-OUTER-BANKS-NC-PARALLEL-SOURCE-READY-001 -- complete accounting.
...
Output:
  launch_packages/pettripfinder/markets/reports/outer_banks_nc_source_ready_accounting_009.json
"""
...
OUT = os.path.join(REPORTS, "outer_banks_nc_source_ready_accounting_009.json")
...
def town_coverage(hotels, pf, np_):
    ...
    from scripts.pettripfinder import outer_banks_nc_geography_001 as GEO
...
def build():
    ...
    partition = _load(os.path.join(STAGING, "launch_package", "outer_banks_nc_final_partition_007.json"))
    geography = _load(os.path.join(REPORTS, "outer_banks_nc_geography_001.json"))
    capture = _load(os.path.join(REPORTS, "outer_banks_nc_attended_capture_001.json"))
    clean = _load(os.path.join(REPORTS, "outer_banks_nc_clean_authority_001.json"))
    recon = _load(os.path.join(REPORTS, "outer_banks_nc_census_reconciliation_001.json"))
    gaps = _load(os.path.join(REPORTS, "outer_banks_nc_competitor_gap_matrix_001.json"))
    osm = _load(os.path.join(REPORTS, "outer_banks_nc_osm_lane_001.json"))
    brand = _load(os.path.join(REPORTS, "outer_banks_nc_brand_inventory_001.json"))
    wyn = _load(os.path.join(REPORTS, "outer_banks_nc_wyndham_lane_001.json"))
    roster = _load(os.path.join(REPORTS, "outer_banks_nc_destination_roster_001.json"))
    shadow = _load(os.path.join(REPORTS, "outer_banks_nc_shadow_package_008.json"))
    ...
    doc = OrderedDict([
        ...
        ("factory_code_changed", "NO -- every changed path is an outer_banks_nc_* helper, the market's discovery config, or a "
        ...
        ("next_order", [
            ...
            "Rebase worker/ptf-outer-banks-nc-market-001 onto it; copy markets/proposed/outer-banks-nc.json -> markets/outer-banks-nc.json, "
            "identity_census_proposed/outer-banks-nc.json -> identity_census/outer-banks-nc.json and the staged launch_package documents "
            "(hotel_policy_facts_outer-banks-nc.json, partition, authority shard) into their registered paths.",
