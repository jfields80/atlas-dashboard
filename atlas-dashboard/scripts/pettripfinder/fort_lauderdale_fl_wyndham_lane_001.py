"""PTF-FORT-LAUDERDALE-FL-HARDENED-SOURCE-READY-001 -- Phase 12B: the Wyndham first-party lane (plain client).

Wyndham's property pages render the pet policy client-side, but both documents the page is built from answer a
plain HTTPS client:

  1. the property's own OVERVIEW page (its Hotel JSON-LD carries the brand property ``identifier``); a route
     that now redirects to the brand's city search (``/hotels/...``) is a RETIRED ROUTE, recorded as such,
     never a refusal;
  2. the brand's own property service
     ``/BWSServices/services/search/property/search?propertyId=<id>`` -- the same JSON the page renders --
     whose ``hotelPolicies.petpolicy`` is the exact text of the page's ``.pet-policy-desc`` element, and whose
     address fields are the property's own.

Both documents are persisted as returned, addressed by sha256, under ``data/acquisition/fort_lauderdale_fl_wyndham_001/``.
The pet quote is read from the property-service document, so ``h``/``b`` on each row are that document's.

Routes selected: every en-us Florida property route the brand's own sitemap lists whose city segment is a
Greater Fort Lauderdale / Broward County place. A route admits nothing -- the postal code the property service states
decides.

Output:
  launch_packages/pettripfinder/markets/staging/fort-lauderdale-fl/raw_captures/wyndham_rows.json
  launch_packages/pettripfinder/markets/reports/fort_lauderdale_fl_wyndham_lane_001.json
"""
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "fort-lauderdale-fl", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "fort_lauderdale_fl_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "fort_lauderdale_fl_wyndham_001")
...
_ROUTE = re.compile(r"^https://www\.wyndhamhotels\.com/(?![a-z]{2}-[a-z]{2}/)[a-z0-9-]+/([a-z-]+)-florida/[a-z0-9-]+/overview$")
PLACES = set(B.LOCALITIES)  # hyphenated city segments, e.g. st-petersburg
