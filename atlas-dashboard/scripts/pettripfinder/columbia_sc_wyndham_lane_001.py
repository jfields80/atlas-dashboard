"""PTF-COLUMBIA-SC-PARALLEL-SOURCE-READY-001 -- Phase 10/11: the Wyndham first-party lane (plain client).
...
``data/acquisition/columbia_sc_wyndham_001/``. The pet quote is read from the
...
lists whose city segment is a Columbia-area or observed-neighbour place. A
route admits nothing -- the postal code the property service states decides.

Output:
  data/acquisition/columbia_sc_attended/wyndham_rows.json   (the capture helper's input shape)
  launch_packages/pettripfinder/markets/reports/columbia_sc_wyndham_lane_001.json
"""
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "columbia-sc", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "columbia_sc_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "columbia_sc_wyndham_001")
