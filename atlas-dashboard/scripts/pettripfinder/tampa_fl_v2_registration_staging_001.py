"""PTF-TAMPA-FL-HARDENED-V2-SOURCE-READY-001 -- Phase 26: STAGE Tampa's authority (shadow until registered).
...
validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE  markets/staging/tampa-fl/launch_package/hotel_policy_facts_tampa-fl.json
     -- schema-1.3 facts, every published fact cited to the quote it rests on.
  2. the PROPOSED AUTHORITY     markets/staging/tampa-fl/tampa_fl_v2_proposed_authority_001.json
...
CLEAN = os.path.join(REPORTS, "tampa_fl_v2_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census_proposed", "tampa-fl.json")
STAGING = os.path.join(PKG, "markets", "staging", "tampa-fl")
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_tampa-fl.json")
AUTHORITY_OUT = os.path.join(STAGING, "tampa_fl_v2_proposed_authority_001.json")
OBSERVED_AT = "2026-09-15"
CAPTURED_AT = "2026-09-15T21:00:00+00:00"
...
def build():
    ...
    package = OrderedDict([
        ("market", "Tampa, Florida"), ("schema_version", PS.SCHEMA_VERSION), ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER), ("as_of", OBSERVED_AT),
        ("note", "Tampa Bay's STAGED policy package (shadow until registered). Every record is one first-party "
    ...
    authority = OrderedDict([
        ...
        ("what_this_is", "Tampa Bay's authority as this order proposes it, STAGED and not registered. "
