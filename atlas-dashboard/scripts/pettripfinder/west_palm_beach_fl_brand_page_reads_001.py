"""PTF-WEST-PALM-BEACH-FL-HARDENED-SOURCE-READY-001 -- the Choice / IHG property pages the Firecrawl route-discovery lane found.

Choice's own sitemap timed out to the plain client and IHG's answered 403, so neither family's routes reached the
census by name: a discovered route names a FLAG and a city ("/florida/west-palm-beach/sleep-inn-hotels/fl123"), never the
...
Outputs:
  launch_packages/pettripfinder/markets/staging/west-palm-beach-fl/raw_captures/brand_page_rows.json
  launch_packages/pettripfinder/markets/reports/west_palm_beach_fl_brand_page_reads_001.json
"""
...
DOCS = os.path.join(_DASH, "data", "acquisition", "west_palm_beach_fl_brand_page_reads_001")
RAW_OUT = os.path.join(PKG, "markets", "staging", "west-palm-beach-fl", "raw_captures", "brand_page_rows.json")
REPORT_OUT = os.path.join(REPORTS, "west_palm_beach_fl_brand_page_reads_001.json")
CAP = 34
FLOOR = 400
#: The market's own towns, as a Choice or IHG route spells them. A route naming another town is another market's.
IN_MARKET = ("west-palm-beach", "palm-beach", "palm-beach-gardens", "north-palm-beach", "royal-palm-beach",
             "palm-beach-shores", "riviera-beach", "singer-island", "lake-park", "mangonia-park",
             "juno-beach", "jupiter", "tequesta", "lake-worth", "lake-worth-beach", "palm-springs",
             "greenacres", "atlantis", "lake-clarke-shores", "haverhill", "lantana", "manalapan",
             "hypoluxo", "boynton-beach", "ocean-ridge", "gulf-stream", "delray-beach", "highland-beach",
             "boca-raton", "wellington", "loxahatchee", "westlake", "belle-glade", "pahokee", "south-bay")
...
def main(argv=None):
    ...
    with open(RAW_OUT, "w", encoding="utf-8", newline="\n") as fh:
        ...
        fh.write("\n")
    report = OrderedDict([
        ("schema", "ptf-brand-page-reads/1.0"), ("work_order", WORK_ORDER), ("market_id", "west-palm-beach-fl"),
