"""PTF-PINEHURST-SOUTHERN-PINES-NC-PARALLEL-SOURCE-READY-001 -- Phase 8: STAGE Pinehurst - Southern Pines's authority (shadow until registered).
...
from, and validates both against the contracts that own them:
  ...
  1. the staged POLICY PACKAGE     markets/staging/pinehurst-southern-pines-nc/launch_package/hotel_policy_facts_pinehurst-southern-pines-nc.json
  ...
  2. the PROPOSED AUTHORITY        markets/staging/pinehurst-southern-pines-nc/pinehurst_southern_pines_nc_proposed_authority_002.json
     -- the ptf-market-proposed-authority/1.0 shape `market_registration_cli`
        reads to write the shard.

SHADOW UNTIL REGISTERED
-----------------------
PTF-PINEHURST-SOUTHERN-PINES-NC-PARALLEL-SOURCE-READY-001 runs while six markets
(Jacksonville, Greenville, Atlanta, Outer Banks, Boone - Blowing Rock ahead of it; Fayetteville now live) wait in the queue. This order therefore writes NOTHING at
the package root and registers nothing: both documents are staged inside the
market's own zone, and the later registration order copies them into place
against the then-current live parent (after Jacksonville, Greenville, Atlanta, Outer Banks and Boone - Blowing Rock).

WHY A LATER ORDER MAY REGISTER WITHOUT A ROW-BY-ROW FOUNDER SIGNATURE
------------------------------------------------------------------
Registration is not publication. Writing `markets/pinehurst-southern-pines-nc.json`, an
...
CLEAN = os.path.join(REPORTS, "pinehurst_southern_pines_nc_clean_authority_001.json")
CENSUS = os.path.join(PKG, "identity_census_proposed", "pinehurst-southern-pines-nc.json")
STAGING = os.path.join(PKG, "markets", "staging", "pinehurst-southern-pines-nc")
PACKAGE_OUT = os.path.join(STAGING, "launch_package", "hotel_policy_facts_pinehurst-southern-pines-nc.json")
AUTHORITY_OUT = os.path.join(STAGING, "pinehurst_southern_pines_nc_proposed_authority_002.json")
OBSERVED_AT = "2026-09-14"
CAPTURED_AT = "2026-09-14T02:30:00+00:00"
...
def _facts(extraction, quote):
    ...
    if "pet_fee" in extraction:
        fee = OrderedDict((
            ...
            # SCOPE IS NEVER DEFAULTED. "Fees - 25USD per night" states no scope, and
            # writing per_pet would publish a fact the hotel did not state. The
            # schema's scope is optional; it is written only when the quote says it.
        ...
        if extraction.get("fee_scope"):
            fee["scope"] = extraction["fee_scope"]
...
def build():
    ...
    for rec in clean["clean_pet_friendly"]:
        ...
        record = OrderedDict((
            ...
            # founder: this order stages Outer Banks without a row-by-row
    ...
    for rec in clean["clean_verified_no_pets"]:
        ...
        exclusions.append(OrderedDict((
            ...
            ("readiness_state", "READY"),
            ("founder_decision", "REGISTERED_NOT_AUTHORIZED_FOR_LAUNCH"),
            # NOT a person. This order stages Outer Banks without a row-by-row
    ...
    package = OrderedDict((
        ("market", "Pinehurst \u2013 Southern Pines \u2013 Aberdeen, North Carolina"),
        ("schema_version", PS.SCHEMA_VERSION),
        ("market_id", MARKET_ID),
        ("work_order", WORK_ORDER),
        ("as_of", OBSERVED_AT),
        ("note",
         "Pinehurst - Southern Pines's STAGED policy package (shadow until registered). Every record is one first-party read of the "
    ...
    authority = OrderedDict((
        ...
        ("what_this_is",
         "Pinehurst - Southern Pines's authority as this order proposes it, STAGED and not registered. Registration makes the market "
