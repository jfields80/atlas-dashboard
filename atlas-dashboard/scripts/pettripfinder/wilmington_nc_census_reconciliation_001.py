"""PTF-WILMINGTON-NC-NORMAL-PRODUCTION-001 -- Phases 4-8: one identity graph.

Independent discovery lanes ran for Wilmington, none of them
authoritative on its own:
  ...
  BRAND_INVENTORY     Marriott's ILM* roster OWNED at zero requests from the
                      committed national harvest; Hilton's roster read from the
                      brand's OWN North Carolina city pages; Wyndham property
                      routes read from its own sitemap.
  PROPERTY_PAGE       the address each property's OWN page states, from the
                      attended pass (Hilton, Marriott, Wyndham, IHG) and the
                      independent / resort properties' own websites.
...
-----------------------
The market contract's postal partition, and nothing else: downtown and midtown
Wilmington, the ILM airport / Mayfaire north side, Wrightsville Beach and
Carolina / Kure Beach (CORE), Monkey Junction, Ogden / Porters Neck and Leland
(CORRIDOR), Castle Hayne (FRINGE). An ILM property code reaches Southport, Oak
Island and Bolivia as readily as Wilmington; every such row is classified on its
own postal code.

VACATION RENTALS ARE NOT HOTELS
-------------------------------
Individual vacation homes, condominium rental programmes, property-management
listings and apartments are recorded NON_LODGING by name, never admitted. A
condo-hotel that operates as a hotel (a front desk, nightly rooms sold under
one name) is a lodging identity like any other.

Nothing here fetches. Nothing here carries a pet policy.

Outputs:
  launch_packages/pettripfinder/identity_census/wilmington-nc.json
  launch_packages/pettripfinder/markets/reports/wilmington_nc_census_reconciliation_001.json
  launch_packages/pettripfinder/markets/reports/wilmington_nc_competitor_gap_matrix_001.json
"""
...
CONTRACT_PATH = _first_existing(os.path.join(PKG, "markets", "wilmington-nc.json"),
                                os.path.join(PKG, "markets", "proposed", "wilmington-nc.json"))
PROPOSED_CONTRACT = os.path.join(REPORTS, "wilmington_nc_corridor_registry_001.json")

OSM_LANE = os.path.join(REPORTS, "wilmington_nc_osm_lane_001.json")
BRAND = os.path.join(REPORTS, "wilmington_nc_brand_inventory_001.json")
ATTENDED_PASS = os.path.join(REPORTS, "wilmington_nc_attended_capture_001.json")
...
NOT_LODGING_NAMES = {
    "hell s kitchen", "mill creek apartments", "wilmington convention center",
    "wilmington international airport", "visit wilmington", "one south lumina",
    "the cottage",
}
#: Why each NON_LODGING name above is refused -- written down so a refusal is a
#: decision. One South Lumina and The Cottage are VACATION RENTALS.
NOT_LODGING_WHY = {
    "hell s kitchen": "a downtown bar and restaurant tagged tourism=hotel on the map",
    "mill creek apartments": "an apartment complex tagged tourism=hotel on the map",
    "one south lumina": "VACATION_RENTAL -- a Wrightsville Beach condominium rental building, not a hotel operation",
    "the cottage": "VACATION_RENTAL -- a single rental cottage",
}
NON_HOTEL_LEAD_TYPES = {"Campground"}
STR_PLACEHOLDERS = {"airbnb wilmington", "vrbo wilmington", "airbnb wrightsville beach"}
...
def read_property_pages():
    ...
    allowed to turn a brand-roster or map row into a Wilmington identity."""
...
IDENTITY_ONLY_PAGES = [
    ("Shell Island Resort", "2700 N. Lumina Avenue", "Wrightsville Beach", "28480", "910-256-8696",
     "https://www.shellisland.com/", "6258a35a3ab812be665dae60414780f188f6914daf188df3e78caabc6af3b6f9",
     "a condo-hotel operated as one resort hotel (front desk, nightly oceanfront suites); the "
     "address is stated in the site footer of the attended read"),
]

#: Phase 6. Competitor directory leads, as the discovery search surfaced them
#: (BringFido's Carolina Beach and Kure Beach lodging pages, via a web search
#: summary). NAMES ONLY -- a lead proposes and never decides, and its pet claims
#: are never read. Each is reconciled in the gap matrix.
COMPETITOR_LEADS = [
    ("Carolina Beach Motel", "https://www.bringfido.com/lodging/hotels/city/carolina_beach_nc_us/"),
    ("SeaBirds Motel at Kure Beach", "https://www.bringfido.com/lodging/city/kure_beach_nc_us/"),
    ("Savannah Inn", "https://www.bringfido.com/lodging/hotels/city/carolina_beach_nc_us/"),
    ("Carolina Beach Inn", "https://www.bringfido.com/lodging/hotels/city/carolina_beach_nc_us/"),
]
...
_IN_MARKET_MUNICIPALITIES = {
    ...
    "castle hayne", "monkey junction", "porters neck", "ogden", "mayfaire", "landfall",
    "smith creek", "riverfront", "historic district", "ilm",
}
_OUT_OF_MARKET_PLACES = {
    "southport", "oak island", "bald head island", "hampstead", "surf city", "topsail beach",
    "north topsail beach", "sneads ferry", "bolivia", "shallotte", "burgaw", "rocky point",
    "holden beach", "ocean isle beach", "sunset beach", "calabash", "jacksonville",
    "myrtle beach", "boiling spring lakes",
}
...
#: Wyndham flags whose own Wilmington-area routes redirected to the brand's city
#: search in this order's attended pass (see the capture report). Baymont is NOT
#: here: the brand's search still lists Baymont by Wyndham Wilmington.
RETIRED_WYNDHAM_FLAGS = ("super 8", "days inn", "ramada", "travelodge", "microtel")
...
def classify(node, zips, name_counts, street_counts):
    ...
    # Wilmington routes for Super 8, Days Inn, Ramada, Travelodge and Microtel now
    ...
    if lanes == {"OSM_OVERPASS"} and any(
        ...
        return IDENTITY_REVIEW_REQUIRED, (
            "a map row under a flag (%s) whose own Wilmington-area route on the brand's site is "
    ...
    if node.postal and node.postal[:5] not in zips:
        return OUTSIDE_MARKET, ("postal code %s is claimed by no corridor of the Wilmington "
                                "contract; Southport, Oak Island, Bolivia, Hampstead, Topsail and "
                                "Burgaw are refused by name in the geography"
                                % node.postal[:5])
...
def build():
    ...
    census = OrderedDict([
        ...
        ("note",
         "PTF-WILMINGTON-NC-NORMAL-PRODUCTION-001 Wilmington census, built from zero "
        ...
        ("source_authorities", [
            ...
            "https://www.hilton.com/en/locations/usa/north-carolina/<city>/ (twelve city pages)",
            "https://www.wyndhamhotels.com/sitemap.xml (the brand's own property shards) and "
            "https://www.wyndhamhotels.com/hotels/wilmington-north-carolina (the brand's own search)",
            ...
            "BringFido Carolina Beach / Kure Beach lodging pages (competitor leads; names only)",
        ]),
        ("count", len(confirmed)),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--census-out",
                    default=os.path.join(CENSUS_DIR, "wilmington-nc.json"))
    ap.add_argument("--report-out",
                    default=os.path.join(REPORTS, "wilmington_nc_census_reconciliation_001.json"))
    ap.add_argument("--gap-out",
                    default=os.path.join(REPORTS, "wilmington_nc_competitor_gap_matrix_001.json"))
