"""PTF-SAN-DIEGO-CA-HARDENED-SOURCE-READY-001 -- Phase 12B: the Wyndham first-party lane (plain client).
...
Both documents are persisted as returned, addressed by sha256, under ``data/acquisition/san_diego_ca_wyndham_001/``.
The pet quote is read from the property-service document, so ``h``/``b`` on each row are that document's.

Routes selected: every en-us California property route the brand's own sitemap lists whose city segment is a
San Diego place. A route admits nothing -- the postal code the property service states
decides.

Output:
  launch_packages/pettripfinder/markets/staging/san-diego-ca/raw_captures/wyndham_rows.json
  launch_packages/pettripfinder/markets/reports/san_diego_ca_wyndham_lane_001.json
"""
...
from scripts.pettripfinder import san_diego_ca_brand_inventory_001 as B  # noqa: E402
...
RAW_OUT = os.path.join(_DASH, "launch_packages", "pettripfinder", "markets", "staging", "san-diego-ca", "raw_captures", "wyndham_rows.json")
REPORT_OUT = os.path.join(REPORTS, "san_diego_ca_wyndham_lane_001.json")
B.DOCS = os.path.join(_DASH, "data", "acquisition", "san_diego_ca_wyndham_001")
...
_ROUTE = re.compile(r"^https://www\.wyndhamhotels\.com/(?![a-z]{2}-[a-z]{2}/)[a-z0-9-]+/([a-z-]+)-california/[a-z0-9-]+/overview$")
