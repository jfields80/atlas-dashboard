"""PTF-RELEASE-FACTORY-BOUNDED-MIGRATION-DECISION-004 stage driver, reused by PTF-RELEASE-STORE-LIVE-CONTENT-KEY-CORRECTION-005 -- Columbia reuse-staging proof.
...
# 005: the parent is the deployed bundle; the release digest is metadata.
parent, found_by = store.resolve_deployed(release_digest=live.digest(), bundle_sha256=live.state.bundle_sha256)
out["parent_release_digest"] = live.digest()
out["parent_found_by"] = found_by
out["parent_release_digest_stored"] = RC.digest_of(RC.release_identity(parent)) if parent is not None else None
out["PARENT_RELEASE_LOOKUP"] = "PASS" if (
    parent is not None and parent["bundle_sha256"] == live.state.bundle_sha256
    and len(RC._fragments_of(parent)) == len(live.participating_markets)) else "FAIL"
parent_fragments = RC._fragments_of(parent) if parent is not None else {}
